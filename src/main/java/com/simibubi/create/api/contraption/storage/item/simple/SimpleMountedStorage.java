...
public class SimpleMountedStorage extends WrapperMountedItemStorage<ItemStackHandler> {
    ...
    protected Optional<IItemHandlerModifiable> validate(IItemHandler handler) {
        if (handler.getSlots() == this.getSlots()
    ...
    public static <T extends SimpleMountedStorage> MapCodec<T> codec(
            Function<IItemHandler, T> factory) {
        return CreateCodecs.ITEM_STACK_HANDLER
                .xmap(factory, storage -> storage.wrapped)
                .fieldOf("value");
    }
}
