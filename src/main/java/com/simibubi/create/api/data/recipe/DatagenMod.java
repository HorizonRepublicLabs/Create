...
public interface DatagenMod {
    ...
    default ResourceLocation ingotOf(String type) {
        return ResourceLocation.fromNamespaceAndPath(
                getId(), reversedMetalPrefix() ? "ingot_" + type : type + "_ingot");
    }

    default ResourceLocation nuggetOf(String type) {
        return ResourceLocation.fromNamespaceAndPath(
                getId(), reversedMetalPrefix() ? "nugget_" + type : type + "_nugget");
    }

    default ResourceLocation oreOf(String type) {
        return ResourceLocation.fromNamespaceAndPath(
                getId(), reversedMetalPrefix() ? "ore_" + type : type + "_ore");
    }

    default ResourceLocation deepslateOreOf(String type) {
        return ResourceLocation.fromNamespaceAndPath(
                getId(),
                reversedMetalPrefix() ? "deepslate_ore_" + type : "deepslate_" + type + "_ore");
    }
