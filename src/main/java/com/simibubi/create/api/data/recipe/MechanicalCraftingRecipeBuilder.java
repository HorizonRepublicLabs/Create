...
public class MechanicalCraftingRecipeBuilder {
    ...
    public void build(RecipeOutput output, String id) {
        ...
        if (idRs.equals(resourcelocation)) {
            throw new IllegalStateException(
                    "Shaped Recipe " + id + " should remove its 'id' argument");
        } else {
    ...
    private void validate(ResourceLocation recipeId) {
        if (pattern.isEmpty()) {
            throw new IllegalStateException(
                    "No pattern is defined for shaped recipe " + recipeId + "!");
        } else {
            ...
            for (String s : pattern) {
                for (int i = 0; i < s.length(); ++i) {
                    ...
                    if (!key.containsKey(c0) && c0 != ' ')
                        throw new IllegalStateException("Pattern in recipe " + recipeId
