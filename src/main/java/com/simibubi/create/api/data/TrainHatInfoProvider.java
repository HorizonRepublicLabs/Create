...
public abstract class TrainHatInfoProvider implements DataProvider {
    ...
    public TrainHatInfoProvider(
            PackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
        this.path = output.createPathProvider(
                PackOutput.Target.RESOURCE_PACK, TrainHatInfoReloadListener.HAT_INFO_DIRECTORY);
    }
    ...
    protected void makeInfoFor(
            EntityType<?> type, Vec3 offset, String part, int cubeIndex, float scale) {
        this.trainHatOffsets.put(
                BuiltInRegistries.ENTITY_TYPE.getKey(type),
                new TrainHatInfo(part, cubeIndex, offset, scale));
    }
    ...
    public CompletableFuture<?> run(CachedOutput output) {
        ...
        this.createOffsets();
        return CompletableFuture.allOf(this.trainHatOffsets.entrySet().stream()
