...
public abstract class CuttingRecipeGen extends StandardProcessingRecipeGen<CuttingRecipe> {
    ...
    protected GeneratedRecipe stripAndMakePlanks(
            Block wood, Block stripped, Block planks, int planksAmount) {
    ...
    protected GeneratedRecipe cuttingCompat(DatagenMod mod, String... woodtypes) {
        for (String type : woodtypes) {
            ...
            if (mod == Mods.ARS_N && type.contains("archwood")) planks = "archwood_planks";

            String strippedPre = mod.strippedIsSuffix() ? "" : "stripped_";
            String strippedPost = mod.strippedIsSuffix() ? "_stripped" : "";
            stripAndMakePlanks(
                    mod, type + "_log", strippedPre + type + "_log" + strippedPost, planks);
    ...
    protected GeneratedRecipe stripOnlyDiffModId(
            DatagenMod mod1, String wood, DatagenMod mod2, String stripped) {
    ...
    protected GeneratedRecipe stripAndMakePlanksDiffPlanksModId(
            DatagenMod mod1, String log, String stripped, DatagenMod mod2, String planks) {
        if (log != null)
            create("compat/" + mod1.getId() + "/" + log, b -> b.duration(50)
                    .require(mod1, log)
                    .output(1, mod1, stripped, 1)
                    .whenModLoaded(mod1.getId()));
        if (planks != null) // Shouldn't be needed as stripAndMakePlanks can already do what this
            // method does
            // if planks is null
    ...
    protected GeneratedRecipe stripAndMakePlanks(
            DatagenMod mod, String wood, String stripped, String planks) {
        if (wood != null)
