...
public class SimpleBuilder<R, T extends R, P>
    ...
    public SimpleBuilder<R, T, P> byBlockEntity(
            SimpleRegistry.Multi<BlockEntityType<?>, R> registry) {
        this.byBlockEntity = SimpleRegistryAccess.of(registry, TagProviderImpl::getBeHolder);
        return this;
    }
    ...
    protected record SimpleRegistryAccess<K, V>(
            BiConsumer<K, V> adder, BiConsumer<TagKey<K>, V> tagAdder) {
        public static <K, V> SimpleRegistryAccess<K, V> of(
                SimpleRegistry<K, V> registry, Function<K, Holder<K>> holderGetter) {
            return new SimpleRegistryAccess<>(
                    registry::register,
                    (tag, value) ->
                            registry.registerProvider(Provider.forTag(tag, holderGetter, value)));
        }

        public static <K, V> SimpleRegistryAccess<K, V> of(
                SimpleRegistry.Multi<K, V> registry, Function<K, Holder<K>> holderGetter) {
            return new SimpleRegistryAccess<>(
                    registry::add,
                    (tag, value) ->
                            registry.addProvider(Provider.forTag(tag, holderGetter, value)));
        }
    }
}
