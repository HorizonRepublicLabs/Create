...
public class MetalScaffoldingBlockItem extends ScaffoldingBlockItem {
    ...
    @Nullable
    public BlockPlaceContext updatePlacementContext(
            BlockPlaceContext pContext) { // TODO replace with placement helper
        BlockPos blockpos = pContext.getClickedPos();
        Level level = pContext.getLevel();
        BlockState blockstate = level.getBlockState(blockpos);
        Block block = this.getBlock();
        if (!blockstate.is(block)) return pContext;

        Direction direction;
        if (pContext.isSecondaryUseActive()) {
            direction = pContext.isInside()
                    ? pContext.getClickedFace().getOpposite()
                    : pContext.getClickedFace();
        } else {
            direction = pContext.getClickedFace() == Direction.UP
