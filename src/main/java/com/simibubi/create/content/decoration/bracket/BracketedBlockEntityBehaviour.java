...
public class BracketedBlockEntityBehaviour extends BlockEntityBehaviour {
    ...
    public BlockState removeBracket(boolean inOnReplacedContext) {
        ...
        blockEntity.notifyUpdate();
        if (world.isClientSide) return removed;
        blockEntity.getBlockState().updateNeighbourShapes(world, getPos(), 3);
        return removed;
    }
    ...
    public void read(CompoundTag nbt, HolderLookup.Provider registries, boolean clientPacket) {
        if (nbt.contains("Bracket")) {
            bracket = null;
            BlockState readBlockState = NbtUtils.readBlockState(
                    blockEntity.blockHolderGetter(), nbt.getCompound("Bracket"));
            if (isBracketValid(readBlockState)) bracket = readBlockState;
        }
        if (clientPacket && nbt.contains("Redraw"))
            getWorld()
                    .sendBlockUpdated(
                            getPos(), blockEntity.getBlockState(), blockEntity.getBlockState(), 16);
