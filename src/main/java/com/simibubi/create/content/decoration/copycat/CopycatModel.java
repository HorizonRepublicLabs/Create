...
public abstract class CopycatModel extends BakedModelWrapperWithData {
    ...
    protected Builder gatherModelData(
        ...
        ModelData wrappedData = getModelOf(material)
                .getModelData(
                        new FilteredBlockAndTintGetter(
                                world,
                                targetPos -> copycatBlock.canConnectTexturesToward(
                                        world, pos, targetPos, state)),
                        pos,
                        material,
                        ModelData.EMPTY);
    ...
    public List<BakedQuad> getQuads(BlockState state, Direction side, RandomSource rand) {
        return getCroppedQuads(
                state,
                side,
                rand,
                getMaterial(ModelData.EMPTY),
                ModelData.EMPTY,
    ...
    public List<BakedQuad> getQuads(
        ...
        // Rubidium: see below
        if (side != null
                && state.getBlock() instanceof CopycatBlock ccb
                && ccb.shouldFaceAlwaysRender(state, side)) return Collections.emptyList();
        ...
        List<BakedQuad> croppedQuads =
                getCroppedQuads(state, side, rand, material, wrappedData, renderType);
        ...
        if (side == null && state.getBlock() instanceof CopycatBlock ccb) {
            ...
            for (Direction nonOcclusionSide : Iterate.directions)
                if (ccb.shouldFaceAlwaysRender(state, nonOcclusionSide)) {
                    ...
                    croppedQuads.addAll(getCroppedQuads(
                            state, nonOcclusionSide, rand, material, wrappedData, renderType));
                }
        }
     ...
     */
    protected abstract List<BakedQuad> getCroppedQuads(
            BlockState state,
            Direction side,
            RandomSource rand,
            BlockState material,
