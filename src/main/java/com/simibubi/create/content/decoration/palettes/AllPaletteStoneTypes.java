...
public enum AllPaletteStoneTypes {
    ...
    ASURINE(STANDARD_RANGE, r -> r.paletteStoneBlock("asurine", () -> Blocks.DEEPSLATE, true, true)
            .properties(p -> p.destroyTime(1.25f).mapColor(MapColor.COLOR_BLUE))
            .register()),

    CRIMSITE(
            STANDARD_RANGE, r -> r.paletteStoneBlock("crimsite", () -> Blocks.DEEPSLATE, true, true)
                    .properties(p -> p.destroyTime(1.25f).mapColor(MapColor.COLOR_RED))
                    .register()),

    LIMESTONE(STANDARD_RANGE, r -> r.paletteStoneBlock(
                    "limestone", () -> Blocks.SANDSTONE, true, false)
            .properties(p -> p.destroyTime(1.25f).mapColor(MapColor.SAND))
            .register()),

    OCHRUM(STANDARD_RANGE, r -> r.paletteStoneBlock("ochrum", () -> Blocks.CALCITE, true, true)
            .properties(p -> p.destroyTime(1.25f).mapColor(MapColor.TERRACOTTA_YELLOW))
            .register()),
    ...
    SCORCHIA(STANDARD_RANGE, r -> r.paletteStoneBlock(
                    "scorchia", () -> Blocks.BLACKSTONE, true, false)
    ...
    VERIDIUM(STANDARD_RANGE, r -> r.paletteStoneBlock("veridium", () -> Blocks.TUFF, true, true)
            .properties(p -> p.destroyTime(1.25f).mapColor(MapColor.WARPED_NYLIUM))
            .register());
    ...
    AllPaletteStoneTypes(
            PaletteBlockPattern[] variantTypes,
    ...
    public static void register(CreateRegistrate registrate) {
        for (AllPaletteStoneTypes paletteStoneVariants : values()) {
            ...
            paletteStoneVariants.materialTag = AllTags.optionalTag(
                    BuiltInRegistries.ITEM, Create.asResource("stone_types/" + id));
