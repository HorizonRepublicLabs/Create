...
public class BeltDeployerCallbacks {
    ...
    public static ProcessingResult whenItemHeld(
        ...
        if (recipeHolder == null) return ProcessingResult.PASS;
    ...
    public static void activate(
        ...
        List<TransportedItemStack> collect = RecipeApplier.applyRecipeOn(
                        blockEntity.getLevel(), transported.stack.copyWithCount(1), recipe, true)
                .stream()
                .map(stack -> {
        ...
        boolean keepHeld = recipe instanceof ItemApplicationRecipe
    ...
    private static void awardAdvancements(DeployerBlockEntity blockEntity, ItemStack created) {
        ...
        else if (AllBlocks.RAILWAY_CASING.isIn(created)) advancement = AllAdvancements.TRAIN_CASING;
        else return;
