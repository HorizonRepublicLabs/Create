...
public class SawRenderer extends SafeBlockEntityRenderer<SawBlockEntity> {
    ...
    protected void renderBlade(
            SawBlockEntity be, PoseStack ms, MultiBufferSource buffer, int light) {
    ...
    protected void renderShaft(
            SawBlockEntity be, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        KineticBlockEntityRenderer.renderRotatingBuffer(
                be, getRotatedModel(be), ms, buffer.getBuffer(RenderType.solid()), light);
    }
    ...
    protected void renderItems(
            ...
            int light,
            int overlay) {
        if (be.getBlockState().getValue(SawBlock.FACING) != Direction.UP) return;
        if (be.inventory.isEmpty()) return;
        ...
        if (moving) {
            offset = Mth.clamp(
                    offset + ((-partialTicks + .5f) * processingSpeed) / duration, 0.125f, 1f);
            if (!be.inventory.appliedRecipe) offset += 1;
            offset /= 2;
        }
        ...
        for (int i = 1; i < be.inventory.getSlots(); i++)
            if (!be.inventory.getStackInSlot(i).isEmpty()) outputs++;
        ...
        for (int i = 0; i < be.inventory.getSlots(); i++) {
            ...
            } else ms.scale(.5f, .5f, .5f);

            if (!box) ms.mulPose(Axis.XP.rotationDegrees(90));
    ...
    protected SuperByteBuffer getRotatedModel(KineticBlockEntity be) {
        ...
        return CachedBuffers.block(
                KineticBlockEntityRenderer.KINETIC_BLOCK, getRenderedBlockState(be));
    }
    ...
    public static void renderInContraption(
        ...
        Vec3 facingVec =
                Vec3.atLowerCornerOf(context.state.getValue(SawBlock.FACING).getNormal());
        ...
        if (SawBlock.isHorizontal(state)) {
            if (shouldAnimate)
                superBuffer =
                        CachedBuffers.partial(AllPartialModels.SAW_BLADE_HORIZONTAL_ACTIVE, state);
            else
                superBuffer = CachedBuffers.partial(
                        AllPartialModels.SAW_BLADE_HORIZONTAL_INACTIVE, state);
        } else {
            if (shouldAnimate)
                superBuffer =
                        CachedBuffers.partial(AllPartialModels.SAW_BLADE_VERTICAL_ACTIVE, state);
            else
                superBuffer =
                        CachedBuffers.partial(AllPartialModels.SAW_BLADE_VERTICAL_INACTIVE, state);
        }
        ...
        if (!SawBlock.isHorizontal(state)) {
            superBuffer.rotateZDegrees(
                    state.getValue(SawBlock.AXIS_ALONG_FIRST_COORDINATE) ? 90 : 0);
        }
        ...
        superBuffer
                ...
                .renderInto(
                        matrices.getViewProjection(), buffer.getBuffer(RenderType.cutoutMipped()));
    }
}
