...
public class KineticNetwork {
    ...
    public void remove(KineticBlockEntity be) {
        if (!members.containsKey(be)) return;
        if (be.isSource()) sources.remove(be);
        members.remove(be);
    ...
    public void sync() {
        for (KineticBlockEntity be : members.keySet()) updateFromNetwork(be);
    }
    ...
    public float calculateCapacity() {
        ...
        for (Iterator<KineticBlockEntity> iterator = sources.keySet().iterator();
                iterator.hasNext(); ) {
    ...
    public float calculateStress() {
        ...
        for (Iterator<KineticBlockEntity> iterator = members.keySet().iterator();
                iterator.hasNext(); ) {
            KineticBlockEntity be = iterator.next();
            if (be.getLevel().getBlockEntity(be.getBlockPos()) != be) {
                iterator.remove();
