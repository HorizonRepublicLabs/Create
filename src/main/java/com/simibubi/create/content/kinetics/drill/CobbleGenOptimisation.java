...
public class CobbleGenOptimisation {
    ...
    public static BlockState determineOutput(
        ...
        Map<FluidType, List<InteractionInformation>> interactions =
                FluidInteractionRegistryAccessor.getInteractions();
        Map<FluidType, Pair<Direction, FluidState>> presentFluidTypes = new HashMap<>();

        for (int i = 0; i < Iterate.directions.length; i++) {
            if (config.statesAroundDrill.size() <= i) break;
        ...
        for (Entry<FluidType, Pair<Direction, FluidState>> type : presentFluidTypes.entrySet()) {
            ...
            FluidState state = FluidHelper.convertToFlowing(
                            type.getValue().getSecond().getType())
                    .defaultFluidState();

            if (list == null) continue;
            for (Direction d : Iterate.horizontalDirections) {
                for (InteractionInformation information : list) {
                    if (d == type.getValue().getFirst()) continue;
                    BlockPos relative = pos.relative(d);
                    HasFluidInteraction predicate = information.predicate();
                    if (!predicate.test(level, pos, relative, state)) continue;
        ...
        interaction.interact(
                cachedLevel, pos, pos.relative(affected.getFirst()), affected.getSecond());
