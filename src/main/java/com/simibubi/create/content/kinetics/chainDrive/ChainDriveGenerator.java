...
public class ChainDriveGenerator extends SpecialBlockStateGen {
    ...
    private final BiFunction<BlockState, String, ModelFile> modelFunc;
    ...
    protected int getXRotation(BlockState state) {
        ...
        boolean connectedAlongFirst =
                state.getValue(ChainDriveBlock.CONNECTED_ALONG_FIRST_COORDINATE);
        Axis axis = state.getValue(ChainDriveBlock.AXIS);

        if (part == Part.NONE) return axis == Axis.Y ? 90 : 0;
        if (axis == Axis.X) return (connectedAlongFirst ? 90 : 0) + (part == Part.START ? 180 : 0);
        if (axis == Axis.Z) return (connectedAlongFirst ? 0 : (part == Part.START ? 270 : 90));
        return 0;
    }
    ...
    protected int getYRotation(BlockState state) {
        ...
        if (axis == Axis.Z) return (connectedAlongFirst && part == Part.END ? 270 : 90);
        boolean flip = part == Part.END && !connectedAlongFirst
                || part == Part.START && connectedAlongFirst;
        if (axis == Axis.Y) return (connectedAlongFirst ? 90 : 0) + (flip ? 180 : 0);
        return 0;
    }
    ...
    protected String getModelSuffix(BlockState state) {
        ...
        if (part == Part.NONE) return "single";
