...
public class ChainConveyorRenderer extends KineticBlockEntityRenderer<ChainConveyorBlockEntity> {
    ...
    public static final ResourceLocation CHAIN_LOCATION =
    ...
    private void renderBox(
        ...
        SuperByteBuffer rigBuffer = CachedBuffers.partial(
                AllPartialModels.PACKAGE_RIGGING.get(physicsData.modelKey), blockState);
        SuperByteBuffer boxBuffer = CachedBuffers.partial(
                AllPartialModels.PACKAGES.get(physicsData.modelKey), blockState);

        Vec3 dangleDiff =
                VecHelper.rotate(targetPosition.add(0, 0.5, 0).subtract(position), -yaw, Axis.Y);
        float zRot =
    ...
    private void renderChains(
            ...
            int overlay) {
        float time =
                AnimationTickHolder.getRenderTime(be.getLevel()) / (360f / Math.abs(be.getSpeed()));
        time %= 1;
        if (time < 0) time += 1;
        ...
        for (BlockPos blockPos : be.connections) {
            ...
            double pitch =
                    Mth.RAD_TO_DEG * Mth.atan2(diff.y, diff.multiply(1, 0, 1).length());
            ...
            if (!VisualizationManager.supportsVisualization(be.getLevel())) {
                SuperByteBuffer guard = CachedBuffers.partial(
                        AllPartialModels.CHAIN_CONVEYOR_GUARD, be.getBlockState());
                guard.center();
            ...
            int light2 = LightTexture.pack(
                    level.getBrightness(LightLayer.BLOCK, tilePos.offset(blockPos)),
    ...
    public static void renderChain(
        ...
        renderPart(
                ms, vc, length, 0.0F, radius, radius, 0.0F, -radius, 0.0F, 0.0F, -radius, minU,
                maxU, minV, maxV, light1, light2, far);
