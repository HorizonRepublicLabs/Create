...
public class DeployerActorVisual extends ActorVisual {
    ...
    public DeployerActorVisual(
        ...
        DeployerBlockEntity.Mode mode =
                NBTHelper.readEnum(context.blockEntityData, "Mode", DeployerBlockEntity.Mode.class);
        ...
        boolean rotatePole =
                state.getValue(AXIS_ALONG_FIRST_COORDINATE) ^ facing.getAxis() == Direction.Axis.Z;
        ...
        pole = instancerProvider
                .instancer(
                        InstanceTypes.TRANSFORMED, Models.partial(AllPartialModels.DEPLOYER_POLE))
                .createInstance();
        hand = instancerProvider
                .instancer(InstanceTypes.TRANSFORMED, Models.partial(handPose))
                .createInstance();
        ...
        shaft.setRotationAxis(axis)
                .setRotationOffset(
                        KineticBlockEntityVisual.rotationOffset(state, axis, context.localPos))
    ...
    private float deploymentDistance() {
        ...
        if (context.disabled) {
            factor = 0;
        } else if (context.contraption.stalled
