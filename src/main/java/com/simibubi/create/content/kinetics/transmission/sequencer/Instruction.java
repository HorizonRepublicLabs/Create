...
public class Instruction {
    ...
    public Instruction(
            SequencerInstructions instruction, InstructionSpeedModifiers speedModifier, int value) {
    ...
    OnIsPoweredResult onRedstonePulse() {
        return instruction == SequencerInstructions.AWAIT
                ? OnIsPoweredResult.CONTINUE
                : OnIsPoweredResult.NOTHING;
    }
    ...
    public static Vector<Instruction> deserializeAll(ListTag list) {
        if (list.isEmpty()) return createDefault();
    ...
    static Instruction deserialize(CompoundTag tag) {
        ...
        instruction.speedModifier =
                NBTHelper.readEnum(tag, "Modifier", InstructionSpeedModifiers.class);
