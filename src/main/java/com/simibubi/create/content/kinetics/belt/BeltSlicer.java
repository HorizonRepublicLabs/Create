...
public class BeltSlicer {
    ...
    public static ItemInteractionResult useWrench(
        ...
        if (state.getValue(BeltBlock.PART) == BeltPart.PULLEY
                && hit.getDirection().getAxis() != Axis.Y)
            return ItemInteractionResult.PASS_TO_DEFAULT_BLOCK_INTERACTION;
        ...
        if (hoveringEnd(state, hit)) {
            ...
            BlockPos next =
                    part == BeltPart.END ? pos.subtract(beltVector) : pos.offset(beltVector);
            ...
            KineticBlockEntity.switchToBlockState(
                    ...
                    ProperWaterloggedBlock.withWater(
                            ...
                            state.setValue(
                                    BeltBlock.CASING,
                                    segmentBE != null && segmentBE.casing != CasingType.NONE),
                            next));
            world.setBlock(
                    pos,
            ...
            if (!creative
                    && AllBlocks.BELT.has(replacedState)
        ...
        if (hitSegment == controllerBE.beltLength - 1
                || hitSegment == controllerBE.beltLength - 2 && towardPositive)
            return ItemInteractionResult.FAIL;
        ...
        if (!world.isClientSide) {
            ...
            KineticBlockEntity.switchToBlockState(
                    ...
                    world.getBlockState(next)
                            .setValue(
                                    BeltBlock.PART,
                                    towardPositive ? BeltPart.START : BeltPart.END));
            world.playSound(
                    null,
                    pos,
                    SoundEvents.WOOL_HIT,
                    player == null ? SoundSource.BLOCKS : SoundSource.PLAYERS,
                    0.5F,
                    2.3F);

            // Transfer items to new controller
            BeltBlockEntity newController =
                    towardPositive ? BeltHelper.getSegmentBE(world, next) : segmentBE;
            if (newController != null && inventory != null) {
                newController.inventory = null;
                newController.setController(newController.getBlockPos());
                for (Iterator<TransportedItemStack> iterator =
                                inventory.getTransportedItems().iterator();
                        iterator.hasNext(); ) {
                    TransportedItemStack transportedItemStack = iterator.next();
                    float newPosition = transportedItemStack.beltPosition
                            - hitSegment
                            - (towardPositive ? 1 : 0);
    ...
    public static ItemInteractionResult useConnector(
        ...
        if (!nextState.canBeReplaced()) {
            if (!AllBlocks.BELT.has(nextState)) return ItemInteractionResult.FAIL;
            ...
            if (!world.isClientSide) {
                ...
                for (BlockPos blockPos :
                        BeltBlock.getBeltChain(world, mergedController.getBlockPos())) {
                    BeltBlockEntity belt = BeltHelper.getSegmentBE(world, blockPos);
                    if (belt == null) continue;
                    belt.detachKinetics();
                    belt.invalidateItemHandler();
                ...
                if (flipBelt && mergedController.inventory != null) {
                    List<TransportedItemStack> transportedItems =
                            mergedController.inventory.getTransportedItems();
                    for (TransportedItemStack transportedItemStack : transportedItems) {
                        transportedItemStack.beltPosition =
                                mergedBeltLength - transportedItemStack.beltPosition;
        ...
        if (!world.isClientSide) {
            ...
            BeltInventory inventory = controllerBE.inventory;
            KineticBlockEntity.switchToBlockState(
                    world, pos, state.setValue(BeltBlock.PART, BeltPart.MIDDLE));
            ...
            if (mergedController == null) {
                ...
                world.setBlock(
                        next,
                        ProperWaterloggedBlock.withWater(
                                world, state.setValue(BeltBlock.CASING, false), next),
                        Block.UPDATE_ALL | Block.UPDATE_MOVE_BY_PISTON);
                BeltBlockEntity segmentBE = BeltHelper.getSegmentBE(world, next);
                ...
                if (part == BeltPart.START && segmentBE != null && inventory != null) {
                    segmentBE.setController(next);
                    for (TransportedItemStack transportedItemStack :
                ...
                for (BlockPos blockPos :
                        BeltBlock.getBeltChain(world, controllerBE.getBlockPos())) {
                    BeltBlockEntity belt = BeltHelper.getSegmentBE(world, blockPos);
                    if (belt == null) continue;
                    belt.invalidateItemHandler();
                }
                ...
                for (int i = 0; i < 10000; i++) {
                    ...
                    if (!AllBlocks.BELT.has(blockState)) break;
                    ...
                    if (newController != mergedController && mergedInventory != null) {
                        ...
                        for (TransportedItemStack transportedItemStack :
                                mergedInventory.getTransportedItems()) {
    ...
    static boolean beltStatesCompatible(BlockState state, BlockState nextState) {
        ...
        switch (slope1) {
            case UPWARD:
                if (slope2 == BeltSlope.DOWNWARD) return facing1 == facing2.getOpposite();
                return slope2 == slope1 && facing1 == facing2;
            case DOWNWARD:
                if (slope2 == BeltSlope.UPWARD) return facing1 == facing2.getOpposite();
                return slope2 == slope1 && facing1 == facing2;
            default:
    ...
    public static void tickHoveringInformation() {
        ...
        if (feedback.langKey != null)
            mc.player.displayClientMessage(
                    CreateLang.translateDirect(feedback.langKey).withStyle(feedback.formatting),
                    true);
        else mc.player.displayClientMessage(CommonComponents.EMPTY, true);

        if (feedback.bb != null)
            Outliner.getInstance()
                    .chaseAABB("BeltSlicer", feedback.bb)
                    .lineWidth(1 / 16f)
