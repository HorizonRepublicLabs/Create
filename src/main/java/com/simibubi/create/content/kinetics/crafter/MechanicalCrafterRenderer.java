...
public class MechanicalCrafterRenderer
    ...
    protected void renderSafe(
        ...
        Vec3 vec = Vec3.atLowerCornerOf(facing.getNormal()).scale(.58).add(.5, .5, .5);

        if (be.phase == Phase.EXPORTING) {
            Direction targetDirection =
                    MechanicalCrafterBlock.getTargetDirection(be.getBlockState());
            float progress = Mth.clamp(
                    (1000 - be.countDown + be.getCountDownSpeed() * partialTicks) / 1000f, 0, 1);
    ...
    public void renderItems(
        ...
        if (be.phase == Phase.IDLE) {
            ...
            if (be.phase == Phase.CRAFTING) {
                ...
                items.calcStats();
                float progress = Mth.clamp(
                        (2000 - be.countDown + be.getCountDownSpeed() * partialTicks) / 1000f,
                        0,
                        1);
            ...
            boolean onlyRenderFirst = be.phase == Phase.INSERTING
                    || be.phase == Phase.CRAFTING && be.countDown < 1000;
            final float spacing = distance;
            items.grid.forEach((pair, stack) -> {
                if (onlyRenderFirst
                        && (pair.getLeft().intValue() != 0 || pair.getRight().intValue() != 0))
                    return;
                ...
                int offset = 0;
                if (be.phase == Phase.EXPORTING
                        && be.getBlockState().hasProperty(MechanicalCrafterBlock.POINTING)) {
                    Pointing value = be.getBlockState().getValue(MechanicalCrafterBlock.POINTING);
                    offset = value == Pointing.UP
                            ? -1
            ...
            if (be.phase == Phase.CRAFTING) {
                ...
                float progress = Mth.clamp(
                        (1000 - be.countDown + be.getCountDownSpeed() * partialTicks) / 1000f,
                        0,
                        1);
                float earlyProgress = Mth.clamp(progress * 2, 0, 1);
                float lateProgress = Mth.clamp(progress * 2 - 1, 0, 1);
                ...
                items.grid.forEach((pair, stack) -> {
                    if (pair.getLeft().intValue() != 0 || pair.getRight().intValue() != 0) return;
                    ms.pushPose();
                    ms.mulPose(Axis.YP.rotationDegrees(180));
                    Minecraft.getInstance()
                            .getItemRenderer()
    ...
    public void renderFast(
        ...
        if (!VisualizationManager.supportsVisualization(be.getLevel())) {
            ...
            superBuffer.rotateCentered(
                    (float)
                            (blockState.getValue(HORIZONTAL_FACING).getAxis() != Direction.Axis.X
                                    ? 0
                                    : Math.PI / 2),
                    Direction.UP);
        ...
        if ((be.covered || be.phase != Phase.IDLE)
                && be.phase != Phase.CRAFTING
                && be.phase != Phase.INSERTING) {
            SuperByteBuffer lidBuffer =
                    renderAndTransform(AllPartialModels.MECHANICAL_CRAFTER_LID, blockState);
            lidBuffer.light(light).renderInto(ms, vb);
        }

        if (MechanicalCrafterBlock.isValidTarget(
                be.getLevel(), pos.relative(targetDirection), blockState)) {
            SuperByteBuffer beltBuffer =
                    renderAndTransform(AllPartialModels.MECHANICAL_CRAFTER_BELT, blockState);
            SuperByteBuffer beltFrameBuffer =
                    renderAndTransform(AllPartialModels.MECHANICAL_CRAFTER_BELT_FRAME, blockState);

            if (be.phase == Phase.EXPORTING) {
                int textureIndex =
                        (int) ((be.getCountDownSpeed() / 128f * AnimationTickHolder.getTicks()));
                beltBuffer.shiftUVtoSheet(
                        AllSpriteShifts.CRAFTER_THINGIES, (textureIndex % 4) / 4f, 0, 1);
            }
            ...
            SuperByteBuffer arrowBuffer =
                    renderAndTransform(AllPartialModels.MECHANICAL_CRAFTER_ARROW, blockState);
