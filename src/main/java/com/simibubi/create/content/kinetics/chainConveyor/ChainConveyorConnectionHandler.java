...
public class ChainConveyorConnectionHandler {
    ...
    public static boolean onRightClick() {
        ...
        if (!isChain(mc.player.getMainHandItem())) return false;
    ...
    public static void onItemUsedOnBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (!AllBlocks.CHAIN_CONVEYOR.has(blockState)) return;
        if (!isChain(itemStack)) return;
    ...
    public static void clientTick() {
        ...
        if (firstDim != player.level().dimension()
                || !(sourceLift instanceof ChainConveyorBlockEntity)) {
            firstPos = null;
        ...
        Outliner.getInstance()
                .showLine("chain_connect_line", from.add(normal), to.add(normal))
                .lineWidth(1 / 16f)
                .colored(color);
        Outliner.getInstance()
                .showLine("chain_connect_line_1", from.subtract(normal), to.subtract(normal))
                .lineWidth(1 / 16f)
                .colored(color);
    }
    ...
    public static boolean validateAndConnect(
            LevelAccessor level, BlockPos pos, Player player, ItemStack chain, boolean simulate) {
        ...
        if (!pos.closerThan(
                firstPos, AllConfigs.server().kinetics.maxChainConveyorLength.get()))
        ...
        if (horizontalDistance <= 0) return fail("chain_conveyor.cannot_connect_vertically");
        if (Math.abs(diff.y) / horizontalDistance > 1) return fail("chain_conveyor.too_steep");
        ...
        if (targetLift.connections.size()
                >= AllConfigs.server().kinetics.maxChainConveyorConnections.get())
        ...
        if (!player.isCreative()) {
            ...
            if (simulate)
                BlueprintOverlayRenderer.displayChainRequirements(
                        chain.getItem(), chainCost, hasEnough);
        ...
        CatnipServices.NETWORK.sendToServer(
                new ChainConveyorConnectionPacket(firstPos, pos, chain, true));
