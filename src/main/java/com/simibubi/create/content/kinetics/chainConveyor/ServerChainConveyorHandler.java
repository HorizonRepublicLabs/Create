...
public class ServerChainConveyorHandler {
    ...
    public static void handleTTLPacket(Player player) {
        ...
        if (hangingPlayers.size() != count) sync();
    }

    public static void handleStopRidingPacket(Player player) {
        if (hangingPlayers.removeInt(player.getUUID()) != 0) sync();
    }
    ...
    public static void tick() {
        ...
        ObjectIterator<Object2IntMap.Entry<UUID>> iterator =
                hangingPlayers.object2IntEntrySet().iterator();
        ...
        if (ticks % 10 != 0 && before == after) return;
    ...
    public static void sync() {
        CatnipServices.NETWORK.sendToAllClients(
                new ClientboundChainConveyorRidingPacket(hangingPlayers.keySet()));
    }
}
