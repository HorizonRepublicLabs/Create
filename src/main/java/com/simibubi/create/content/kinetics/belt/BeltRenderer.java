...
public class BeltRenderer extends SafeBlockEntityRenderer<BeltBlockEntity> {
    ...
    protected void renderSafe(
        ...
        if (!VisualizationManager.supportsVisualization(be.getLevel())) {
            ...
            if (downward
                    || beltSlope == BeltSlope.VERTICAL && axisDirection == AxisDirection.POSITIVE) {
                boolean b = start;
                start = end;
                end = b;
            }
            ...
            for (boolean bottom : Iterate.trueAndFalse) {
                ...
                SuperByteBuffer beltBuffer =
                        CachedBuffers.partial(beltPartial, blockState).light(light);
                ...
                if (speed != 0 || be.color.isPresent()) {
                    ...
                    if (diagonal && (downward ^ alongX)
                            || !sideways && !diagonal && alongX
            ...
            if (be.hasPulley()) {
                ...
                SuperByteBuffer superBuffer = CachedBuffers.partialDirectional(
                        AllPartialModels.BELT_PULLEY, blockState, dir, matrixStackSupplier);
    ...
    public static SpriteShiftEntry getSpriteShiftEntry(
            DyeColor color, boolean diagonal, boolean bottom) {
        if (color != null) {
            return (diagonal
                            ? AllSpriteShifts.DYED_DIAGONAL_BELTS
                            : bottom
                                    ? AllSpriteShifts.DYED_OFFSET_BELTS
                                    : AllSpriteShifts.DYED_BELTS)
                    .get(color);
        } else
            return diagonal
                    ? AllSpriteShifts.BELT_DIAGONAL
                    : bottom ? AllSpriteShifts.BELT_OFFSET : AllSpriteShifts.BELT;
    }

    public static PartialModel getBeltPartial(
    ...
    private void renderItem(
        ...
        boolean tiltForward = (slope == BeltSlope.DOWNWARD
                ...
                == (beltFacing.getAxis() == Direction.Axis.Z);
        ...
        if (onContraption) {
            ...
            mutablePos
                    .set(be.getBlockPos())
                    .move(
                            directionVec.getX() * segment,
                            verticality * segment,
                            directionVec.getZ() * segment);
