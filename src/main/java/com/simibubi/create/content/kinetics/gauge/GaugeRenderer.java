...
public class GaugeRenderer extends ShaftRenderer<GaugeBlockEntity> {
    ...
    protected void renderSafe(
        ...
        PartialModel partialModel = (type == Type.SPEED
        ...
        for (Direction facing : Iterate.directions) {
            if (!((GaugeBlock) gaugeState.getBlock())
                    .shouldRenderHeadOnFace(be.getLevel(), be.getBlockPos(), gaugeState, facing))
                continue;
    ...
    protected SuperByteBuffer rotateBufferTowards(SuperByteBuffer buffer, Direction target) {
        return buffer.rotateCentered(
                (float) ((-target.toYRot() - 90) / 180 * Math.PI), Direction.UP);
    }
}
