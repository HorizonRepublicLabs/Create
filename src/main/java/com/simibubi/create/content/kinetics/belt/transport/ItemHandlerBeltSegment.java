...
public class ItemHandlerBeltSegment implements IItemHandler {
    ...
    public ItemStack getStackInSlot(int slot) {
        ...
        if (stackAtOffset == null) return ItemStack.EMPTY;
        ...
        if (this.beltInventory.canInsertAt(offset)) {
            ...
            if (!simulate) {
                ...
                newStack.beltPosition =
                        offset + .5f + (beltInventory.beltMovementPositive ? -1 : 1) / 16f;
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (transported == null) return ItemStack.EMPTY;

        amount = Math.min(amount, transported.stack.getCount());
        ItemStack extracted =
                simulate ? transported.stack.copy().split(amount) : transported.stack.split(amount);
        if (!simulate) {
            if (transported.stack.isEmpty()) beltInventory.toRemove.add(transported);
            else beltInventory.belt.notifyUpdate();
        }
