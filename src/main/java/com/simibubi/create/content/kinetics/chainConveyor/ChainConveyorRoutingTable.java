...
public class ChainConveyorRoutingTable {
    ...
    public BlockPos getExitFor(ItemStack box) {
        for (RoutingTableEntry entry : entriesByDistance)
            if (PackageItem.matchAddress(box, entry.port())) return entry.nextConnection();
    ...
    public void advertiseTo(BlockPos connection, ChainConveyorRoutingTable otherTable) {
        ...
        otherTable.entriesByDistance.removeIf(e -> e.timeout().intValue() < ENTRY_TIMEOUT
                && !e.endOfRoute()
                && backConnection.equals(e.nextConnection()));
    }
    ...
    private void insert(RoutingTableEntry entry) {
        ...
        for (int i = 0; i < entriesByDistance.size(); i++) {
            ...
            if (otherEntry.distance() > entry.distance())
                // Still not found at this distance : this either means it's an new port or a
                // shorter route
                // to a known port
            ...
            if (otherEntry.port().equals(entry.port())) {
                if (otherEntry.distance() == entry.distance()
                        && otherEntry.nextConnection().equals(entry.nextConnection()))
                    // We know this port, and we are given a route we already have (same distant,
                    // same
                    // connection)
                    // In that case, we can keep our existing entry and simply refresh its life time
                    otherEntry.timeout.setValue(ENTRY_TIMEOUT);
                // ...else...
                // We know this port, but we are given a different route, either longer or going
                // through a
                // different connection
                // It's time to question our existing entry. As it might be obsolete, we let it
                // decay (ie.
                // we do nothing)
                return;
            }
