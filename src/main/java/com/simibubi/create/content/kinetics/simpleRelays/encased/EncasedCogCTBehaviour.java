...
public class EncasedCogCTBehaviour extends EncasedCTBehaviour {
    ...
    public boolean connectsTo(
        ...
        if (!ICogWheel.isLargeCog(blockState)) return false;
    ...
    protected boolean reverseUVsHorizontally(BlockState state, Direction face) {
        if (large) return super.reverseUVsHorizontally(state, face);

        if (state.getValue(AXIS).isVertical() && face.getAxis().isHorizontal()) return true;

        if (state.getValue(AXIS) == Axis.Z && face == Direction.DOWN) return true;
            ...
            if (axis == direction.getAxis()
                    && state.getValue(
                            direction.getAxisDirection() == AxisDirection.POSITIVE
                                    ...
                                    : EncasedCogwheelBlock.BOTTOM_SHAFT)) return null;
