...
public class ManualApplicationRecipe extends ItemApplicationRecipe {
    ...
    public static void manualApplicationRecipesApplyInWorld(
        ...
        Optional<RecipeHolder<Recipe<RecipeWrapper>>> foundRecipe =
                level.getRecipeManager().getAllRecipesFor(type).stream()
                        .filter(r -> {
                            ManualApplicationRecipe mar = (ManualApplicationRecipe) r.value();
                            return mar.testBlock(blockState)
                                    && mar.ingredients.get(1).test(heldItem);
                        })
                        .findFirst();
        ...
        ManualApplicationRecipe recipe =
                (ManualApplicationRecipe) foundRecipe.get().value();
        ...
        recipe.rollResults(level.random).forEach(stack -> Block.popResource(level, pos, stack));
    ...
    public static RecipeHolder<DeployerApplicationRecipe> asDeploying(RecipeHolder<?> recipe) {
        ...
        ResourceLocation id = AllRecipeTypes.CAN_BE_AUTOMATED.test(recipe)
                ? recipe.id().withSuffix("_using_deployer")
                : recipe.id();
