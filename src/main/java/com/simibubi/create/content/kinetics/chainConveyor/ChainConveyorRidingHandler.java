...
public class ChainConveyorRidingHandler {
    ...
    public static void clientTick() {
        ...
        Vec3 playerPosition = mc.player
                .position()
                .add(0, mc.player.getBoundingBox().getYsize() + chainYOffset, 0);
        ...
        if (ridingConnection != null) {
            ...
            targetPosition = stats.start()
                    .add((stats.end().subtract(stats.start()))
                            .normalize()
        ...
        if (AnimationTickHolder.getTicks() % 10 == 0)
            CatnipServices.NETWORK.sendToServer(
                    new ServerboundChainConveyorRidingPacket(ridingChainConveyor, false));
    }

    private static void stopRiding() {
        if (ridingChainConveyor != null)
            CatnipServices.NETWORK.sendToServer(
                    new ServerboundChainConveyorRidingPacket(ridingChainConveyor, true));
    ...
    private static void updateTargetPosition(Minecraft mc, ChainConveyorBlockEntity clbe) {
        ...
        if (ridingConnection != null) {
            ...
            if (chainPosition < stats.chainLength()) return;

            // transfer to other
            if (mc.level.getBlockEntity(clbe.getBlockPos().offset(ridingConnection))
                    instanceof ChainConveyorBlockEntity clbe2) {
                chainPosition = clbe.wrapAngle(
                        stats.tangentAngle() + 180 + 2 * 35 * (clbe.reversed ? -1 : 1));
        ...
        if (!clbe.loopThresholdCrossed(chainPosition, prevChainPosition, offBranchAngle)) return;
