...
public class ChainPackageInteractionHandler {
    ...
    public static boolean onUse() {
        ...
        ChainConveyorPackage.physicsDataCache.get(mc.level).asMap().forEach((i, data) -> {
            if (success.booleanValue()) return;
            if (data == null || data.targetPos == null || data.beReference == null) return;
            AABB bounds = new AABB(data.targetPos, data.targetPos)
                    .move(0, -.25, 0)
            ...
            if (bounds.clip(from, to).isEmpty()) return;

            ChainConveyorBlockEntity ccbe = data.beReference.get();
            if (ccbe == null || ccbe.isRemoved()) return;
            ...
            for (ChainConveyorPackage pckg : ccbe.getLoopingPackages()) {
                if (pckg.netId == i) {
                    CatnipServices.NETWORK.sendToServer(new ChainPackageInteractionPacket(
                            ccbe.getBlockPos(), null, pckg.chainPosition, true));
                    success.setTrue();
                    return;
                }
            }
            ...
            for (BlockPos connection : ccbe.connections) {
                ...
                for (ChainConveyorPackage pckg : list) {
                    if (pckg.netId == i) {
                        CatnipServices.NETWORK.sendToServer(new ChainPackageInteractionPacket(
                                ccbe.getBlockPos(), connection, pckg.chainPosition, true));
                        success.setTrue();
                        return;
                    }
                }
