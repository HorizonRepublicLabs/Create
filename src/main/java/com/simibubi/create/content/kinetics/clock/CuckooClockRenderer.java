...
public class CuckooClockRenderer extends KineticBlockEntityRenderer<CuckooClockBlockEntity> {
    ...
    protected void renderSafe(
        ...
        // Render Hands
        SuperByteBuffer hourHand =
                CachedBuffers.partial(AllPartialModels.CUCKOO_HOUR_HAND, blockState);
        SuperByteBuffer minuteHand =
                CachedBuffers.partial(AllPartialModels.CUCKOO_MINUTE_HAND, blockState);
        ...
        // Doors
        SuperByteBuffer leftDoor =
                CachedBuffers.partial(AllPartialModels.CUCKOO_LEFT_DOOR, blockState);
        SuperByteBuffer rightDoor =
                CachedBuffers.partial(AllPartialModels.CUCKOO_RIGHT_DOOR, blockState);
        float angle = 0;
        float offset = 0;
        ...
        if (be.animationType != Animation.NONE) {
            ...
            PartialModel partialModel = (be.animationType == Animation.PIG
            ...
            figure.rotateCentered(
                    AngleHelper.rad(AngleHelper.horizontalAngle(direction.getCounterClockWise())),
                    Direction.UP);
    ...
    protected SuperByteBuffer getRotatedModel(CuckooClockBlockEntity be, BlockState state) {
        return CachedBuffers.partialFacing(
                ...
                state,
                state.getValue(CuckooClockBlock.HORIZONTAL_FACING).getOpposite());
    }
    ...
    private SuperByteBuffer rotateHand(SuperByteBuffer buffer, float angle, Direction facing) {
        ...
        buffer.rotateCentered(
                AngleHelper.rad(AngleHelper.horizontalAngle(facing.getCounterClockWise())),
                Direction.UP);
    ...
    private SuperByteBuffer rotateDoor(
            SuperByteBuffer buffer, float angle, boolean left, Direction facing) {
        float pivotX = 2 / 16f;
        float pivotY = 0;
