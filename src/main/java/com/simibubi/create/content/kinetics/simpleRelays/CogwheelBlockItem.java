...
public class CogwheelBlockItem extends BlockItem {
    ...
    public InteractionResult onItemUseFirst(ItemStack stack, UseOnContext context) {
        ...
        BlockHitResult ray =
                new BlockHitResult(context.getClickLocation(), context.getClickedFace(), pos, true);
    ...
    private static class SmallCogHelper extends DiagonalCogHelper {
        ...
        public Predicate<ItemStack> getItemPredicate() {
            return ((Predicate<ItemStack>) ICogWheel::isSmallCogItem)
                    .and(ICogWheel::isDedicatedCogItem);
        }
        ...
        public PlacementOffset getOffset(
            ...
            if (!ICogWheel.isLargeCog(state)) {
                ...
                List<Direction> directions =
                        IPlacementHelper.orderedByDistanceExceptAxis(pos, ray.getLocation(), axis);
    ...
    private static class LargeCogHelper extends DiagonalCogHelper {
        ...
        @Override
        public Predicate<ItemStack> getItemPredicate() {
            return ((Predicate<ItemStack>) ICogWheel::isLargeCogItem)
                    .and(ICogWheel::isDedicatedCogItem);
        }
        ...
        public PlacementOffset getOffset(
            ...
            if (ICogWheel.isLargeCog(state)) {
                Axis axis = ((IRotate) state.getBlock()).getRotationAxis(state);
                Direction side = IPlacementHelper.orderedByDistanceOnlyAxis(
                                pos, ray.getLocation(), axis)
                        .get(0);
                List<Direction> directions =
                        IPlacementHelper.orderedByDistanceExceptAxis(pos, ray.getLocation(), axis);
                for (Direction dir : directions) {
    ...
    public abstract static class DiagonalCogHelper implements IPlacementHelper {
        ...
        public PlacementOffset getOffset(
            ...
            Axis axis = ((IRotate) state.getBlock()).getRotationAxis(state);
            Direction closest = IPlacementHelper.orderedByDistanceExceptAxis(
                            pos, ray.getLocation(), axis)
                    .get(0);
            List<Direction> directions = IPlacementHelper.orderedByDistanceExceptAxis(
                    pos, ray.getLocation(), axis, d -> d.getAxis() != closest.getAxis());

            for (Direction dir : directions) {
                BlockPos newPos = pos.relative(dir).relative(closest);
                if (!world.getBlockState(newPos).canBeReplaced()) continue;

                if (!CogWheelBlock.isValidCogwheelPosition(
                        ICogWheel.isLargeCog(state), world, newPos, axis)) continue;
    ...
    public static class IntegratedLargeCogHelper implements IPlacementHelper {
        ...
        public PlacementOffset getOffset(
            ...
            if (state.hasProperty(HorizontalKineticBlock.HORIZONTAL_FACING))
                newAxis =
                        state.getValue(HorizontalKineticBlock.HORIZONTAL_FACING).getAxis();
            ...
            List<Direction> directions = IPlacementHelper.orderedByDistanceExceptAxis(
                    pos, ray.getLocation(), face.getAxis(), newAxis);
            ...
            for (Direction d : directions) {
                ...
                return PlacementOffset.success(
                        newPos, s -> s.setValue(CogWheelBlock.AXIS, newAxis));
            }
    ...
    public static class IntegratedSmallCogHelper implements IPlacementHelper {
        ...
        public PlacementOffset getOffset(
            ...
            List<Direction> directions =
                    IPlacementHelper.orderedByDistanceExceptAxis(pos, ray.getLocation(), newAxis);
