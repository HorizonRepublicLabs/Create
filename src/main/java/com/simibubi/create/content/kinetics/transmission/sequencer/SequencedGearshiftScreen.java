...
public class SequencedGearshiftScreen extends AbstractSimiScreen {
    ...
    public void updateParamsOfRow(int row) {
        ...
        if (def == SequencerInstructions.DELAY) {
            value.withStepFunction(context -> {
                ...
                if (!context.forward) v--;
                if (v < 20) return context.shift ? 20 : 1;
                return context.shift ? 100 : 20;
            });
        } else value.withStepFunction(value.standardStep());

        ScrollInput modifier = rowInputs.get(2);
        modifier.active = modifier.visible = hasModifier;
        if (hasModifier) modifier.setState(instruction.speedModifier.ordinal());
    }
    ...
    private void renderAdditional(
        ...
        GuiGameElement.of(renderedItem)
                .<GuiGameElement.GuiRenderBuilder>at(
                        guiLeft + background.getWidth() + 6,
                        guiTop + background.getHeight() - 56,
                        100)
                .scale(5)
                .render(graphics);
    }
    ...
    public void sendPacket() {
        CatnipServices.NETWORK.sendToServer(
                new ConfigureSequencedGearshiftPacket(be.getBlockPos(), instructions));
    }
