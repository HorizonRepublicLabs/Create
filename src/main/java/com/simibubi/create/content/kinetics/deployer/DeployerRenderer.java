...
public class DeployerRenderer extends SafeBlockEntityRenderer<DeployerBlockEntity> {
    ...
    protected void renderItem(
        ...
        Vec3 offset = getHandOffset(be, partialTicks, deployerState)
        ...
        if (punching) ms.translate(0, 1 / 8f, -1 / 16f);
    ...
    protected void renderComponents(
        ...
        if (!VisualizationManager.supportsVisualization(be.getLevel())) {
            KineticBlockEntityRenderer.renderRotatingKineticBlock(
                    be, getRenderedBlockState(be), ms, vb, light);
        }
    ...
    protected Vec3 getHandOffset(
            DeployerBlockEntity be, float partialTicks, BlockState blockState) {
    ...
    private static SuperByteBuffer transform(
            SuperByteBuffer buffer, BlockState deployerState, boolean axisDirectionMatters) {
        Direction facing = deployerState.getValue(FACING);

        float yRot = AngleHelper.horizontalAngle(facing);
        float xRot = facing == Direction.UP ? 270 : facing == Direction.DOWN ? 90 : 0;
        float zRot = axisDirectionMatters
                        && (deployerState.getValue(AXIS_ALONG_FIRST_COORDINATE)
    ...
    public static void renderInContraption(
        ...
        if (context.contraption.stalled
                ...
                || context.data.contains("StationaryTimer")) {
            ...
            factor = .5f
                    - Mth.clamp(
                            Mth.lerp(AnimationTickHolder.getPartialTicks(), distance, nextDistance),
                            0,
                            1);
        }

        Vec3 offset =
                Vec3.atLowerCornerOf(blockState.getValue(FACING).getNormal()).scale(factor);
        ...
        if (!context.disabled) m.translate(offset.x, offset.y, offset.z);
        pole.transform(m);
        hand.transform(m);
    ...
    static PartialModel getHandPose(DeployerBlockEntity.Mode mode) {
        return mode == DeployerBlockEntity.Mode.PUNCH
