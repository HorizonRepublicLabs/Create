...
public class BlockBreakingMovementBehaviour implements MovementBehaviour {
    ...
    public void startMoving(MovementContext context) {
        ...
        context.data.putInt(
                "BreakerId", -BlockBreakingKineticBlockEntity.NEXT_BREAKER_ID.incrementAndGet());
    }
    ...
    public void visitNewPosition(MovementContext context, BlockPos pos) {
        ...
        if (!canBreak(world, pos, stateVisited)) return;
            ...
            if (entity instanceof AbstractMinecart)
                for (Entity passenger : entity.getIndirectPassengers())
                    if (passenger instanceof AbstractContraptionEntity
                            && ((AbstractContraptionEntity) passenger).getContraption()
                                    == context.contraption) continue Entities;

            if (damageSource != null && !world.isClientSide) {
                float damage = (float)
                        Mth.clamp(6 * Math.pow(context.relativeMotion.length(), 0.4) + 1, 2, 10);
    ...
    protected void throwEntity(MovementContext context, Entity entity) {
        ...
        if (motionBoost.length() > maxBoost) {
            motionBoost = motionBoost.subtract(
                    motionBoost.normalize().scale(motionBoost.length() - maxBoost));
        }
    ...
    public void cancelStall(MovementContext context) {
        ...
        if (context.world.isClientSide) return;
        if (!data.contains("BreakingPos")) return;
    ...
    public void tick(MovementContext context) {
        ...
        CompoundTag data = context.data;
        if (!data.contains("WaitingTicks")) return;
    ...
    public void tickBreaker(MovementContext context) {
        ...
        world.playSound(
                null,
                breakingPos,
                stateToBreak.getSoundType(world, breakingPos, null).getHitSound(),
                SoundSource.NEUTRAL,
                .25f,
                1);
        ...
        if (destroyProgress >= 10) {
            ...
            context.stall = false;
            if (shouldDestroyStartBlock(stateToBreak)) destroyBlock(context, breakingPos);
    ...
    protected void destroyBlock(MovementContext context, BlockPos breakingPos) {
        BlockHelper.destroyBlock(
                context.world, breakingPos, 1f, stack -> this.dropItem(context, stack));
    }
    ...
    protected void onBlockBroken(MovementContext context, BlockPos pos, BlockState brokenState) {
        ...
        if (!(brokenState.getBlock() instanceof FallingBlock)) return;
