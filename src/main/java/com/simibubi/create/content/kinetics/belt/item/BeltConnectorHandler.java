...
public class BeltConnectorHandler {
    public static void tick() {
        ...
        for (InteractionHand hand : InteractionHand.values()) {
            ...
            if (!AllItems.BELT_CONNECTOR.isIn(heldItem)) continue;

            if (!heldItem.has(AllDataComponents.BELT_FIRST_SHAFT)) continue;
            ...
            if (rayTrace == null || !(rayTrace instanceof BlockHitResult)) {
                if (random.nextInt(50) == 0) {
                    level.addParticle(
                            new DustParticleOptions(new Vector3f(.3f, .9f, .5f), 1),
            ...
            if (!selected.closerThan(
                    first, AllConfigs.server().kinetics.maxBeltLength.get())) return;

            boolean canConnect = BeltConnectorItem.validateAxis(level, selected)
                ...
                if (random.nextInt(10) == 0) {
                    level.addParticle(
                            new DustParticleOptions(
                                    new Vector3f(
                                            canConnect ? .3f : .9f, canConnect ? .9f : .3f, .5f),
                                    1),
                            position.x + .5f,
                            position.y + .5f,
                            position.z + .5f,
