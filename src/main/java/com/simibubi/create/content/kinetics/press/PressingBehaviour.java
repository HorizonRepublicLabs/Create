...
public class PressingBehaviour extends BeltProcessingBehaviour {
	...
	public void read(CompoundTag compound, HolderLookup.Provider registries, boolean clientPacket) {
		...
		if (clientPacket) {
			NBTHelper.iterateCompoundList(compound.getListOrEmpty("ParticleItems"),
				c -> particleItems.add(StackNbt.parse(registries, c)));
			spawnParticles();
		}
	}
	...
	public void write(CompoundTag compound, HolderLookup.Provider registries, boolean clientPacket) {
		...
		if (clientPacket) {
			compound.put("ParticleItems", NBTHelper.writeCompoundList(particleItems, s -> (CompoundTag) StackNbt.save(registries, s)));
			particleItems.clear();
		}
	}
	...
	public void makePressingParticleEffect(Vec3 pos, ItemStack stack, int amount) {
		...
		for (int i = 0; i < amount; i++) {
			...
			level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, ItemHelper.asTemplate(stack)), pos.x, pos.y - .25f, pos.z, motion.x,
				motion.y, motion.z);
		}
	}
	...
	public void makeCompactingParticleEffect(Vec3 pos, ItemStack stack) {
		...
		for (int i = 0; i < 20; i++) {
			Vec3 motion = VecHelper.offsetRandomly(Vec3.ZERO, level.getRandom(), .175f)
				.multiply(1, 0, 1);
			level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, ItemHelper.asTemplate(stack)), pos.x, pos.y, pos.z, motion.x,
