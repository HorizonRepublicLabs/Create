...
public class PressingBehaviour extends BeltProcessingBehaviour {
    ...
    public interface PressingBehaviourSpecifics {
        ...
        boolean tryProcessOnBelt(
                TransportedItemStack input, List<ItemStack> outputList, boolean simulate);
    ...
    public void read(CompoundTag compound, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        if (clientPacket) {
            NBTHelper.iterateCompoundList(
                    compound.getList("ParticleItems", Tag.TAG_COMPOUND),
    ...
    public void write(
        ...
        if (clientPacket) {
            compound.put("ParticleItems", NBTHelper.writeCompoundList(particleItems, s ->
                    (CompoundTag) s.saveOptional(registries)));
            particleItems.clear();
        }
    }
    ...
    public void tick() {
        ...
        if (!running || level == null) {
            if (level != null && !level.isClientSide) {
                ...
                if (specifics.getKineticSpeed() == 0) return;
                ...
                if (entityScanCooldown <= 0) {
                    ...
                    if (BlockEntityBehaviour.get(
                            ...
                            != null) return;
                    if (BasinBlock.isBasin(level, worldPosition.below(2))) return;

                    for (ItemEntity itemEntity : level.getEntitiesOfClass(
                            ItemEntity.class, new AABB(worldPosition.below()).deflate(.125f))) {
        ...
        if (runningTicks == CYCLE / 2 && specifics.getKineticSpeed() != 0) {
            ...
            if (level.getBlockState(worldPosition.below(2)).getSoundType() == SoundType.WOOL)
                AllSoundEvents.MECHANICAL_PRESS_ACTIVATION_ON_BELT.playOnServer(
                        level, worldPosition);
            else
    ...
    protected void applyOnBasin() {
        ...
        particleItems.clear();
        if (specifics.tryProcessInBasin(false)) blockEntity.sendData();
    }
    ...
    protected void applyInWorld() {
        ...
        for (Entity entity : level.getEntities(null, bb)) {
            ...
            if (specifics.tryProcessInWorld(itemEntity, false)) blockEntity.sendData();
            if (!bulk) break;
        }
    }
    ...
    protected void spawnParticles() {
        ...
        if (mode == Mode.BASIN)
            particleItems.forEach(stack -> makeCompactingParticleEffect(
                    VecHelper.getCenterOf(worldPosition.below(2)), stack));
        if (mode == Mode.BELT)
    ...
    public void makePressingParticleEffect(Vec3 pos, ItemStack stack, int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            Vec3 motion =
                    VecHelper.offsetRandomly(Vec3.ZERO, level.random, .125f).multiply(1, 0, 1);
    ...
    public void makeCompactingParticleEffect(Vec3 pos, ItemStack stack) {
        ...
        for (int i = 0; i < 20; i++) {
            Vec3 motion =
                    VecHelper.offsetRandomly(Vec3.ZERO, level.random, .175f).multiply(1, 0, 1);
            level.addParticle(
                    new ItemParticleOption(ParticleTypes.ITEM, stack),
                    pos.x,
                    pos.y,
