...
public class ContraptionControlsRenderer
    ...
    protected void renderSafe(
        ...
        Vec3 buttonMovementAxis = VecHelper.rotate(
                new Vec3(0, 1, -.325), AngleHelper.horizontalAngle(facing), Axis.Y);
        Vec3 buttonMovement =
                buttonMovementAxis.scale(-0.07f + -1 / 24f * blockEntity.button.getValue(pt));
        ...
        CachedBuffers.partialFacing(
                        AllPartialModels.CONTRAPTION_CONTROLS_BUTTON, blockState, facing)
                .light(light)
                .renderInto(ms, vc);
    ...
    public static void renderInContraption(
        ...
        if (!(ctx.temporaryData instanceof ElevatorFloorSelection efs)) return;
        if (!AllBlocks.CONTRAPTION_CONTROLS.has(ctx.state)) return;

        Entity cameraEntity = Minecraft.getInstance().getCameraEntity();
        float playerDistance = (float)
                (ctx.position == null || cameraEntity == null
                        ? 0
                        : ctx.position.distanceToSqr(cameraEntity.getEyePosition()));

        float flicker = renderWorld.random.nextFloat();
        Couple<Integer> couple = DyeHelper.getDyeColors(
        ...
        if (ctx.contraption.getBlockEntityClientSide(ctx.localPos)
            ...
            buttondepth = -1
                    / 24f
                    * cbe.button.getValue(AnimationTickHolder.getPartialTicks(renderWorld));
        ...
        CachedBuffers.partialFacing(
                        ...
                        ctx.state,
                        ctx.state.getValue(ContraptionControlsBlock.FACING).getOpposite())
        ...
        if (!text.isBlank() && playerDistance < 100) {
            ...
            NixieTubeRenderer.drawInWorldString(
                    ms, buffer, text, Color.mixColors(darkColor, 0, .35f));
            ms.popPose();
        }
