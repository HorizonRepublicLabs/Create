...
public class ContraptionHandler {
    ...
    public static WorldAttached<Map<Integer, WeakReference<AbstractContraptionEntity>>>
            loadedContraptions;
    ...
    public static void tick(Level world) {
        ...
        for (Iterator<WeakReference<AbstractContraptionEntity>> iterator = values.iterator();
                iterator.hasNext(); ) {
    ...
    public static void entitiesWhoJustDismountedGetSentToTheRightLocation(
            LivingEntity entityLiving, Level world) {
        ...
        Vec3 position =
                VecHelper.readNBT(data.getList("ContraptionDismountLocation", Tag.TAG_DOUBLE));
        if (entityLiving.getVehicle() == null)
            entityLiving.absMoveTo(
                    position.x,
                    position.y,
                    position.z,
