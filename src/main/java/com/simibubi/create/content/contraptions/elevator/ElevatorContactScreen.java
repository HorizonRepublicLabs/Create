...
public class ElevatorContactScreen extends AbstractSimiScreen {
    ...
    public ElevatorContactScreen(
            BlockPos pos, String prevShortName, String prevLongName, DoorControl prevDoorControl) {
    ...
    private EditBox editBox(int x, int width, int chars) {
        EditBox editBox =
                new EditBox(font, guiLeft + x, guiTop + 30, width, 10, CommonComponents.EMPTY);
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.drawString(
                ...
                (float) (x
                        + (background.getWidth() - 8) / 2
                        - font.width(formattedcharsequence) / 2),
                (float) y + 6,
                0x2F3738,
                false);

        GuiGameElement.of(AllBlocks.ELEVATOR_CONTACT.asStack())
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        ...
        if (shortNameInput.isHoveredOrFocused()) longNameInput.mouseClicked(0, 0, 0);
    ...
    public boolean keyPressed(int keyCode, int p_keyPressed_2_, int p_keyPressed_3_) {
        if (super.keyPressed(keyCode, p_keyPressed_2_, p_keyPressed_3_)) return true;
    ...
    private void confirm() {
        CatnipServices.NETWORK.sendToServer(
                new ElevatorContactEditPacket(pos, shortName, longName, doorControl));
        onClose();
    }
}
