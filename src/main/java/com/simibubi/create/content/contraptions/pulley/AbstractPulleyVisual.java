...
public abstract class AbstractPulleyVisual<T extends KineticBlockEntity> extends ShaftVisual<T>
    ...
    public AbstractPulleyVisual(VisualizationContext dispatcher, T blockEntity, float partialTick) {
        ...
        rope = new SmartRecycler<>(
                b -> b ? getHalfRopeModel().createInstance() : getRopeModel().createInstance());
        ...
        public void setSize(int size) {
            if (size != data.size()) {
                ...
                int sectionCount = MoreMath.ceilingDiv(
                        size + 15 - pos.getY() + pos.getY() / 4 * 4, SectionPos.SECTION_SIZE);
