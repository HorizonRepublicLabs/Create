...
public class MinecartCouplingItem extends Item {
    ...
    public static void handleInteractionWithMinecart(PlayerInteractEvent.EntityInteract event) {
        ...
        MinecartController controller = minecart.getData(AllAttachmentTypes.MINECART_CONTROLLER);
        if (controller == MinecartController.EMPTY || !controller.isPresent()) return;
    ...
    private static boolean onWrenchInteractOnMinecart(
            ...
            Player player,
            MinecartController controller) {
        int couplings = (controller.isConnectedToCoupling() ? 1 : 0)
                + (controller.isLeadingCoupling() ? 1 : 0);
