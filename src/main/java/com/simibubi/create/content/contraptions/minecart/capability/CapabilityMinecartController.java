...
public class CapabilityMinecartController {
    ...
    public static void tick(Level world) {
        ...
        for (AbstractMinecart cart : queued) {
            ...
            if (controller != MinecartController.EMPTY) {
                ...
                if (controller.isLeadingCoupling()) cartsWithCoupling.add(uniqueID);
                if (!world.isClientSide && controller != null) controller.sendData();
            }
        }
    ...
    public static void entityTick(EntityTickEvent event) {
        ...
        if (data != MinecartController.EMPTY) data.tick();
    }
    ...
    public static void onChunkUnloaded(ChunkEvent.Unload event) {
        ...
        for (MinecartController minecartController : carts.values()) {
            if (minecartController == null) continue;
            if (!minecartController.isPresent()) continue;
            AbstractMinecart cart = minecartController.cart();
            if (cart.chunkPosition().equals(chunkPos))
                queuedUnloads.get(event.getLevel()).add(cart.getUUID());
        }
    }
    ...
    protected static void onCartRemoved(Level world, AbstractMinecart entity) {
        ...
        if (!carts.containsKey(uniqueID) || unloads.contains(uniqueID)) return;
        if (world.isClientSide) return;
        handleKilledMinecart(world, carts.get(uniqueID), entity.position());
    }

    protected static void handleKilledMinecart(
            Level world, MinecartController controller, Vec3 removedPos) {
        if (controller == null) return;
        for (boolean forward : Iterate.trueAndFalse) {
            MinecartController next =
                    CouplingHandler.getNextInCouplingChain(world, controller, forward);
            ...
            ItemEntity itemEntity = new ItemEntity(
                    world, itemPos.x, itemPos.y, itemPos.z, AllItems.MINECART_COUPLING.asStack());
    ...
    public static MinecartController getIfPresent(Level world, UUID cartId) {
        ...
        if (!carts.containsKey(cartId)) return null;
    ...
    public static void attach(EntityJoinLevelEvent event) {
        Entity entity = event.getEntity();
        if (!(entity instanceof AbstractMinecart abstractMinecart)) return;
    ...
    public static void startTracking(PlayerEvent.StartTracking event) {
        ...
        if (controller != MinecartController.EMPTY) controller.sendData(abstractMinecart);
    }
}
