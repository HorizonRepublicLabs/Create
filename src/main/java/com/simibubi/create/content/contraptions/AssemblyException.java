...
public class AssemblyException extends Exception {
    ...
    public static void write(
            CompoundTag compound, HolderLookup.Provider registries, AssemblyException exception) {
    ...
    public static AssemblyException read(CompoundTag compound, HolderLookup.Provider registries) {
        if (!compound.contains("LastException")) return null;

        CompoundTag nbt = compound.getCompound("LastException");
        String string = nbt.getString("Component");
        AssemblyException exception =
                new AssemblyException(Component.Serializer.fromJson(string, registries));
        if (nbt.contains("Position")) exception.position = BlockPos.of(nbt.getLong("Position"));
    ...
    public static AssemblyException unloadedChunk(BlockPos pos) {
        AssemblyException e =
                new AssemblyException("chunkNotLoaded", pos.getX(), pos.getY(), pos.getZ());
        e.position = pos;
        return e;
    }
    ...
    public static AssemblyException structureTooLarge() {
        return new AssemblyException(
                "structureTooLarge", AllConfigs.server().kinetics.maxBlocksMoved.get());
    }
