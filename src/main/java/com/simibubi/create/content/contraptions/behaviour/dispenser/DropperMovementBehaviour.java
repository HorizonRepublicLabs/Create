...
public class DropperMovementBehaviour implements MovementBehaviour {
    ...
    public void visitNewPosition(MovementContext context, BlockPos pos) {
        ...
        int slot = getSlot(
                storage, context.world.random, context.contraption.getStorage().getAllItems());
        if (slot == -1) {
            // all slots empty
    ...
    private static int getSlot(
            MountedItemStorage storage, RandomSource random, IItemHandler contraptionInventory) {
    ...
    private static ItemStack tryTopOff(ItemStack stack, IItemHandler from) {
        Predicate<ItemStack> test =
                otherStack -> ItemStack.isSameItemSameComponents(stack, otherStack);
        int needed = stack.getMaxStackSize() - stack.getCount();

        ItemStack extracted =
                ItemHelper.extract(from, test, ItemHelper.ExtractionCountMode.UPTO, needed, false);
        return extracted.isEmpty()
                ? null
                : stack.copyWithCount(stack.getCount() + extracted.getCount());
    }
