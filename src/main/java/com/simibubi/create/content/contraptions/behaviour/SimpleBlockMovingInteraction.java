...
public abstract class SimpleBlockMovingInteraction extends MovingInteractionBehaviour {
    ...
    public boolean handlePlayerInteraction(
        ...
        if (info.state() == newState) return false;

        setContraptionBlockData(
                contraptionEntity,
                localPos,
                new StructureBlockInfo(info.pos(), newState, info.nbt()));
        if (updateColliders()) contraption.invalidateColliders();
        return true;
    }
    ...
    protected void playSound(Player player, SoundEvent sound, float pitch) {
        player.level()
                .playSound(player, player.blockPosition(), sound, SoundSource.BLOCKS, 0.3f, pitch);
    }

    protected abstract BlockState handle(
