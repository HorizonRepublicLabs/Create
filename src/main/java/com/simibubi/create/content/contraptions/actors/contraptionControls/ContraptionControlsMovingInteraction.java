...
public class ContraptionControlsMovingInteraction extends MovingInteractionBehaviour {
    ...
    public boolean handlePlayerInteraction(
        ...
        if (!(contraptionEntity instanceof CarriageContraptionEntity cce)) return true;
        if (!filter.is(ItemTags.DOORS)) return true;
    ...
    private void send(
            AbstractContraptionEntity contraptionEntity, ItemStack filter, boolean disable) {
    ...
    private boolean elevatorInteraction(
        ...
        if (!(ctx.temporaryData instanceof ElevatorFloorSelection efs)) return false;
        if (efs.currentTargetY == contraption.clientYTarget) return true;
