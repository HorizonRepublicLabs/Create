...
public class MovementContext {
    ...
    public MovementContext(Level world, StructureBlockInfo info, Contraption contraption) {
        ...
        this.fluidStorage = Suppliers.memoize(
                () -> contraption.getStorage().getFluids().storages.get(this.localPos));
    }
    ...
    public float getAnimationSpeed() {
        ...
        if (world.isClientSide && contraption.stalled) return 700;
        if (Math.abs(length) < 1 / 512f) return 0;
        return (((int) (length * modifier + 100 * Math.signum(length))) / 100) * 100;
    }

    public static MovementContext readNBT(
            Level world, StructureBlockInfo info, CompoundTag nbt, Contraption contraption) {
    ...
    public CompoundTag writeToNBT(CompoundTag nbt) {
        ...
        if (position != null) nbt.put("Position", VecHelper.writeNBT(position));
    ...
    public FilterItemStack getFilterFromBE() {
        ...
        return filter =
                FilterItemStack.of(world.registryAccess(), blockEntityData.getCompound("Filter"));
    }
