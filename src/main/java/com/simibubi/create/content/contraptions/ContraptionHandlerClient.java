...
public class ContraptionHandlerClient {
    ...
    public static void preventRemotePlayersWalkingAnimations(PlayerTickEvent.Post event) {
        if (!(event.getEntity() instanceof RemotePlayer remotePlayer)) return;

        CompoundTag data = remotePlayer.getPersistentData();
        if (!data.contains("LastOverrideLimbSwingUpdate")) return;
    ...
    @SubscribeEvent
    @OnlyIn(Dist.CLIENT)
    public static void rightClickingOnContraptionsGetsHandledLocally(
        ...
        for (WeakReference<AbstractContraptionEntity> ref : contraptions) {
            ...
            double distance = contraptionEntity
                    .toGlobalVector(rayTraceResult.getLocation(), 1)
                    .distanceTo(origin);
        ...
        if (bestEntity.handlePlayerInteraction(player, pos, face, hand)) {
            CatnipServices.NETWORK.sendToServer(
                    new ContraptionInteractionPacket(bestEntity, hand, pos, face));
        } else handleSpecialInteractions(bestEntity, player, pos, face, hand);
    ...
    private static boolean handleSpecialInteractions(
        ...
        if (AllItems.WRENCH.isIn(player.getItemInHand(interactionHand))
            ...
            return TrainRelocator.carriageWrenched(
                    car.toGlobalVector(VecHelper.getCenterOf(localPos), 1), car);
        return false;
    }
    ...
    public static BlockHitResult rayTraceContraption(
            Vec3 origin, Vec3 target, AbstractContraptionEntity contraptionEntity) {
        ...
        PredicateTraceResult predicateResult =
                RaycastHelper.rayTraceUntil(localOrigin, localTarget, p -> {
                    for (Direction d : Iterate.directions) {
                        ...
                        VoxelShape raytraceShape = state.getShape(
                                contraption.getContraptionWorld(), BlockPos.ZERO.below());
