...
public record ClientMotionPacket(Vec3 motion, boolean onGround, float limbSwing)
        implements ServerboundPacketPayload {
    public static final StreamCodec<ByteBuf, ClientMotionPacket> STREAM_CODEC =
    ...
    public void handle(ServerPlayer sender) {
        ...
        if (onGround) {
            sender.causeFallDamage(
                    sender.fallDistance, 1, sender.damageSources().fall());
        ...
        CatnipServices.NETWORK.sendToClientsTrackingEntity(
                sender, new LimbSwingUpdatePacket(sender.getId(), sender.position(), limbSwing));
    }
}
