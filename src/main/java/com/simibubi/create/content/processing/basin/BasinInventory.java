...
public class BasinInventory extends SmartInventory {
    ...
    public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
        ...
        for (int i = 0; i < getSlots(); i++) {
            ...
            if (i != slot && ItemStack.isSameItemSameComponents(stack, inv.getStackInSlot(i)))
                return stack;
            if (inv.getStackInSlot(i).isEmpty() && firstFreeSlot == -1) firstFreeSlot = i;
        }

        // Only insert if this is the first empty slot, prevents overfilling in the
        // simulation pass
        if (inv.getStackInSlot(slot).isEmpty() && firstFreeSlot != slot) return stack;
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!simulate && !extractItem.isEmpty()) blockEntity.notifyChangeOfContents();
        return extractItem;
    }
}
