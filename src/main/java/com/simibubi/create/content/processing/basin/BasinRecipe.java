...
public class BasinRecipe extends StandardProcessingRecipe<RecipeInput> {
    ...
    private static boolean apply(BasinBlockEntity basin, Recipe<?> recipe, boolean test) {
        ...
        IItemHandler availableItems = basin.getLevel()
                .getCapability(Capabilities.ItemHandler.BLOCK, basin.getBlockPos(), null);
        IFluidHandler availableFluids = basin.getLevel()
        ...
        for (boolean simulate : Iterate.trueAndFalse) {
            ...
            for (Ingredient ingredient : ingredients) {
                for (int slot = 0; slot < availableItems.getSlots(); slot++) {
                    if (simulate
                            && availableItems.getStackInSlot(slot).getCount()
                                    <= extractedItemsFromSlot[slot]) continue;
            ...
            if (simulate) {
                CraftingInput remainderInput = new DummyCraftingContainer(
                                availableItems, extractedItemsFromSlot)
                        .asCraftInput();
    ...
    public static RecipeHolder<BasinRecipe> convertShapeless(RecipeHolder<?> recipe) {
        BasinRecipe basinRecipe = new Builder<>(BasinRecipe::new, recipe.id())
                ...
                .withSingleItemOutput(recipe.value()
