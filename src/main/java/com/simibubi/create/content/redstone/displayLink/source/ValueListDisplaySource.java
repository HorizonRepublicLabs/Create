...
public abstract class ValueListDisplaySource extends DisplaySource {
    ...
    protected abstract Stream<IntAttached<MutableComponent>> provideEntries(
    ...
    public List<MutableComponent> provideText(
        ...
        List<MutableComponent> list = provideEntries(
                        context, stats.maxRows() * (isBook ? ENTRIES_PER_PAGE : 1))
                .map(e -> createComponentsFromEntry(context, e))
                .map(l -> {
                    MutableComponent combined = l.get(0).append(l.get(1));
                    if (l.size() > 2) combined.append(l.get(2));
                    return combined;
                })
                .toList();

        if (isBook) list = condensePages(list);
    ...
    private List<MutableComponent> condensePages(List<MutableComponent> list) {
        ...
        if (current != null) condensed.add(current);
        return condensed;
    }
    ...
    public List<List<MutableComponent>> provideFlapDisplayText(
        ...
        return provideEntries(context, stats.maxRows())
                .map(e -> {
    ...
    protected List<MutableComponent> createComponentsFromEntry(
            DisplayLinkContext context, IntAttached<MutableComponent> entry) {
        ...
        if (shortenNumbers(context)) {
            ...
            return valueFirst()
                    ? Arrays.asList(shortened.getFirst(), shortened.getSecond(), name)
                    : Arrays.asList(name, shortened.getFirst(), shortened.getSecond());
        }

        MutableComponent formattedNumber =
                Component.literal(String.valueOf(number)).append(WHITESPACE);
        return valueFirst()
                ? Arrays.asList(formattedNumber, name)
    ...
    public void loadFlapDisplayLayout(
        ...
        int valueFormat = shortenNumbers
                ? 0
                : Math.max(
                        4,
        ...
        FlapDisplaySection name =
                new FlapDisplaySection(MONOSPACE * nameLength, "alphabet", false, !valueFirst);
        FlapDisplaySection value = new FlapDisplaySection(
                        MONOSPACE * numberLength, "number", false, !shortenNumbers && valueFirst)
                .rightAligned();

        if (shortenNumbers) {
            FlapDisplaySection suffix =
                    new FlapDisplaySection(MONOSPACE, "shortened_numbers", false, valueFirst);
            layout.configure(
                    layoutKey,
                    valueFirst
                            ? Arrays.asList(value, suffix, name)
                            : Arrays.asList(name, value, suffix));
            return;
        }

        layout.configure(
                layoutKey, valueFirst ? Arrays.asList(value, name) : Arrays.asList(name, value));
    }
    ...
    public void initConfigurationWidgets(
        ...
        if (isFirstLine) addFullNumberConfig(builder);
    }
    ...
    protected void addFullNumberConfig(ModularGuiLineBuilder builder) {
        builder.addSelectionScrollInput(
                ...
                (si, l) -> si.forOptions(CreateLang.translatedOptions(
                                "display_source.value_list", "shortened", "full_number"))
