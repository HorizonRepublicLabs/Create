...
public class FluidListDisplaySource extends ValueListDisplaySource {
    ...
    protected Stream<IntAttached<MutableComponent>> provideEntries(
        ...
        if (!(sourceBE instanceof SmartObserverBlockEntity cobe)) return Stream.empty();
        ...
        return fluids.entrySet().stream()
                .sorted(Comparator.<Map.Entry<Fluid, Integer>>comparingInt(
    ...
    protected List<MutableComponent> createComponentsFromEntry(
        ...
        Couple<MutableComponent> formatted =
                FluidFormatter.asComponents(amount, shortenNumbers(context));
    ...
    public void loadFlapDisplayLayout(
        ...
        FlapDisplaySection value = new FlapDisplaySection(
                        FlapDisplaySection.MONOSPACE * numberLength, "number", false, false)
                .rightAligned();
        FlapDisplaySection unit =
                new FlapDisplaySection(FlapDisplaySection.MONOSPACE * 2, "fluid_units", true, true);
        FlapDisplaySection name = new FlapDisplaySection(
                FlapDisplaySection.MONOSPACE * nameLength, "alphabet", false, false);
