...
public abstract class SingleLineDisplaySource extends DisplaySource {
    ...
    protected abstract MutableComponent provideLine(
    ...
    public void initConfigurationWidgets(
        ...
        if (isFirstLine && allowsLabeling(context)) addLabelingTextBox(builder);
    }
    ...
    public List<MutableComponent> provideText(
        ...
        if (line == EMPTY_LINE) return EMPTY;
        ...
        if (allowsLabeling(context)) {
            ...
            if (!label.isEmpty()) {
                return ImmutableList.of(ImmutableList.of(
                        Component.literal(label + " "), provideLine(context, stats)));
            }
        }
    ...
    public void loadFlapDisplayLayout(
        ...
        if (!allowsLabeling(context)) {
            if (!layout.isLayout(layoutKey))
                layout.configure(
                        layoutKey,
                        ImmutableList.of(
                                createSectionForValue(context, flapDisplay.getMaxCharCount())));
            return;
        }
        ...
        if (label.length() + 1 < maxCharCount)
            layout.configure(
                    ...
                    ImmutableList.of(
                            labelSection,
                            createSectionForValue(context, maxCharCount - label.length() - 1)));
        else layout.configure(layoutName, ImmutableList.of(labelSection));
    }
