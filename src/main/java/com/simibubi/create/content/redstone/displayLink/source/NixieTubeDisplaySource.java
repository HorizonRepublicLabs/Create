...
public class NixieTubeDisplaySource extends SingleLineDisplaySource {
    ...
    protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
        ...
        if (!(sourceBE instanceof NixieTubeBlockEntity nbe)) return EMPTY_LINE;
    ...
    protected String getFlapDisplayLayoutName(DisplayLinkContext context) {
        if (isNumeric(context)) return "Number";
    ...
    protected FlapDisplaySection createSectionForValue(DisplayLinkContext context, int size) {
        if (isNumeric(context))
            return new FlapDisplaySection(
                    size * FlapDisplaySection.MONOSPACE, "numeric", false, false);
