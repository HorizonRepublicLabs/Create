...
public class StationSummaryDisplaySource extends DisplaySource {
    ...
    static {
        EMPTY_ENTRY_5 = ImmutableList.of(
                WHITESPACE, Component.literal(" . "), WHITESPACE, WHITESPACE, WHITESPACE);
    }
    ...
    public List<List<MutableComponent>> provideFlapDisplayText(
        ...
        GlobalTrainDisplayData.prepare(filter, stats.maxRows()).forEach(prediction -> {
                ...
                lines.add(
                        min > 0
                                ? CreateLang.translateDirect(
                                        "display_source.station_summary.minutes")
                                : CreateLang.translateDirect(
                                        "display_source.station_summary.seconds", sec));
            }
            ...
            for (String string : filter.split("\\*"))
                if (!string.isEmpty()) platform = platform.replace(string, "");
    ...
    public void loadFlapDisplayLayout(
        ...
        if (hasPlatform) totalSize = totalSize - gapSize - platformWidth;
        if (platformWidth == 0 && hasPlatform) totalSize += gapSize;

        int trainNameWidth = (int) ((columnWidth / 100f) * totalSize / MONOSPACE);
        int destinationWidth = Math.round((1 - columnWidth / 100f) * totalSize / MONOSPACE);

        FlapDisplaySection trainName = new FlapDisplaySection(
                trainNameWidth * MONOSPACE, "alphabet", false, trainNameWidth > 0);
        ...
        FlapDisplaySection platform =
                new FlapDisplaySection(platformWidth, "numeric", false, false).rightAligned();
        ...
        if (hasPlatform) list.add(platform);
    ...
    public void populateData(DisplayLinkContext context) {
        ...
        if (!conf.contains("PlatformColumn")) conf.putInt("PlatformColumn", 3);
        if (!conf.contains("NameColumn")) conf.putInt("NameColumn", 50);

        if (conf.contains("Filter")) return;
        if (!(context.getSourceBlockEntity() instanceof StationBlockEntity stationBe)) return;
    ...
    public void initConfigurationWidgets(
        ...
        builder.addScrollInput(
                ...
                32,
                (si, l) -> {
                    si.titled(CreateLang.translateDirect(
                                    "display_source.station_summary.train_name_column"))
                            .withRange(0, 73)
                            .withShiftStep(12);
                    si.setState(50);
                    l.withSuffix("%");
        ...
        builder.addScrollInput(
                ...
                (si, l) -> {
                    si.titled(CreateLang.translateDirect(
                                    "display_source.station_summary.platform_column"))
                            .withRange(0, 16)
                            .withShiftStep(4);
                    si.setState(3);
                },
