...
public class ThresholdSwitchScreen extends AbstractSimiScreen {
    ...
    protected void init() {
        ...
        offBelow = new ScrollInput(x + 48, y + 47, 1, 18)
                .withRange(
                        blockEntity.getMinLevel(), blockEntity.getMaxLevel() + 1 - getValueStep())
                .titled(CreateLang.translateDirect("gui.threshold_switch.lower_threshold"))
                .calling(state -> {
                    lastModification = 0;
                    int valueStep = getValueStep();

                    if (onAbove.getState() / valueStep == 0 && state / valueStep == 0) return;
        ...
        onAbove = new ScrollInput(x + 48, y + 23, 1, 18)
                .withRange(
                        blockEntity.getMinLevel() + getValueStep(), blockEntity.getMaxLevel() + 1)
                .titled(CreateLang.translateDirect("gui.threshold_switch.upper_threshold"))
                .calling(state -> {
                    lastModification = 0;
                    int valueStep = getValueStep();

                    if (offBelow.getState() / valueStep == 0 && state / valueStep == 0) return;
        ...
        flipSignals = new IconButton(
                x + background.getWidth() - 62, y + background.getHeight() - 24, AllIcons.I_FLIP);
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (typeOfCurrentTarget == ThresholdType.FLUID) valueStep = 1000;
        ...
        graphics.drawString(
                font,
                Component.literal("\u2265 "
                        + (typeOfCurrentTarget == ThresholdType.UNSUPPORTED
                                ? ""
                                : forItems
                                        ? onAbove.getState() / valueStep
                                        : blockEntity
                                                .format(onAbove.getState() / valueStep, stacks)
                                                .getString())),
                x + 53,
                y + 28,
                0xFFFFFFFF,
                ...
                Component.literal("\u2264 "
                        + (typeOfCurrentTarget == ThresholdType.UNSUPPORTED
                                ...
                                : forItems
                                        ...
                                        : blockEntity
                                                .format(offBelow.getState() / valueStep, stacks)
                                                .getString())),
                x + 53,
                y + 28 + 24,
                0xFFFFFFFF,
        ...
        GuiGameElement.of(renderedItem)
                .<GuiGameElement.GuiRenderBuilder>at(
                        x + background.getWidth() + 6, y + background.getHeight() - 56, -200)
                .scale(5)
                .render(graphics);
        ...
        AllGuiTextures.THRESHOLD_SWITCH_CURRENT_STATE.render(
                graphics, torchX - 3, torchY - 4 + (highlightTopRow ? 0 : 24));

        PoseStack ms = graphics.pose();
        ms.pushPose();
        ms.translate(torchX - 5, torchY + 14, 200);
        TransformStack.of(ms).rotateXDegrees(-22.5f).rotateYDegrees(45);

        for (boolean power : Iterate.trueAndFalse) {
            GuiGameElement.of(Blocks.REDSTONE_TORCH
                            .defaultBlockState()
    ...
    private int getValueStep() {
        boolean stacks = inStacks.getState() == 1;
        int valueStep = 1;
        if (blockEntity.getTypeOfCurrentTarget() == ThresholdType.FLUID) valueStep = 1000;
        else if (stacks) valueStep = 64;
        return valueStep;
    }
    ...
    protected void send(boolean invert) {
        CatnipServices.NETWORK.sendToServer(new ConfigureThresholdSwitchPacket(
