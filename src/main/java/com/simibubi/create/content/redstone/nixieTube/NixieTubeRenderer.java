...
public class NixieTubeRenderer extends SafeBlockEntityRenderer<NixieTubeBlockEntity> {
    ...
    protected void renderSafe(
        ...
        float yRot = AngleHelper.horizontalAngle(blockState.getValue(NixieTubeBlock.FACING))
                - 90
                + (face == DoubleAttachFace.WALL_REVERSED ? 180 : 0);
        float xRot = face == DoubleAttachFace.WALL
                ? -90
                : face == DoubleAttachFace.WALL_REVERSED ? 90 : 0;
    ...
    public static void drawInWorldString(
            PoseStack ms, MultiBufferSource buffer, String c, int color) {
        Font fontRenderer = Minecraft.getInstance().font;
        ...
        if (buffer instanceof BufferSource) {
            BakedGlyph texturedglyph =
                    fontRenderer.getFontSet(Style.DEFAULT_FONT).whiteGlyph();
    ...
    private void renderAsSignal(
        ...
        if (facing == Direction.DOWN) msr.center().rotateZDegrees(180).uncenter();

        boolean invertTubes = facing == Direction.DOWN
                || blockState.getValue(NixieTubeBlock.FACE) == DoubleAttachFace.WALL_REVERSED;
        ...
        if (be.signalState != null) {
            for (boolean first : Iterate.trueAndFalse) {
                if (first && !be.signalState.isRedLight(renderTime)) continue;
                if (!first
                        && !be.signalState.isGreenLight(renderTime)
                        && !be.signalState.isYellowLight(renderTime)) continue;
            ...
            for (boolean first : Iterate.trueAndFalse) {
                NixieTubeBlockEntity.ComputerSignal.TubeDisplay tubeDisplay =
                        first ? be.computerSignal.first : be.computerSignal.second;
                if (tubeDisplay.blinkPeriod == 0
                        || tubeDisplay.blinkPeriod > 1
                                && renderTime % tubeDisplay.blinkPeriod < tubeDisplay.blinkOffTime)
                    continue;
                ...
                if (diff.lengthSqr() < GLOW_VIEW_DISTANCE * GLOW_VIEW_DISTANCE) {
                    ...
                    CachedBuffers.partial(AllPartialModels.SIGNAL_COMPUTER_WHITE_GLOW, blockState)
                            .light(0xf000f0)
                            .color(
                ...
                CachedBuffers.partial(AllPartialModels.SIGNAL_COMPUTER_WHITE_BASE, blockState)
                        .light(0xF000F0)
                        .color(12, 12, 12, 255)
                        .disableDiffuse()
                        .scale(1 + 1.25f / 16f)
                ...
                CachedBuffers.partial(AllPartialModels.SIGNAL_COMPUTER_WHITE, blockState)
                        .light(0xF000F0)
