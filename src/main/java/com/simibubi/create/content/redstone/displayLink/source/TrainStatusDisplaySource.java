...
public class TrainStatusDisplaySource extends SingleLineDisplaySource {
    ...
    protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
        ...
        if (observer == null) return EMPTY_LINE;
        Train currentTrain = observer.getPresentTrain();
        if (currentTrain == null) return EMPTY_LINE;

        ScheduleRuntime runtime = currentTrain.runtime;
        Schedule schedule = runtime.getSchedule();
        if (schedule == null) return EMPTY_LINE;
        if (runtime.paused) return EMPTY_LINE;
        if (runtime.state != State.POST_TRANSIT) return EMPTY_LINE;
