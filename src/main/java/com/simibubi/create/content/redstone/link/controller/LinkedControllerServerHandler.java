...
public class LinkedControllerServerHandler {
    ...
    public static void tick(LevelAccessor world) {
        ...
        for (Iterator<Entry<UUID, Collection<ManualFrequencyEntry>>> iterator =
            ...
            for (Iterator<ManualFrequencyEntry> entryIterator = list.iterator();
    ...
    public static void receivePressed(
        ...
        Collection<ManualFrequencyEntry> list =
                map.computeIfAbsent(uniqueID, $ -> new ArrayList<>());
        ...
        for (Couple<Frequency> activated : collect) {
            ...
            for (IRedstoneLinkable linkable :
                    Create.REDSTONE_LINK_NETWORK_HANDLER.getNetworkOf(world, entry))
    ...
    static class ManualFrequencyEntry extends IntAttached<Couple<Frequency>>
