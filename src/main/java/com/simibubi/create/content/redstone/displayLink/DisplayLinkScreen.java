...
public class DisplayLinkScreen extends AbstractSimiScreen {
    ...
    public void tick() {
        ...
        if (sourceState != null
                        && sourceState.getBlock()
                                != minecraft
                                        .level
                                        .getBlockState(blockEntity.getSourcePosition())
                                        .getBlock()
                || targetState != null
                        && targetState.getBlock()
                                != minecraft
                                        .level
                                        .getBlockState(blockEntity.getTargetPosition())
                                        .getBlock()) initGathererOptions();
    }
    ...
    private void initGathererOptions() {
        ...
        if (target != null) {
            DisplayTargetStats stats =
                    target.provideStats(new DisplayLinkContext(level, blockEntity));
            ...
            if (rows > 1) {
                targetLineSelector = new ScrollInput(x + 61, y + 105, 135, 16)
                        .withRange(0, rows)
        ...
        sourceWidget
                ...
                .addAll(List.of(
                        ...
                        sourceState
                                ...
                                .getName()
                                .withStyle(
                                        s -> s.withColor(sources.isEmpty() ? 0xF68989 : 0xF2C16D)),
        ...
        if (!sources.isEmpty()) {
            ...
            if (sources.size() > 1) {
                List<Component> options =
                        sources.stream().map(DisplaySource::getName).toList();
                sourceTypeSelector = new SelectionScrollInput(x + 61, y + 26, 135, 16)
                        .forOptions(options)
            ...
            } else initGathererSourceSubOptions(0);
    ...
    private void initGathererSourceSubOptions(int i) {
        ...
        configWidgets.forEachWithContext((s, first) -> source.initConfigurationWidgets(
                context,
                new ModularGuiLineBuilder(font, s, guiLeft + 60, guiTop + (first ? 51 : 72)),
                first));
        configWidgets.forEach(s -> s.loadValues(
                blockEntity.getSourceConfig(), this::addRenderableWidget, this::addRenderableOnly));
    }
    ...
    public void onClose() {
        ...
        if (!sources.isEmpty()) {
            DisplaySource source =
                    sources.get(sourceTypeSelector == null ? 0 : sourceTypeSelector.getState());
        ...
        CatnipServices.NETWORK.sendToServer(new DisplayLinkConfigurationPacket(
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        TransformStack.of(ms)
                ...
                .rotateYDegrees(63);
        GuiGameElement.of(
                        blockEntity.getBlockState().setValue(DisplayLinkBlock.FACING, Direction.UP))
                .render(graphics);
        ms.popPose();
    }
    ...
    protected void removeWidget(GuiEventListener p_169412_) {
        if (p_169412_ != null) super.removeWidget(p_169412_);
    }
}
