...
public class LinkedControllerItemRenderer extends CustomRenderedItemModelRenderer {
    ...
    protected static final PartialModel POWERED =
            PartialModel.of(Create.asResource("item/linked_controller/powered"));
    protected static final PartialModel BUTTON =
            PartialModel.of(Create.asResource("item/linked_controller/button"));
    ...
    static {
        ...
        for (int i = 0; i < 6; i++) buttons.add(LerpedFloat.linear().startWithValue(0));
    }
    ...
    static void tick() {
        ...
        for (int i = 0; i < buttons.size(); i++) {
            ...
            lerpedFloat.chase(
                    LinkedControllerClientHandler.currentlyPressed.contains(i) ? 1 : 0,
                    .4f,
                    Chaser.EXP);
    ...
    protected static void render(
        ...
        if (renderType == RenderType.NORMAL) {
            ...
            boolean noControllerInMain =
                    !AllItems.LINKED_CONTROLLER.isIn(mc.player.getMainHandItem());

            if (transformType == mainHand || (transformType == offHand && noControllerInMain)) {
                float equip = equipProgress.getValue(pt);
                int handModifier =
                        transformType == ItemDisplayContext.FIRST_PERSON_LEFT_HAND ? -1 : 1;
        ...
        if (renderType == RenderType.NORMAL) {
            if (LinkedControllerClientHandler.MODE == Mode.BIND) {
                int i = (int) Mth.lerp(
                        (Mth.sin(AnimationTickHolder.getRenderTime() / 4f) + 1) / 2, 5, 15);
                light = i << 20;
            }
        }
