...
public class DisplayBoardTarget extends DisplayTarget {
    ...
    public void acceptFlapText(
            int line, List<List<MutableComponent>> text, DisplayLinkContext context) {
        FlapDisplayBlockEntity controller = getController(context);
        if (controller == null) return;
        if (!controller.isSpeedRequirementFulfilled()) return;
        ...
        for (int i = 0; i + line < lines.size(); i++) {
            ...
            if (i == 0) reserve(i + line, controller, context);
            if (i > 0 && isReserved(i + line, controller, context)) break;
            ...
            for (int sectionIndex = 0; sectionIndex < layout.getSections().size(); sectionIndex++) {
                ...
                if (textLine.size() <= sectionIndex) break;
    ...
    public boolean isReserved(int line, BlockEntity target, DisplayLinkContext context) {
        return super.isReserved(line, target, context)
                || target instanceof FlapDisplayBlockEntity fdte
                        && fdte.manualLines.length > line
                        && fdte.manualLines[line];
    }

    @Override
    public DisplayTargetStats provideStats(DisplayLinkContext context) {
        FlapDisplayBlockEntity controller = getController(context);
        if (controller == null) return new DisplayTargetStats(1, 1, this);
    ...
    private FlapDisplayBlockEntity getController(DisplayLinkContext context) {
        ...
        if (!(teIn instanceof FlapDisplayBlockEntity be)) return null;
    ...
    public AABB getMultiblockBounds(LevelAccessor level, BlockPos pos) {
        ...
        if (!(be instanceof FlapDisplayBlockEntity fdbe)) return baseShape;

        FlapDisplayBlockEntity controller = fdbe.getController();
        if (controller == null) return baseShape;

        Vec3i normal = controller.getDirection().getClockWise().getNormal();
        return baseShape
                .move(controller.getBlockPos().subtract(pos))
                .expandTowards(
