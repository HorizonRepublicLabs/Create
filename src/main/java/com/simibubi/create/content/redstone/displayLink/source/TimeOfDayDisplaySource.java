...
public class TimeOfDayDisplaySource extends SingleLineDisplaySource {
    ...
    protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
        if (!(context.level() instanceof ServerLevel sLevel)) return EMPTY_TIME;
        if (!(context.getSourceBlockEntity() instanceof CuckooClockBlockEntity ccbe))
            return EMPTY_TIME;
        if (ccbe.getSpeed() == 0) return EMPTY_TIME;
        ...
        MutableComponent suffix =
                CreateLang.translateDirect("generic.daytime." + (hours > 11 ? "pm" : "am"));
        ...
        MutableComponent component = Component.literal((hours < 10 ? " " : "") + hours + ":"
                + (minutes < 10 ? "0" : "") + minutes + (c12 ? " " : ""));
        ...
        builder.addSelectionScrollInput(
                ...
                (si, l) -> {
                    si.forOptions(CreateLang.translatedOptions(
                                    "display_source.time", "12_hour", "24_hour"))
