...
public class ClipboardScreen extends AbstractSimiScreen {
    ...
    public ClipboardScreen(int targetSlot, DataComponentMap components, @Nullable BlockPos pos) {
        ...
        reopenWith(components.getOrDefault(
    ...
    public void reopenWith(ClipboardContent content) {
        ...
        if (startEmpty) currentEntries.add(new ClipboardEntry(false, Component.empty()));
        editingIndex = 0;
            ...
            for (int i = 0; i < currentEntries.size(); i++) {
                ...
                totalHeight += Math.max(
                        ...
                        font.split(
                                                        ...
                                                        clipboardEntry.icon.isEmpty() ? 150 : 130)
                                                .size()
                                        * 9
                                + 3);
    ...
    private void setClipboard(String p_98148_) {
        if (minecraft != null) TextFieldHelper.setClipboardContents(minecraft, p_98148_);
    }
    ...
    private void changePage(boolean next) {
        ...
        if (next) return;
        if (pages.get(currentPage + 1).stream()
                .allMatch(ce -> ce.text.getString().isBlank())) pages.remove(currentPage + 1);
    }
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < currentEntries.size(); i++) {
            ...
            if (isAddress) {
                ...
                (checked
                                ? AllGuiTextures.CLIPBOARD_ADDRESS_INACTIVE
                                : AllGuiTextures.CLIPBOARD_ADDRESS)
                        .render(graphics, x + 44, y + 50);
                text = Component.literal(string.substring(1).stripLeading());
            } else {
                graphics.drawString(
                        font, "\u25A1", x + 45, y + 51, checked ? 0x668D7F6B : 0xff8D7F6B, false);
            ...
            for (FormattedCharSequence sequence : split) {
                if (i != editingIndex)
                    graphics.drawString(
                            ...
                            x + 58 + iconOffset,
                            y + 50,
                            checked ? isAddress ? 0x668D7F6B : 0x31B25D : 0x311A00,
                            false);
                y += 9;
            }
            y += 3;
    ...
    private void sendIfEditingBlock() {
        ...
        if (handler.getOnlinePlayers().size() > 1 && targetedBlock != null) send();
    }
    ...
    private void send() {
        ...
        CatnipServices.NETWORK.sendToServer(
                new ClipboardEditPacket(targetSlot, content, targetedBlock));
    }
    ...
    public boolean charTyped(char pCodePoint, int pModifiers) {
        ...
        if (!StringUtil.isAllowedChatCharacter(pCodePoint)) return false;
        ...
        if (Screen.isSelectAll(pKeyCode)) {
            ...
            switch (pKeyCode) {
                ...
                case 335:
                    if (hasShiftDown()) {
                        ...
                        if (currentEntries.size() <= editingIndex + 1
                                || !currentEntries
                                        ...
                                        .isEmpty())
                            currentEntries.add(
                                    editingIndex + 1, new ClipboardEntry(false, Component.empty()));
                        editingIndex += 1;
                        editContext.setCursorToEnd();
                        if (validateTextForEntry(" ")) return true;
                    ...
                    return true;
                case 259:
                    if (currentEntries.get(editingIndex).text.getString().isEmpty()
    ...
    private void renderCursor(GuiGraphics graphics, Pos2i pCursorPos, boolean pIsEndOfText) {
        ...
        if (!pIsEndOfText) {
            graphics.fill(
                    pCursorPos.x, pCursorPos.y - 1, pCursorPos.x + 1, pCursorPos.y + 9, -16777216);
        } else {
    ...
    private void renderHighlight(Rect2i[] pSelected) {
        ...
        BufferBuilder bufferbuilder =
                tesselator.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION);
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        ...
        DisplayCache cache = getDisplayCache();
        int j = cache.getIndexAtPosition(
                font, convertScreenToLocal(new Pos2i((int) pMouseX, (int) pMouseY)));
        if (j >= 0) {
    ...
    public boolean mouseDragged(
        ...
        DisplayCache cache = getDisplayCache();
        int i = cache.getIndexAtPosition(
                font, convertScreenToLocal(new Pos2i((int) pMouseX, (int) pMouseY)));
    ...
    private DisplayCache rebuildDisplayCache() {
        ...
        if (i != j) {
            ...
            if (j1 == k1) {
                ...
                list1.add(createPartialLineSelection(
                        s, stringsplitter, aint[k1], i1, k1 * 9, aint[k1]));
            }
        }

        return new DisplayCache(
                s, pos, flag, aint, list.toArray(new LineInfo[0]), list1.toArray(new Rect2i[0]));
    }
    ...
    static class DisplayCache {
        ...
        public int getIndexAtPosition(Font pFont, Pos2i pCursorPosition) {
            ...
            if (i < 0) return 0;
            if (i >= lines.length) return fullText.length();
