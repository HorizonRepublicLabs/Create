...
public class SoulPulseEffect {
    ...
    public List<BlockPos> tick(Level world) {
        ...
        ticks--;
        if (ticks < 0 || ticks % TICKS_PER_LAYER != 0) return null;
    ...
    public List<BlockPos> getPotentialSoulSpawns(Level world) {
        ...
        return getLayer(currentLayerIdx())
                .map(p -> p.offset(pos))
    ...
    public static boolean canSpawnSoulAt(Level world, BlockPos at, boolean ignoreLight) {
        ...
        return world != null
                && SpawnPlacementTypes.ON_GROUND.isSpawnPositionOk(world, at, dummy)
    ...
    public void spawnParticles(Level world, BlockPos at) {
        ...
        if (canOverlap())
            world.addAlwaysVisibleParticle(
                    ((int) Math.round(VecHelper.getCenterOf(pos)
                                            .distanceTo(VecHelper.getCenterOf(at))))
                                    >= distance
                            ? new SoulParticle.PerimeterData()
        ...
        if (SoulPulseEffect.isDark(world, at)) {
            world.addAlwaysVisibleParticle(
                    new SoulParticle.Data(), p.x + 0.5, p.y + 0.5, p.z + 0.5, 0, 0, 0);
            world.addParticle(
                    new SoulBaseParticle.Data(), p.x + 0.5, p.y + 0.01, p.z + 0.5, 0, 0, 0);
        }
    }

    private static List<List<BlockPos>> genLayers() {
        List<List<BlockPos>> layers = new ArrayList<>();
        for (int i = 0; i < MAX_DISTANCE; i++) layers.add(new ArrayList<>());
    ...
    public static Stream<BlockPos> getLayer(int idx) {
        if (idx < 0 || idx >= MAX_DISTANCE) return Stream.empty();
