...
public class ToolboxScreen extends AbstractSimiContainerScreen<ToolboxMenu> {
    ...
    protected void init() {
        ...
        disposeButton.withCallback(() -> {
            CatnipServices.NETWORK.sendToServer(
                    new ToolboxDisposeAllPacket(menu.contentHolder.getBlockPos()));
        });
        ...
        extraAreas = ImmutableList.of(new Rect2i(
                leftPos + 30 + BG.getWidth(), topPos + BG.getHeight() - 15 - 34 - 6, 72, 68));
    }
    ...
    protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
        ...
        for (int compartment = 0; compartment < 8; compartment++) {
            ...
            if (itemstack.isEmpty()) itemstack = menu.getFilter(compartment);
    ...
    private void renderToolbox(GuiGraphics graphics, int x, int y, float partialTicks) {
        ...
        GuiGameElement.of(AllBlocks.TOOLBOXES.get(color).getDefaultState()).render(graphics);
        ...
        TransformStack.of(ms)
                ...
                .translate(0, 6 / 16f, -12 / 16f);
        GuiGameElement.of(AllPartialModels.TOOLBOX_LIDS.get(color)).render(graphics);
        ms.popPose();
