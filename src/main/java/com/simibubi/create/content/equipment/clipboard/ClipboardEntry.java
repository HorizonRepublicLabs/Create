...
public class ClipboardEntry {
    public static final Codec<ClipboardEntry> CODEC = RecordCodecBuilder.create(i -> i.group(
            ...
            .apply(i, (checked, text, icon, itemAmount) -> {
                ...
                if (!icon.isEmpty()) entry.displayItem(icon, itemAmount);

                return entry;
            }));

    public static final StreamCodec<RegistryFriendlyByteBuf, ClipboardEntry> STREAM_CODEC =
    ...
    public static List<List<ClipboardEntry>> readAll(@Nullable ClipboardContent content) {
        ...
        // Both these lists are immutable, so we unfortunately need to re-create them to make them
        // mutable
        List<List<ClipboardEntry>> saved = content.pages();

        List<List<ClipboardEntry>> entries = new ArrayList<>(saved.size());
        for (List<ClipboardEntry> inner : saved) entries.add(new ArrayList<>(inner));
    ...
    public static List<ClipboardEntry> getLastViewedEntries(ItemStack heldItem) {
        ...
        int previouslyOpenedPage = heldItem.getOrDefault(
    ...
    public final boolean equals(Object o) {
        ...
        return checked == that.checked
                ...
                && ItemStack.isSameItemSameComponents(icon, that.icon);
    }
