...
public abstract class ShootableGadgetRenderHandler {
    ...
    protected abstract void transformTool(
            PoseStack ms, float flip, float equipProgress, float recoil, float pt);

    protected abstract void transformHand(
            PoseStack ms, float flip, float equipProgress, float recoil, float pt);
    ...
    protected void onRenderPlayerHand(RenderHandEvent event) {
        ...
        boolean rightHand = event.getHand() == InteractionHand.MAIN_HAND
                ^ mc.player.getMainArm() == HumanoidArm.LEFT;
        float recoil = rightHand
                ? Mth.lerp(pt, lastRightHandAnimation, rightHandAnimation)
                : Mth.lerp(pt, lastLeftHandAnimation, leftHandAnimation);
        float equipProgress = event.getEquipProgress();

        if (rightHand && (rightHandAnimation > .01f || dontReequipRight)) equipProgress = 0;
        if (!rightHand && (leftHandAnimation > .01f || dontReequipLeft)) equipProgress = 0;
        ...
        if (rightHand) playerrenderer.renderRightHand(ms, buffer, light, player);
        else playerrenderer.renderLeftHand(ms, buffer, light, player);
        ms.popPose();
