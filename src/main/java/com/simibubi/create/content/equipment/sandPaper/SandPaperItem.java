...
public class SandPaperItem extends Item implements CustomUseEffectsItem {
    ...
    public InteractionResultHolder<ItemStack> use(
        ...
        InteractionResultHolder<ItemStack> FAIL =
        ...
        InteractionHand otherHand = handIn == InteractionHand.MAIN_HAND
        ...
        BlockHitResult raytraceresult =
                getPlayerPOVHitResult(worldIn, playerIn, ClipContext.Fluid.NONE);
        ...
        for (ItemEntity itemEntity : worldIn.getEntitiesOfClass(ItemEntity.class, bb)) {
            ...
            if (itemEntity.position().distanceTo(playerIn.position()) > 3) continue;
            ItemStack stack = itemEntity.getItem();
            if (!SandPaperPolishingRecipe.canPolish(worldIn, stack)) continue;
            pickUp = itemEntity;
            break;
        }

        if (pickUp == null) return FAIL;
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entityLiving) {
        ...
        if (stack.has(AllDataComponents.SAND_PAPER_POLISHING)) {
            ItemStack toPolish =
                    stack.get(AllDataComponents.SAND_PAPER_POLISHING).item();
            //noinspection DataFlowIssue - toPolish won't be null as we do call .has before calling
            // .get
            ItemStack polished = SandPaperPolishingRecipe.applyPolish(
            ...
            if (level.isClientSide) {
                spawnParticles(
                        entityLiving
                                .getEyePosition(1)
                                .add(entityLiving.getLookAngle().scale(.5f)),
                        toPolish,
                        level);
                return stack;
            }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (newState != null) {
            ...
            if (player != null)
                stack.hurtAndBreak(
                        1, player, LivingEntity.getSlotForHand(player.getUsedItemHand()));
