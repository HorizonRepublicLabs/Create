...
public class RadialToolboxMenu extends AbstractSimiScreen {
    ...
    public RadialToolboxMenu(
        ...
        if (selectedBox != null) this.selectedBox = selectedBox;
    }
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        float fade =
                Mth.clamp((ticksOpen + AnimationTickHolder.getPartialTicks()) / 10f, 1 / 512f, 1);
        ...
        if (distance > 25 && distance < 10000)
            hoveredSlot =
                    (Mth.floor((AngleHelper.deg(Mth.atan2(hoveredY, hoveredX)) + 360 + 180 - 22.5f))
                                    % 360)
                            / 45;
        boolean renderCenterSlot = state == State.SELECT_ITEM_UNEQUIP;
        if (scrollMode && distance > 150) scrollMode = false;
        if (renderCenterSlot && distance <= 150) hoveredSlot = UNEQUIP;
        ...
        if (state == State.DETACH) {
            ...
            GuiGameElement.of(AllBlocks.TOOLBOXES.get(DyeColor.BROWN).asStack())
                    .at(-9, -9)
                    .render(graphics);
            ...
            for (int slot = 0; slot < 8; slot++) {
                ...
                if (state == State.SELECT_ITEM || state == State.SELECT_ITEM_UNEQUIP) {
                    ...
                    if (!stackInSlot.isEmpty()) {
                        ...
                        (empty
                                        ? AllGuiTextures.TOOLBELT_INACTIVE_SLOT
                                        : AllGuiTextures.TOOLBELT_SLOT)
                                .render(graphics, 0, 0);
                        GuiGameElement.of(stackInSlot).at(3, 3).render(graphics);
                    ...
                    } else AllGuiTextures.TOOLBELT_EMPTY_SLOT.render(graphics, 0, 0);
                    ...
                    if (slot < toolboxes.size()) {
                        ...
                        GuiGameElement.of(AllBlocks.TOOLBOXES
                                        .get(toolboxBlockEntity.getColor())
                                        .asStack())
                                .at(3, 3)
                                .render(graphics);
    ...
    public void renderBackground(
        ...
        Color color = BACKGROUND_COLOR.scaleAlpha(
                Math.min(1, (ticksOpen + AnimationTickHolder.getPartialTicks()) / 20f));
    ...
    public void removed() {
        ...
        if (selected == DEPOSIT) {
            if (state == State.DETACH) return;
                ...
                toolboxes.forEach(be -> CatnipServices.NETWORK.sendToServer(
                        new ToolboxDisposeAllPacket(be.getBlockPos())));
            else
                CatnipServices.NETWORK.sendToServer(
                        new ToolboxDisposeAllPacket(selectedBox.getBlockPos()));
            return;
        }

        if (state == State.SELECT_BOX) return;

        if (state == State.DETACH) {
            if (selected == UNEQUIP)
                CatnipServices.NETWORK.sendToServer(new ToolboxEquipPacket(
                        null, selected, minecraft.player.getInventory().selected));
            return;
        }

        if (selected == UNEQUIP)
            CatnipServices.NETWORK.sendToServer(new ToolboxEquipPacket(
                    selectedBox.getBlockPos(), selected, minecraft.player.getInventory().selected));

        if (selected < 0) return;
        ToolboxInventory inv = selectedBox.inventory;
        ItemStack stackInSlot = inv.filters.get(selected);
        if (stackInSlot.isEmpty()) return;
        if (inv.getStackInSlot(selected * STACKS_PER_COMPARTMENT).isEmpty()) return;
        ...
        if (distance <= 150) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (state == State.SELECT_ITEM || state == State.SELECT_ITEM_UNEQUIP) {
                    ...
                    if (!stackInSlot.isEmpty()
                            && !inv.getStackInSlot(scrollSlot * STACKS_PER_COMPARTMENT)
                                    .isEmpty()) break;
                }

                if (state == State.SELECT_BOX) if (scrollSlot < toolboxes.size()) break;

                if (state == State.DETACH) break;
    ...
    public boolean mouseClicked(double x, double y, int button) {
        ...
        if (button == 0) {
            ...
            if (state == State.DETACH
                    || state == State.SELECT_ITEM
                    || state == State.SELECT_ITEM_UNEQUIP) {
        ...
        for (int i = 0; i < hotbarBinds.length && i < 8; i++) {
            if (hotbarBinds[i].matches(code, scanCode)) {
                ...
                if (state == State.SELECT_ITEM || state == State.SELECT_ITEM_UNEQUIP) {
                    ...
                    if (stackInSlot.isEmpty()
                            || inv.getStackInSlot(i * STACKS_PER_COMPARTMENT).isEmpty())
                        return false;
                }

                if (state == State.SELECT_BOX) if (i >= toolboxes.size()) return false;
