...
public record ExtendoGripInteractionPacket(InteractionHand hand, int target, Vec3 point)
        implements ServerboundPacketPayload {
    public static final StreamCodec<ByteBuf, ExtendoGripInteractionPacket> STREAM_CODEC =
            StreamCodec.composite(
                    CatnipStreamCodecBuilders.nullable(CatnipStreamCodecs.HAND),
                    ExtendoGripInteractionPacket::hand,
                    ByteBufCodecs.INT,
                    ExtendoGripInteractionPacket::target,
                    CatnipStreamCodecBuilders.nullable(CatnipStreamCodecs.VEC3),
                    ExtendoGripInteractionPacket::point,
    ...
    public void handle(ServerPlayer sender) {
        ...
        if (entityByID != null && ExtendoGripItem.isHoldingExtendoGrip(sender)) {
            ...
            if (this.hand == null) sender.attack(entityByID);
            else if (this.point == null) sender.interactOn(entityByID, this.hand);
            else entityByID.interactAt(sender, this.point, this.hand);
        }
    }
}
