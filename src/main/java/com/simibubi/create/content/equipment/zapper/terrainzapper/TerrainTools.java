...
public enum TerrainTools implements StringRepresentable {
    ...
    public static final Codec<TerrainTools> CODEC =
            StringRepresentable.fromValues(TerrainTools::values);
    public static final StreamCodec<ByteBuf, TerrainTools> STREAM_CODEC =
            CatnipStreamCodecBuilders.ofEnum(TerrainTools.class);
        ...
        switch (this) {
            case Clear:
                targetPositions.forEach(
                        p -> world.setBlockAndUpdate(p, Blocks.AIR.defaultBlockState()));
                break;
            case Fill:
                targetPositions.forEach(p -> {
                    BlockState toReplace = world.getBlockState(p);
                    if (!isReplaceable(toReplace)) return;
            ...
            case Overlay:
                targetPositions.forEach(p -> {
                    ...
                    if (isReplaceable(toOverlay)) return;
                    if (toOverlay == paintedState) return;
