...
public class EntityHats {
    ...
    public static PartialModel getHatFor(LivingEntity entity) {
        ...
        if (shouldRenderTrainHat(entity)) return AllPartialModels.TRAIN_HAT;
    ...
    public static PartialModel getLogisticsHatFor(LivingEntity entity) {
        ...
        if (!(entity.getVehicle() instanceof SeatEntity cce)) return null;
        ...
        for (Direction d : Iterate.horizontalDirections) {
            for (int y : Iterate.zeroAndOne) {
                if (!(level.getBlockState(pos.relative(d).above(y)).getBlock()
                        instanceof StockTickerBlock lw)) continue;
                PartialModel hatOfStation = lw.getHat(level, pos, entity);
                if (hatOfStation == null) continue;
                hat = hatOfStation;
                stations++;
            }
        }

        if (stations == 1) return hat;
    ...
    public static boolean shouldRenderTrainHat(LivingEntity entity) {
        ...
        if (!(entity.getVehicle() instanceof CarriageContraptionEntity cce)) return false;
