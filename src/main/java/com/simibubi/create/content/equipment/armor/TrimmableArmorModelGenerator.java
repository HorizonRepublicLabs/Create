...
public class TrimmableArmorModelGenerator {
    ...
    static {
        try {
            MethodHandles.Lookup lookup =
                    MethodHandles.privateLookupIn(ModelBuilder.class, MethodHandles.lookup());
    ...
    public static <T extends ArmorItem> void generate(
            DataGenContext<Item, T> c, RegistrateItemModelProvider p) {
        T item = c.get();
        ItemModelBuilder builder = p.generated(c);
        for (ItemModelGenerators.TrimModelData data :
                ItemModelGeneratorsAccessor.create$getGENERATED_TRIM_MODELS()) {
            ...
            ResourceLocation trimLoc = ResourceLocation.withDefaultNamespace(
                    "trims/items/" + item.getType().getName() + "_trim_" + trimId);
            String parent = "item/generated";
            if (item.getMaterial() == AllArmorMaterials.CARDBOARD) {
                trimLoc = Create.asResource(
                        "trims/items/card_" + item.getType().getName() + "_trim_" + trimId);
            }
