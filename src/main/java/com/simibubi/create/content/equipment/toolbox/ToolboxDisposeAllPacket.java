...
public record ToolboxDisposeAllPacket(BlockPos toolboxPos) implements ServerboundPacketPayload {
    ...
    public void handle(ServerPlayer player) {
        ...
        if (!(blockEntity instanceof ToolboxBlockEntity toolbox)) return;
        ...
        toolbox.inventory.inLimitedMode(inventory -> {
            for (int i = 0; i < 36; i++) {
                ...
                if (compound.contains(key)
                        && NBTHelper.readBlockPos(compound.getCompound(key), "Pos")
                                .equals(toolboxPos)) {
                ...
                ItemStack remainder =
                        ItemHandlerHelper.insertItemStacked(toolbox.inventory, itemStack, false);
                if (remainder.getCount() != itemStack.getCount())
                    player.getInventory().setItem(i, remainder);
            }
        });

        if (sendData.booleanValue()) ToolboxHandler.syncData(player);
    }
}
