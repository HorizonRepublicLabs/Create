...
public class ShootableGadgetItemMethods {
    ...
    public static void sendPackets(
            Player player, Function<Boolean, ? extends ShootGadgetPacket> factory) {
    ...
    public static boolean shouldSwap(
            Player player, ItemStack item, InteractionHand hand, Predicate<ItemStack> predicate) {
        ...
        // Pass To Offhand
        if (mainHand && isSwap && gunInOtherHand) return true;
        if (mainHand && !isSwap && gunInOtherHand) item.set(AllDataComponents.SHAPER_SWAP, true);
        if (!mainHand && isSwap) item.remove(AllDataComponents.SHAPER_SWAP);
    ...
    public static Vec3 getGunBarrelVec(Player player, boolean mainHand, Vec3 rightHandForward) {
        ...
        Vec3 barrelPosNoTransform =
                new Vec3(flip * rightHandForward.x, rightHandForward.y, rightHandForward.z);
