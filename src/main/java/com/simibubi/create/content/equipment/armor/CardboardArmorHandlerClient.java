...
public class CardboardArmorHandlerClient {
    ...
    private static final Cache<UUID, Integer> BOXES_PLAYERS_ARE_HIDING_AS =
            new TickBasedCache<>(20, true);

    @SubscribeEvent
    public static void keepCacheAliveDesignDespiteNotRendering(PlayerTickEvent.Post event) {
        Player player = event.getEntity();
        if (!CardboardArmorHandler.testForStealth(player)) return;
        try {
    ...
    public static void playerRendersAsBoxWhenSneaking(RenderPlayerEvent.Pre event) {
        ...
        if (player.onGround())
            ms.translate(
                    0,
                    Math.min(
                            Math.abs(Mth.cos((AnimationTickHolder.getRenderTime() % 256) / 2.0f))
                                    * -renderOffset.y,
                            movement * 5),
                    0);
        ...
        try {
            PartialModel model =
                    AllPartialModels.PACKAGES_TO_HIDE_AS.get(getCurrentBoxIndex(player));
