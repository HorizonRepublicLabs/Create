...
public class SoulParticle extends CustomRotationParticle {
    ...
    public SoulParticle(
        ...
        this.animationStage =
                !isPerimeter ? new StartAnimation(this) : new PerimeterAnimation(this);
        if (isPerimeter) {
            yo = y -= .5f - 1 / 128f;
    ...
    public void tick() {
        ...
        if (!SoulPulseEffect.isDark(level, pos)) {
            isVisible = true;
            if (!isPerimeter) remove();
        } else if (isPerimeter) isVisible = false;
    }
    ...
    public void setFrame(int frame) {
        if (frame >= 0 && frame < totalFrames) setSprite(animatedSprite.get(frame, totalFrames));
    }

    @Override
    public Quaternionf getCustomRotation(Camera camera, float partialTicks) {
        if (isPerimeter) return Axis.XP.rotationDegrees(90);
    ...
    public static class Data extends BasicParticleData<SoulParticle> {
        ...
        public IBasicParticleFactory<SoulParticle> getBasicFactory() {
            return (worldIn, x, y, z, vx, vy, vz, spriteSet) ->
                    new SoulParticle(worldIn, x, y, z, vx, vy, vz, spriteSet, this);
        }
    ...
    public static class StartAnimation extends AnimationStage {
        ...
        public void tick() {
            ...
            particle.setFrame(particle.firstStartFrame
                    + (int) (getAnimAge() / (float) particle.startTicks * particle.startFrames));
        }

        @Override
        public AnimationStage getNext() {
            if (animAge < particle.startTicks) return this;
            else return new LoopAnimation(particle);
        }
    }
    ...
    public static class LoopAnimation extends AnimationStage {
        ...
        public void tick() {
            ...
            particle.setFrame(
                    particle.firstLoopFrame + loopTick); // (int) (((float) loopTick / (float)
        ...
        @Override
        public AnimationStage getNext() {
            if (loops <= particle.numLoops) return this;
            else return new EndAnimation(particle);
        }
    }
    ...
    public static class EndAnimation extends AnimationStage {
        ...
        public void tick() {
            ...
            particle.setFrame(particle.firstEndFrame
                    + (int) ((getAnimAge() / (float) particle.endTicks) * particle.endFrames));
        }

        @Override
        public AnimationStage getNext() {
            if (animAge < particle.endTicks) return this;
            else return null;
        }
    }
    ...
    public static class PerimeterAnimation extends AnimationStage {
        ...
        public AnimationStage getNext() {
            if (animAge
                    < (particle.isExpandingPerimeter
                            ? 8
                            : particle.startTicks
                                    + particle.endTicks
                                    + particle.numLoops * particle.loopLength)) return this;
            else return null;
        }
    }
}
