...
public class ClipboardValueSettingsHandler {
                ...
                && smartBE.getAllBehaviours().stream()
                        .noneMatch(b -> b instanceof ClipboardCloneable cc
                                && cc.writeToClipboard(
                                        ...
                                        new CompoundTag(),
                                        target.getDirection()))
                && !(smartBE instanceof ClipboardCloneable)) return;
    ...
    public static void clientTick() {
        Minecraft mc = Minecraft.getInstance();
        if (!(mc.hitResult instanceof BlockHitResult target)) return;
        if (!AllBlocks.CLIPBOARD.isIn(mc.player.getMainHandItem())) return;
        BlockPos pos = target.getBlockPos();
        if (!(mc.level.getBlockEntity(pos) instanceof SmartBlockEntity smartBE)) return;
        ...
        if (smartBE instanceof ClipboardBlockEntity) {
            ...
            tip.add(CreateLang.translateDirect(
                    "clipboard.copy_other_clipboard", Component.keybind("key.use")));
        ...
        boolean canCopy = smartBE.getAllBehaviours().stream()
                ...
                && (smartBE.getAllBehaviours().stream()
                                .anyMatch(b -> b instanceof ClipboardCloneable cc
                                        && cc.readFromClipboard(
                                                mc.level.registryAccess(),
                                                tagElement.getCompound(cc.getClipboardKey()),
                                                mc.player,
                                                target.getDirection(),
                                                true))
                        || smartBE instanceof ClipboardCloneable ccbe
                                && ccbe.readFromClipboard(
                                        mc.level.registryAccess(),
                                        tagElement.getCompound(ccbe.getClipboardKey()),
                                        mc.player,
                                        target.getDirection(),
                                        true));
        ...
        if (canPaste)
            tip.add(CreateLang.translateDirect(
                    "clipboard.to_paste", Component.keybind("key.attack")));
    ...
    private static void interact(PlayerInteractEvent event, boolean paste) {
        ...
        if (!AllBlocks.CLIPBOARD.isIn(itemStack)) return;
        ...
        if (smartBE instanceof ClipboardBlockEntity cbe) {
            if (event instanceof ICancellableEvent cancellableEvent) {
                ...
                switch (event) {
                    case EntityInteractSpecific e -> e.setCancellationResult(
                    ...
                    for (ClipboardEntry entry : page) {
                        ...
                        for (List<ClipboardEntry> pageTo : listTo)
                            for (ClipboardEntry existing : pageTo)
                                if (entryToAdd.equals(existing.text.getString())) continue Copy;
            ...
            player.displayClientMessage(
                    CreateLang.translate(
                                    "clipboard.copied_from_clipboard",
        ...
        for (BlockEntityBehaviour behaviour : smartBE.getAllBehaviours()) {
            if (!(behaviour instanceof ClipboardCloneable cc)) continue;
            anyValid = true;
            String clipboardKey = cc.getClipboardKey();
            if (paste) {
                anySuccess |= cc.readFromClipboard(
                        world.registryAccess(),
                        tag.getCompound(clipboardKey),
                        player,
                        event.getFace(),
                        world.isClientSide());
                continue;
            ...
            boolean success =
                    cc.writeToClipboard(world.registryAccess(), compoundTag, event.getFace());
            anySuccess |= success;
            if (success) tag.put(clipboardKey, compoundTag);
        }
        ...
        if (smartBE instanceof ClipboardCloneable ccbe) {
            ...
            if (paste) {
                ...
                CompoundTag compoundTag = new CompoundTag();
                boolean success =
                        ccbe.writeToClipboard(world.registryAccess(), compoundTag, event.getFace());
                anySuccess |= success;
                if (success) tag.put(clipboardKey, compoundTag);
            }
        }
