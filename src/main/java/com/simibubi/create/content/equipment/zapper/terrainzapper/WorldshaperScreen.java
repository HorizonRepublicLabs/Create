...
public class WorldshaperScreen extends ZapperScreen {
    ...
    protected final Component placementSection =
            CreateLang.translateDirect("gui.terrainzapper.placement");
    ...
    protected final List<Component> brushOptions = CreateLang.translatedOptions(
            "gui.terrainzapper.brush", "cuboid", "sphere", "cylinder", "surface", "cluster");
    ...
    public WorldshaperScreen(ItemStack zapper, InteractionHand hand) {
        ...
        currentPlacement = zapper.getOrDefault(
                AllDataComponents.SHAPER_PLACEMENT_OPTIONS, PlacementOptions.Merged);
    }
    ...
    protected void init() {
        ...
        brushInput = new SelectionScrollInput(x + 56, y + 20, 77, 18)
    ...
    protected void initBrushParams(int x, int y) {
        ...
        for (int index = 0; index < 3; index++) {
            Label label =
                    new Label(x + 65 + 20 * index, y + 45, CommonComponents.EMPTY).withShadow();
        ...
        if (currentBrush.hasConnectivityOptions()) {
            ...
            followDiagonals.withCallback(() -> {
                followDiagonalsIndicator.state =
                        followDiagonalsIndicator.state == State.OFF ? State.ON : State.OFF;
                currentFollowDiagonals = !currentFollowDiagonals;
            });
            followDiagonals.setToolTip(
                    CreateLang.translateDirect("gui.terrainzapper.searchDiagonal"));
            acrossMaterials.withCallback(() -> {
                acrossMaterialsIndicator.state =
                        acrossMaterialsIndicator.state == State.OFF ? State.ON : State.OFF;
            ...
            if (currentFollowDiagonals) followDiagonalsIndicator.state = State.ON;
            if (currentAcrossMaterials) acrossMaterialsIndicator.state = State.ON;
        }

        // Tools

        if (toolButtons != null) removeWidgets(toolButtons);
        ...
        for (int id = 0; id < toolValues.length; id++) {
            ...
            toolButton.setToolTip(
                    CreateLang.translateDirect("gui.terrainzapper.tool." + tool.translationKey));
            toolButtons.add(toolButton);
        }

        int toolIndex = -1;
        for (int i = 0; i < toolValues.length; i++) if (currentTool == toolValues[i]) toolIndex = i;
        if (toolIndex == -1) {
        ...
        if (placementButtons != null) removeWidgets(placementButtons);
        ...
        if (currentBrush.hasPlacementOptions()) {
            ...
            for (int id = 0; id < placementValues.length; id++) {
                ...
                placementButton.setToolTip(CreateLang.translateDirect(
                        "gui.terrainzapper.placement." + option.translationKey));
    ...
    protected void drawOnBackground(GuiGraphics graphics, int x, int y) {
        ...
        for (int index = 2; index >= currentBrush.amtParams; index--)
            AllGuiTextures.TERRAINZAPPER_INACTIVE_PARAM.render(
                    graphics, x + 56 + 20 * index, y + 40);
    ...
    protected ConfigureZapperPacket getConfigurationPacket() {
        ...
        int brushParamY = followDiagonalsIndicator != null
                ? followDiagonalsIndicator.state == State.ON ? 0 : 1
                : currentBrushParams[1];
        int brushParamZ = acrossMaterialsIndicator != null
                ? acrossMaterialsIndicator.state == State.ON ? 0 : 1
