            ...
            for (int j = 0; j < values[i].length; j++) {
                ...
                for (int iOffset = -2; iOffset <= 2; iOffset++) {
                    for (int jOffset = -2; jOffset <= 2; jOffset++) {
                        ...
                        int ref = 0;
                        if (iTarget < 0
                                || iTarget >= values.length
                                || jTarget < 0
                                || jTarget >= values[0].length) ref = value;
    ...
    public static void apply(Level world, List<BlockPos> targetPositions, Direction facing) {
        ...
        for (BlockPos p : targetPositions) {
            ...
            if (TerrainTools.isReplaceable(belowSurface)) {
                if (!heightMap.containsKey(coords)) heightMap.put(coords, Integer.MIN_VALUE);
                continue;
            }
            ...
            if (!heightMap.containsKey(coords)
                    || heightMap.get(coords).equals(Integer.MAX_VALUE)
                    || heightMap.get(coords).equals(Integer.MIN_VALUE)
        ...
        for (BlockPos p : surfaces) {
            ...
            int targetCoord =
                    heightMapArray[coords.getKey() - minCoord1][coords.getValue() - minCoord2]
                            * offset;

            // Keep surface
            if (surfaceCoord == targetCoord) continue;
