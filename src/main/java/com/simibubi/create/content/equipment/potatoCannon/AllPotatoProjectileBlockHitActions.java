...
public class AllPotatoProjectileBlockHitActions {
    ...
    private static void register(
            String name, MapCodec<? extends PotatoProjectileBlockHitAction> codec) {
        Registry.register(
    ...
    public record PlantCrop(Holder<Block> cropBlock) implements PotatoProjectileBlockHitAction {
        ...
        public boolean execute(LevelAccessor level, ItemStack projectile, BlockHitResult ray) {
            ...
            if (level instanceof Level l && !l.isLoaded(hitPos)) return true;
            ...
            if (!(cropBlock.value() instanceof SpecialPlantable specialPlantable)) return false;
    ...
    public record PlaceBlockOnGround(Holder<Block> block)
            implements PotatoProjectileBlockHitAction {
        public static final MapCodec<PlaceBlockOnGround> CODEC =
                RecordCodecBuilder.mapCodec(instance -> instance.group(BuiltInRegistries.BLOCK
                                .holderByNameCodec()
                                .fieldOf("block")
                                .forGetter(PlaceBlockOnGround::block))
        ...
        public boolean execute(
                LevelAccessor levelAccessor, ItemStack projectile, BlockHitResult ray) {
            if (levelAccessor.isClientSide()) return true;

            BlockPos hitPos = ray.getBlockPos();
            if (levelAccessor instanceof Level l && !l.isLoaded(hitPos)) return true;
            Direction face = ray.getDirection();
            BlockPos placePos = hitPos.relative(face);
            ...
            if (face == Direction.UP) {
                ...
                if (!level.isEmptyBlock(placePos.below())) y = Math.max(y, placePos.getY());
