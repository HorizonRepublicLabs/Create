...
public class BlueprintOverlayRenderer {
    ...
    public static void tick() {
        ...
        if (mouseOver.getType() != Type.ENTITY) return;

        EntityHitResult entityRay = (EntityHitResult) mouseOver;
        if (!(entityRay.getEntity() instanceof BlueprintEntity blueprintEntity)) return;

        BlueprintSection sectionAt = blueprintEntity.getSectionAt(
                entityRay.getLocation().subtract(blueprintEntity.position()));

        lastTargetedSection = last;
        active = true;

        boolean sneak = mc.player.isShiftKeyDown();
        if (sectionAt != lastTargetedSection
                || AnimationTickHolder.getTicks() % 10 == 0
                || lastSneakState != sneak) rebuild(sectionAt, sneak);
    ...
    public static void displayTrackRequirements(PlacementInfo info, ItemStack pavementItem) {
        ...
        while (pavement > 0) {
            ingredients.add(Pair.of(
                    pavementItem.copyWithCount(Math.min(64, pavement)), info.hasRequiredPavement));
            pavement -= 64;
        }
    }
    ...
    public static void displayClothShop(
            TableClothBlockEntity dce, int alreadyPurchased, ShoppingList list) {
        if (active) return;
        prepareCustomOverlay();
        noOutput = false;

        shopContext = new BlueprintOverlayShopContext(
                false, dce.getStockLevelForTrade(list), alreadyPurchased);

        ingredients.add(Pair.of(
                dce.getPaymentItem().copyWithCount(dce.getPaymentAmount()),
                !dce.getPaymentItem().isEmpty()
                        && shopContext.stockLevel() > shopContext.purchases()));
    ...
    public static void displayShoppingList(Couple<InventorySummary> bakedList) {
        if (active || bakedList == null) return;
        Minecraft mc = Minecraft.getInstance();
        prepareCustomOverlay();
        noOutput = false;
        ...
        for (BigItemStack entry : bakedList.getSecond().getStacksByCount()) {
            ingredients.add(
                    Pair.of(entry.stack.copyWithCount(entry.count), canAfford(mc.player, entry)));
        }
    ...
    private static boolean canAfford(Player player, BigItemStack entry) {
        ...
        for (int i = 0; i < player.getInventory().items.size(); i++) {
            ...
            if (item.isEmpty() || !ItemStack.isSameItemSameComponents(item, entry.stack)) continue;
    ...
    public static void rebuild(BlueprintSection sectionAt, boolean sneak) {
        ...
        Minecraft mc = Minecraft.getInstance();
        ItemStackHandler playerInv =
                new ItemStackHandler(mc.player.getInventory().getContainerSize());
        ...
        do {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                for (int slot = 0; slot < playerInv.getSlots(); slot++) {
                    if (!requestedItem.test(mc.level, playerInv.getStackInSlot(slot))) continue;
            ...
            if (success) {
                ...
                if (!recipe.isPresent())
                    recipe = mc.level
                            ...
                            .getRecipeFor(
                                    RecipeType.CRAFTING,
                                    craftingInventory.asCraftInput(),
                                    mc.level);
                ItemStack resultFromRecipe = recipe.filter(
                                r -> r.value().matches(craftingInventory.asCraftInput(), mc.level))
                        .map(r -> r.value()
                                .assemble(
                                        craftingInventory.asCraftInput(),
                ...
                if (resultFromRecipe.isEmpty()) {
                    if (!recipe.isPresent()) invalid = true;
                    success = false;
                } else if (resultFromRecipe.getCount() + amountCrafted > 64) {
                    success = false;
                } else {
                    amountCrafted += resultFromRecipe.getCount();
                    if (results.isEmpty()) results.add(resultFromRecipe.copy());
                    else results.get(0).grow(resultFromRecipe.getCount());
            ...
            if (success || firstPass) {
                newlyAdded.forEach(
                        s -> ItemHandlerHelper.insertItemStacked(availableItems, s, false));
                newlyMissing.forEach(
                        s -> ItemHandlerHelper.insertItemStacked(missingItems, s, false));
            }

            if (!success) {
                if (firstPass) {
                    results.clear();
                    if (!invalid) results.add(items.getStackInSlot(9));
    ...
    public static void renderOverlay(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        boolean invalidShop = shopContext != null
                && (ingredients.isEmpty()
                        || ingredients.get(0).getFirst().isEmpty()
                        || shopContext.stockLevel() == 0);
        ...
        if (shopContext != null) {
            TooltipRenderUtil.renderTooltipBackground(
                    guiGraphics, x - 2, y + 1, w + 4, 19, 0, 0x55_000000, 0x55_000000, 0, 0);

            AllGuiTextures.TRADE_OVERLAY.render(
                    guiGraphics, guiGraphics.guiWidth() / 2 - 48, y - 19);
            if (shopContext.purchases() > 0) {
                guiGraphics.renderItem(
                        AllItems.SHOPPING_LIST.asStack(), guiGraphics.guiWidth() / 2 + 20, y - 20);
        ...
        for (Pair<ItemStack, Boolean> pair : ingredients) {
            ...
            (pair.getSecond() ? AllGuiTextures.HOTSLOT_ACTIVE : AllGuiTextures.HOTSLOT)
        ...
        if (results.isEmpty()) {
            ...
            for (ItemStack result : results) {
                AllGuiTextures slot = resultCraftable
                        ? AllGuiTextures.HOTSLOT_SUPER_ACTIVE
                        : AllGuiTextures.HOTSLOT;
                if (!invalidShop
                        && shopContext != null
                        && shopContext.stockLevel() > shopContext.purchases())
            ...
            for (boolean count : Iterate.trueAndFalse)
                for (int i = 0; i < results.size(); i++) {
                    ...
                    List<Component> tooltipLines = result.getTooltipLines(
                            TooltipContext.of(mc.level), mc.player, TooltipFlag.NORMAL);
    ...
    public static void drawItemStack(
            GuiGraphics graphics, Minecraft mc, int x, int y, ItemStack itemStack, String count) {
