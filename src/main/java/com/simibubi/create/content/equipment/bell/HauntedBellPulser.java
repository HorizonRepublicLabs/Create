...
public class HauntedBellPulser {
    ...
    public static void hauntedBellCreatesPulse(PlayerTickEvent.Post event) {
        ...
        if (!player.isHolding(AllBlocks.HAUNTED_BELL::isIn)) return;

        boolean firstPulse = false;

        try {
            IntAttached<Entity> ticker =
                    WARMUP.get(player.getUUID(), () -> IntAttached.with(WARMUP_TICKS, player));
            firstPulse = ticker.getFirst() == 1;
            ticker.decrement();
            if (!ticker.isOrBelowZero()) return;
    ...
    public static void sendPulse(
            ServerLevel world, BlockPos pos, int distance, boolean canOverlap) {
        ChunkPos chunk = world.getChunkAt(pos).getPos();
        CatnipServices.NETWORK.sendToClientsTrackingChunk(
                world, chunk, new SoulPulseEffectPacket(pos, distance, canOverlap));
    }
}
