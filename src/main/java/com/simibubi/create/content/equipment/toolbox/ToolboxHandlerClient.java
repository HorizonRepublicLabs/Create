...
public class ToolboxHandlerClient {
    ...
    public static void clientTick() {
        if (COOLDOWN > 0 && !AllKeys.TOOLBELT.isPressed()) COOLDOWN--;
    }
    ...
    public static boolean onPickItem() {
        ...
        if (toolboxes.isEmpty()) return false;
        ...
        for (ToolboxBlockEntity toolboxBlockEntity : toolboxes) {
            ...
            for (int comp = 0; comp < 8; comp++) {
                ...
                if (inSlot.getItem() != result.getItem()) continue;
                if (!ItemStack.matches(inSlot, result)) continue;

                CatnipServices.NETWORK.sendToServer(new ToolboxEquipPacket(
                        toolboxBlockEntity.getBlockPos(), comp, player.getInventory().selected));
                return true;
            }
        }
    ...
    public static void onKeyInput(int key, boolean pressed) {
        ...
        if (!AllKeys.TOOLBELT.doesModifierAndCodeMatch(key)) return;
        ...
        if (equipped) {
            ...
            ScreenOpener.open(new RadialToolboxMenu(
                    ImmutableList.of(), RadialToolboxMenu.State.DETACH, null));
            return;
        }

        if (toolboxes.isEmpty()) return;

        if (toolboxes.size() == 1)
            ScreenOpener.open(new RadialToolboxMenu(
                    toolboxes, RadialToolboxMenu.State.SELECT_ITEM, toolboxes.get(0)));
        else
            ScreenOpener.open(
    ...
    public static void renderOverlay(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (!persistentData.contains("CreateToolboxData")) return;
