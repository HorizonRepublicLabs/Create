...
public class CylinderBrush extends ShapedBrush {
        ...
        for (int i = 0; i <= MAX_RADIUS; i++) {
            ...
            List<BlockPos> positions = BlockPos.betweenClosedStream(
                    ...
                    .filter(p -> VecHelper.getCenterOf(p)
                                    .distanceTo(VecHelper.getCenterOf(BlockPos.ZERO))
                            < radius + .42f)
            ...
            for (int h = 0; h <= MAX_HEIGHT; h++) {
                ...
                for (int layer = 0; layer < h; layer++) {
                    ...
                    for (BlockPos p : positions) stackedPositions.add(p.above(yOffset));
                }
    ...
    public BlockPos getOffset(Vec3 ray, Direction face, PlacementOptions option) {
        if (option == PlacementOptions.Merged) return BlockPos.ZERO;
        ...
        return BlockPos.ZERO.relative(
                face,
                (face.getAxis().isVertical() ? y : r)
                        * (option == PlacementOptions.Attached ? 1 : -1));
    }
    ...
    Component getParamLabel(int paramIndex) {
        return paramIndex == 0
                ? CreateLang.translateDirect("generic.radius")
                : super.getParamLabel(paramIndex);
    }
