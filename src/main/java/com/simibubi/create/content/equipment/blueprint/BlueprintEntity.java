...
public class BlueprintEntity extends HangingEntity
    ...
    public BlueprintEntity(
            Level world, BlockPos pos, Direction facing, Direction verticalOrientation) {
    ...
    public static EntityType.Builder<?> build(EntityType.Builder<?> builder) {
        @SuppressWarnings("unchecked")
        EntityType.Builder<BlueprintEntity> entityBuilder =
                (EntityType.Builder<BlueprintEntity>) builder;
    ...
    protected void updateFacingWithBoundingBox(Direction facing, Direction verticalOrientation) {
        ...
        if (facing.getAxis().isHorizontal()) {
            ...
            setYRot(
                    verticalOrientation.getAxis().isHorizontal()
                            ? 180 + verticalOrientation.toYRot()
                            : 0);
        }
        ...
        if (size == 2)
            pos = pos.add(Vec3.atLowerCornerOf(
                                    axis.isHorizontal()
                                            ...
                                            : verticalOrientation.getClockWise().getNormal())
                            .scale(0.5))
                    .add(Vec3.atLowerCornerOf(
                                    axis.isHorizontal()
                                            ? Direction.UP.getNormal()
                                            : direction == Direction.UP
                                                    ? verticalOrientation.getNormal()
                                                    : verticalOrientation
                                                            .getOpposite()
                                                            .getNormal())
                            .scale(0.5));
    ...
    public boolean survives() {
        ...
        Direction upDirection = direction.getAxis().isHorizontal()
                ? Direction.UP
                : direction == Direction.UP
                        ? verticalOrientation
                        : verticalOrientation.getOpposite();
        Direction newDirection = direction.getAxis().isVertical()
        ...
        return this.level()
                .getEntities(this, this.getBoundingBox(), HANGING_ENTITY)
                .isEmpty();
    }
    ...
    public boolean skipAttackInteraction(Entity source) {
        ...
        if (!rayTrace.isPresent()) return super.skipAttackInteraction(source);

        Vec3 hitVec = rayTrace.get();
        BlueprintSection sectionAt = getSectionAt(hitVec.subtract(position()));
        ItemStackHandler items = sectionAt.getItems();

        if (items.getStackInSlot(9).isEmpty()) return super.skipAttackInteraction(source);
    ...
    public void lerpTo(double pX, double pY, double pZ, float pYRot, float pXRot, int pSteps) {
        BlockPos blockpos = this.pos.offset(
                BlockPos.containing(pX - this.getX(), pY - this.getY(), pZ - this.getZ()));
            ...
            do {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    for (int slot = 0; slot < playerInv.getSlots(); slot++) {
                        if (!requestedItem.test(level(), playerInv.getStackInSlot(slot))) continue;
                        ItemStack currentItem = playerInv.extractItem(slot, 1, false);
                        if (stacksTaken.containsKey(slot)) stacksTaken.get(slot).grow(1);
                        else stacksTaken.put(slot, currentItem.copy());
                ...
                if (success) {
                    CraftingContainer craftingInventory =
                            new BlueprintCraftingInventory(craftingGrid);
                    ...
                    if (!recipe.isPresent())
                        recipe = level().getRecipeManager()
                                .getRecipeFor(
                                        ...
                                        level());
                    ItemStack result = recipe.filter(r ->
                                    r.value().matches(craftingInventory.asCraftInput(), level()))
                            .map(r -> r.value()
                                    .assemble(craftingInventory.asCraftInput(), registryAccess()))
    ...
    static class BlueprintCraftingInventory extends TransientCraftingContainer {
        ...
        private static final AbstractContainerMenu dummyContainer =
    ...
    public CompoundTag getOrCreateRecipeCompound() {
        ...
        if (!persistentData.contains("Recipes")) persistentData.put("Recipes", new CompoundTag());
    ...
    class BlueprintSection implements MenuProvider, IInteractionChecker {
        ...
        public Couple<ItemStack> getDisplayItems() {
            if (cachedDisplayItems != null) return cachedDisplayItems;
            ItemStackHandler items = getItems();
            return cachedDisplayItems =
                    Couple.create(items.getStackInSlot(9), items.getStackInSlot(10));
        }
        ...
        public ItemStackHandler getItems() {
            ...
            if (!invNBT.isEmpty()) newInv.deserializeNBT(registryAccess(), invNBT);
            return newInv;
        }
        ...
        public void save(ItemStackHandler inventory) {
            ...
            if (!level().isClientSide) syncPersistentDataWithTracking(BlueprintEntity.this);
        }
        ...
        public Component getDisplayName() {
            return AllItems.CRAFTING_BLUEPRINT.get().getDescription();
        }
