...
public class ZapperRenderHandler extends ShootableGadgetRenderHandler {
    ...
    public void tick() {
        ...
        if (cachedBeams == null) cachedBeams = new LinkedList<>();

        cachedBeams.removeIf(b -> b.itensity < .1f);
        if (cachedBeams.isEmpty()) return;

        cachedBeams.forEach(beam -> {
            Outliner.getInstance()
                    .endChasingLine(beam, beam.start, beam.end, 1 - beam.itensity, false)
                    .disableLineNormals()
                    .colored(0xffffff)
