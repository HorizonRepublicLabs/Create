...
public interface IWrenchable {
    ...
    default BlockState updateAfterWrenched(BlockState newState, UseOnContext context) {
        ...
        return Block.updateFromNeighbourShapes(
                newState, context.getLevel(), context.getClickedPos());
    }
    ...
    static void playRemoveSound(Level level, BlockPos pos) {
        AllSoundEvents.WRENCH_REMOVE.playOnServer(
                level, pos, 1, Create.RANDOM.nextFloat() * .5f + .5f);
    }
    ...
    default BlockState getRotatedBlockState(BlockState originalState, Direction targetedFace) {
        ...
        if (targetedFace.getAxis() == Direction.Axis.Y) {
            if (originalState.hasProperty(HorizontalAxisKineticBlock.HORIZONTAL_AXIS))
                return originalState.setValue(
                        ...
                        VoxelShaper.axisAsFace(originalState.getValue(
                                        HorizontalAxisKineticBlock.HORIZONTAL_AXIS))
        ...
        if (!originalState.hasProperty(DirectionalKineticBlock.FACING)) return originalState;
        ...
        if (stateFacing.getAxis().equals(targetedFace.getAxis())) {
            ...
            do {
                newState = newState.setValue(
                        ...
                        newState.getValue(DirectionalKineticBlock.FACING)
                                .getClockWise(targetedFace.getAxis()));
                if (targetedFace.getAxis() == Direction.Axis.Y
                        && newState.hasProperty(
                                DirectionalAxisKineticBlock.AXIS_ALONG_FIRST_COORDINATE))
                    newState =
                            newState.cycle(DirectionalAxisKineticBlock.AXIS_ALONG_FIRST_COORDINATE);
