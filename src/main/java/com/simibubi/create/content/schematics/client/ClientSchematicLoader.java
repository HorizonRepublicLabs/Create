...
public class ClientSchematicLoader {
    ...
    public void startNewUpload(String schematic) {
        ...
        try {
            ...
            // Too big
            if (!validateSizeLimitation(size)) return;
            ...
            if (!isGZIPEncoded(path.toFile())) {
                ...
                if (player != null)
                    player.displayClientMessage(
                            CreateLang.translateDirect("schematics.wrongFormat"), false);
                return;
            }
        ...
        if (size > maxSize * 1000) {
            ...
            if (player != null) {
                player.displayClientMessage(
                        ...
                        false);
                player.displayClientMessage(
                        CreateLang.translateDirect("schematics.maxAllowedSize")
                                .append(" " + maxSize + " KB"),
                        false);
            }
            return false;
    ...
    private void continueUpload(String schematic) {
        if (activeUploads.containsKey(schematic)) {
            int maxPacketSize =
                    AllConfigs.server().schematics.maxSchematicPacketSize.get();
            byte[] data = new byte[maxPacketSize];
            try {
                int status = activeUploads.get(schematic).read(data);

                if (status != -1) {
                    if (status < maxPacketSize) data = Arrays.copyOf(data, status);
    ...
    public void refresh() {
        ...
        try (Stream<Path> paths = Files.list(CreatePaths.SCHEMATICS_DIR)) {
            paths.filter(f ->
                            !Files.isDirectory(f) && f.getFileName().toString().endsWith(".nbt"))
                    .forEach(path -> {
        ...
        availableSchematics.sort((aT, bT) -> {
            ...
            if (asNumeric)
                if (aLength > bLength
                        && a.charAt(bLength) >= '0'
                        && a.charAt(bLength) <= '9') // as number
                return 1; // a has bigger size, thus b is smaller
                else if (bLength > aLength
                        && b.charAt(aLength) >= '0'
                        && b.charAt(aLength) <= '9') // as number
