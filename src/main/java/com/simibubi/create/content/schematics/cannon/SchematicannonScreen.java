...
public class SchematicannonScreen extends AbstractSimiContainerScreen<SchematicannonMenu> {
    ...
    private final Component listPrinter =
            CreateLang.translateDirect("gui.schematicannon.listPrinter");
    ...
    private final Component optionDisabled =
            CreateLang.translateDirect("gui.schematicannon.optionDisabled");
    ...
    protected void init() {
        ...
        extraAreas = ImmutableList.of(new Rect2i(
                x + BG_TOP.getWidth(),
                y + BG_TOP.getHeight() + BG_BOTTOM.getHeight() - 62,
                84,
                92));
    ...
    private void initPlacementSettings() {
        ...
        for (int i = 0; i < 4; i++) {
            replaceLevelIndicators.add(
                    new Indicator(x + 33 + i * 18, y + 111, CommonComponents.EMPTY));
        ...
        skipMissingButton.setToolTip(
                CreateLang.translateDirect("gui.schematicannon.option.skipMissing"));
        ...
        skipBlockEntitiesButton.setToolTip(
                CreateLang.translateDirect("gui.schematicannon.option.skipBlockEntities"));
    ...
    protected void containerTick() {
        ...
        if (!placementSettingsHidden()) {
            for (int replaceMode = 0; replaceMode < replaceLevelButtons.size(); replaceMode++) {
                ...
                replaceLevelIndicators.get(replaceMode).state =
                        replaceMode == be.replaceMode ? State.ON : State.OFF;
            }
    ...
    protected void handleTooltips() {
        ...
        if (hasShiftDown()) {
            ...
            fillToolTip(
                    replaceLevelButtons.get(0), replaceLevelIndicators.get(0), "dontReplaceSolid");
            fillToolTip(
                    replaceLevelButtons.get(1), replaceLevelIndicators.get(1), "replaceWithSolid");
            fillToolTip(
                    replaceLevelButtons.get(2), replaceLevelIndicators.get(2), "replaceWithAny");
            fillToolTip(
                    replaceLevelButtons.get(3), replaceLevelIndicators.get(3), "replaceWithEmpty");
        }
    }
    ...
    private void fillToolTip(IconButton button, Indicator indicator, String tooltipKey) {
        ...
        tip.add((enabled ? optionEnabled : optionDisabled)
                .plainCopy()
        ...
        tip.addAll(TooltipHelper.cutTextComponent(
                CreateLang.translateDirect(
                        "gui.schematicannon.option." + tooltipKey + ".description"),
                Palette.ALL_GRAY));
    }
    ...
    protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
        ...
        if (!be.inventory.getStackInSlot(0).isEmpty()) renderBlueprintHighlight(graphics, x, y);

        GuiGameElement.of(renderedItem)
                .<GuiGameElement.GuiRenderBuilder>at(
                        x + BG_TOP.getWidth(),
                        y + BG_TOP.getHeight() + BG_BOTTOM.getHeight() - 48,
                        -200)
                .scale(5)
                .render(graphics);
        ...
        graphics.drawString(
                font,
                title,
                x + (BG_TOP.getWidth() - 8 - font.width(title)) / 2,
                y + 2,
                0x505050,
                false);
    ...
    protected void renderChecklistPrinterProgress(
            GuiGraphics graphics, int x, int y, float progress) {
    ...
    protected void renderForeground(
        ...
        if (hoveredSlot != null && !hoveredSlot.hasItem()) {
            if (hoveredSlot.index == 0)
                graphics.renderComponentTooltip(
                        ...
                        TooltipHelper.cutTextComponent(
                                CreateLang.translateDirect(_slotSchematic), Palette.GRAY_AND_BLUE),
                        mouseX,
                        mouseY);
            if (hoveredSlot.index == 2)
                graphics.renderComponentTooltip(
                        font,
                        TooltipHelper.cutTextComponent(
                                CreateLang.translateDirect(_slotListPrinter),
                                Palette.GRAY_AND_BLUE),
                        mouseX,
                        mouseY);
            if (hoveredSlot.index == 4)
                graphics.renderComponentTooltip(
                        font,
                        TooltipHelper.cutTextComponent(
                                CreateLang.translateDirect(_slotGunpowder), Palette.GRAY_AND_BLUE),
                        mouseX,
                        mouseY);
        }
        ...
        if (be.missingItem != null) {
            ...
            if (mouseX >= missingBlockX
                    && mouseY >= missingBlockY
                    && mouseX <= missingBlockX + 16
    ...
    protected List<Component> getFuelLevelTooltip(SchematicannonBlockEntity be) {
        ...
        int shotsLeftWithItems =
                shotsLeft + be.inventory.getStackInSlot(4).getCount() * be.getShotsPerGunpowder();
        List<Component> tooltip = new ArrayList<>();

        if (be.hasCreativeCrate) {
            tooltip.add(CreateLang.translateDirect(_gunpowderLevel, "" + 100));
            tooltip.add(Component.literal("(")
                    .append(AllBlocks.CREATIVE_CRATE.get().getName())
                    .append(")")
        ...
        tooltip.add(CreateLang.translateDirect(
                        _shotsRemaining,
                        Component.literal(Integer.toString(shotsLeft)).withStyle(BLUE))
                .withStyle(GRAY));
        if (shotsLeftWithItems != shotsLeft) {
            tooltip.add(CreateLang.translateDirect(
                            _shotsRemainingWithBackup,
                            Component.literal(Integer.toString(shotsLeftWithItems))
                                    .withStyle(BLUE))
                    .withStyle(GRAY));
        }
