...
public abstract class LaunchedItem {
    ...
    public static LaunchedItem fromNBT(
            CompoundTag c, HolderLookup.Provider registries, HolderGetter<Block> holderGetter) {
        LaunchedItem launched = c.contains("Length")
                ? new LaunchedItem.ForBelt()
                : c.contains("BlockState")
    ...
    public static class ForBelt extends ForBlockState {
        ...
        public CompoundTag serializeNBT(HolderLookup.Provider registries) {
            ...
            serializeNBT.putIntArray(
                    "Casing", Arrays.stream(casings).map(CasingType::ordinal).toList());
            return serializeNBT;
        }
        ...
        void readNBT(
            ...
            for (int i = 0; i < casings.length; i++)
                casings[i] = i >= intArray.length
                        ? CasingType.NONE
                        : CasingType.values()[
                                Mth.clamp(intArray[i], 0, CasingType.values().length - 1)];
            super.readNBT(nbt, registries, holderGetter);
        }
        ...
        void place(Level world) {
            ...
            for (int segment = 0; segment < length; segment++) {
                if (casings[segment] == CasingType.NONE) continue;
                BlockPos casingTarget = target.offset(
                        offset.getX() * segment, offset.getY() * segment, offset.getZ() * segment);
    ...
    public static class ForEntity extends LaunchedItem {
        ...
        public boolean update(Level world) {
            if (deferredTag != null && entity == null) {
                try {
                    ...
                    if (!loadEntityUnchecked.isPresent()) return true;
        ...
        public CompoundTag serializeNBT(HolderLookup.Provider registries) {
            CompoundTag serializeNBT = super.serializeNBT(registries);
            if (entity != null) serializeNBT.put("Entity", entity.serializeNBT(registries));
            return serializeNBT;
        }
        ...
        void readNBT(
                ...
                HolderGetter<Block> holderGetter) {
            super.readNBT(nbt, registries, holderGetter);
            if (nbt.contains("Entity")) deferredTag = nbt.getCompound("Entity");
        }
