...
public class SchematicTransformation {
    ...
    public void applyTransformations(PoseStack ms, Vec3 camera) {
        ...
        TransformStack.of(ms)
                .translate(VecHelper.lerp(pt, prevChasingPos, chasingPos).subtract(camera));
    ...
    public StructurePlaceSettings toSettings() {
        ...
        if (mirrorfb) settings.setMirror(Mirror.FRONT_BACK);
        if (mirrorlr) settings.setMirror(Mirror.LEFT_RIGHT);
    ...
    public int getMirrorModifier(Axis axis) {
        if (axis == Axis.Z) return (int) getScaleLR().getChaseTarget();
    ...
    public void flip(Axis axis) {
        if (axis == Axis.X) getScaleLR().updateChaseTarget(getScaleLR().getChaseTarget() * -1);
        if (axis == Axis.Z) getScaleFB().updateChaseTarget(getScaleFB().getChaseTarget() * -1);
    }
