...
public class SchematicannonRenderer extends SafeBlockEntityRenderer<SchematicannonBlockEntity> {
    ...
    protected void renderSafe(
        ...
        SuperByteBuffer connector =
                CachedBuffers.partial(AllPartialModels.SCHEMATICANNON_CONNECTOR, state);
    ...
    public static double[] getCannonAngles(
            SchematicannonBlockEntity blockEntity, BlockPos pos, float partialTicks) {
        double yaw;
        double pitch;
    ...
    public static double getRecoil(SchematicannonBlockEntity blockEntity, float partialTicks) {
        ...
        for (LaunchedItem launched : blockEntity.flyingBlocks) {
            ...
            if ((launched.ticksRemaining + 1 - partialTicks) > launched.totalTicks - 10)
                recoil = Math.max(
                        recoil,
                        (launched.ticksRemaining + 1 - partialTicks) - launched.totalTicks + 10);
        }
    ...
    private static void renderLaunchedBlocks(
        ...
        for (LaunchedItem launched : blockEntity.flyingBlocks) {
            ...
            float progress =
                    ((float) launched.totalTicks - (launched.ticksRemaining + 1 - partialTicks))
                            / launched.totalTicks;
