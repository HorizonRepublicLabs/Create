...
public enum TrackShape implements StringRepresentable {
    ...
    private final List<Vec3> axes;
    private final int modelRotation;
    ...
    public TrackShape mirror(Mirror mirror) {
        return mirror == Mirror.NONE
                ...
                : mirror == Mirror.FRONT_BACK
                        ? xMirror.getOrDefault(this, this)
                        : zMirror.getOrDefault(this, this);
    }

    public TrackShape rotate(Rotation rotation) {
        TrackShape shape = this;
        for (int i = 0; i < rotation.ordinal(); i++) shape = clockwise.getOrDefault(shape, shape);
        return shape;
    }
}
