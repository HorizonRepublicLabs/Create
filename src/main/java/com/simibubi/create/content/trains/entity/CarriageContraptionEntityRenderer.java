...
public class CarriageContraptionEntityRenderer
    ...
    public boolean shouldRender(
        ...
        if (carriage != null)
            for (CarriageBogey bogey : carriage.bogeys)
                if (bogey != null) bogey.couplingAnchors.replace(v -> null);
    ...
    public void render(
            ...
            int overlay) {
        if (!entity.validForRender || entity.firstPositionUpdate) return;
        ...
        carriage.bogeys.forEach(bogey -> {
            ...
            BlockPos bogeyPos = bogey.isLeading
                    ? BlockPos.ZERO
                    : BlockPos.ZERO.relative(
                            entity.getInitialOrientation().getCounterClockWise(), bogeySpacing);

            if (!VisualizationManager.supportsVisualization(entity.level())
                    && !entity.getContraption().isHiddenInPortal(bogeyPos)) {
            ...
            bogey.updateCouplingAnchor(
                    position, viewXRot, viewYRot, bogeySpacing, partialTicks, bogey.isLeading);
