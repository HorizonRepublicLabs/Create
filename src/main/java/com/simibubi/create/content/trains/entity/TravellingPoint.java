...
public class TravellingPoint {
    ...
    public interface ITrackSelector
            extends BiFunction<
                    TrackGraph,
                    Pair<Boolean, List<Entry<TrackNode, TrackEdge>>>,
                    Entry<TrackNode, TrackEdge>> {}

    public interface IEdgePointListener
    ...
    public TravellingPoint(
            TrackNode node1, TrackNode node2, TrackEdge edge, double position, boolean upsideDown) {
        this.node1 = node1;
        this.node2 = node2;
        this.edge = edge;
    ...
    public ITrackSelector random() {
        return (graph, pair) ->
                pair.getSecond().get(Create.RANDOM.nextInt(pair.getSecond().size()));
    }
        ...
        return (graph, pair) -> {
            ...
            for (Entry<TrackNode, TrackEdge> entry : validTargets)
                if (entry.getKey() == target || entry.getKey() == secondary) {
                    if (success != null) success.accept(true);
                    return entry;
                }

            List<List<Entry<TrackNode, TrackEdge>>> frontiers =
                    new ArrayList<>(validTargets.size());
            ...
            for (int i = 0; i < 20; i++) {
                for (int j = 0; j < validTargets.size(); j++) {
                    ...
                    for (Entry<TrackNode, TrackEdge> nextEntry :
                            graph.getConnectionsFrom(currentEntry.getKey()).entrySet()) {
                        TrackEdge nextEdge = nextEntry.getValue();
                        if (!visiteds.get(j).add(nextEdge)) continue;
                        if (!currentEntry.getValue().canTravelTo(nextEdge)) continue;
            ...
            if (success != null) success.accept(false);
    ...
    public double travel(
        ...
        return travel(
                graph, distance, trackSelector, signalListener, turnListener, ignorePortals());
    }
    ...
    public double travel(
        ...
        if (Mth.equal(distance, 0)) return 0;
        ...
        Double blockedLocation = edgeTraversedFrom(
                graph, forward, signalListener, turnListener, prevPos, collectedDistance);
        ...
        if (forward) {
            ...
            while (position > edgeLength) {
                ...
                for (Entry<TrackNode, TrackEdge> entry :
                        graph.getConnectionsFrom(node2).entrySet()) {
                    TrackNode newNode = entry.getKey();
                    if (newNode == node1) continue;

                    TrackEdge newEdge = entry.getValue();
                    if (!edge.canTravelTo(newEdge)) continue;
                ...
                Entry<TrackNode, TrackEdge> entry = validTargets.size() == 1
                        ? validTargets.get(0)
                        : trackSelector.apply(graph, Pair.of(true, validTargets));

                if (entry.getValue().getLength() == 0
                        && portalListener.test(Couple.create(
                                node2.getLocation(), entry.getKey().getLocation()))) {
                    traveled -= position - edgeLength;
                    position = edgeLength;
                    blocked = true;
                    break;
                ...
                if (edge.isTurn()) turnListener.accept(collectedDistance, edge);
            ...
            while (position < 0) {
                ...
                for (Entry<TrackNode, TrackEdge> entry :
                        graph.getConnectionsFrom(node1).entrySet()) {
                    TrackNode newNode = entry.getKey();
                    if (newNode == node2) continue;
                    if (!graph.getConnectionsFrom(newNode).get(node1).canTravelTo(edge)) continue;
                ...
                if (entry.getValue().getLength() == 0
                        && portalListener.test(
                                Couple.create(entry.getKey().getLocation(), node1.getLocation()))) {
                    traveled -= position;
                    position = 0;
                    blocked = true;
                    break;
    ...
    protected Double edgeTraversedFrom(
        ...
        if (edge.isTurn()) turnListener.accept(Math.max(0, totalDistance), edge);
        ...
        for (int i = 0; i < edgePoints.size(); i++) {
            ...
            if (forward
                    ? (locationOn < from || locationOn >= to)
                    : (locationOn <= from || locationOn > to)) continue;
            Couple<TrackNode> nodes = Couple.create(node1, node2);
            if (edgePointListener.test(
                    totalDistance + distance,
                    Pair.of(nextBoundary, forward ? nodes : nodes.swap()))) return locationOn;
        }
    ...
    public Vec3 getPositionWithOffset(
            @Nullable TrackGraph trackGraph, double offset, boolean flipUpsideDown) {
    ...
    public CompoundTag write(DimensionPalette dimensions) {
        ...
        Couple<TrackNode> nodes = Couple.create(node1, node2);
        if (nodes.either(Objects::isNull)) return tag;
        tag.put(
                "Nodes",
                nodes.map(TrackNode::getLocation).serializeEach(loc -> loc.write(dimensions)));
        tag.putDouble("Position", position);
        tag.putBoolean("UpsideDown", upsideDown);
        return tag;
    }

    public static TravellingPoint read(
            CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
        if (graph == null) return new TravellingPoint(null, null, null, 0, false);
        ...
        if (locs.either(Objects::isNull)) return new TravellingPoint(null, null, null, 0, false);
