...
public class TrackModel extends BakedModelWrapper<BakedModel> {
    ...
    public @NotNull List<BakedQuad> getQuads(
        ...
        if (templateQuads.isEmpty()) return templateQuads;
        if (!extraData.has(TrackBlockEntityTilt.ASCENDING_PROPERTY)) return templateQuads;
        ...
        for (BakedQuad templateQuad : templateQuads) {
            ...
            for (int j = 0; j < 4; j++)
                BakedQuadHelper.setXYZ(
                        vertexData, j, transform.apply(BakedQuadHelper.getXYZ(vertexData, j)));
            quads.add(quad);
        }
