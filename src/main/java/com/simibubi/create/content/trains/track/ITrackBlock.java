...
public interface ITrackBlock {
    ...
    BlockState getBogeyAnchor(
            BlockGetter world, BlockPos pos, BlockState state); // should be on bogey side

    boolean trackEquals(BlockState state1, BlockState state2);

    default BlockState overlay(
    ...
    static Collection<DiscoveredLocation> walkConnectedTracks(
            BlockGetter worldIn, TrackNodeLocation location, boolean linear) {
        BlockGetter world = location != null && worldIn instanceof ServerLevel sl
                ? sl.getServer().getLevel(location.dimension)
                : worldIn;
    ...
    default Collection<DiscoveredLocation> getConnected(
        ...
        trackAxes.forEach(axis -> {
            BiFunction<Double, Boolean, Vec3> offsetFactory =
                    (d, b) -> axis.scale(b ? d : -d).add(center);
    ...
    static TrackMaterial getMaterialSimple(
            BlockGetter world, Vec3 pos, TrackMaterial defaultMaterial) {
        if (defaultMaterial == null) defaultMaterial = TrackMaterial.ANDESITE;
        if (world != null) {
    ...
    static void addToListIfConnected(
        ...
        DiscoveredLocation firstLocation = new DiscoveredLocation(
                        dimensionFactory.apply(true), firstOffset)
                .viaTurn(viaTurn)
        ...
        DiscoveredLocation secondLocation = new DiscoveredLocation(
                        dimensionFactory.apply(false), secondOffset)
                .viaTurn(viaTurn)
        ...
        if (fromEnd != null) {
            ...
            if (equalsFirst) skipFirst = true;
            if (equalsSecond) skipSecond = true;
        }

        if (!skipFirst) list.add(firstLocation);
    ...
    @OnlyIn(Dist.CLIENT)
    PartialModel prepareAssemblyOverlay(
    ...
    default Pair<Vec3, AxisDirection> getNearestTrackAxis(
            BlockGetter world, BlockPos pos, BlockState state, Vec3 lookVec) {
        Vec3 best = null;
        ...
        return Pair.of(
                best,
                lookVec.dot(best.multiply(1, 0, 1).normalize()) < 0
