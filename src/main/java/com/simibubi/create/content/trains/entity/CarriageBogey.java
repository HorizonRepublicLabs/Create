...
public class CarriageBogey {
    ...
    public ResourceKey<Level> getDimension() {
        ...
        if (leading.edge == null || trailing.edge == null) return null;
        if (leading.edge.isInterDimensional() || trailing.edge.isInterDimensional()) return null;
        ...
        if (dimension1.equals(dimension2)) return dimension1;
        return null;
    }
    ...
    public double getStress() {
        if (getDimension() == null) return 0;
        if (carriage.train.derailed) return 0;
        return type.getWheelPointSpacing()
                - leading()
    ...
    public Vec3 getAnchorPosition(boolean flipUpsideDown) {
        if (leading().edge == null) return null;
    ...
    public CompoundTag write(DimensionPalette dimensions) {
        ...
        tag.putString("Type", RegisteredObjectsHelper.getKeyOrThrow(type).toString());
    ...
    public static CarriageBogey read(
        ...
        Couple<TravellingPoint> points = Couple.deserializeEach(
                tag.getList("Points", Tag.TAG_COMPOUND),
