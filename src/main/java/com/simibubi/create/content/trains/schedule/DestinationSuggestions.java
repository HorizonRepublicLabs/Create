...
public class DestinationSuggestions extends CommandSuggestions {
    ...
    public void tick() {
        if (suggestions == null) textBox.setSuggestion("");
        if (active == textBox.isFocused()) return;
    ...
    public void updateCommandInfo() {
        if (textBox.getValue().length() < textBox.getCursorPosition()) return;
        ...
        currentSuggestions = viableStations.stream()
                .filter(ia -> !ia.getValue().equals(value)
                        && ia.getValue().toLowerCase().startsWith(value.toLowerCase()))
    ...
    public void showSuggestions(boolean pNarrateFirstSuggestion) {
        ...
        int x = Mth.clamp(
                textBox.getScreenX(0), 0, textBox.getScreenX(0) + textBox.getInnerWidth() - width);
        suggestions = new CommandSuggestions.SuggestionsList(
                x, 72 + yOffset, width, currentSuggestions, false);
    }
