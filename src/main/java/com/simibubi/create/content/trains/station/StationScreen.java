...
public class StationScreen extends AbstractStationScreen {
    ...
    private final LerpedFloat trainPosition;
    ...
    protected void init() {
        ...
        disassembleTrainButton =
                new WideIconButton(x + 94, y + 65, AllGuiTextures.I_DISASSEMBLE_TRAIN);
        ...
        dropScheduleButton.withCallback(() -> CatnipServices.NETWORK.sendToServer(
                StationEditPacket.dropSchedule(blockEntity.getBlockPos())));
        addRenderableWidget(dropScheduleButton);

        colorTypeScroll = new ScrollInput(x + 166, y + 17, 22, 14)
        ...
        trainNameBox = new EditBox(
                font, x + 23, y + 47, background.getWidth() - 75, 10, CommonComponents.EMPTY);
    ...
    public void tick() {
        ...
        if (messedWithColors > 0) {
            messedWithColors--;
            if (messedWithColors == 0) syncTrainNameAndColor();
        }

        super.tick();

        updateAssemblyTooltip(
                blockEntity.edgePoint.isOnCurve()
                        ? "no_assembly_curve"
                        : !blockEntity.edgePoint.isOrthogonal()
                                ? "no_assembly_diagonal"
                                : trainPresent() && !blockEntity.trainCanDisassemble
                                        ? "train_not_aligned"
                                        : null);
    }
    ...
    private void tickTrainDisplay() {
        ...
        if (train == null) {
            ...
            if (imminentTrain != null) {
                ...
                if (trainIconWidth > 130) targetPos -= trainIconWidth - 130;
        ...
        disassembleTrainButton.active = trainAtStation
                && blockEntity.trainCanDisassemble
                && blockEntity.edgePoint.isOrthogonal();
        dropScheduleButton.active = blockEntity.trainHasSchedule;

        if (blockEntity.trainHasSchedule)
            dropScheduleButton.setToolTip(CreateLang.translateDirect(
                    blockEntity.trainHasAutoSchedule
                            ? "station.remove_auto_schedule"
                            : "station.remove_schedule"));
        else dropScheduleButton.getToolTip().clear();
    ...
    private void updateAssemblyTooltip(String key) {
        if (key == null) {
            disassembleTrainButton.setToolTip(
                    CreateLang.translateDirect("station.disassemble_train"));
        ...
        for (IconButton ib : new IconButton[] {disassembleTrainButton, newTrainButton}) {
            ...
            toolTip.clear();
            toolTip.add(
                    CreateLang.translateDirect("station." + key).withStyle(ChatFormatting.GRAY));
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (!nameBox.isFocused())
            AllGuiTextures.STATION_EDIT_NAME.render(
                    graphics, nameBoxX(text, nameBox) + font.width(text) + 5, y + 1);
        ...
        if (train == null) {
            ...
            graphics.drawString(
                    font, header, x + 97 - font.width(header) / 2, y + 47, 0x7A7A7A, false);
            return;
        }
        ...
        for (int i = carriages.size() - 1; i > 0; i--) {
            RenderSystem.setShaderColor(
                    ...
                    Math.min(
                            1f,
                            Math.min(
                                    (position + offset - 10) / 30f,
                                    (background.getWidth() - 40 - position - offset) / 30f)));
            Carriage carriage =
                    carriages.get(blockEntity.trainBackwards ? carriages.size() - i - 1 : i);
        ...
        UIRenderHelper.drawStretched(
                graphics, x + 21, y + 60, 150, 26, 0, AllGuiTextures.STATION_TEXTBOX_MIDDLE);
        ...
        if (!trainNameBox.isFocused()) {
            ...
            AllGuiTextures.STATION_EDIT_TRAIN_NAME.render(
                    graphics, Math.min(buttonX, guiLeft + 156), y + 44);
            if (font.width(text) > trainNameBox.getWidth())
                graphics.drawString(font, "...", guiLeft + 26, guiTop + 47, 0xa6a6a6);
        }

        if (!mapModsPresent()) return;
    ...
    public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
        boolean hitEnter = getFocused() instanceof EditBox
                && (pKeyCode == InputConstants.KEY_RETURN
                        || pKeyCode == InputConstants.KEY_NUMPADENTER);
    ...
    private void syncTrainNameAndColor() {
        ...
        if (train != null && !trainNameBox.getValue().equals(train.name.getString()))
            CatnipServices.NETWORK.sendToServer(new TrainEditPacket.Serverbound(
                    train.id, trainNameBox.getValue(), train.icon.getId(), train.mapColorIndex));
    }

    private void syncStationName() {
        if (!nameBox.getValue().equals(station.name))
            CatnipServices.NETWORK.sendToServer(StationEditPacket.configure(
                    blockEntity.getBlockPos(), false, nameBox.getValue(), doorControl));
    }

    @Override
    public void removed() {
        super.removed();
        if (nameBox == null || trainNameBox == null) return;
        CatnipServices.NETWORK.sendToServer(StationEditPacket.configure(
        ...
        if (!switchingToAssemblyMode)
            CatnipServices.NETWORK.sendToServer(new TrainEditPacket.Serverbound(
                    train.id, trainNameBox.getValue(), train.icon.getId(), train.mapColorIndex));
        else blockEntity.imminentTrain = null;
    }

    @Override
    protected PartialModel getFlag(float partialTicks) {
        return blockEntity.flag.getValue(partialTicks) > 0.75f
                ? AllPartialModels.STATION_ON
