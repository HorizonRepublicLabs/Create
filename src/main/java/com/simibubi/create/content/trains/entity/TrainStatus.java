...
public class TrainStatus {
    ...
    public void crash() {
        Component component = Component.literal(" - ")
                ...
                .append(CreateLang.translateDirect("train.status.collision")
                        .withStyle(st -> st.withColor(0xFFD3B4)));
        ...
        Stream<Component> locationComponents = presentDimensions.stream().map(key -> {
            return Component.literal(" - ")
                    .withStyle(ChatFormatting.GRAY)
                    .append(CreateLang.translateDirect(
    ...
    public void tick(Level level) {
        ...
        if (owner instanceof Player player) {
            player.displayClientMessage(
                    CreateLang.translateDirect("train.status", train.name)
    ...
    public void addMessage(StatusMessage message) {
        ...
        if (queued.size() > 3) queued.remove(0);
    }
    ...
    public record StatusMessage(Component... messages) {
        public void displayToPlayer(Player player) {
            Arrays.stream(messages)
                    .forEach(messages -> player.displayClientMessage(messages, false));
        }
    }
}
