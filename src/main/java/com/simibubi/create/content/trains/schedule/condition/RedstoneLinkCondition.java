...
public class RedstoneLinkCondition extends ScheduleWaitCondition {
    ...
    public List<Component> getTitleAs(String type) {
        return ImmutableList.of(
                CreateLang.translateDirect("schedule.condition.redstone_link.frequency_"
    ...
    protected void writeAdditional(HolderLookup.Provider registries, CompoundTag tag) {
        tag.put("Frequency", freq.serializeEach(f ->
                (CompoundTag) f.getStack().saveOptional(registries)));
    }
    ...
    protected void readAdditional(HolderLookup.Provider registries, CompoundTag tag) {
        if (tag.contains("Frequency"))
            freq = Couple.deserializeEach(
                    tag.getList("Frequency", Tag.TAG_COMPOUND),
                    c -> Frequency.of(ItemStack.parseOptional(registries, c)));
    }
    ...
    public void initConfigurationWidgets(ModularGuiLineBuilder builder) {
        builder.addSelectionScrollInput(
                ...
                (i, l) -> i.forOptions(CreateLang.translatedOptions(
                                "schedule.condition.redstone_link", "powered", "unpowered"))
                        .titled(CreateLang.translateDirect(
                                "schedule.condition.redstone_link.frequency_state")),
                "Inverted");
    }
