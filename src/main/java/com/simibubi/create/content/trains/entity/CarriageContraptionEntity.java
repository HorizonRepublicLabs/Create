...
public class CarriageContraptionEntity extends OrientedContraptionEntity {
    ...
    private static final EntityDataAccessor<Boolean> SCHEDULED = SynchedEntityData.defineId(
            CarriageContraptionEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public CarriageContraptionEntity(EntityType<?> type, Level world) {
        ...
        derailParticleOffset =
                VecHelper.offsetRandomly(Vec3.ZERO, world.random, 1.5f).multiply(1, .25f, 1);
    }
    ...
    public void onSyncedDataUpdated(EntityDataAccessor<?> key) {
        ...
        if (TRACK_GRAPH.equals(key)) updateTrackGraph();
    ...
    public Vec3 getPrevPositionVec() {
        if (!level().isClientSide() && serverPrevPos != null) return serverPrevPos;
        return super.getPrevPositionVec();
    }

    public boolean isLocalCoordWithin(BlockPos localPos, int min, int max) {
        if (!(getContraption() instanceof CarriageContraption cc)) return false;
    ...
    public void tick() {
        ...
        if (contraption instanceof CarriageContraption cc)
            for (Entity entity : getPassengers()) {
                ...
                if (seatOf == null) continue;
                if (cc.conductorSeats.get(seatOf) == null) continue;
    ...
    public void setBlock(BlockPos localPos, StructureBlockInfo newInfo) {
        ...
        carriage.forEachPresentEntity(cce -> {
            ...
            CatnipServices.NETWORK.sendToClientsTrackingEntity(
                    cce, new ContraptionBlockChangedPacket(cce.getId(), localPos, newInfo.state()));
        });
    }
    ...
    protected void tickContraption() {
        ...
        if (carriage == null) {
            if (level().isClientSide) bindCarriage();
            else discard();
            return;
        }
        ...
        if (!level().isClientSide) {
            ...
            boolean shouldCarriageSyncThisTick = carriage.train.shouldCarriageSyncThisTick(
                    level().getGameTime(), getType().updateInterval());
            ...
            if (navigation.announceArrival
                    && Math.abs(navigation.distanceToDestination) < 60
                    && carriageIndex
                            == (carriage.train.speed < 0
                                    ? carriage.train.carriages.size() - 1
                                    : 0)) {
            ...
            if (arrivalSoundPlaying) tickArrivalSound(cc);

            entityData.set(
                    TRACK_GRAPH, Optional.ofNullable(carriage.train.graph).map(g -> g.id));
        ...
        if (tickCount % 10 == 0) updateTrackGraph();

        if (!dce.pointsInitialised) return;

        carriageData.approach(this, carriage, 1f / getType().updateInterval());

        if (!carriage.train.derailed) carriage.updateContraptionAnchors();
        ...
        if (sounds == null) sounds = new CarriageSounds(this);
        sounds.tick(dce);

        if (particles == null) particles = new CarriageParticles(this);
        particles.tick(dce);
        ...
        if (carriage.isOnTwoBogeys()) carriage.bogeys.getSecond().updateAngles(this, distanceTo);

        if (carriage.train.derailed) spawnDerailParticles(carriage);
        if (dce.pivot != null) spawnPortalParticles(dce);
    ...
    private void bindCarriage() {
        ...
        if (train == null || train.carriages.size() <= carriageIndex) return;
    ...
    private void tickArrivalSound(CarriageContraption cc) {
        ...
        if (arrivalSoundTicks == Integer.MIN_VALUE) {
            ...
            for (int index = 0; index < carriageCount; index++) {
                ...
                CarriageContraptionEntity entity =
                        carriage.getDimensional(level()).entity.get();
                if (entity == null || !(entity.contraption instanceof CarriageContraption otherCC))
                    break;
                tick = arrivalSoundReversed
                        ? otherCC.soundQueue.lastTick()
                        : otherCC.soundQueue.firstTick();
                if (tick != null) break;
            }
    ...
    protected boolean isActorActive(MovementContext context, MovementBehaviour actor) {
        ...
        if (!super.isActorActive(context, actor)) return false;
    ...
    protected void addPassenger(Entity pPassenger) {
        ...
        if (!(pPassenger instanceof Player player)) return;
        player.getPersistentData()
                .put("ContraptionMountLocation", VecHelper.writeNBT(player.position()));
    }

    private final Set<BlockPos> particleSlice = new HashSet<>();
    ...
    private void spawnPortalParticles(DimensionalCarriageEntity dce) {
        ...
        if (particleSlice.isEmpty()) return;
        ...
        for (BlockPos pos : particleSlice) {
            if (size != 0 && random.nextInt(size) != 0) continue;
            if (alongX) pos = new BlockPos(0, pos.getY(), pos.getX());
            Vec3 v = pivot.add(pos.getX() * extraFlip, pos.getY(), pos.getZ() * extraFlip);
            CubeParticleData data = new CubeParticleData(
                    .25f, 0, .5f, .65f + (random.nextFloat() - .5f) * .25f, 4, false);
    ...
    @Override
    protected void writeAdditional(
            CompoundTag compound, HolderLookup.Provider registries, boolean spawnPacket) {
    ...
    public Component getContraptionName() {
        if (carriage != null) return carriage.train.name;
    ...
    public Couple<Boolean> checkConductors() {
        ...
        if (!(contraption instanceof CarriageContraption cc)) return sides;
    ...
    public Component getDisplayName() {
        if (carriage == null) return CreateLang.translateDirect("train");
    ...
    public boolean control(
            BlockPos controlsLocalPos, Collection<Integer> heldControls, Player player) {
        if (carriage == null) return false;
        if (carriage.train.derailed) return false;
        ...
        if (hudPacketCooldown-- <= 0 && player instanceof ServerPlayer sp) {
            CatnipServices.NETWORK.sendToClient(
                    sp, new TrainHUDUpdatePacket.Clientbound(carriage.train));
            hudPacketCooldown = 5;
        }
        ...
        int targetSteer = 0;
        if (heldControls.contains(2)) targetSteer++;
        if (heldControls.contains(3)) targetSteer--;
        ...
        if (targetSpeed != 0) carriage.train.burnFuel();
        ...
        if (currentStation != null && spaceDown) {
            sendPrompt(
                    ...
                    CreateLang.translateDirect(
                            "train.arrived_at",
                            Component.literal(currentStation.name)
                                    .withStyle(s -> s.withColor(0x704630))),
                    false);
            return true;
        }

        if (carriage.train.speedBeforeStall != null
                && targetSpeed != 0
        ...
        if (currentStation == null) {
            ...
            if (nav.destination != null) {
                if (!spaceDown) nav.cancelNavigation();
                if (spaceDown) {
                    double f = (nav.distanceToDestination / navDistanceTotal);
                    int progress = (int) (Mth.clamp(1 - ((1 - f) * (1 - f)), 0, 1) * 30);
                    boolean arrived = progress == 0;
                    MutableComponent whiteComponent =
                            Component.literal(Strings.repeat("|", progress));
                    MutableComponent greenComponent =
                            Component.literal(Strings.repeat("|", 30 - progress));
                    ...
                    MutableComponent component = greenComponent
                            .withStyle(st -> st.withColor(mixedColor))
            ...
            GlobalStation lookAhead = nav.findNearestApproachable(!carriage.train.doubleEnded
                    || (directedSpeed != 0 ? directedSpeed > 0 : !inverted));
            ...
            } else cleanUpApproachStationMessage(player);
        }

        carriage.train.manualSteer = targetSteer < 0
                ? SteerDirection.RIGHT
                : targetSteer > 0 ? SteerDirection.LEFT : SteerDirection.NONE;

        double topSpeed = carriage.train.maxSpeed()
                * AllConfigs.server().trains.manualTrainSpeedModifier.getF();
        double cappedTopSpeed = topSpeed * carriage.train.throttle;

        if (carriage.getLeadingPoint().edge != null
                        && carriage.getLeadingPoint().edge.isTurn()
                || carriage.getTrailingPoint().edge != null
                        && carriage.getTrailingPoint().edge.isTurn())
        ...
        boolean counteringAcceleration =
                Math.abs(Math.signum(targetSpeed) - Math.signum(carriage.train.speed)) > 1.5f;

        if (slow && !counteringAcceleration) carriage.train.backwardsDriver = player;
    ...
    private void displayApproachStationMessage(Player player, GlobalStation station) {
        sendPrompt(
                player,
                CreateLang.translateDirect(
                        "contraption.controls.approach_station",
    ...
    public void setCarriage(Carriage carriage) {
        ...
        if (contraption instanceof CarriageContraption cc) cc.swapStorageAfterAssembly(this);
