...
public class ArrivalSoundQueue {
    ...
    public boolean tick(CarriageContraptionEntity entity, int tick, boolean backwards) {
        tick = tick - offset;
        if (!sources.containsKey(tick)) return backwards ? tick > min : tick < max;
    ...
    public Pair<Boolean, Integer> getFirstWhistle(CarriageContraptionEntity entity) {
        ...
        if (firstTick == null || lastTick == null || firstTick > lastTick) return null;
        for (int i = firstTick; i <= lastTick; i++) {
            if (!sources.containsKey(i - offset)) continue;
            Contraption contraption = entity.getContraption();
            for (BlockPos blockPos : sources.get(i - offset)) {
                ...
                if (state.getBlock() instanceof WhistleBlock && info.nbt() != null) {
                    ...
                    return Pair.of(
                            size == WhistleSize.LARGE,
                            (size == WhistleSize.SMALL ? 12 : 0) - pitch);
                }
            }
        }
        return null;
    ...
    public void deserialize(CompoundTag tagIn) {
        ...
        NBTHelper.iterateCompoundList(
                tag.getList("Sources", Tag.TAG_COMPOUND),
    ...
    public static boolean isPlayable(BlockState state) {
        if (state.getBlock() instanceof BellBlock) return true;
        if (state.getBlock() instanceof NoteBlock) return true;
    ...
    public static void play(CarriageContraptionEntity entity, StructureBlockInfo info) {
        ...
        if (state.getBlock() instanceof BellBlock) {
            ...
            else playSimple(entity, SoundEvents.BELL_BLOCK, 1, 1);
        }

        if (state.getBlock() instanceof NoteBlock nb) {
            float f = (float) Math.pow(2, (state.getValue(NoteBlock.NOTE) - 12) / 12.0);
            playSimple(
                    entity, state.getValue(NoteBlock.INSTRUMENT).getSoundEvent().value(), 1, f);
        }
        ...
        if (state.getBlock() instanceof WhistleBlock && info.nbt() != null) {
            ...
            playSimple(
                    entity,
                    (size == WhistleSize.LARGE
                                    ? AllSoundEvents.WHISTLE_TRAIN_LOW
                                    : AllSoundEvents.WHISTLE_TRAIN)
                            .getMainEvent(),
                    1,
                    f);
    ...
    private static void playSimple(
            CarriageContraptionEntity entity, SoundEvent event, float volume, float pitch) {
