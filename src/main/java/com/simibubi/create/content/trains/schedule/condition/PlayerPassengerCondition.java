...
public class PlayerPassengerCondition extends ScheduleWaitCondition {
    ...
    public Pair<ItemStack, Component> getSummary() {
        ...
        return Pair.of(
                AllBlocks.SEATS.get(DyeColor.YELLOW).asStack(),
                CreateLang.translateDirect(
                        "schedule.condition.player_count."
                                + (target == 1 ? "summary" : "summary_plural"),
                        target));
    }
        ...
        builder.addSelectionScrollInput(
                ...
                (i, l) -> {
                    i.forOptions(CreateLang.translatedOptions(
                                    "schedule.condition.player_count", "exactly", "or_above"))
                            .titled(CreateLang.translateDirect(
                                    "schedule.condition.player_count.condition"));
                },
                "Exact");
    }
    ...
    public boolean tickCompletion(Level level, Train train, CompoundTag context) {
        ...
        if (prev != present) requestStatusToUpdate(context);
    ...
    public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
        return CreateLang.translateDirect(
                "schedule.condition.player_count.status",
