...
public class EdgeData {
    ...
    public void refreshIntersectingSignalGroups(TrackGraph graph) {
        ...
        for (TrackEdgeIntersection intersection : intersections) {
            if (intersection.groupId == null) continue;
            SignalEdgeGroup group = groups.get(intersection.groupId);
            if (group != null) group.removeIntersection(intersection.id);
        }
        if (hasIntersections()) graph.deferIntersectionUpdate(edge);
    }
    ...
    public void addIntersection(
        ...
        for (TrackEdgeIntersection existing : intersections)
            if (existing.isNear(position) && existing.targets(loc1, loc2)) return;
    ...
    public void removeIntersection(TrackGraph graph, UUID id) {
        refreshIntersectingSignalGroups(graph);
        for (Iterator<TrackEdgeIntersection> iterator = intersections.iterator();
                iterator.hasNext(); ) {
            TrackEdgeIntersection existing = iterator.next();
            if (existing.id.equals(id)) iterator.remove();
        }
    }

    public UUID getGroupAtPosition(TrackGraph graph, double position) {
        if (!hasSignalBoundaries()) return getEffectiveEdgeGroupId(graph);
        ...
        for (TrackEdgePoint trackEdgePoint : getPoints()) {
            if (!(trackEdgePoint instanceof SignalBoundary sb)) continue;
            if (sb.getLocationOn(edge) >= position) return currentGroup;
    ...
    public UUID getEffectiveEdgeGroupId(TrackGraph graph) {
        return singleSignalGroup == null
                ? null
                : singleSignalGroup.equals(passiveGroup) ? graph.id : singleSignalGroup;
    }
    ...
    public <T extends TrackEdgePoint> void addPoint(TrackGraph graph, TrackEdgePoint point) {
        if (point.getType() == EdgePointType.SIGNAL) setSingleSignalGroup(graph, null);
        double locationOn = point.getLocationOn(edge);
        int i = 0;
        for (; i < points.size(); i++) if (points.get(i).getLocationOn(edge) > locationOn) break;
        points.add(i, point);
    }
    ...
    public TrackEdgePoint next(double minPosition) {
        for (TrackEdgePoint point : points)
            if (point.getLocationOn(edge) > minPosition) return point;
        return null;
    }
    ...
    public <T extends TrackEdgePoint> T get(EdgePointType<T> type, double exactPosition) {
        ...
        if (next != null && Mth.equal(next.getLocationOn(edge), exactPosition)) return next;
        return null;
    }

    public CompoundTag write(DimensionPalette dimensions) {
        CompoundTag nbt = new CompoundTag();
        if (singleSignalGroup == passiveGroup) NBTHelper.putMarker(nbt, "PassiveGroup");
        else if (singleSignalGroup != null) nbt.putUUID("SignalGroup", singleSignalGroup);
        ...
        if (hasIntersections())
            nbt.put(
                    "Intersections",
                    NBTHelper.writeCompoundList(intersections, tei -> tei.write(dimensions)));
        return nbt;
    }

    public static EdgeData read(
            CompoundTag nbt, TrackEdge edge, TrackGraph graph, DimensionPalette dimensions) {
        EdgeData data = new EdgeData(edge);
        if (nbt.contains("SignalGroup")) data.singleSignalGroup = nbt.getUUID("SignalGroup");
        else if (!nbt.contains("PassiveGroup")) data.singleSignalGroup = null;
        ...
        if (nbt.contains("Points"))
            NBTHelper.iterateCompoundList(nbt.getList("Points", Tag.TAG_COMPOUND), tag -> {
                ...
                if (type == null || !tag.contains("Id")) return;
                TrackEdgePoint point = graph.getPoint(type, tag.getUUID("Id"));
                if (point != null) data.points.add(point);
            });
        if (nbt.contains("Intersections"))
            data.intersections = NBTHelper.readCompoundList(
                    nbt.getList("Intersections", Tag.TAG_COMPOUND),
