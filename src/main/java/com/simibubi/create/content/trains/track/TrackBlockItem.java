...
public class TrackBlockItem extends BlockItem {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        ...
        if (pContext.getHand() == InteractionHand.OFF_HAND) return super.useOn(pContext);

        Vec3 lookAngle = player.getLookAngle();

        if (!isFoil(stack)) {
            if (state.getBlock() instanceof TrackBlock track
                    && track.getTrackAxes(level, pos, state).size() > 1) {
            ...
            if (select(level, pos, lookAngle, stack)) {
                level.playSound(
                        null, pos, SoundEvents.ITEM_FRAME_ADD_ITEM, SoundSource.BLOCKS, 0.75f, 1);
                return InteractionResult.SUCCESS;
            }
        ...
        if (placing) {
            if (!state.canBeReplaced()) pos = pos.relative(pContext.getClickedFace());
        ...
        PlacementInfo info =
                TrackPlacement.tryConnect(level, player, pos, state, stack, hasGirder, extend);
    ...
    public static InteractionResultHolder<ItemStack> clearSelection(
        ...
        if (level.isClientSide) {
            ...
            player.displayClientMessage(
                    CreateLang.translateDirect("track.selection_cleared"), true);
            stack.remove(AllDataComponents.TRACK_CONNECTING_FROM);
        }
    ...
    public static boolean select(
            LevelAccessor world, BlockPos pos, Vec3 lookVec, ItemStack heldItem) {
        BlockState blockState = world.getBlockState(pos);
        Block block = blockState.getBlock();
        if (!(block instanceof ITrackBlock track)) return false;

        Pair<Vec3, AxisDirection> nearestTrackAxis =
                track.getNearestTrackAxis(world, pos, blockState, lookVec);
        Vec3 axis = nearestTrackAxis
                .getFirst()
        ...
        heldItem.set(
                AllDataComponents.TRACK_CONNECTING_FROM,
                new TrackPlacement.ConnectingFrom(pos, axis, normal, end));
        return true;
    }
    ...
    public static void sendExtenderPacket(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (Minecraft.getInstance().options.keySprint.isDown())
            CatnipServices.NETWORK.sendToServer(new PlaceExtendedCurvePacket(
                    event.getHand() == InteractionHand.MAIN_HAND, true));
    }
