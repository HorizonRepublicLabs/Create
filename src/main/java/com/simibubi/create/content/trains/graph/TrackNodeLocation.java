...
public class TrackNodeLocation extends Vec3i {
    ...
    public TrackNodeLocation(double x, double y, double z) {
        super(Mth.floor((double) Math.round(x * 2)), Mth.floor(y) * 2, Mth.floor((double)
                Math.round(z * 2)));
    }
    ...
    public boolean equals(Object pOther) {
        return equalsIgnoreDim(pOther)
                && pOther instanceof TrackNodeLocation tnl
    ...
    public boolean equalsIgnoreDim(Object pOther) {
        return super.equals(pOther)
                && pOther instanceof TrackNodeLocation tnl
                && tnl.yOffsetPixels == yOffsetPixels;
    }

    @Override
    public int hashCode() {
        return (getY() + ((getZ() + yOffsetPixels * 31) * 31 + dimension.hashCode()) * 31) * 31
                + getX();
    }
    ...
    public CompoundTag write(DimensionPalette dimensions) {
        ...
        if (dimensions != null) c.putInt("D", dimensions.encode(dimension));
        if (yOffsetPixels != 0) c.putInt("YO", yOffsetPixels);
        return c;
    }

    public static TrackNodeLocation read(CompoundTag tag, DimensionPalette dimensions) {
        TrackNodeLocation location = fromPackedPos(
                tag.contains("Pos")
                        ? NBTHelper.readBlockPos(tag, "Pos")
                        : new BlockPos(tag.getInt("X"), tag.getInt("Y"), tag.getInt("Z")));
        if (dimensions != null) location.dimension = dimensions.decode(tag.getInt("D"));
    ...
    public static TrackNodeLocation receive(FriendlyByteBuf buffer, DimensionPalette dimensions) {
        TrackNodeLocation location = fromPackedPos(
                new BlockPos(buffer.readVarInt(), buffer.readShort(), buffer.readVarInt()));
