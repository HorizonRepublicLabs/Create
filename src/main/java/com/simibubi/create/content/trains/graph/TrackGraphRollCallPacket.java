...
public record TrackGraphRollCallPacket(List<Entry> entries) implements ClientboundPacketPayload {
    public static final StreamCodec<ByteBuf, TrackGraphRollCallPacket> STREAM_CODEC =
            CatnipStreamCodecBuilders.list(Entry.STREAM_CODEC)
    ...
    public void handle(LocalPlayer player) {
        ...
        for (Entry entry : this.entries) {
            ...
            Create.LOGGER.warn(
                    "Track network: {} failed its checksum; Requesting refresh",
        ...
        for (UUID unused : unusedIds) manager.trackNetworks.remove(unused);
    }
