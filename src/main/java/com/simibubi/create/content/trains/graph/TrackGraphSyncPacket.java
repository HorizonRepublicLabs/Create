...
public class TrackGraphSyncPacket extends TrackGraphPacket {
    ...
    public static final StreamCodec<FriendlyByteBuf, TrackGraphSyncPacket> STREAM_CODEC =
            StreamCodec.of((b, v) -> v.write(b), TrackGraphSyncPacket::new);
    ...
    public TrackGraphSyncPacket(FriendlyByteBuf buffer) {
        ...
        for (int i = 0; i < size; i++) removedNodes.add(buffer.readVarInt());
        ...
        for (int i = 0; i < size; i++)
            addedEdges.add(Pair.of(
                    Pair.of(
                            Couple.create(buffer::readVarInt),
                            TrackMaterial.deserialize(buffer.readUtf())),
                    buffer.readBoolean() ? new BezierConnection(buffer) : null));

        size = buffer.readVarInt();
        for (int i = 0; i < size; i++) addedEdgePoints.add(EdgePointType.read(buffer, dimensions));

        size = buffer.readVarInt();
        for (int i = 0; i < size; i++) removedEdgePoints.add(buffer.readUUID());
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < size2; j++) list.add(buffer.readUUID());
    ...
    public void write(FriendlyByteBuf buffer) {
        ...
        addedEdgePoints.forEach(
                ep -> ep.edgeLocation.forEach(loc -> dimensions.encode(loc.dimension)));
        ...
        addedEdges.forEach(pair -> {
            ...
            if (turn != null) turn.write(buffer);
        });
    ...
    protected void handle(GlobalRailwayManager manager, TrackGraph graph) {
        ...
        if (fullWipe) {
            manager.removeGraph(graph);
            graph = Create.RAILWAYS.sided(null).getOrCreateGraph(graphId, netId);
        }

        for (int nodeId : removedNodes) {
            TrackNode node = graph.getNode(nodeId);
            if (node != null) graph.removeNode(null, node.getLocation());
        }
        ...
        for (Pair<Pair<Couple<Integer>, TrackMaterial>, BezierConnection> pair : addedEdges) {
            ...
            if (node1 != null && node2 != null)
                graph.putConnection(
                        ...
                        node2,
                        new TrackEdge(
                                node1, node2, pair.getSecond(), pair.getFirst().getSecond()));
        }
        ...
        if (!splitSubGraphs.isEmpty())
            graph.findDisconnectedGraphs(null, splitSubGraphs).forEach(manager::putGraph);
    }
    ...
    protected void handleEdgeData(GlobalRailwayManager manager, TrackGraph graph) {
        for (Entry<Couple<Integer>, Pair<Integer, List<UUID>>> entry : updatedEdgeData.entrySet()) {
            ...
            if (groupType == NULL_GROUP) edgeData.setSingleSignalGroup(null, null);
            else if (groupType == PASSIVE_GROUP)
                edgeData.setSingleSignalGroup(null, EdgeData.passiveGroup);
            else edgeData.setSingleSignalGroup(null, idList.get(0));
    ...
    public void syncEdgeData(TrackNode node1, TrackNode node2, TrackEdge edge) {
        ...
        int groupType = edgeData.hasSignalBoundaries()
                ? NULL_GROUP
                : EdgeData.passiveGroup.equals(edgeData.getSingleSignalGroup())
                        ? PASSIVE_GROUP
                        : GROUP;
        if (groupType == GROUP) list.add(edgeData.getSingleSignalGroup());
        for (TrackEdgePoint point : edgeData.getPoints()) list.add(point.getId());
