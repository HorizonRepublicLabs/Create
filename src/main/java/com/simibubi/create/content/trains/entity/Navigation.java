...
public class Navigation {
    ...
    public void tick(Level level) {
        ...
        if (!train.runtime.paused) {
            ...
            if (destinationBehindTrain && !backDriver) {
                if (frontDriver) train.status.missingCorrectConductor();
                else train.status.missingConductor();
                cancelNavigation();
                return;
            }
        ...
        if (train.graph != null) {
            if (waitingForSignal != null && currentSignalResolved()) {
                ...
                if (signal != null
                        && signal.types.get(waitingForSignal.getSecond())
                                == SignalType.CROSS_SIGNAL) waitingForChainedGroups.clear();
                waitingForSignal = null;
            }

            TravellingPoint leadingPoint = !destinationBehindTrain
            ...
            if (distanceToSignal > 1 / 16f) {
                ...
                double scanDistance = Mth.clamp(
                        brakingDistanceNoFlicker, preDepartureLookAhead, distanceToDestination);
                ...
                signalScout.travel(
                        ...
                        (distance, couple) -> {
                            ...
                            if (!crossSignalTracked) {
                                ...
                                if (occupied) { // Section is occupied
                                    ...
                                    if (!crossSignal)
                                        return true; // Standard entry signal, do not collect any
                                    // further segments
                                }
                                if (!occupied
                                        && !crossSignal
                                        && distance < distanceToSignal + .25
                                        && distance < brakingDistanceNoFlicker)
                                    signalEdgeGroup.reserved =
                                            signal; // Reserve group for traversal
                                return false;
                            }

                            if (crossSignalTracked) {
                                waitingForChainedGroups.put(
                                        entering, Pair.of(signal, primary)); // Add group to chain
                                if (occupied) { // Section is occupied, but wait at the cross signal
                                    // that started
                                    // the chain
                                    waitingForSignal = trackingCrossSignal.getValue();
                                    distanceToSignal = crossSignalDistanceTracker.doubleValue();
                                    if (!crossSignal) return true; // Entry signals end a chain
                                }
                                if (!crossSignal) {
                                    if (distance < distanceToSignal + .25) {
                                        // Collect and reset the signal chain because none were
                                        // blocked
                                        trackingCrossSignal.setValue(null);
                                        reserveChain();
                                        return false;
                                    } else return true; // End of a blocked signal chain
                                }
                            }
                        ...
                        (distance, edge) -> {
                            ...
                            double vDistance =
                                    Math.abs(turn.starts.getFirst().y - turn.starts.getSecond().y);
                            ...
                            if (turn != null && vDistance > 1 / 16f) {
                                if (turn.axes
                                                        .getFirst()
                                                        .multiply(1, 0, 1)
                                                        .distanceTo(turn.axes
                                                                .getSecond()
                                                                .multiply(1, 0, 1)
                                                                .scale(-1))
                                                < 1 / 64f
                                        && vDistance / turn.getLength() < .225f) return;
                            }
        ...
        if (distanceToNextCurve != -1) {
            double slowingDistance =
                    brakingDistance - (turnTopSpeed * turnTopSpeed) / (2 * acceleration);
            double targetTurnSpeed = distanceToNextCurve > slowingDistance
                    ? topSpeed * speedMod
                    : turnTopSpeed * speedMod;
            if (Math.abs(targetTurnSpeed) < Math.abs(targetSpeed)) targetSpeed = targetTurnSpeed;
        }
    ...
    private void reserveChain() {
        ...
        waitingForChainedGroups.forEach((groupId, boundary) -> {
            ...
            if (signalEdgeGroup != null) signalEdgeGroup.reserved = boundary.getFirst();
        });
        waitingForChainedGroups.clear();
    }

    private boolean currentSignalResolved() {
        if (train.manualTick) return true;
        if (distanceToDestination < .5f) return true;
        SignalBoundary signal =
                train.graph.getPoint(EdgePointType.SIGNAL, waitingForSignal.getFirst());
        if (signal == null) return true;

        // Cross Signal
        if (signal.types.get(waitingForSignal.getSecond()) == SignalType.CROSS_SIGNAL) {
            for (Entry<UUID, Pair<SignalBoundary, Boolean>> entry :
                    waitingForChainedGroups.entrySet()) {
                Pair<SignalBoundary, Boolean> boundary = entry.getValue();
                SignalEdgeGroup signalEdgeGroup =
                        Create.RAILWAYS.signalEdgeGroups.get(entry.getKey());
                ...
                if (signalEdgeGroup.isOccupiedUnless(train)) return false;
            }
            return true;
        }
        ...
        SignalEdgeGroup signalEdgeGroup = Create.RAILWAYS.signalEdgeGroups.get(groupId);
        if (signalEdgeGroup == null) return true;
        return !signalEdgeGroup.isOccupiedUnless(train);
    }
    ...
    public double startNavigation(DiscoveredPath pathTo) {
        ...
        if (noneFound) {
            ...
            currentPath = new ArrayList<>();
            if (this.destination != null) cancelNavigation();
            return -1;
        }

        if (Math.abs(distanceToDestination) > 100) announceArrival = true;
        ...
        if (this.destination == null) distanceStartedAt = distance;

        if (this.destination == pathTo.destination) return 0;
        ...
        if (!train.runtime.paused) {
            ...
            if (!destinationBehindTrain && !frontDriver) {
                if (backDriver) train.status.missingCorrectConductor();
                else train.status.missingConductor();
                return -1;
            }
    ...
    public DiscoveredPath findPathTo(ArrayList<GlobalStation> destinations, double maxCost) {
                    ...
                    (distance, cost, reachedVia, currentEntry, globalStation) -> {
                        for (GlobalStation destination : destinations) {
                            if (globalStation == destination) {
                                ...
                                double position =
                                        edge.getLength() - destination.getLocationOn(edge);
        ...
        boolean canDriveBackward =
                train.doubleEnded && train.hasBackwardConductor() || train.runtime.paused;
        ...
        boolean frontBetter =
                maxCost == -1 ? -back.distance > front.distance : back.cost > front.cost;
    ...
    public GlobalStation findNearestApproachable(boolean forward) {
        ...
        search(
                ...
                (distance, cost, reachedVia, currentEntry, globalStation) -> {
                    ...
                    if (presentTrain != null && presentTrain != train) return false;
    ...
    public void search(
        ...
        for (int i = 0; i < train.carriages.size(); i++) {
            ...
            if (i == 0) {
                validTypes.addAll(carriage.leadingBogey()
                        .type
                        .getValidPathfindingTypes(carriage.leadingBogey().getStyle()));
            } else {
                validTypes.retainAll(carriage.leadingBogey()
                        .type
                        .getValidPathfindingTypes(carriage.leadingBogey().getStyle()));
            }
            if (carriage.isOnTwoBogeys())
                validTypes.retainAll(carriage.trailingBogey()
                        .type
                        .getValidPathfindingTypes(carriage.trailingBogey().getStyle()));
        }
        ...
        double distanceToNode2 =
                forward ? initialEdge.getLength() - startingPoint.position : startingPoint.position;
        ...
        if (costRelevant) initialPenalty += penalties.getOrDefault(initialEdge, 0);
        ...
        if (initialSignalData.hasPoints()) {
            for (TrackEdgePoint point : initialSignalData.getPoints()) {
                if (point.getLocationOn(initialEdge) < initialEdge.getLength() - distanceToNode2)
                    continue;
                if (costRelevant && distanceToNode2 + initialPenalty > maxCost) return;
                if (!point.canNavigateVia(initialNode2)) return;
                ...
                if (point instanceof GlobalStation station) {
                    ...
                    if (station.canApproachFrom(initialNode2)
                            && stationTest.test(
                                    ...
                                    station)) return;
                    if (!isOwnStation) initialPenalty += Train.Penalties.STATION;
                }
            }
        }
        ...
        frontier.add(new FrontierEntry(
                distanceToNode2, initialPenalty, initialNode1, initialNode2, initialEdge));
        ...
        while (!frontier.isEmpty()) {
            ...
            if (entry.hasDestination) {
                ...
                if (signalData.hasPoints()) {
                    for (TrackEdgePoint point : signalData.getPoints()) {
                        if (point instanceof GlobalStation station) {
                            if (station.canApproachFrom(node2)
                                    && stationTest.test(
                                            distance,
                                            penalty,
                                            reachedVia,
            ...
            for (Entry<TrackNode, TrackEdge> connection : connectionsFrom.entrySet()) {
                ...
                if (edge.canTravelTo(connection.getValue())) validTargets.add(connection);
            }
            ...
            for (Entry<TrackNode, TrackEdge> target : validTargets) {
                ...
                if (costRelevant) newPenalty += penalties.getOrDefault(newEdge, 0);
                ...
                if (signalData.hasPoints()) {
                    for (TrackEdgePoint point : signalData.getPoints()) {
                        if (node2 == initialNode1
                                && point.getLocationOn(newEdge) < edgeLength - distanceToNode2)
                            continue;
                        if (costRelevant && newDistance + newPenalty > maxCost) continue Search;
                        if (!point.canNavigateVia(newNode)) continue Search;
                        if (point instanceof SignalBoundary signal) {
                            ...
                            if (group == null) continue;
                            SignalEdgeGroup signalEdgeGroup =
                                    Create.RAILWAYS.signalEdgeGroups.get(group);
                            if (signalEdgeGroup == null) continue;
                            if (signalEdgeGroup.isOccupiedUnless(signal)) {
                        ...
                        if (point instanceof GlobalStation station) {
                            ...
                            if (!isOwnStation) newPenalty += Train.Penalties.STATION;
                        }
                    }
                }

                if (costRelevant && newDistance + newPenalty > maxCost) continue;
                ...
                if (destinations != null && !destinations.isEmpty()) {
                    ...
                    for (GlobalStation destination : destinations) {
                        ...
                        double currentRemaining = 0.317837245195782 * dMin
                                + 0.414213562373095 * dMid
                                + dMax
                                + destination.position;
                        if (node2.getLocation().equals(destinationNode))
                            currentRemaining -= newEdge.getLength()
                                    * 2; // Correct the distance estimator for station edge
                        remainingDist = Math.min(remainingDist, currentRemaining);
                    }
                }

                reachedVia.putIfAbsent(
                        newEdge, Pair.of(validTargets.size() > 1, Couple.create(node1, node2)));
                frontier.add(new FrontierEntry(
                        newDistance,
                        newPenalty,
                        remainingDist,
    ...
    private class FrontierEntry implements Comparable<FrontierEntry> {
        ...
        public FrontierEntry(
                double distance, int penalty, TrackNode node1, TrackNode node2, TrackEdge edge) {
        ...
        public int compareTo(FrontierEntry o) {
            return Double.compare(
                    distance + penalty + remaining, o.distance + o.penalty + o.remaining);
        }
    }
    ...
    public CompoundTag write(DimensionPalette dimensions) {
        ...
        tag.put("Path", NBTHelper.writeCompoundList(currentPath, c -> {
            ...
            nbt.put(
                    "Nodes",
                    c.map(TrackNode::getLocation).serializeEach(loc -> loc.write(dimensions)));
            return nbt;
        }));
        if (waitingForSignal == null) return tag;
    ...
    public void read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
        ...
        NBTHelper.iterateCompoundList(
                tag.getList("Path", Tag.TAG_COMPOUND),
                c -> currentPath.add(Couple.deserializeEach(
                                c.getList("Nodes", Tag.TAG_COMPOUND),
                                c2 -> TrackNodeLocation.read(c2, dimensions))
        ...
        waitingForSignal = tag.contains("BlockingSignal")
                ...
                : null;
        if (waitingForSignal == null) return;
    ...
    private void removeBrokenPathEntries() {
        ...
        if (nullEntriesPresent)
            Create.LOGGER.error("Found null values in path of train with name: "
                    + train.name.getString() + ", id: " + train.id.toString());
    }
}
