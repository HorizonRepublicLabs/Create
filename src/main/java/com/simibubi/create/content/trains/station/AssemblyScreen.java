...
public class AssemblyScreen extends AbstractStationScreen {
    ...
    protected void init() {
        ...
        iconTypes = TrainIconType.REGISTRY.keySet().stream().toList();
        iconTypeScroll = new ScrollInput(x + 4, y + 17, 162, 14)
        ...
        iconTypeScroll.calling(s -> {
            ...
            if (train != null) train.icon = TrainIconType.byId(iconTypes.get(s));
        });
        ...
        toggleAssemblyButton.withCallback(() -> {
            CatnipServices.NETWORK.sendToServer(
                    StationEditPacket.tryAssemble(blockEntity.getBlockPos()));
        });
        ...
        quitAssembly.withCallback(() -> {
            CatnipServices.NETWORK.sendToServer(StationEditPacket.configure(
                    blockEntity.getBlockPos(), false, station.name, null));
    ...
    public void tick() {
        ...
        if (train != null) {
            CatnipServices.NETWORK.sendToServer(StationEditPacket.configure(
                    blockEntity.getBlockPos(), false, station.name, null));
            minecraft.setScreen(new StationScreen(blockEntity, station));
            for (Carriage carriage : train.carriages) carriage.updateConductors();
        }
    }
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (lastAssemblyException != null) {
            ...
            if (blockEntity.failedCarriageIndex != -1) {
                graphics.drawString(
                        ...
                        CreateLang.translateDirect(
                                "station.carriage_number", blockEntity.failedCarriageIndex),
                        x + 30,
                        y + 67,
                        0x7A7A7A,
                        false);
            ...
            graphics.drawWordWrap(
                    font, lastAssemblyException.component, x + 30, y + 67 + offset, 134, 0x775B5B);
        ...
        MutableComponent text = CreateLang.translateDirect(
                bogeyCount == 0
                        ? "station.no_bogeys"
                        : bogeyCount == 1 ? "station.one_bogey" : "station.more_bogeys",
                bogeyCount);
        graphics.drawString(font, text, x + 97 - font.width(text) / 2, y + 47, 0x7A7A7A, false);

        graphics.drawWordWrap(
                font, CreateLang.translateDirect("station.how_to"), x + 28, y + 62, 134, 0x7A7A7A);
        graphics.drawWordWrap(
                font,
