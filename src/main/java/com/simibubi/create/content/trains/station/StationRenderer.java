...
public class StationRenderer extends SafeBlockEntityRenderer<StationBlockEntity> {
    ...
    protected void renderSafe(
        ...
        DepotRenderer.renderItemsOf(
                be, partialTicks, ms, buffer, light, overlay, be.depotBehaviour);
        ...
        if (!isAssembling
                || (station == null || station.getPresentTrain() != null) && !be.isVirtual()) {
            renderFlag(
                    be.flag.getValue(partialTicks) > 0.75f
        ...
        if (be.isVirtual() && be.bogeyLocations == null) be.refreshAssemblyInfo();

        if (direction == null || be.assemblyLength == 0 || be.bogeyLocations == null) return;
        ...
        PartialModel assemblyOverlay =
                track.prepareAssemblyOverlay(level, targetPosition, trackState, direction, ms);
    ...
    public static void transformFlag(
            Transform<?> flag,
            StationBlockEntity be,
            float partialTicks,
            int yRot,
        ...
        if (be.flag.getChaseTarget() > 0 && !be.flag.settled() && progress == 1) {
            ...
            progress += (Math.sin(wiggleProgress * (2 * Mth.PI) * 4) / 8f)
