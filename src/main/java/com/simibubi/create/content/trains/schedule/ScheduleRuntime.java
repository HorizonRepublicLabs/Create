...
public class ScheduleRuntime {
    ...
    public void destinationReached() {
        if (state != State.IN_TRANSIT) return;
        state = State.POST_TRANSIT;
        conditionProgress.clear();
        conditionContext.clear();
        displayLinkUpdateRequested = true;
        for (Carriage carriage : train.carriages) carriage.storage.resetIdleCargoTracker();

        if (ticksInTransit > 0) {
            int current = predictionTicks.get(currentEntry);
            if (current > 0) ticksInTransit = (current + ticksInTransit) / 2;
            predictionTicks.set(currentEntry, ticksInTransit);
        }

        if (currentEntry >= schedule.entries.size()) return;
        List<List<ScheduleWaitCondition>> conditions =
                schedule.entries.get(currentEntry).conditions;
    ...
    public void transitInterrupted() {
        if (schedule == null || state != State.IN_TRANSIT) return;
    ...
    public void tick(Level level) {
        ...
        if (checkEndOfScheduleReached()) return;
    ...
    private boolean checkEndOfScheduleReached() {
        if (currentEntry < schedule.entries.size()) return false;
    ...
    public void tickConditions(Level level) {
        ...
        for (Carriage carriage : train.carriages) carriage.storage.tickIdleCargoTracker();
    }

    public DiscoveredPath startCurrentInstruction(Level level) {
        if (checkEndOfScheduleReached()) return null;
    ...
    public Collection<TrainDeparturePrediction> submitPredictions() {
        ...
        if (state == State.POST_TRANSIT || current >= entryCount) {
            ...
            if (departureTime == INVALID) accumulatedTime = INVALID;
            else accumulatedTime += departureTime;
            ...
            if (destination != null) {
                ...
                predictions.add(
                        createPrediction(current, destination.name, currentTitle, accumulatedTime));

                int departureTime = estimateStayDuration(current);
                if (departureTime != INVALID) accumulatedTime += departureTime;
                else accumulatedTime = INVALID;

            } else predictForEntry(current, currentTitle, accumulatedTime, predictions);
        }
        ...
        for (int i = 1; i < entryCount; i++) {
            ...
            if (index == 0 && !schedule.cyclic) break;
    ...
    private int predictForEntry(
        ...
        if (!(entry.instruction instanceof DestinationInstruction filter)) return accumulatedTime;
        if (predictionTicks.size() <= currentEntry) return accumulatedTime;

        int departureTime = estimateStayDuration(index);

        if (accumulatedTime < 0) {
            predictions.add(
                    createPrediction(index, filter.getFilter(), currentTitle, accumulatedTime));
        ...
        if (predictedTime == TBD) accumulatedTime = TBD;

        predictions.add(createPrediction(index, filter.getFilter(), currentTitle, accumulatedTime));

        if (accumulatedTime != TBD) accumulatedTime += departureTime;
    ...
    private int estimateStayDuration(int index) {
        if (index >= schedule.entries.size()) {
            if (!schedule.cyclic) return INVALID;
            index = 0;
        }
        ...
        for (List<ScheduleWaitCondition> list : scheduleEntry.conditions) {
            ...
            for (ScheduleWaitCondition condition : list) {
                if (!(condition instanceof ScheduledDelay wait)) continue Columns;
    ...
    private TrainDeparturePrediction createPrediction(
            int index, String destination, String currentTitle, int time) {
        if (time == INVALID) return null;

        int size = schedule.entries.size();
        if (index >= size) {
            if (!schedule.cyclic) {
                return new TrainDeparturePrediction(
                        train, time, CommonComponents.space(), destination);
            }
            index %= size;
        }
    ...
    public CompoundTag write(HolderLookup.Provider registries) {
        ...
        if (schedule != null) tag.put("Schedule", schedule.write(registries));
        NBTHelper.writeEnum(tag, "State", state);
        tag.putIntArray("ConditionProgress", conditionProgress);
        tag.put(
                "ConditionContext",
                NBTHelper.writeCompoundList(conditionContext, CompoundTag::copy));
    ...
    public void read(HolderLookup.Provider registries, CompoundTag tag) {
        ...
        for (int i : tag.getIntArray("ConditionProgress")) conditionProgress.add(i);
        NBTHelper.iterateCompoundList(
                tag.getList("ConditionContext", Tag.TAG_COMPOUND), conditionContext::add);
    ...
    public ItemStack returnSchedule(HolderLookup.Provider registries) {
        if (schedule == null) return ItemStack.EMPTY;
    ...
    public MutableComponent getWaitingStatus(Level level) {
        ...
        if (progress >= list.size()) return Component.empty();
