...
public class TrackBlockOutline {
    ...
    public static void pickCurves() {
        ...
        if (!(mc.cameraEntity instanceof LocalPlayer player)) return;
        if (mc.level == null) return;

        Vec3 origin = player.getEyePosition(AnimationTickHolder.getPartialTicks(mc.level));

        double maxRange = mc.hitResult == null
                ? Double.MAX_VALUE
        ...
        for (TrackBlockEntity be : turns.values()) {
            for (BezierConnection bc : be.connections.values()) {
                ...
                for (int i = 0; i < stepLUT.length - 2; i++) {
                    ...
                    localDirection =
                            VecHelper.rotate(localDirection, AngleHelper.deg(-angles.x), Axis.X);
                    localDirection =
                            VecHelper.rotate(localDirection, AngleHelper.deg(-angles.y), Axis.Y);

                    Optional<Vec3> clip =
                            segmentBounds.clip(localOrigin, localOrigin.add(localDirection));
                    ...
                    BezierTrackPointLocation location =
                            new BezierTrackPointLocation(bc.getKey(), i);
                    result = new BezierPointSelection(
                            be, location, anchor, angles, diff.normalize());
                }
        ...
        if (mc.hitResult != null && mc.hitResult.getType() != Type.MISS) {
            ...
            mc.hitResult =
                    BlockHitResult.miss(priorLoc, Direction.UP, BlockPos.containing(priorLoc));
        }
    }
    ...
    public static void drawCurveSelection(PoseStack ms, MultiBufferSource buffer, Vec3 camera) {
        ...
        boolean holdingTrack = AllTags.AllBlockTags.TRACKS.matches(
                Minecraft.getInstance().player.getMainHandItem());
        renderShape(
                AllShapes.TRACK_ORTHO.get(Direction.SOUTH), ms, vb, holdingTrack ? false : null);
        ms.popPose();
    }
    ...
    public static void drawCustomBlockSelection(RenderHighlightEvent.Block event) {
        ...
        if (!(blockstate.getBlock() instanceof TrackBlock)) return;
        ...
        boolean canConnectFrom = !shape.isJunction()
                && !(mc.level.getBlockEntity(pos) instanceof TrackBlockEntity tbe
                        && tbe.isTilted());
    ...
    private static void walkShapes(
            TrackShape shape, TransformStack<?> msr, Consumer<VoxelShape> renderer) {
        ...
        } else if (shape == TrackShape.ND
                || shape == TrackShape.CR_NDX
                || shape == TrackShape.CR_NDZ) {
        ...
        if (!(shape == TrackShape.AE
                || shape == TrackShape.AN
                || shape == TrackShape.AW
                || shape == TrackShape.AS)) return;
