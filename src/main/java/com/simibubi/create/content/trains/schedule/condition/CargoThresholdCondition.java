...
public abstract class CargoThresholdCondition extends LazyTickedScheduleCondition {
    public enum Ops {
        ...
        public static List<? extends Component> translatedOptions() {
            return Arrays.stream(values())
                    .map(op -> CreateLang.translateDirect(
                            "schedule.condition.threshold." + Lang.asId(op.name())))
                    .toList();
        }
    }
    ...
    public boolean lazyTickCompletion(Level level, Train train, CompoundTag context) {
        ...
        int status = 0;
        for (Carriage carriage : train.carriages) status += carriage.storage.getVersion();
    ...
    protected int getLastDisplaySnapshot(CompoundTag context) {
        if (!context.contains("CurrentDisplay")) return -1;
    ...
    public Pair<ItemStack, Component> getSummary() {
        return Pair.of(
                getIcon(),
                Component.literal(getOperator().formatted + " " + getThreshold())
                        .append(getUnit()));
    }
