...
public class CarriageParticles {
    ...
    public void tick(DimensionalCarriageEntity dce) {
        ...
        if (leadingAnchor == null || !leadingAnchor.closerThan(camEntity.position(), 64)) return;
        ...
        if (arrived && length > 0.01f) arrived = false;
        ...
        if (stopped) {
            if (!arrived) {
                arrived = true;
                depressurise =
                        (int) (20 * entity.getCarriage().train.accumulatedSteamRelease / 10f);
            }
        } else depressurise = 0;

        if (depressurise > 0) depressurise--;
        ...
        for (CarriageBogey bogey : entity.getCarriage().bogeys) {
            ...
            if (length > 1 / 6f) cutoff = Math.max(cutoff, brakes.getValue() * 1.15f);
            ...
            for (int j : Iterate.positiveAndNegative) {
                ...
                for (int i : Iterate.positiveAndNegative) {
                    ...
                    level.addParticle(
                            spark
                                    ? bogey.getStyle().contactParticle
                                    : bogey.getStyle().smokeParticle,
                            v.x,
                            v.y,
                            v.z,
                            m.x,
