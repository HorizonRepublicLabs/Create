...
public class CurvedTrackSelectionPacket extends BlockEntityConfigurationPacket<TrackBlockEntity> {
    ...
    public CurvedTrackSelectionPacket(
    ...
    protected void applySettings(ServerPlayer player, TrackBlockEntity be) {
        ...
        if (!(stack.getItem() instanceof TrackTargetingBlockItem)) return;
        ...
        EdgePointType<?> type =
                AllBlocks.TRACK_SIGNAL.isIn(stack) ? EdgePointType.SIGNAL : EdgePointType.STATION;
        MutableObject<OverlapResult> result = new MutableObject<>(null);
        BezierTrackPointLocation bezierTrackPointLocation =
                new BezierTrackPointLocation(targetPos, segment);
