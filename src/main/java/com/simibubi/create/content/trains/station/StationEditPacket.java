...
public class StationEditPacket extends BlockEntityConfigurationPacket<StationBlockEntity> {
    public static final StreamCodec<ByteBuf, StationEditPacket> STREAM_CODEC =
    ...
    public static StationEditPacket configure(
            BlockPos pos, boolean assemble, String name, DoorControl doorControl) {
        return new StationEditPacket(pos, false, assemble, null, doorControl, name);
    }

    private StationEditPacket(
            BlockPos pos,
    ...
    protected void applySettings(ServerPlayer player, StationBlockEntity be) {
        ...
        if (name != null && !name.isBlank()) be.updateName(name);
