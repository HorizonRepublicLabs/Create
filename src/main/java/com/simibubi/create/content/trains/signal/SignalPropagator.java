...
public class SignalPropagator {
    ...
    public static void onSignalRemoved(TrackGraph graph, SignalBoundary signal) {
        ...
        for (boolean front : Iterate.trueAndFalse) {
            if (signal.sidesToUpdate.get(front)) continue;
    ...
    public static void propagateSignalGroup(
            TrackGraph graph, SignalBoundary signal, boolean front) {
    ...
    public static void walkSignals(
        ...
        Couple<TrackEdge> startEdges = startNodes.mapWithParams(
                (l1, l2) -> graph.getConnectionsFrom(l1).get(l2), startNodes.swap());
        ...
        SignalBoundary immediateBoundary =
                startEdge.getEdgeData().next(EdgePointType.SIGNAL, signal.getLocationOn(startEdge));
    ...
    private static void walkSignals(
        ...
        while (!frontier.isEmpty()) {
            ...
            for (Entry<TrackNode, TrackEdge> entry :
                    graph.getConnectionsFrom(currentNode).entrySet()) {
                ...
                if (forCollection
                        && !graph.getConnectionsFrom(prevNode).get(currentNode).canTravelTo(edge))
                    continue;
                ...
                for (boolean flip : Iterate.falseAndTrue) {
                    ...
                    if (!signalData.hasSignalBoundaries()) {
                        if (nonBoundaryCallback.test(signalData)) {
                            ...
                            Create.RAILWAYS.sync.edgeDataChanged(
                                    graph, currentNode, nextNode, edge, oppositeEdge);
                        }
                        continue;
                    }
        ...
        for (TrackEdge trackEdge : edges) {
            for (Train train : Create.RAILWAYS.trains.values()) {
                ...
                train.forEachTravellingPoint(tp -> {
                    if (tp.edge == trackEdge) train.updateSignalBlocks = true;
                });
            }
        }
    }
