...
public class TrackEdge {
    ...
    public TrackEdge(
            TrackNode node1, TrackNode node2, BezierConnection turn, TrackMaterial trackMaterial) {
    ...
    public Vec3 getDirection(boolean fromFirst) {
        return getPosition(null, fromFirst ? 0.25f : 1)
                .subtract(getPosition(null, fromFirst ? 0 : 0.75f))
                .normalize();
    }
    ...
    public boolean canTravelTo(TrackEdge other) {
        if (isInterDimensional() || other.isInterDimensional()) return true;
    ...
    public double getLength() {
        return isInterDimensional()
                ...
                : isTurn()
                        ? turn.getLength()
                        : node1.location.getLocation().distanceTo(node2.location.getLocation());
    }
    ...
    public Vec3 getPosition(@Nullable TrackGraph trackGraph, double t) {
        ...
        if (trackGraph != null
                && (node1.location.yOffsetPixels != 0 || node2.location.yOffsetPixels != 0)) {
        ...
        return VecHelper.lerp(
                (float) t, node1.location.getLocation(), node2.location.getLocation());
    }
    ...
    public Vec3 getPositionSmoothed(TrackGraph trackGraph, double t) {
        ...
        for (TrackEdge trackEdge : trackGraph.getConnectionsFrom(node1).values())
            if (trackEdge.isTurn()) node1Location = trackEdge.getPosition(trackGraph, 0);
        for (TrackEdge trackEdge : trackGraph.getConnectionsFrom(node2).values())
            if (trackEdge.isTurn()) node2Location = trackEdge.getPosition(trackGraph, 0);
        if (node1Location == null || node2Location == null) return null;
    ...
    public Vec3 getNormalSmoothed(TrackGraph trackGraph, double t) {
        ...
        for (TrackEdge trackEdge : trackGraph.getConnectionsFrom(node1).values())
            if (trackEdge.isTurn()) node1Normal = trackEdge.getNormal(trackGraph, 0);
        for (TrackEdge trackEdge : trackGraph.getConnectionsFrom(node2).values())
            if (trackEdge.isTurn()) node2Normal = trackEdge.getNormal(trackGraph, 0);
        if (node1Normal == null || node2Normal == null) return null;
    ...
    public Collection<double[]> getIntersection(
        ...
        if (!isTurn()) {
            ...
            return other.getIntersection(other1, other2, this, node1, node2).stream()
        ...
        for (int i = 0; i < turn.getSegmentCount(); i++) {
            ...
            for (int j = 0; j < other.turn.getSegmentCount(); j++) {
                ...
                double[] intersection =
                        VecHelper.intersectRanged(seg1, otherSeg1, seg2, otherSeg2, Axis.Y);
                otherSeg1 = otherSeg2;
    ...
    public static TrackEdge read(
        ...
        TrackEdge trackEdge = new TrackEdge(
                node1,
                node2,
                tag.contains("Positions") ? new BezierConnection(tag, BlockPos.ZERO) : null,
                TrackMaterial.deserialize(tag.getString("Material")));
        trackEdge.edgeData =
                EdgeData.read(tag.getCompound("Signals"), trackEdge, graph, dimensions);
        return trackEdge;
    }
}
