...
public class FlapDisplaySection {
    ...
    public FlapDisplaySection(float width, String cycle, boolean singleFlap, boolean hasGap) {
        ...
        this.spinning = new boolean
                [singleFlap ? 1 : Math.max(0, (int) (width / FlapDisplaySection.MONOSPACE))];
    ...
    public void refresh(boolean transition) {
        ...
        if (!singleFlap) {
            if (rightAligned) newText = newText.trim();
        ...
        } else if (!text.isEmpty()) spinning[0] |= transition && !newText.equals(text);
    ...
    public CompoundTag write(HolderLookup.Provider registries) {
        ...
        if (rightAligned) NBTHelper.putMarker(tag, "RightAligned");
        if (singleFlap) NBTHelper.putMarker(tag, "SingleFlap");
    ...
    public static FlapDisplaySection load(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        if (!tag.contains("Text")) return section;
    ...
    public void update(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        if (!text.isEmpty()) component = Component.Serializer.fromJson(text, registries);
        if (cyclingOptions == null) cyclingOptions = getFlapCycle(cycle);
    ...
    public static String[] getFlapCycle(String key) {
        return LOADED_FLAP_CYCLES.computeIfAbsent(
                key, k -> CreateLang.translateDirect("flap_display.cycles." + key)
                        .getString()
                        .split(";"));
    }
}
