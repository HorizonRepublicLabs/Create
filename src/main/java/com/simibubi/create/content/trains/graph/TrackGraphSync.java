...
public class TrackGraphSync {
        ...
        if (!queuedPackets.isEmpty()) {
            for (TrackGraphPacket packet : queuedPackets) {
                if (!packet.packetDeletesGraph
                        && !Create.RAILWAYS.trackNetworks.containsKey(packet.graphId)) continue;
    ...
    public void nodeAdded(TrackGraph graph, TrackNode node) {
        ...
        currentGraphSyncPacket.addedNodes.put(
                node.getNetId(), Pair.of(node.getLocation(), node.getNormal()));
        currentPayload++;
    }
    ...
    public void edgeDataChanged(
            TrackGraph graph, TrackNode node1, TrackNode node2, TrackEdge edge) {
        flushGraphPacket(graph);
    ...
    public void edgeDataChanged(
            TrackGraph graph, TrackNode node1, TrackNode node2, TrackEdge edge, TrackEdge edge2) {
        flushGraphPacket(graph);
        currentGraphSyncPacket.syncEdgeData(node1, node2, edge);
    ...
    public void sendFullGraphTo(TrackGraph graph, ServerPlayer player) {
        ...
        for (TrackNode node : graph.nodes.values()) {
            TrackGraphSyncPacket currentPacket = packet;
            if (!graph.connectionsByNode.containsKey(node)) continue;

            for (Entry<TrackNode, TrackEdge> entry :
                    graph.connectionsByNode.get(node).entrySet()) {
                ...
                currentPacket.addedEdges.add(
                        Pair.of(Pair.of(key, edge.getTrackMaterial()), edge.getTurn()));
        ...
        if (sent > 0) flushAndCreateNew(graph, player, packet);
    }
    ...
    private TrackGraphSyncPacket flushAndCreateNew(
            TrackGraph graph, ServerPlayer player, TrackGraphSyncPacket packet) {
    ...
    private void flushGraphPacket(@Nullable UUID graphId, int netId) {
        if (currentGraphSyncPacket != null) {
            if (currentGraphSyncPacket.graphId.equals(graphId) && currentPayload < 1000) return;
