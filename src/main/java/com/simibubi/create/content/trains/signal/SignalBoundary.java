...
public class SignalBoundary extends TrackEdgePoint {
    ...
    public void setGroup(boolean primary, UUID groupId) {
        ...
        if (opposite != null && signalEdgeGroups.containsKey(opposite)) {
            ...
            if (previous != null) oppositeGroup.removeAdjacent(previous);
            if (groupId != null) oppositeGroup.putAdjacent(groupId);
        }

        if (groupId != null && signalEdgeGroups.containsKey(groupId)) {
            SignalEdgeGroup group = signalEdgeGroups.get(groupId);
            if (opposite != null) group.putAdjacent(opposite);
        }
    }
        ...
        for (boolean first : Iterate.trueAndFalse) {
            ...
            for (BlockPos blockPos : set.keySet()) {
                if (blockPos.equals(blockEntity))
                    return blockEntities.get(!first).isEmpty()
                            ? OverlayState.RENDER
                            : OverlayState.DUAL;
    ...
    public SignalState getStateFor(BlockPos blockEntity) {
        for (boolean first : Iterate.trueAndFalse) {
            Map<BlockPos, Boolean> set = blockEntities.get(first);
            if (set.containsKey(blockEntity)) return cachedStates.get(first);
        }
    ...
    public void tick(TrackGraph graph, boolean preTrains) {
        ...
        for (boolean front : Iterate.trueAndFalse) {
            if (!sidesToUpdate.get(front)) continue;
    ...
    private void tickState(TrackGraph graph) {
        for (boolean current : Iterate.trueAndFalse) {
            ...
            cachedStates.set(
                    current,
                    occupiedUnlessBySelf ? SignalState.RED : resolveSignalChain(graph, current));
        }
    }
    ...
    public boolean isForcedRed(boolean primary) {
        ...
        for (Boolean b : values) if (b) return true;
        return false;
    }

    private SignalState resolveSignalChain(TrackGraph graph, boolean side) {
        if (types.get(side) != SignalType.CROSS_SIGNAL) return SignalState.GREEN;
        ...
        if (allPathsFree) return SignalState.GREEN;
        if (noPathsFree) return SignalState.RED;
        ...
        for (int i = 1; i <= 2; i++)
            if (nbt.contains("Tiles" + i)) {
                ...
                NBTHelper.iterateCompoundList(
                        nbt.getList("Tiles" + i, Tag.TAG_COMPOUND), c -> blockEntities
                                .get(first)
                                .put(NBTHelper.readBlockPos(c, "Pos"), c.getBoolean("Power")));
            }

        for (int i = 1; i <= 2; i++)
            if (nbt.contains("Group" + i)) groups.set(i == 1, nbt.getUUID("Group" + i));
        for (int i = 1; i <= 2; i++) sidesToUpdate.set(i == 1, nbt.contains("Update" + i));
        for (int i = 1; i <= 2; i++)
    ...
    public void read(FriendlyByteBuf buffer, DimensionPalette dimensions) {
        ...
        for (int i = 1; i <= 2; i++) {
            if (buffer.readBoolean()) groups.set(i == 1, buffer.readUUID());
        }
    }
    ...
    public void write(
        ...
        for (int i = 1; i <= 2; i++)
            if (!blockEntities.get(i == 1).isEmpty())
                nbt.put(
                        "Tiles" + i,
                        NBTHelper.writeCompoundList(blockEntities.get(i == 1).entrySet(), e -> {
        ...
        for (int i = 1; i <= 2; i++)
            if (groups.get(i == 1) != null) nbt.putUUID("Group" + i, groups.get(i == 1));
        for (int i = 1; i <= 2; i++)
            if (sidesToUpdate.get(i == 1)) nbt.putBoolean("Update" + i, true);
        for (int i = 1; i <= 2; i++) NBTHelper.writeEnum(nbt, "Type" + i, types.get(i == 1));
        for (int i = 1; i <= 2; i++)
    ...
    public void write(FriendlyByteBuf buffer, DimensionPalette dimensions) {
        ...
        for (int i = 1; i <= 2; i++) {
            ...
            if (hasGroup) buffer.writeUUID(groups.get(i == 1));
        }
    }
