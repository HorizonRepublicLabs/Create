    ...
    protected void renderSafe(
        ...
        TransformStack.of(ms)
                ...
                .rotateYDegrees(AngleHelper.horizontalAngle(
                        be.getBlockState().getValue(FlapDisplayBlock.HORIZONTAL_FACING)))
                .uncenter()
                .translate(0, 0, -3 / 16f);
        ...
        for (int j = 0; j < lines.size(); j++) {
            ...
            for (int i = 0; i < line.size(); i++) {
                ...
                String text = section.renderCharsIndividually() || !section.spinning[0]
                        ? section.text
                        : section.cyclingOptions[
                                ((ticks / 3) + i * 13) % section.cyclingOptions.length];
    ...
    static class FlapDisplayRenderOutput implements FormattedCharSink {
        ...
        public boolean accept(int charIndex, Style style, int glyph) {
            ...
            if (section.renderCharsIndividually()
                    && section.spinning[Math.min(charIndex, section.spinning.length)]) {
                float speed =
                        section.spinningTicks > 5 && section.spinningTicks < 20 ? 1.75f : 2.5f;
                float cycle = (time / speed) + charIndex * 16.83f + lineIndex * 0.75f;
                float partial = cycle % 1;
                char cyclingGlyph =
                        section.cyclingOptions[((int) cycle) % section.cyclingOptions.length]
                                .charAt(0);
                glyph = paused
                        ? cyclingGlyph
                        : partial > 1 / 2f ? partial > 3 / 4f ? '_' : '-' : cyclingGlyph;
                dim = 0.75f;
            }
            ...
            if (!section.renderCharsIndividually() && section.spinning[0]) {
                glyph = ticks % 3 == 0
                        ? glyphWidth == 6 ? '-' : glyphWidth == 1 ? '\'' : glyph
                        : glyph;
                glyph = ticks % 3 == 2
                        ? glyphWidth == 6 ? '_' : glyphWidth == 1 ? '.' : glyph
                        : glyph;
                if (ticks % 3 != 1) dim = 0.75f;
            }
            ...
            float standardWidth = section.wideFlaps
                    ? FlapDisplaySection.WIDE_MONOSPACE
                    : FlapDisplaySection.MONOSPACE;

            if (section.renderCharsIndividually()) x += (standardWidth - glyphWidth) / 2f;

            if (isNotEmpty(bakedglyph)) {
                VertexConsumer vertexconsumer = bufferSource.getBuffer(renderTypeOf(bakedglyph));
                bakedglyph.render(
                        style.isItalic(), x, 0, pose, vertexconsumer, red, green, blue, a, light);
            }
        ...
        public float finish(int bgColor) {
            ...
            bakedglyph.renderEffect(
                    new BakedGlyph.Effect(-1f, 9f, section.size, -2f, 0.01f, r, g, b, a),
                    this.pose,
                    vertexconsumer,
                    light);
