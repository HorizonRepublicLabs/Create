...
public class CarriageContraptionVisual extends ContraptionVisual<CarriageContraptionEntity> {
    ...
    public CarriageContraptionVisual(
            VisualizationContext context, CarriageContraptionEntity entity, float partialTick) {
    ...
    protected <T extends BlockEntity> void setupVisualizer(T be, float partialTicks) {
        if (entity.getContraption() instanceof CarriageContraption cc
                && cc.isHiddenInPortal(be.getBlockPos())) {
            return;
        }
    ...
    protected void setupActor(
        ...
        if (entity.getContraption() instanceof CarriageContraption cc
                && cc.isHiddenInPortal(actor.left.pos())) {
            return;
        }
    ...
    private boolean checkCarriage(float pt) {
        ...
        if (entity.validForRender && carriage != null) {
            ...
            for (var bogey : carriage.bogeys) {
                if (bogey != null) {
                    visuals[numBogeys] = bogey.getStyle()
                            .createVisual(bogey.getSize(), visualizationContext, pt, true);
                    bogeys[numBogeys] = bogey;
                    bogeyPos[numBogeys] = bogey.isLeading
                            ? 0
    ...
    private void animate(float partialTick) {
        ...
        for (int bogeyIdx = 0; bogeyIdx < numBogeys; bogeyIdx++) {
            ...
            CarriageContraptionEntityRenderer.translateBogey(
                    poseStack, bogey, bogeySpacing, viewYRot, viewXRot, partialTick);
