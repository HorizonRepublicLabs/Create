...
public class TrackPlacementOverlay implements LayeredDraw.Layer {
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (TrackPlacement.cached == null
                || TrackPlacement.cached.curve == null
                || !TrackPlacement.cached.valid) return;
        if (TrackPlacement.extraTipWarmup < 4) return;

        if (((GuiAccessor) mc.gui).create$getToolHighlightTimer() > 0) return;
        ...
        Color color = new Color(0x4ADB4A)
                .setAlpha(Mth.clamp((TrackPlacement.extraTipWarmup - 4) / 3f, 0.1f, 1));
