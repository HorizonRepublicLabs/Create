...
public class DimensionPalette {
    public static final StreamCodec<ByteBuf, DimensionPalette> STREAM_CODEC =
            CatnipStreamCodecBuilders.list(ResourceKey.streamCodec(Registries.DIMENSION))
    ...
    public ResourceKey<Level> decode(int index) {
        if (gatheredDims.size() <= index || index < 0) return Level.OVERWORLD;
    ...
    public static DimensionPalette read(CompoundTag tag) {
        ...
        NBTHelper.iterateCompoundList(
                tag.getList("DimensionPalette", Tag.TAG_COMPOUND),
                c -> palette.gatheredDims.add(ResourceKey.create(
                        Registries.DIMENSION, ResourceLocation.parse(c.getString("Id")))));
        return palette;
    }
}
