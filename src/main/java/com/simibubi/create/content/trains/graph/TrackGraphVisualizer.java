...
public class TrackGraphVisualizer {
    ...
    public static void visualiseSignalEdgeGroups(TrackGraph graph) {
        ...
        if (box == null || !box.intersects(cameraEntity.getBoundingBox().inflate(50))) return;
        ...
        for (Entry<TrackNodeLocation, TrackNode> nodeEntry : graph.nodes.entrySet()) {
            ...
            if (location.distanceTo(camera) > 50) continue;
            if (!mc.level.dimension().equals(nodeLocation.dimension)) continue;
            ...
            for (Entry<TrackNode, TrackEdge> entry : map.entrySet()) {
                ...
                if (!edge.node1.location.dimension.equals(edge.node2.location.dimension)) continue;
                if (other.hashCode() > hashCode
                        && other.location.getLocation().distanceTo(camera) <= 50) continue;
                ...
                if (!edge.isTurn()) {
                    ...
                    if (signalData.hasSignalBoundaries()) {
                        ...
                        for (TrackEdgePoint trackEdgePoint : signalData.getPoints()) {
                            if (!(trackEdgePoint instanceof SignalBoundary boundary)) continue;
                            ...
                            if (group != null)
                                outliner.showLine(
                                                ...
                                                edge.getPosition(
                                                                graph,
                                                                (prev = boundary.getLocationOn(edge)
                                                                                / length)
                                                                        - 1 / 16f / length)
                                                        .add(yOffset))
                        ...
                        if (prevBoundary != null) {
                            ...
                            if (group != null)
                                outliner.showLine(
                                                edge,
                                                edge.getPosition(graph, prev + 1 / 16f / length)
                                                        .add(yOffset),
                    ...
                    SignalEdgeGroup singleEdgeGroup =
                            singleGroup == null ? null : allGroups.get(singleGroup);
                    ...
                    if (signalData.hasSignalBoundaries()) {
                        ...
                        while (points.hasNext()) {
                            ...
                            if (!(next instanceof SignalBoundary signal)) continue;
                        ...
                        if (initialGroupId == null) continue;
                        SignalEdgeGroup initialGroup = allGroups.get(initialGroupId);
                        if (initialGroup == null) continue;
    ...
    public static void debugViewGraph(TrackGraph graph, boolean extended) {
        ...
        for (Entry<TrackNodeLocation, TrackNode> nodeEntry : graph.nodes.entrySet()) {
            ...
            Outliner.getInstance()
                    .showLine(Integer.valueOf(node.netId), v1, v2)
                ...
                if (!edge.isTurn()) {
                    if (extended) {
                        ...
                        Outliner.getInstance()
                                .showAABB(
                                        edge.edgeData,
                                        AABB.ofSize(materialPos, .25, 0, .25).move(0, -0.5, 0))
                                .lineWidth(1 / 16f)
                                .colored(graph.color);
                    }
                    Outliner.getInstance()
