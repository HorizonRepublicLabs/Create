...
public class TrainHUD {
    ...
    public static void tick() {
        if (promptKeepAlive > 0) promptKeepAlive--;
        else currentPrompt = null;

        Minecraft mc = Minecraft.getInstance();
        displayedPromptSize.chase(
                currentPrompt != null ? mc.font.width(currentPrompt) + 17 : 0, .5f, Chaser.EXP);
        ...
        double value = Math.abs(train.speed)
                / (train.maxSpeed()
        ...
        displayedThrottle.chase(
                editedThrottle != null ? editedThrottle : train.throttle, .75f, Chaser.EXP);
        ...
        if (hudPacketCooldown-- <= 0) {
            CatnipServices.NETWORK.sendToServer(
                    new TrainHUDUpdatePacket.Serverbound(train, editedThrottle));
            hudPacketCooldown = 5;
        }
    }
    ...
    private static void renderOverlay(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (!(ControlsHandler.getContraption() instanceof CarriageContraptionEntity cce)) return;
        ...
        int w = (int)
                (AllGuiTextures.TRAIN_HUD_SPEED.getWidth() * displayedSpeed.getValue(partialTicks));
        ...
        if (promptSize > 1) {
            ...
            if (currentPrompt != null && font.width(currentPrompt) < promptSize - 10) {
                ...
                else guiGraphics.drawString(font, currentPrompt, 9, 4, 0x544D45, false);
                poseStack.popPose();
            }
        }
        ...
        w = (int) (AllGuiTextures.TRAIN_HUD_THROTTLE.getWidth()
                * (1 - displayedThrottle.getValue(partialTicks)));
        ...
        AllGuiTextures.TRAIN_HUD_THROTTLE_POINTER.render(
                guiGraphics, Math.max(1, AllGuiTextures.TRAIN_HUD_THROTTLE.getWidth() - w) - 3, -2);
        ...
        if (reversing) angleOffset *= -1;

        float snapSize = 22.5f;
        float diff = AngleHelper.getShortestAngleDiff(cameraEntity.getYRot(), cce.yaw)
                + (inverted ? -90 : 90);
