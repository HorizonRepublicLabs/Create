...
public class GlobalTrainDisplayData {
    ...
    public static final Map<String, Collection<TrainDeparturePrediction>> statusByDestination =
            new HashMap<>();
    ...
    public static void refresh() {
        ...
        for (Train train : Create.RAILWAYS.trains.values()) {
            if (train.runtime.paused || train.runtime.getSchedule() == null) continue;
            if (train.derailed || train.graph == null) continue;
            for (TrainDeparturePrediction prediction : train.runtime.submitPredictions())
                statusByDestination
                        .computeIfAbsent(prediction.destination, $ -> new ArrayList<>())
                        .add(prediction);
        }
    }
    ...
    public static List<TrainDeparturePrediction> prepare(String filter, int maxLines) {
        ...
        return statusByDestination.entrySet().stream()
    ...
    public static class TrainDeparturePrediction implements Comparable<TrainDeparturePrediction> {
        ...
        public TrainDeparturePrediction(
                Train train, int ticks, MutableComponent scheduleTitle, String destination) {
        ...
        private int getCompareTicks() {
            if (ticks == -1) return Integer.MAX_VALUE;
            if (ticks < 200) return 0;
            return ticks;
        }
        ...
        public int compareTo(TrainDeparturePrediction o) {
            ...
            if (compare == 0) return train.name.getString().compareTo(o.train.name.getString());
            return compare;
        }
    }
}
