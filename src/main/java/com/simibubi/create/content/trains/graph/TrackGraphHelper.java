...
public class TrackGraphHelper {
    ...
    public static TrackGraphLocation getGraphLocationAt(
            Level level, BlockPos pos, AxisDirection targetDirection, Vec3 targetAxis) {
        BlockState trackBlockState = level.getBlockState(pos);
        if (!(trackBlockState.getBlock() instanceof ITrackBlock track)) return null;
        ...
        if (graph != null) {
            ...
            if (node != null) {
                ...
                for (Entry<TrackNode, TrackEdge> entry : connectionsFrom.entrySet()) {
                    ...
                    if (direction.scale(length).distanceToSqr(axis.scale(-1)) > 1 / 4096f) continue;
        ...
        Collection<DiscoveredLocation> ends =
                track.getConnected(level, pos, trackBlockState, true, null);
        ...
        for (DiscoveredLocation current : ends) {
            ...
            boolean forward =
                    compareOffset.distanceToSqr(axis.multiply(-1, 0, -1).normalize()) < 1 / 4096f;
            boolean backwards =
                    compareOffset.distanceToSqr(axis.multiply(1, 0, 1).normalize()) < 1 / 4096f;
            ...
            for (int i = 0; i < 100 && distance < 32; i++) {
                ...
                if (graph == null) graph = Create.RAILWAYS.sided(level).getGraph(level, loc);

                if (graph == null || graph.locateNode(loc) == null) {
                    singleTrackPiece = false;
                    Collection<DiscoveredLocation> list =
                            ITrackBlock.walkConnectedTracks(level, loc, true);
                    for (DiscoveredLocation discoveredLocation : list) {
                        ...
                        if ((forward ? axis.scale(-1) : axis)
                                        .distanceToSqr(diff.normalize().scale(length))
                                > 1 / 4096f) continue;
        ...
        if (frontNode == null || backNode == null) return null;

        if (singleTrackPiece)
            position = frontNode
                            .getLocation()
                            .getLocation()
                            .distanceTo(backNode.getLocation().getLocation())
                    / 2.0;
    ...
    public static TrackGraphLocation getBezierGraphLocationAt(
        ...
        if (!(level.getBlockEntity(pos) instanceof TrackBlockEntity trackBE)) return null;
        BezierConnection bc = trackBE.getConnections().get(targetBezier.curveTarget());
        if (bc == null || !bc.isPrimary()) return null;

        TrackNodeLocation targetLoc = new TrackNodeLocation(bc.starts.getSecond()).in(level);
        if (bc.smoothing != null) targetLoc.yOffsetPixels = bc.smoothing.getSecond();
