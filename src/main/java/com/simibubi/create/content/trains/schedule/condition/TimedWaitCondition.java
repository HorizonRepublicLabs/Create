...
public abstract class TimedWaitCondition extends ScheduleWaitCondition {
    ...
    protected Component formatTime(boolean compact) {
        ...
        return Component.literal(getValue() + " ")
                .append(CreateLang.translateDirect(getUnit().key));
    }
    ...
    public List<Component> getTitleAs(String type) {
        return ImmutableList.of(
                Component.translatable(
                        getId().getNamespace() + ".schedule." + type + "." + getId().getPath()),
    ...
    public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
        ...
        int num = (int)
                (showInMinutes
                        ? Math.floor(ticksUntilDeparture / (20 * 60f))
                        : Math.ceil(ticksUntilDeparture / 100f) * 5);
        String key = "generic."
                + (showInMinutes
                        ? num == 1 ? "daytime.minute" : "unit.minutes"
                        : num == 1 ? "daytime.second" : "unit.seconds");
        return CreateLang.translateDirect(
                "schedule.condition." + getId().getPath() + ".status",
