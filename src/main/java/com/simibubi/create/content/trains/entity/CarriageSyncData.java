...
public class CarriageSyncData {
    ...
    public CarriageSyncData copy() {
        ...
        for (int i = 0; i < 4; i++) data.wheelLocations.set(i, wheelLocations.get(i));
    ...
    public void update(CarriageContraptionEntity entity, Carriage carriage) {
        ...
        for (boolean first : Iterate.trueAndFalse) {
            ...
            for (boolean firstPoint : Iterate.trueAndFalse) {
                ...
                wheelLocations.set(
                        index, Pair.of(nodes.map(TrackNode::getNetId), (float) point.position));
            }
        }
        ...
        for (int i = 0; i < wheelLocations.size(); i++) {
            ...
            if (dce.pointsInitialised) {
                ...
                for (boolean forward : Iterate.trueAndFalse) {
                    float distanceTo =
                            getDistanceTo(graph, bogeyPoint, point, foundDistance, forward);
        ...
        for (boolean first : Iterate.trueAndFalse) {
            ...
            for (boolean firstPoint : Iterate.trueAndFalse) {
                ...
                ITrackSelector trackSelector = point.follow(
                        toApproach, b -> success.setValue(success.booleanValue() && b));
    ...
    public float getDistanceTo(
        ...
        double distanceToNode2 =
                forward ? initialEdge.getLength() - current.position : current.position;

        frontier.add(Pair.of(
                distanceToNode2, Pair.of(Couple.create(initialNode1, initialNode2), initialEdge)));
        ...
        while (!frontier.isEmpty()) {
            ...
            if (edge == targetEdge)
                return (float) (distance
                        - (forward ? edge.getLength() - target.position : target.position));
