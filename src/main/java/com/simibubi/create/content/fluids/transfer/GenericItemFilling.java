...
public class GenericItemFilling {
    ...
    public static boolean canItemBeFilled(Level world, ItemStack stack) {
        ...
        if (!isFluidHandlerValid(stack, capability)) return false;
    ...
    public static int getRequiredAmountForItem(
        ...
        if (stack.getItem() == Items.BUCKET && canFillBucketInternally(availableFluid)) return 1000;
        ...
        if (capability instanceof FluidBucketWrapper) {
            ...
            if (filledBucket == null || filledBucket == Items.AIR) return -1;
            if (!((FluidBucketWrapper) capability).getFluid().isEmpty()) return -1;
            return 1000;
        }
    ...
    private static boolean canFillGlassBottleInternally(FluidStack availableFluid) {
        ...
        if (fluid.isSame(Fluids.WATER)) return true;
        if (fluid.isSame(AllFluids.POTION.get())) return true;
    ...
    public static ItemStack fillItem(
        ...
        if (stack.getItem() == Items.GLASS_BOTTLE && canFillGlassBottleInternally(toFill)) {
            ...
            else fillBottle = PotionFluidHandler.fillBottle(stack, toFill);
            stack.shrink(1);
            return fillBottle;
        }
