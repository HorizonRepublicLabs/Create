...
public class BoilerData {
    ...
    public void tick(FluidTankBlockEntity controller) {
        ...
        if (level.isClientSide) {
            ...
            if (current > 1 && level.random.nextFloat() < 1 / 2f)
                gauge.setValueNoUpdate(
                        current + Math.min(-(current - 1) * level.random.nextFloat(), 0));
            return;
        }
        if (needsHeatLevelUpdate && updateTemperature(controller)) controller.notifyUpdate();
        ticksUntilNextSample--;
        if (ticksUntilNextSample > 0) return;
        ...
        if (currentIndex == 0) {
            waterSupply = 0;
            for (float i : supplyOverTime) waterSupply = Math.max(i, waterSupply);
        }
    ...
    public void updateOcclusion(FluidTankBlockEntity controller) {
        ...
        if (attachedEngines + attachedWhistles == 0) return;
        for (Direction d : Iterate.horizontalDirections) {
            AABB aabb = new AABB(controller.getBlockPos())
                    .move(controller.width / 2f - .5f, 0, controller.width / 2f - .5f)
                    .deflate(5f / 8);
            aabb = aabb.move(
    ...
    public float getEngineEfficiency(int boilerSize) {
        if (isPassive(boilerSize)) return passiveEngineEfficiency / attachedEngines;
    ...
    public boolean addToGoggleTooltip(
            List<Component> tooltip, boolean isPlayerSneaking, int boilerSize) {
        ...
        CreateLang.translate(
                        "boiler.status",
                        getHeatLevelTextComponent().withStyle(ChatFormatting.GREEN))
                .forGoggles(tooltip);
        ...
        if (attachedEngines == 0) return true;

        int boilerLevel = Math.min(activeHeat, Math.min(maxHeatForWater, maxHeatForSize));
        double totalSU = getEngineEfficiency(boilerSize)
                * 16
                * Math.max(boilerLevel, attachedEngines)
        ...
        CreateLang.number(totalSU)
                ...
                .space()
                .add((attachedEngines == 1
                                ? CreateLang.translate("boiler.via_one_engine")
                                : CreateLang.translate("boiler.via_engines", attachedEngines))
                        .style(ChatFormatting.DARK_GRAY))
                .forGoggles(tooltip, 1);
    ...
    public void calcMinMaxForSize(int boilerSize) {
        ...
        minValue =
                Math.min(passiveHeat ? 1 : activeHeat, Math.min(maxHeatForWater, maxHeatForSize));
        maxValue =
                Math.max(passiveHeat ? 1 : activeHeat, Math.max(maxHeatForWater, maxHeatForSize));
    }
    ...
    public MutableComponent getHeatLevelTextComponent() {
        ...
        return isPassive()
                ...
                : (boilerLevel == 0
                        ...
                        : boilerLevel == 18
                                ? CreateLang.translateDirect("boiler.max_lvl")
                                : CreateLang.translateDirect(
                                        "boiler.lvl", String.valueOf(boilerLevel)));
    }

    public MutableComponent getSizeComponent(
            boolean forGoggles, boolean useBlocksAsBars, ChatFormatting... styles) {
    ...
    public MutableComponent getHeatComponent(
        ...
        return componentHelper(
                "heat", passiveHeat ? 1 : activeHeat, forGoggles, useBlocksAsBars, styles);
    }
    ...
    private MutableComponent blockComponent(int level) {
        return Component.literal("\u2588".repeat(minValue)
            ...
            for (int xOffset = 0; xOffset < controller.width; xOffset++) {
                for (int zOffset = 0; zOffset < controller.width; zOffset++) {
                    ...
                    for (Direction d : Iterate.directions) {
                        ...
                        if (AllBlocks.STEAM_ENGINE.has(attachedState)
    ...
    public void checkPipeOrganAdvancement(FluidTankBlockEntity controller) {
        if (!controller.getBehaviour(AdvancementBehaviour.TYPE).isOwnerPresent()) return;
        ...
        if (whistlePitches.size() >= 12) controller.award(AllAdvancements.PIPE_ORGAN);
    }
    ...
    public void read(CompoundTag nbt, int boilerSize) {
        ...
        int forWaterSupply = getMaxHeatLevelForWaterSupply();
        int actualHeat = Math.min(activeHeat, Math.min(forWaterSupply, forBoilerSize));
        float target = isPassive(boilerSize)
                ? 1 / 8f
                : forBoilerSize == 0 ? 0 : actualHeat / (forBoilerSize * 1f);
    ...
    public class BoilerFluidHandler implements IFluidHandler {
        ...
        public int fill(FluidStack resource, FluidAction action) {
            ...
            if (action.execute()) gatheredSupply += amount;
            return amount;
        }
