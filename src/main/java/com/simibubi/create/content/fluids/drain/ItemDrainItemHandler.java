...
public class ItemDrainItemHandler implements IItemHandler {
    ...
    public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
        if (!blockEntity.getHeldItemStack().isEmpty()) return stack;

        ItemStack returned = ItemStack.EMPTY;
        if (stack.getCount() > 1
                && GenericItemEmptying.canItemBeEmptied(blockEntity.getLevel(), stack)) {
            returned = stack.copyWithCount(stack.getCount() - 1);
            stack = stack.copyWithCount(1);
        } else returned = ItemHelper.limitCountToMaxStackSize(stack, simulate);
        if (!simulate) {
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!simulate) {
            ...
            if (stack.isEmpty()) blockEntity.heldItem = null;
            blockEntity.notifyUpdate();
        }
        return extracted;
    }
