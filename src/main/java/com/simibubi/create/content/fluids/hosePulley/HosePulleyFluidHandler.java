...
public class HosePulleyFluidHandler implements IFluidHandler {
    ...
    public int fill(FluidStack resource, FluidAction action) {
        if (!internalTank.isEmpty()
                && !FluidStack.isSameFluidSameComponents(resource, internalTank.getFluid()))
            return 0;
        if (resource.isEmpty() || !FluidHelper.hasBlockState(resource.getFluid())) return 0;
    ...
    public FluidStack getFluidInTank(int tank) {
        if (internalTank.isEmpty()) return drainer.getDrainableFluid(rootPosGetter.get());
    ...
    private FluidStack drainInternal(
            int maxDrain, @Nullable FluidStack resource, FluidAction action) {
        if (resource != null
                && !internalTank.isEmpty()
                && !FluidStack.isSameFluidSameComponents(resource, internalTank.getFluid()))
            return FluidStack.EMPTY;
        if (internalTank.getFluidAmount() >= 1000) return internalTank.drain(maxDrain, action);
        ...
        if (!internalTank.isEmpty()
                        && !FluidStack.isSameFluidSameComponents(internalTank.getFluid(), returned)
                || returned.isEmpty()) return internalTank.drain(maxDrain, action);
        ...
        if (action.execute() && !leftover.isEmpty()) internalTank.setFluid(leftover);
        return returned;
    }
