...
public class BasinFluidParticle extends FluidStackParticle {
    ...
    public BasinFluidParticle(
        ...
        if (vx != 0) {
            ...
            Vec3 diff =
                    currentPos.subtract(centerOf).multiply(1, 0, 1).normalize().scale(.375);
    ...
    public void tick() {
        ...
        if (targetPos != null) {
            ...
            Vec3 currentPos =
                    centerOfBasin.add(targetPos.subtract(centerOfBasin).scale(progess));
            x = currentPos.x;
            z = currentPos.z;
        }
    }
