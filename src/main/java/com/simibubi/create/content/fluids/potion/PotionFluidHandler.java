...
public class PotionFluidHandler {
    ...
    public static SizedFluidIngredient potionIngredient(Holder<Potion> potion, int amount) {
        FluidStack stack = FluidHelper.copyStackWithAmount(
    ...
    public static BottleType bottleTypeFromItem(Item item) {
        if (item == Items.LINGERING_POTION) return BottleType.LINGERING;
        if (item == Items.SPLASH_POTION) return BottleType.SPLASH;
    ...
    public static ItemStack fillBottle(ItemStack stack, FluidStack availableFluid) {
        ...
        potionStack.set(
                DataComponents.POTION_CONTENTS, availableFluid.get(DataComponents.POTION_CONTENTS));
        return potionStack;
    }
    ...
    public static void addPotionTooltip(
            FluidStack fs, Consumer<Component> tooltipAdder, float durationFactor) {
        PotionContents contents =
                fs.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY);
        ...
        for (MobEffectInstance mobeffectinstance : effects) {
            ...
            holder.value()
                    .createModifiers(
                            mobeffectinstance.getAmplifier(), (h, m) -> list.add(Pair.of(h, m)));
            if (mobeffectinstance.getAmplifier() > 0) {
                mutablecomponent
                        .append(" ")
                        .append(Component.translatable(
                                        "potion.potency." + mobeffectinstance.getAmplifier())
                                .getString());
            }
            ...
            tooltipAdder.accept(mutablecomponent.withStyle(
                    holder.value().getCategory().getTooltipFormatting()));
        }

        if (flag) tooltipAdder.accept(NO_EFFECT);
                ...
                if (d1 > 0.0D) {
                    tooltipAdder.accept((Component.translatable(
                                    ...
                                    Component.translatable(
                                            pair.getFirst().value().getDescriptionId())))
