...
public class GenericItemEmptying {
    ...
    public static boolean canItemBeEmptied(Level world, ItemStack stack) {
        ...
        for (int i = 0; i < capability.getTanks(); i++) {
            if (capability.getFluidInTank(i).getAmount() > 0) return true;
        }
        return false;
    }
    ...
    public static Pair<FluidStack, ItemStack> emptyItem(
        ...
        if (capability == null) return Pair.of(resultingFluid, resultingItem);
        resultingFluid =
                capability.drain(1000, simulate ? FluidAction.SIMULATE : FluidAction.EXECUTE);
