...
public class PotionMixingRecipes {
    ...
    private static List<RecipeHolder<MixingRecipe>> createRecipesImpl(Level level) {
        ...
        for (Item container : allowedSupportedContainers) {
            ...
            for (PotionBrewing.Mix<Potion> mix :
                ...
                mixingRecipes.add(createRecipe(
                        "potion_mixing_vanilla_" + recipeIndex++,
                        mix.ingredient(),
                        fromFluid,
                        toFluid));
            }
        ...
        for (PotionBrewing.Mix<Item> mix :
                ((PotionBrewingAccessor) potionBrewing).create$getContainerMixes()) {
            ...
            for (Reference<Potion> potion : potions) {
                ...
                mixingRecipes.add(createRecipe(
                        "potion_mixing_vanilla_" + recipeIndex++, ingredient, fromFluid, toFluid));
            }
        }
    ...
    private static RecipeHolder<MixingRecipe> createRecipe(
    ...
    private static Map<Item, List<MixingRecipe>> sortRecipesByItem(
            List<RecipeHolder<MixingRecipe>> all) {
            ...
            for (Ingredient ingredient : recipe.value().getIngredients()) {
                for (ItemStack itemStack : ingredient.getItems()) {
                    ...
                    if (processedItems.add(item)) {
                        byItem.computeIfAbsent(item, i -> new ArrayList<>()).add(recipe.value());
                    }
                }
            }
