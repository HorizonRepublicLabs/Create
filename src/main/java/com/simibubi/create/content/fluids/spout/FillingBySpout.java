...
public class FillingBySpout {
    ...
    public static boolean canItemBeFilled(Level world, ItemStack stack) {
        ...
        Optional<RecipeHolder<FillingRecipe>> assemblyRecipe = SequencedAssemblyRecipe.getRecipe(
                world, input, AllRecipeTypes.FILLING.getType(), FillingRecipe.class);
        if (assemblyRecipe.isPresent()) return true;

        if (AllRecipeTypes.FILLING.find(input, world).isPresent()) return true;
        return GenericItemFilling.canItemBeFilled(world, stack);
    }

    public static int getRequiredAmountForItem(
            Level world, ItemStack stack, FluidStack availableFluid) {
        SingleRecipeInput input = new SingleRecipeInput(stack);
    ...
    public static ItemStack fillItem(
        ...
        RecipeHolder<FillingRecipe> fillingRecipe = SequencedAssemblyRecipe.getRecipe(
                        level,
                        input,
                        AllRecipeTypes.FILLING.getType(),
                        FillingRecipe.class,
                ...
                .orElseGet(() -> {
                    for (RecipeHolder<Recipe<SingleRecipeInput>> recipe : level.getRecipeManager()
                        ...
                        if (requiredFluid.test(toFill)) return new RecipeHolder<>(recipe.id(), fr);
                    }
                    return null;
                });
    ...
    private static Predicate<RecipeHolder<FillingRecipe>> matchItemAndFluid(
            Level world, FluidStack availableFluid, SingleRecipeInput input) {
        return r -> r.value().matches(input, world)
                && r.value().getRequiredFluid().test(availableFluid);
    }
}
