...
public class HatchFilterSlot extends ValueBoxTransform {
    ...
    public Vec3 getLocalOffset(LevelAccessor level, BlockPos pos, BlockState state) {
        return VecHelper.rotateCentered(
                VecHelper.voxelSpace(8, 5.15, 9.5), angle(state), Direction.Axis.Y);
    }
    ...
    public boolean testHit(LevelAccessor level, BlockPos pos, BlockState state, Vec3 localHit) {
        return localHit.distanceTo(getLocalOffset(level, pos, state).subtract(0, 0.125, 0))
                < scale / 2;
    }
    ...
    private float angle(BlockState state) {
        return AngleHelper.horizontalAngle(
