...
public class BeltTunnelShapes {
    ...
    private static final VoxelShape block = box(0, -5, 0, 16, 16, 16);

    private static final VoxelShaper opening =
            VoxelShaper.forHorizontal(box(2, -5, 14, 14, 10, 16), Direction.SOUTH);
    ...
    public static VoxelShape getShape(BlockState state) {
        ...
        if (shape == BeltTunnelBlock.Shape.CROSS) return CROSS;

        if (BeltTunnelBlock.isStraight(state)) return STRAIGHT.get(axis);
