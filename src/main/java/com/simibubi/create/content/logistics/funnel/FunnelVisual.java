...
public class FunnelVisual extends AbstractBlockEntityVisual<FunnelBlockEntity>
    ...
    public FunnelVisual(
        ...
        PartialModel flapPartial = (blockState.getBlock() instanceof FunnelBlock
                ? AllPartialModels.FUNNEL_FLAP
                : AllPartialModels.BELT_FUNNEL_FLAP);

        var commonTransform = FlapStuffs.commonTransform(
                getVisualPosition(), funnelFacing, -blockEntity.getFlapOffset());
        flaps = new FlapStuffs.Visual(
                instancerProvider(),
                commonTransform,
                FlapStuffs.FUNNEL_PIVOT,
                Models.partial(flapPartial));
    ...
    public void updateLight(float partialTick) {
        if (flaps != null) flaps.updateLight(computePackedLight());
    }
