...
public class FactoryPanelConnection {
    public static final Codec<FactoryPanelConnection> CODEC =
    ...
    public List<Direction> getPath(Level level, BlockState state, FactoryPanelPosition to) {
        if (!path.isEmpty() && arrowBendModeCurrentPathUses == arrowBendMode) return path;
        ...
        final Vec3 start = fromBehaviour != null
                ? fromBehaviour
                        .getSlotPositioning()
        ...
        for (int actualMode = 0; actualMode <= 4; actualMode++) {
            path.clear();
            if (!findSuitable && actualMode != arrowBendMode) continue;
            ...
            for (BlockPos toTravel : List.of(toTravelFirst, toTravelLast)) {
                ...
                for (int i = 0; i < 100; i++) {
                    if (toTravel.equals(BlockPos.ZERO)) break;

                    for (Direction d : directionOrder) {
                        if (lastDirection != null && d == lastDirection.getOpposite()) continue;
                        if (currentDirection == null
                                || toTravel.relative(d).distManhattan(BlockPos.ZERO)
                                        < toTravel.relative(currentDirection)
                                                .distManhattan(BlockPos.ZERO)) currentDirection = d;
                    }
            ...
            if (findSuitable && !desperateOption) {
                ...
                for (int i = 0; i < path.size() - 1; i++) {
                    ...
                    if (!level.noCollision(new AABB(v, v).inflate(1 / 128f))) continue ModeFinder;
                }
            }
