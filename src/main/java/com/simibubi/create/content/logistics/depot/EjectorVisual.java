...
public class EjectorVisual extends ShaftVisual<EjectorBlockEntity> implements SimpleDynamicVisual {
    ...
    public EjectorVisual(
            VisualizationContext dispatcher, EjectorBlockEntity blockEntity, float partialTick) {
        super(dispatcher, blockEntity, partialTick);

        plate = instancerProvider()
    ...
    private void pivotPlate(float lidProgress) {
        ...
        EjectorRenderer.applyLidAngle(
                blockEntity, angle, plate.setIdentityTransform().translate(getVisualPosition()));
        plate.setChanged();
    }
