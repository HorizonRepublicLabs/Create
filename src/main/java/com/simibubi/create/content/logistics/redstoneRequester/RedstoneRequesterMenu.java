...
public class RedstoneRequesterMenu extends GhostItemMenu<RedstoneRequesterBlockEntity> {
    ...
    public RedstoneRequesterMenu(
            MenuType<?> type, int id, Inventory inv, RedstoneRequesterBlockEntity contentHolder) {
    ...
    public static RedstoneRequesterMenu create(
            int id, Inventory inv, RedstoneRequesterBlockEntity be) {
    ...
    protected RedstoneRequesterBlockEntity createOnClient(RegistryFriendlyByteBuf extraData) {
        ...
        return AllBlocks.REDSTONE_REQUESTER
                .get()
    ...
    protected void saveData(RedstoneRequesterBlockEntity contentHolder) {
        ...
        for (int i = 0; i < ghostInventory.getSlots(); i++) {
            ...
            list.add(new BigItemStack(
                    stackInSlot.copyWithCount(1), i < stacks.size() ? stacks.get(i).count : 1));
        }
