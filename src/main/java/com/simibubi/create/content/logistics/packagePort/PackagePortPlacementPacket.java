...
public record PackagePortPlacementPacket(PackagePortTarget target, BlockPos pos)
        implements ServerboundPacketPayload {
    public static final StreamCodec<RegistryFriendlyByteBuf, PackagePortPlacementPacket>
    ...
    public void handle(ServerPlayer player) {
        ...
        if (!(blockEntity instanceof PackagePortBlockEntity ppbe)) return;
        if (!target.canSupport(ppbe)) return;
        ...
        if (targetLocation == Vec3.ZERO
                || !targetLocation.closerThan(
                        ...
                        AllConfigs.server().logistics.packagePortRange.get() + 2)) return;
