...
public class FunnelGenerator extends SpecialBlockStateGen {
    ...
    public <T extends Block> ModelFile getModel(
            DataGenContext<Block, T> c, RegistrateBlockstateProvider p, BlockState s) {
        ...
        BlockModelBuilder model = p.models()
                .withExistingParent(
                        "block/" + type + "_funnel_" + parent + extracting + powered,
        ...
        if (horizontal) return model.texture("block", blockTexture);

        return model.texture("frame", p.modLoc(prefix + type + "_funnel_frame"))
                .texture("open", p.modLoc(prefix + "funnel" + closed));
    }

    public static NonNullBiConsumer<DataGenContext<Item, FunnelItem>, RegistrateItemModelProvider>
            itemModel(String type) {
        String prefix = "block/funnel/";
