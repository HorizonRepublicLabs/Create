...
public record FilterScreenPacket(Option option, @Nullable CompoundTag data)
    ...
    public static final StreamCodec<ByteBuf, FilterScreenPacket> STREAM_CODEC =
    ...
    public void handle(ServerPlayer player) {
        ...
        if (player.containerMenu instanceof FilterMenu c) {
            if (this.option == Option.WHITELIST) c.blacklist = false;
            if (this.option == Option.BLACKLIST) c.blacklist = true;
            if (this.option == Option.RESPECT_DATA) c.respectNBT = true;
            if (this.option == Option.IGNORE_DATA) c.respectNBT = false;
        ...
        if (player.containerMenu instanceof AttributeFilterMenu c) {
            ...
            if (option == Option.ADD_TAG)
                c.appendSelectedAttribute(
                        ItemAttribute.loadStatic(data, player.registryAccess()), false);
            if (option == Option.ADD_INVERTED_TAG)
                c.appendSelectedAttribute(
                        ItemAttribute.loadStatic(data, player.registryAccess()), true);
        }

        if (player.containerMenu instanceof PackageFilterMenu c) {
            if (option == Option.UPDATE_ADDRESS) c.address = tag.getString("Address");
        }
    }
    ...
    public enum Option {
        ...
        public static final StreamCodec<ByteBuf, Option> STREAM_CODEC =
                CatnipStreamCodecBuilders.ofEnum(Option.class);
    }
}
