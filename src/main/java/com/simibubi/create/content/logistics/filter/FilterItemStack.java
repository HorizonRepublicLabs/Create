...
public class FilterItemStack {
    ...
    public static class ListFilterItemStack extends FilterItemStack {
        ...
        public ListFilterItemStack(ItemStack filter) {
            ...
            ItemStackHandler items =
                    ((ListFilterItem) filter.getItem()).getFilterItemHandler(filter);
            for (int i = 0; i < items.getSlots(); i++) {
                ItemStack stackInSlot = items.getStackInSlot(i);
                if (!stackInSlot.isEmpty()) containedItems.add(FilterItemStack.of(stackInSlot));
            }

            shouldRespectNBT = hasFilterItems
                    && filter.getOrDefault(AllDataComponents.FILTER_ITEMS_RESPECT_NBT, false);
            isBlacklist = hasFilterItems
                    && filter.getOrDefault(AllDataComponents.FILTER_ITEMS_BLACKLIST, false);
        }

        @Override
        public boolean test(Level world, ItemStack stack, boolean matchNBT) {
            for (FilterItemStack filterItemStack : containedItems)
                if (filterItemStack.test(world, stack, shouldRespectNBT)) return !isBlacklist;
            return isBlacklist;
        }
    ...
    public static class AttributeFilterItemStack extends FilterItemStack {
        ...
        public AttributeFilterItemStack(ItemStack filter) {
            ...
            List<ItemAttribute.ItemAttributeEntry> attributes = defaults
                    ? new ArrayList<>()
        ...
        @Override
        public boolean test(Level world, ItemStack stack, boolean matchNBT) {
            if (attributeTests.isEmpty()) return super.test(world, stack, matchNBT);
