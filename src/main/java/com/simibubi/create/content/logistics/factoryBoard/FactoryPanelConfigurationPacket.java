...
public class FactoryPanelConfigurationPacket
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, FactoryPanelConfigurationPacket>
            STREAM_CODEC = CatnipLargerStreamCodecs.composite(
                    FactoryPanelPosition.STREAM_CODEC,
                    packet -> packet.position,
                    ByteBufCodecs.STRING_UTF8,
                    packet -> packet.address,
                    ByteBufCodecs.map(
                            HashMap::new, FactoryPanelPosition.STREAM_CODEC, ByteBufCodecs.INT),
    ...
    protected void applySettings(ServerPlayer player, FactoryPanelBlockEntity be) {
        ...
        for (Entry<FactoryPanelPosition, Integer> entry : inputAmounts.entrySet()) {
            ...
            if (connection != null) connection.amount = entry.getValue();
        }

        if (removeConnection != null) {
            behaviour.targetedBy.remove(removeConnection);
            FactoryPanelBehaviour source =
                    FactoryPanelBehaviour.at(be.getLevel(), removeConnection);
            if (source != null) {
        ...
        if (clearPromises) behaviour.forceClearPromises = true;
