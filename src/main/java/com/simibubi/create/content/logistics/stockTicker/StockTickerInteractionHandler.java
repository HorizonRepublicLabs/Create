...
public class StockTickerInteractionHandler {
    ...
    @SubscribeEvent
    public static void interactWithLogisticsManager(
    ...
    public static boolean interactWithLogisticsManagerAt(
            Player player, Level level, BlockPos targetPos) {
        ...
        if (!(level.getBlockEntity(targetPos) instanceof StockTickerBlockEntity stbe)) return false;

        if (!stbe.behaviour.mayInteract(player)) {
            player.displayClientMessage(
                    CreateLang.translate("stock_keeper.locked")
        ...
        if (player instanceof ServerPlayer sp) {
            boolean showLockOption = stbe.behaviour.mayAdministrate(player)
                    && Create.LOGISTICS.isLockable(stbe.behaviour.freqId);
    ...
    private static void interactWithShop(
        ...
        for (int i = 0; i < tickerBE.receivedPayments.getSlots(); i++)
            if (tickerBE.receivedPayments.getStackInSlot(i).isEmpty()) occupiedSlots--;
        ...
        for (boolean simulate : Iterate.trueAndFalse) {
            ...
            for (int i = 0; i < player.getInventory().items.size(); i++) {
                ...
                player.getInventory()
                        .setItem(
                                ...
                                newStackSize == 0
                                        ? ItemStack.EMPTY
                                        : item.copyWithCount(newStackSize));
            ...
            toTransfer.forEach(
                    s -> ItemHandlerHelper.insertItemStacked(tickerBE.receivedPayments, s, false));
        }

        tickerBE.broadcastPackageRequest(
                RequestType.PLAYER, order, null, ShoppingListItem.getAddress(mainHandItem));
    ...
    public static BlockPos getStockTickerPosition(Entity entity) {
        ...
        if (!(rootVehicle instanceof SeatEntity)) return null;
        if (!(entity instanceof LivingEntity)) return null;
        if (AllEntityTypes.PACKAGE.is(entity)) return null;
        ...
        for (Direction d : Iterate.horizontalDirections) {
            for (int y : Iterate.zeroAndOne) {
                ...
                if (!(entity.level().getBlockState(workstationPos).getBlock()
                        instanceof StockTickerBlock)) continue;
                targetPos = workstationPos;
                stations++;
            }
        }

        if (stations != 1) return null;
        return targetPos;
    }
}
