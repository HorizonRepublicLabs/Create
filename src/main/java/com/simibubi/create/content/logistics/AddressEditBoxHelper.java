...
public class AddressEditBoxHelper {
    ...
    private static final Cache<BlockPos, WeakReference<ClipboardBlockEntity>> NEARBY_CLIPBOARDS =
    ...
    public static void advertiseClipboard(ClipboardBlockEntity blockEntity) {
        ...
        if (player.distanceToSqr(Vec3.atCenterOf(blockPos)) > 32 * 32) return;
        NEARBY_CLIPBOARDS.put(blockPos, new WeakReference<>(blockEntity));
    }

    public static DestinationSuggestions createSuggestions(
            Screen screen, EditBox pInput, boolean anchorToBottom, String localAddress) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        ...
        if (player == null) return destinationSuggestions;
    ...
    private static void appendAddresses(
        ...
        pages.forEach(page -> page.forEach(entry -> {
            ...
            if (!alreadyAdded.add(trim)) return;
