...
public class PackagerGenerator extends SpecialBlockStateGen {
    ...
    public <T extends Block> ModelFile getModel(
        ...
        String suffix = state.getOptionalValue(PackagerBlock.LINKED).orElse(false)
                ? "linked"
                : state.getValue(PackagerBlock.POWERED) ? "powered" : "";
        return state.getValue(PackagerBlock.FACING).getAxis() == Axis.Y
                ? AssetLookup.partialBaseModel(ctx, prov, "vertical", suffix)
