...
public class PackagePortTargetSelectionHandler {
    ...
    public static void flushSettings(BlockPos pos) {
        ...
        if (validateDiff(exactPositionOfTarget, pos) == null) {
            ...
            CatnipServices.NETWORK.sendToServer(
                    new PackagePortPlacementPacket(activePackageTarget, pos));
        }
    ...
    public static boolean onUse() {
        ...
        if (hitResult == null || hitResult.getType() == Type.MISS) return false;
        if (!(hitResult instanceof BlockHitResult bhr)) return false;

        BlockPos pos = bhr.getBlockPos();
        if (!(mc.level.getBlockEntity(pos) instanceof StationBlockEntity sbe)) return false;
        if (sbe.edgePoint == null) return false;
        if (!AllItemTags.POSTBOXES.matches(mainHandItem)) return false;
        ...
        PackagePortTargetSelectionHandler.activePackageTarget =
                new PackagePortTarget.TrainStationFrogportTarget(pos);
    ...
    public static void tick() {
        ...
        if (!isWrench) {
            ...
            if (!AllBlocks.PACKAGE_FROGPORT.isIn(player.getMainHandItem()) && !isPostbox) return;
        }
        ...
        if (isWrench) {
            ...
            if (!(mc.level.getBlockEntity(pos) instanceof PackagePortBlockEntity ppbe)) return;
        ...
        Outliner.getInstance()
                .chaseAABB(
                        "TargetedFrogPos", new AABB(pos).contract(0, 1, 0).deflate(0.125, 0, 0.125))
                .colored(color)
                .lineWidth(1 / 16f)
                .disableLineNormals();
    ...
    public static String validateDiff(Vec3 target, BlockPos placedPos) {
        ...
        if (diff.y < 0 && !isPostbox) return "package_port.cannot_reach_down";
