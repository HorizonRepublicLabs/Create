...
public record AutoRequestData(
    ...
    public static AutoRequestData readFromItem(
        ...
        mutable.isValid = mutable.targetOffset.closerThan(BlockPos.ZERO, 128)
                && requestData.targetDim.equals(level.dimension().location().toString());

        if (player != null)
            CreateLang.translate(
                            mutable.isValid
                                    ? "redstone_requester.keeper_connected"
