...
public record ShulkerFillLevelAttribute(ShulkerLevels levels) implements ItemAttribute {
    ...
    public static final StreamCodec<ByteBuf, ShulkerFillLevelAttribute> STREAM_CODEC =
            ShulkerLevels.STREAM_CODEC.map(
                    ShulkerFillLevelAttribute::new, ShulkerFillLevelAttribute::levels);
    ...
    public Object[] getTranslationParameters() {
        String parameter = "";
        if (levels != null)
            parameter = CreateLang.translateDirect(
                            "item_attributes." + getTranslationKey() + "." + levels.key)
                    .getString();
    ...
    enum ShulkerLevels implements StringRepresentable {
        ...
        public static final Codec<ShulkerLevels> CODEC =
                StringRepresentable.fromValues(ShulkerLevels::values);
        public static final StreamCodec<ByteBuf, ShulkerLevels> STREAM_CODEC =
                CatnipStreamCodecBuilders.ofEnum(ShulkerLevels.class);
        ...
        public static ShulkerFillLevelAttribute.ShulkerLevels fromKey(String key) {
            return Arrays.stream(values())
                    .filter(shulkerLevels -> shulkerLevels.key.equals(key))
                    .findFirst()
                    .orElse(null);
        }
        ...
        public boolean canApply(ItemStack testStack) {
            if (!isShulker(testStack)) return false;
            ItemContainerContents contents =
                    testStack.getOrDefault(DataComponents.CONTAINER, ItemContainerContents.EMPTY);
            if (contents == ItemContainerContents.EMPTY) return requiredSize.test(0);
            if (testStack.has(DataComponents.CONTAINER_LOOT)) return false;
            if (contents.getSlots() > 0) {
                int rawSize = contents.getSlots();
                if (rawSize < 27) return requiredSize.test(rawSize);

                NonNullList<ItemStack> inventory = NonNullList.withSize(27, ItemStack.EMPTY);
                contents.copyInto(inventory);
                boolean isFull = inventory.stream()
                        .allMatch(itemStack -> !itemStack.isEmpty()
