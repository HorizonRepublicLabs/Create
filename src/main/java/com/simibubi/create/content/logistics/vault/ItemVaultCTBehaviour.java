...
public class ItemVaultCTBehaviour extends ConnectedTextureBehaviour.Base {
    ...
    public CTSpriteShiftEntry getShift(
        ...
        if (direction.getAxis() == vaultBlockAxis) return AllSpriteShifts.VAULT_FRONT.get(small);
        if (direction == Direction.UP) return AllSpriteShifts.VAULT_TOP.get(small);
        if (direction == Direction.DOWN) return AllSpriteShifts.VAULT_BOTTOM.get(small);
    ...
    protected Direction getUpDirection(
        ...
        return Direction.fromAxisAndDirection(
                vaultBlockAxis, alongX ? AxisDirection.POSITIVE : AxisDirection.NEGATIVE);
    }
    ...
    public boolean connectsTo(
        ...
        return state == other
                && ConnectivityHandler.isConnected(
                        reader, pos,
                        otherPos); // ItemVaultConnectivityHandler.isConnected(reader, pos,
        // otherPos);
    }
}
