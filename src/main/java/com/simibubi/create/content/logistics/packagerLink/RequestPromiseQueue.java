...
public class RequestPromiseQueue {
    ...
    public void add(RequestPromise promise) {
        promisesByItem
                .computeIfAbsent(promise.promisedStack.stack.getItem(), $ -> new LinkedList<>())
                .add(promise);
        onChanged.run();
    }
    ...
    public int getTotalPromisedAndRemoveExpired(ItemStack stack, int expiryTime) {
        ...
        if (list == null) return promised;

        for (Iterator<RequestPromise> iterator = list.iterator(); iterator.hasNext(); ) {
            RequestPromise promise = iterator.next();
            if (!ItemStack.isSameItemSameComponents(promise.promisedStack.stack, stack)) continue;
    ...
    public void forceClear(ItemStack stack) {
        ...
        if (list.isEmpty()) promisesByItem.remove(stack.getItem());
    }
    ...
    public List<RequestPromise> flatten(boolean sorted) {
        ...
        if (sorted) all.sort(RequestPromise.ageComparator());
        return all;
    }
    ...
    public CompoundTag write(HolderLookup.Provider registries) {
        ...
        tag.put(
                ...
                CatnipCodecUtils.encode(
                                Codec.list(RequestPromise.CODEC), registries, flatten(false))
                        .orElseThrow());
        return tag;
    }

    public static RequestPromiseQueue read(
            CompoundTag tag, HolderLookup.Provider registries, Runnable onChanged) {
        RequestPromiseQueue queue = new RequestPromiseQueue(onChanged);
        List<RequestPromise> promises = CatnipCodecUtils.decode(
                        Codec.list(RequestPromise.CODEC), registries, tag.get("List"))
                .orElse(List.of());
    ...
    public void tick() {
        promisesByItem.forEach(
                (key, list) -> list.forEach(RequestPromise::tick)); // delete old entries?
    }
