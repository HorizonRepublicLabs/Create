...
public class TableClothOverlayRenderer {
    ...
    public static void tick() {
        ...
        if (mouseOver.getType() != Type.ENTITY) {
            if (!(mouseOver instanceof BlockHitResult bhr)) return;
            if (!(mc.level.getBlockEntity(bhr.getBlockPos()) instanceof TableClothBlockEntity dcbe))
                return;
            if (!dcbe.isShop()) return;
            if (AllBlocks.CLIPBOARD.isIn(heldItem)) return;
            if (dcbe.targetsPriceTag(mc.player, bhr)) return;

            int alreadyPurchased = 0;
            ShoppingList list = ShoppingListItem.getList(heldItem);
            if (list != null) alreadyPurchased = list.getPurchases(dcbe.getBlockPos());
        ...
        if (!AllItems.SHOPPING_LIST.isIn(heldItem)) return;

        ShoppingList list = ShoppingListItem.getList(heldItem);
        BlockPos stockTickerPosition =
                StockTickerInteractionHandler.getStockTickerPosition(entityRay.getEntity());

        if (list == null || stockTickerPosition == null) return;
        if (!(mc.level.getBlockEntity(stockTickerPosition)
                instanceof StockTickerBlockEntity tickerBE)) return;
        if (!tickerBE.behaviour.freqId.equals(list.shopNetwork())) return;
