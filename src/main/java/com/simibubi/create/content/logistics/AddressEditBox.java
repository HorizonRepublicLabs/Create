...
public class AddressEditBox extends EditBox {
    ...
    public AddressEditBox(
        ...
        destinationSuggestions =
                AddressEditBoxHelper.createSuggestions(screen, this, anchorToBottom, localAddress);
        destinationSuggestions.setAllowSuggestions(true);
        destinationSuggestions.updateCommandInfo();
        mainResponder = t -> {
            if (!t.equals(prevValue)) destinationSuggestions.updateCommandInfo();
            prevValue = t;
        };
    ...
    public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
        if (destinationSuggestions.keyPressed(pKeyCode, pScanCode, pModifiers)) return true;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (destinationSuggestions.mouseScrolled(Mth.clamp(scrollY, -1.0D, 1.0D))) return true;
    ...
    public void renderWidget(
        ...
        if (!destinationSuggestions.isEmpty()) return;
        ...
        if (pMouseX >= itemX && pMouseX < itemX + 16 && pMouseY >= itemY && pMouseY < itemY + 16) {
            ...
            promiseTip = List.of(
                    CreateLang.translate("gui.address_box.clipboard_tip")
            ...
            pGuiGraphics.renderComponentTooltip(
                    Minecraft.getInstance().font, promiseTip, pMouseX, pMouseY);
        }
    }
    ...
    public void setResponder(Consumer<String> pResponder) {
        super.setResponder(
                pResponder == mainResponder ? mainResponder : mainResponder.andThen(pResponder));
    }

    public void tick() {
        if (!isFocused()) destinationSuggestions.hide();
