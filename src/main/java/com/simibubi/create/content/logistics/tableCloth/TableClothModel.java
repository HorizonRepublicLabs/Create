...
public class TableClothModel extends BakedModelWrapperWithData {
    ...
    private List<BakedQuad> getCorner(
            TableClothBlock block,
            int corner,
        ...
        if (!CORNERS.containsKey(block)) {
            ...
            for (PartialModel pm : List.of(
                    AllPartialModels.TABLE_CLOTH_SW,
                    AllPartialModels.TABLE_CLOTH_NW,
                    AllPartialModels.TABLE_CLOTH_NE,
    ...
    private List<BakedQuad> getCornerQuads(
            RandomSource rand,
        ...
        for (BakedQuad quad : pm.get().getQuads(null, null, rand, ModelData.EMPTY, renderType)) {
            ...
            for (int vertex = 0; vertex < 4; vertex++) {
                BakedQuadHelper.setU(
                        vertexData,
                        vertex,
                        targetSprite.getU(SpriteShiftEntry.getUnInterpolatedU(
                                original, BakedQuadHelper.getU(vertexData, vertex))));
                BakedQuadHelper.setV(
                        vertexData,
                        vertex,
                        targetSprite.getV(SpriteShiftEntry.getUnInterpolatedV(
                                original, BakedQuadHelper.getV(vertexData, vertex))));
            }
            quads.add(newQuad);
        }
    ...
    @Override
    protected Builder gatherModelData(
            Builder builder,
        ...
        if (culledSides.isEmpty()) return builder;
    ...
    public @NotNull List<BakedQuad> getQuads(
        ...
        if (side == null || side.getAxis() == Axis.Y) return mainQuads;

        CullData cullData = extraData.get(CULL_PROPERTY);
        if (cullData != null && cullData.culled().contains(side.getClockWise())) return mainQuads;
        if (state == null || !(state.getBlock() instanceof TableClothBlock dcb)) return mainQuads;
