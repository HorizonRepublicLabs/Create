...
public class EntityLauncher {
    ...
    public void clamp(int max) {
        set(
                Math.min(horizontalDistance, max),
    ...
    public Vec3 getGlobalVelocity(double t, Direction d, BlockPos launcher) {
        return VecHelper.rotate(
                new Vec3(0, dy(t), dx(t)), 180 + AngleHelper.horizontalAngle(d), Axis.Y);
    }
    ...
    public float y(double t) {
        ...
        return (float)
                (yMotion * -49.4983 * f + 49.4983 * yMotion - 194.033 * f - 3.92 * t + 194.033);
    }
    ...
    protected void recalculateTrajectory() {
        ...
        double xError = (-0.0007 * Math.pow(xTarget + .5, 2) + 0.484)
                - (Math.min(5, yTarget) / 5)
                        * Math.min(1, 0.076 * xTarget - 0.0014 * xTarget * xTarget);

        double yPeak = Math.max(0, yTarget + (xTarget + .5) / 8f) + (xTarget <= 1 ? 1 : 4);
        yMotion = Math.sqrt(2 * yPeak / 13) + 0.015 * yPeak;
        double tPeak = Math.log(98 / (25 * yMotion + 98))
                / (2 * Math.log(7) - 2 * Math.log(5) - Math.log(2));
