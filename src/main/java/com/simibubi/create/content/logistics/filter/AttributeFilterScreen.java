...
public class AttributeFilterScreen extends AbstractFilterScreen<AttributeFilterMenu> {
    ...
    private final Component addInvertedDESC =
            CreateLang.translateDirect(PREFIX + "add_inverted_attribute");

    private final Component allowDisN =
            CreateLang.translateDirect(PREFIX + "allow_list_disjunctive");
    private final Component allowDisDESC =
            CreateLang.translateDirect(PREFIX + "allow_list_disjunctive.description");
    private final Component allowConN =
            CreateLang.translateDirect(PREFIX + "allow_list_conjunctive");
    private final Component allowConDESC =
            CreateLang.translateDirect(PREFIX + "allow_list_conjunctive.description");
    ...
    private final Component noSelectedT =
            CreateLang.translateDirect(PREFIX + "no_selected_attributes");
    ...
    protected void init() {
        ...
        attributeSelectorLabel = new Label(x + 43, y + 31, CommonComponents.EMPTY)
                .colored(0xF3EBDE)
                .withShadow();
        ...
        selectedAttributes.add((menu.selectedAttributes.isEmpty() ? noSelectedT : selectedT)
                .plainCopy()
    ...
    private void referenceItemChanged(ItemStack stack) {
        ...
        List<Component> options =
                attributesOfItem.stream().map(a -> a.format(false)).collect(Collectors.toList());
    ...
    protected void containerTick() {
        ...
        if (!ItemStack.matches(stackInSlot, lastItemScanned)) referenceItemChanged(stackInSlot);
    }
    ...
    protected List<MutableComponent> getTooltipDescriptions() {
        return Arrays.asList(
                denyDESC.plainCopy(), allowConDESC.plainCopy(), allowDisDESC.plainCopy());
    }

    protected boolean handleAddedAttibute(boolean inverted) {
        int index = attributeSelector.getState();
        if (index >= attributesOfItem.size()) return false;
        add.active = false;
        addInverted.active = false;
        ItemAttribute itemAttribute = attributesOfItem.get(index);
        CompoundTag tag = ItemAttribute.saveStatic(
                itemAttribute, Minecraft.getInstance().level.registryAccess());
