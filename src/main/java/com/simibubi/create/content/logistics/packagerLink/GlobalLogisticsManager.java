...
public class GlobalLogisticsManager {
    ...
    public boolean mayInteract(UUID networkId, Player player) {
        ...
        return network == null
                || network.owner == null
                || !network.locked
                || network.owner.equals(player.getUUID());
    }
    ...
    public void linkAdded(UUID networkId, GlobalPos pos, UUID ownedBy) {
        LogisticsNetwork network =
                logisticsNetworks.computeIfAbsent(networkId, $ -> new LogisticsNetwork(networkId));
        network.totalLinks.add(pos);
        if (ownedBy != null && network.owner == null) network.owner = ownedBy;
        markDirty();
    }

    public void linkLoaded(UUID networkId, GlobalPos pos) {
        logisticsNetworks
                .computeIfAbsent(networkId, $ -> new LogisticsNetwork(networkId))
                .loadedLinks
                .add(pos);
    }
    ...
    public void linkRemoved(UUID networkId, GlobalPos pos) {
        ...
        if (logisticsNetwork.totalLinks.size() <= 0) logisticsNetworks.remove(networkId);
        markDirty();
    }
    ...
    public int getUnloadedLinkCount(UUID networkId) {
        LogisticsNetwork logisticsNetwork = logisticsNetworks.get(networkId);
        if (logisticsNetwork == null) return 0;
    ...
    public RequestPromiseQueue getQueuedPromises(UUID networkId) {
        return !logisticsNetworks.containsKey(networkId)
                ? null
                : logisticsNetworks.get(networkId).panelPromises;
    }

    public boolean hasQueuedPromises(UUID networkId) {
        return logisticsNetworks.containsKey(networkId)
                && !logisticsNetworks.get(networkId).panelPromises.isEmpty();
    }
