    ...
    protected void renderSafe(
        ...
        if (!VisualizationManager.supportsVisualization(be.getLevel())) {
            SuperByteBuffer model =
                    CachedBuffers.partial(AllPartialModels.EJECTOR_TOP, be.getBlockState());
        ...
        float maxTime = (float)
                (be.earlyTarget != null ? be.earlyTargetTime : be.launcher.getTotalFlyingTicks());
        ...
        if (behaviour == null || behaviour.isEmpty()) return;
        ...
        msr.center()
                .rotateYDegrees(-180
                        - AngleHelper.horizontalAngle(
                                be.getBlockState().getValue(EjectorBlock.HORIZONTAL_FACING)))
                .uncenter();
        DepotRenderer.renderItemsOf(be, partialTicks, ms, buffer, light, overlay, behaviour);
        ms.popPose();
    }

    static <T extends Translate<T> & Rotate<T>> void applyLidAngle(
            KineticBlockEntity be, float angle, T tr) {
        applyLidAngle(be, pivot, angle, tr);
    }

    static <T extends Translate<T> & Rotate<T>> void applyLidAngle(
            KineticBlockEntity be, Vec3 rotationOffset, float angle, T tr) {
        tr.center()
                .rotateYDegrees(180
                        + AngleHelper.horizontalAngle(
                                be.getBlockState().getValue(EjectorBlock.HORIZONTAL_FACING)))
