...
public class SharedDepotBlockMethods {
    ...
    public static ItemInteractionResult onUse(
            ItemStack stack,
            BlockState state,
            Level level,
            BlockPos pos,
        ...
        if (behaviour == null) return ItemInteractionResult.PASS_TO_DEFAULT_BLOCK_INTERACTION;
    ...
    public static void onLanded(BlockGetter worldIn, Entity entityIn) {
        ...
        if (entityIn instanceof ItemEntity) ((ItemEntity) entityIn).setItem(remainder);
        if (remainder.isEmpty()) entityIn.discard();
    }

    public static int getComparatorInputOverride(
            BlockState blockState, Level worldIn, BlockPos pos) {
