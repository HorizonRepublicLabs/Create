...
public class PackagePortScreen extends AbstractSimiContainerScreen<PackagePortMenu> {
    ...
    protected void init() {
        setWindowSize(
                background.getWidth(),
                background.getHeight() + AllGuiTextures.PLAYER_INVENTORY.getHeight());
        super.init();
        clearWidgets();
        ...
        acceptPackages = new IconButton(
                x + 37, y + background.getHeight() - 24, AllIcons.I_SEND_AND_RECEIVE);
        ...
        dontAcceptPackages =
                new IconButton(x + 37 + 18, y + background.getHeight() - 24, AllIcons.I_SEND_ONLY);
    ...
    protected void renderBg(GuiGraphics graphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        AllGuiTextures header =
                frogMode ? AllGuiTextures.FROGPORT_HEADER : AllGuiTextures.POSTBOX_HEADER;
        ...
        if (!addressBox.isFocused()) {
            if (addressBox.getValue().isEmpty()) {
                ...
                graphics.drawString(
                        font, text, nameBoxX(text, addressBox), y - 11, 0x3D3C48, false);
            }
            AllGuiTextures.FROGPORT_EDIT_NAME.render(
                    graphics, nameBoxX(text, addressBox) + font.width(text) + 5, y - 14);
        }
        ...
        if (menu.contentHolder.target == null) return;
    ...
    public void removed() {
        CatnipServices.NETWORK.sendToServer(new PackagePortConfigurationPacket(
                menu.contentHolder.getBlockPos(), addressBox.getValue(), acceptPackages.green));
        super.removed();
    }
