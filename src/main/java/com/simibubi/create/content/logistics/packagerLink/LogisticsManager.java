...
public class LogisticsManager {
    ...
    public static final Cache<UUID, InventorySummary> ACCURATE_SUMMARIES =
            new TickBasedCache<>(1, false);
    ...
    public static Multimap<PackagerBlockEntity, PackagingRequest> findPackagersForRequest(
        ...
        for (LogisticallyLinkedBehaviour link : allAvailableLinks) {
            ...
            if (inventoryId != null) {
                linksByInventory
                        .computeIfAbsent(inventoryId, k -> new ArrayList<>())
                        .add(link);
            } else {
        ...
        for (int i = 0; i < stacks.size(); i++) {
            ...
            for (LogisticallyLinkedBehaviour link : availableLinks) {
                ...
                if (finalEntry) finalLinkTracker.setTrue();
                break;
            }
        }
        return requests;
    ...
    public static void performPackageRequests(
        ...
        for (Entry<PackagerBlockEntity, Collection<PackagingRequest>> entry : asMap.entrySet()) {
            ...
            if (!queuedRequests.isEmpty()) packager.flashLink();
            for (int i = 0; i < 100; i++) {
                if (queuedRequests.isEmpty()) break;
