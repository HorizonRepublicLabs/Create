...
public class RedstoneRequesterScreen extends AbstractSimiContainerScreen<RedstoneRequesterMenu> {
    ...
    public RedstoneRequesterScreen(
            RedstoneRequesterMenu container, Inventory inv, Component title) {
        super(container, inv, title);

        for (int i = 0; i < 9; i++) amounts.add(1);

        List<BigItemStack> stacks = menu.contentHolder.encodedRequest.stacks();
        for (int i = 0; i < stacks.size(); i++) amounts.set(i, Math.max(1, stacks.get(i).count));
    }
    ...
    protected void containerTick() {
        ...
        for (int i = 0; i < amounts.size(); i++)
            if (menu.ghostInventory.getStackInSlot(i).isEmpty()) amounts.set(i, 1);
    }
    ...
    protected void init() {
        ...
        if (addressBox == null) {
            addressBox = new AddressEditBox(
                    this, new NoShadowFontWrapper(font), x + 55, y + 68, 110, 10, false);
        ...
        allowPartial.setToolTip(
                CreateLang.translate("gui.redstone_requester.allow_partial").component());
        ...
        dontAllowPartial.setToolTip(
                CreateLang.translate("gui.redstone_requester.dont_allow_partial")
                        .component());
    ...
    protected void renderBg(
            GuiGraphics pGuiGraphics, float pPartialTick, int pMouseX, int pMouseY) {
        int x = getGuiLeft();
        int y = getGuiTop();
        ...
        pGuiGraphics.drawString(
                font, title, x + 117 - font.width(title) / 2, y + 4, 0x3D3C48, false);

        GuiGameElement.of(stack).scale(3).render(pGuiGraphics, x + 245, y + 80);
    }
        ...
        if (addressBox.isHovered() && !addressBox.isFocused()) {
            if (addressBox.getValue().isBlank())
                graphics.renderComponentTooltip(
                        ...
                        List.of(
                                ...
                                CreateLang.translate(
                                                "gui.redstone_requester.requester_address_tip_1")
                                        .style(ChatFormatting.GRAY)
                                        .component(),
                ...
                graphics.renderComponentTooltip(
                        ...
                        List.of(
                                CreateLang.translate(
                                                "gui.redstone_requester.requester_address_given")
                                        .color(ScrollInput.HEADER_RGB)
                                        .component(),
        ...
        for (int i = 0; i < amounts.size(); i++) {
            ...
            if (mouseX >= inputX
                ...
                amounts.set(
                        ...
                        Mth.clamp(
                                (int) (amounts.get(i)
                                        + Math.signum(scrollY) * (hasShiftDown() ? 10 : 1)),
                                1,
                                256));
                return true;
            }
    ...
    protected List<Component> getTooltipFromContainerItem(ItemStack pStack) {
        ...
        if (slotIndex >= amounts.size()) return tooltip;

        return List.of(
                CreateLang.translate(
                                "gui.factory_panel.send_item",
                                CreateLang.itemName(pStack)
