...
public class FunnelFilterSlotPositioning extends ValueBoxTransform.Sided {
            ...
            switch (state.getValue(BeltFunnelBlock.SHAPE)) {
                case EXTENDED:
                    return VecHelper.rotateCentered(
                            VecHelper.voxelSpace(8, 15.5f, 13), stateAngle, Axis.Y);
                case PULLING:
                case PUSHING:
                    return VecHelper.rotateCentered(
                            VecHelper.voxelSpace(8, 12.0f, 8.675f), horizontalAngle, Axis.Y);
                default:
                case RETRACTED:
                    return VecHelper.rotateCentered(
                            VecHelper.voxelSpace(8, 13, 7.5f), horizontalAngle, Axis.Y);
            }
        }

        if (!funnelFacing.getAxis().isHorizontal()) {
            Vec3 southLocation =
                    VecHelper.voxelSpace(8, funnelFacing == Direction.DOWN ? 14 : 2, 15.5f);
            return VecHelper.rotateCentered(southLocation, horizontalAngle, Axis.Y);
        }

        return VecHelper.rotateCentered(
                VecHelper.voxelSpace(8, 12.2, 8.55f), horizontalAngle, Axis.Y);
    }
    ...
    protected boolean isSideActive(BlockState state, Direction direction) {
        ...
        if (state.getBlock() instanceof BeltFunnelBlock
                && state.getValue(BeltFunnelBlock.SHAPE) == Shape.EXTENDED)
