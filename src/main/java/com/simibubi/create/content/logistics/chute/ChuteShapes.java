...
public class ChuteShapes {
    ...
    public static VoxelShape createShape(BlockState state) {
        if (AllBlocks.SMART_CHUTE.has(state)) return Shapes.block();
        ...
        if (direction == Direction.DOWN) return intersection ? Shapes.block() : AllShapes.CHUTE;
        ...
        if (intersection) result = Shapes.joinUnoptimized(INTERSECTION_MASK, result, BooleanOp.AND);
        return result;
    }
    ...
    public static VoxelShape getCollisionShape(BlockState state) {
        if (collisionCache.containsKey(state)) return collisionCache.get(state);
        VoxelShape createdShape =
                Shapes.joinUnoptimized(COLLISION_MASK, getShape(state), BooleanOp.AND);
