...
public class FactoryPanelModel extends BakedModelWrapperWithData {
    ...
    private static final ModelProperty<FactoryPanelModelData> PANEL_PROPERTY =
            new ModelProperty<>();
    ...
    protected ModelData.Builder gatherModelData(
        ...
        for (PanelSlot slot : PanelSlot.values()) {
            FactoryPanelBehaviour behaviour =
                    FactoryPanelBehaviour.at(world, new FactoryPanelPosition(pos, slot));
    ...
    public List<BakedQuad> getQuads(
        ...
        if (side != null || !data.has(PANEL_PROPERTY)) return Collections.emptyList();
        FactoryPanelModelData modelData = data.get(PANEL_PROPERTY);
        List<BakedQuad> quads =
                new ArrayList<>(super.getQuads(state, null, rand, data, renderType));
        for (PanelSlot panelSlot : PanelSlot.values())
            if (modelData.states.containsKey(panelSlot))
                addPanel(
                        ...
                        panelSlot,
                        modelData.type,
                        modelData.states.get(panelSlot),
                        rand,
                        data,
                        renderType,
                        modelData.ponder);
    ...
    public void addPanel(
        ...
        PartialModel factoryPanel = panelState == PanelState.PASSIVE
                ? type == PanelType.NETWORK
                        ...
                        : AllPartialModels.FACTORY_PANEL_RESTOCKER
                : type == PanelType.NETWORK
                        ? AllPartialModels.FACTORY_PANEL_WITH_BULB
                        : AllPartialModels.FACTORY_PANEL_RESTOCKER_WITH_BULB;

        List<BakedQuad> quadsToAdd =
                factoryPanel.get().getQuads(state, null, rand, data, RenderType.solid());
        ...
        for (BakedQuad bakedQuad : quadsToAdd) {
            ...
            Direction newNormal = Direction.fromDelta(
                    (int) Math.round(quadNormal.x), (int) Math.round(quadNormal.y), (int)
                            Math.round(quadNormal.z));
