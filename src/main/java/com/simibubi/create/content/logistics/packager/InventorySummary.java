...
public class InventorySummary {
    ...
    public void addAllItemStacks(List<ItemStack> list) {
        for (ItemStack stack : list) add(stack, stack.getCount());
    }

    public void addAllBigItemStacks(List<BigItemStack> list) {
        for (BigItemStack entry : list) add(entry.stack, entry.count);
    }

    public InventorySummary copy() {
        InventorySummary inventorySummary = new InventorySummary();
        items.forEach(
                (i, list) -> list.forEach(entry -> inventorySummary.add(entry.stack, entry.count)));
    ...
    public void add(ItemStack stack, int count) {
        ...
        if (totalCount < BigItemStack.INF) totalCount += count;

        List<BigItemStack> stacks =
                items.computeIfAbsent(stack.getItem(), $ -> Lists.newArrayList());
        for (BigItemStack existing : stacks) {
            ...
            if (ItemStack.isSameItemSameComponents(existingStack, stack)) {
                if (existing.count < BigItemStack.INF) existing.count += count;
                return;
            }
        }

        if (stack.getCount() > stack.getMaxStackSize()) stack = stack.copyWithCount(1);
    ...
    public boolean erase(ItemStack stack) {
        ...
        for (Iterator<BigItemStack> iterator = stacks.iterator(); iterator.hasNext(); ) {
            ...
            ItemStack existingStack = existing.stack;
            if (!ItemStack.isSameItemSameComponents(existingStack, stack)) continue;
    ...
    public int getCountOf(ItemStack stack) {
        ...
        for (BigItemStack entry : list)
            if (ItemStack.isSameItemSameComponents(entry.stack, stack)) return entry.count;
        return 0;
    }
    ...
    public int getTotalOfMatching(Predicate<ItemStack> filter) {
        ...
        items.forEach(($, list) -> {
            for (BigItemStack entry : list) if (filter.test(entry.stack)) sum.add(entry.count);
        });
        return sum.getValue();
    }
    ...
    public void divideAndSendTo(ServerPlayer player, BlockPos pos) {
        ...
        if (stacks.isEmpty())
            CatnipServices.NETWORK.sendToClient(
                    player, new LogisticalStockResponsePacket(true, pos, Collections.emptyList()));

        for (BigItemStack entry : stacks) {
            if (currentList == null) currentList = new ArrayList<>(Math.min(100, remaining));
            ...
            CatnipServices.NETWORK.sendToClient(
                    player, new LogisticalStockResponsePacket(false, pos, currentList));
            currentList = null;
        }

        if (currentList != null)
            CatnipServices.NETWORK.sendToClient(
                    player, new LogisticalStockResponsePacket(true, pos, currentList));
    }
