...
public class XaeroTrainMap {
    ...
    public static void tick() {
        if (!AllConfigs.client().showTrainMapOverlay.get()
                || !isMapOpen(Minecraft.getInstance().screen)) {
    ...
    public static void mouseClick(InputEvent.MouseButton.Pre event) {
        ...
        try {
            if (!(mc.screen instanceof GuiMap)) return;
        } catch (Exception e) {
            Create.LOGGER.error(
    ...
    public static void onRender(GuiGraphics graphics, GuiMap screen, int mX, int mY, float pt) {
        ...
        renderedDimension = ((XaeroFullscreenMapAccessor) screen)
                .getMapProcessor()
                .getMapWorld()
                .getCurrentDimension()
                .getDimId();
        ...
        List<FormattedText> tooltip = TrainMapManager.renderAndPick(
        ...
        if (!renderToggleWidgetAndTooltip(graphics, screen, mX, mY) && tooltip != null)
            RemovedGuiUtils.drawHoveringText(
                    graphics, tooltip, mX, mY, screen.width, screen.height, 256, mc.font);
    }

    private static boolean renderToggleWidgetAndTooltip(
            GuiGraphics graphics, GuiMap screen, int mouseX, int mouseY) {
