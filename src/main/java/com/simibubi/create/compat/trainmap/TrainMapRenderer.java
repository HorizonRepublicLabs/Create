...
public class TrainMapRenderer implements AutoCloseable {
    ...
    private final Object2ObjectMap<Couple<Integer>, TrainMapInstance> maps =
    ...
    public void startDrawing() {
        ...
        maps.values().forEach(tmi -> {
            tmi.getImage().fillRect(0, 0, WIDTH, HEIGHT, 0);
            tmi.untouched = true;
        });
    }
    ...
    public int getPixel(int xCoord, int zCoord) {
        ...
        if (!maps.containsKey(sectionKey)) return 0;
    ...
    public void blendPixel(int xCoord, int zCoord, int color, int alpha) {
        ...
        xCoord = Mth.positiveModulo(xCoord, WIDTH);
        zCoord = Mth.positiveModulo(zCoord, HEIGHT);
        instance.getImage().blendPixel(xCoord, zCoord, FastColor.ABGR32.color(alpha, color));
    }

    public void blendPixels(
            int xCoordFrom, int zCoordFrom, int xCoordTo, int zCoordTo, int color, int alpha) {
        for (int x = Math.min(xCoordFrom, xCoordTo); x <= Math.max(xCoordFrom, xCoordTo); x++)
            for (int z = Math.min(zCoordFrom, zCoordTo); z <= Math.max(zCoordFrom, zCoordTo); z++)
    ...
    public void render(GuiGraphics graphics, boolean linearFiltering, Rect2i bounds) {
        ...
        maps.forEach((key, tmi) -> {
            if (tmi.canBeSkipped(bounds)) return;
            int x = key.getFirst();
            int y = key.getSecond();
            pose.pushPose();
    ...
    public TrainMapInstance getOrCreateAt(int xCoord, int zCoord) {
        ...
        return maps.compute(
                sectionKey,
                (key, instance) -> instance == null ? new TrainMapInstance(key) : instance);
    }
    ...
    public void resetData() {
        for (TrainMapInstance instance : maps.values()) instance.close();
        maps.clear();
    }
    ...
    public class TrainMapInstance implements AutoCloseable {
        ...
        private final Couple<Integer> sectionKey;
        ...
        public TrainMapInstance(Couple<Integer> sectionKey) {
            ...
            location = textureManager.register(
                    "create_trainmap/" + sectionKey.getFirst() + "_" + sectionKey.getSecond(),
                    texture);
            renderType = RenderTypes.TRAIN_MAP.apply(location, linearFiltering);
            bounds = new Rect2i(
                    sectionKey.getFirst() * WIDTH, sectionKey.getSecond() * HEIGHT, WIDTH, HEIGHT);
        }
        ...
        public void draw(
                PoseStack pPoseStack, MultiBufferSource pBufferSource, boolean linearFiltering) {
            ...
            vertexconsumer
                    .addVertex(matrix4f, 0.0F, HEIGHT, 0)
                    .setColor(255, 255, 255, 255)
                    .setUv(0.0F, 1.0F)
                    .setLight(pPackedLight);
            vertexconsumer
                    .addVertex(matrix4f, WIDTH, HEIGHT, 0)
                    .setColor(255, 255, 255, 255)
                    .setUv(1.0F, 1.0F)
                    .setLight(pPackedLight);
            vertexconsumer
                    .addVertex(matrix4f, WIDTH, 0.0F, 0)
                    .setColor(255, 255, 255, 255)
                    .setUv(1.0F, 0.0F)
                    .setLight(pPackedLight);
            vertexconsumer
                    .addVertex(matrix4f, 0.0F, 0.0F, 0)
                    .setColor(255, 255, 255, 255)
                    .setUv(0.0F, 0.0F)
                    .setLight(pPackedLight);
        }
