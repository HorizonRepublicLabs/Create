...
public class TrainMapSync {
    ...
    public enum TrainState {
        ...
        public static final StreamCodec<ByteBuf, TrainState> STREAM_CODEC =
                CatnipStreamCodecBuilders.ofEnum(TrainState.class);
    }
    ...
    public enum SignalState {
        ...
        public static final StreamCodec<ByteBuf, SignalState> STREAM_CODEC =
                CatnipStreamCodecBuilders.ofEnum(SignalState.class);
    }

    public static class TrainMapSyncEntry {
        public static final StreamCodec<FriendlyByteBuf, TrainMapSyncEntry> STREAM_CODEC =
                CatnipLargerStreamCodecs.composite(
                        CatnipStreamCodecBuilders.array(ByteBufCodecs.FLOAT, Float.class),
                        packet -> packet.positions,
                        CatnipStreamCodecBuilders.list(CatnipStreamCodecBuilders.nullable(
                                ResourceKey.streamCodec(Registries.DIMENSION))),
                        packet -> packet.dimensions,
                        TrainState.STREAM_CODEC,
                        packet -> packet.state,
                        SignalState.STREAM_CODEC,
        ...
        public Vec3 getPosition(int carriageIndex, boolean firstBogey, double time) {
            ...
            if (positions == null || positions.length <= startIndex + 2) return Vec3.ZERO;
            Vec3 position = new Vec3(
                    positions[startIndex], positions[startIndex + 1], positions[startIndex + 2]);
            if (prevPositions == null || prevPositions.length <= startIndex + 2) return position;
            Vec3 prevPosition = new Vec3(
                    prevPositions[startIndex],
                    prevPositions[startIndex + 1],
                    prevPositions[startIndex + 2]);
            return prevPosition.lerp(position, time);
        }
    }

    public static Cache<UUID, WeakReference<ServerPlayer>> requestingPlayers =
            new TickBasedCache<>(20, false);
    ...
    public static void requestReceived(ServerPlayer sender) {
        ...
        if (sendImmediately) send(sender.server, false);
    }

    public static void serverTick(ServerTickEvent event) {
        ticks++;
        if (ticks % fullPacketInterval == 0) send(event.getServer(), false);
        else if (ticks % lightPacketInterval == 0) send(event.getServer(), true);
    }

    public static void send(MinecraftServer minecraftServer, boolean light) {
        if (requestingPlayers.size() == 0) return;

        TrainMapSyncPacket packet = new TrainMapSyncPacket(light);
        for (Train train : Create.RAILWAYS.trains.values())
            packet.add(train.id, createEntry(minecraftServer, train));

        for (WeakReference<ServerPlayer> weakReference :
                requestingPlayers.asMap().values()) {
    ...
    private static TrainMapSyncEntry createEntry(MinecraftServer minecraftServer, Train train) {
        ...
        for (int i = 0; i < carriages.size(); i++) {
            ...
            if (train.graph == null) {
                ...
                ResourceKey<Level> leadingDim = (leading.node1 == null
                                || leading.edge == null
                                || leading.edge.isInterDimensional())
                        ? null
                        : leading.node1.getLocation().getDimension();

                ResourceKey<Level> trailingDim = (trailing.node1 == null
                                || trailing.edge == null
                                || trailing.edge.isInterDimensional())
                        ? null
                        : trailing.node1.getLocation().getDimension();

                ResourceKey<Level> carriageDim =
                        (leadingDim == null || leadingDim != trailingDim) ? null : leadingDim;
        ...
        if (train.owner != null) {
            ...
            if (owner != null) entry.ownerName = owner.getName().getString();
        }
        ...
        if (currentStation != null) {
            ...
            entry.targetStationDistance =
                    Math.max(0, Mth.floor(train.navigation.distanceToDestination));
        }
        ...
        if (stopped && train.navigation.waitingForSignal != null) {
            ...
            if (signal != null) {
                ...
                entry.signalState =
                        chainSignal ? SignalState.CHAIN_SIGNAL : SignalState.BLOCK_SIGNAL;
                if (signal.isForcedRed(side)) entry.signalState = SignalState.WAITING_FOR_REDSTONE;
                else {
                    SignalEdgeGroup group =
                            Create.RAILWAYS.signalEdgeGroups.get(signal.groups.get(side));
                    if (group != null) {
        ...
        if (train.fuelTicks > 0 && !stopped) entry.fueled = true;
