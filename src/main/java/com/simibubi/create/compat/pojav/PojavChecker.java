...
public class PojavChecker {
    ...
    public static final boolean IS_PRESENT = Util.make(() -> {
        if (System.getenv("POJAV_RENDERER") != null) {
            LOGGER.warn("[Create]: Detected presence of environment variable POJAV_LAUNCHER, which"
                    + " seems to indicate we are running on Android");
            return true;
        }
        ...
        if (librarySearchPaths != null) {
            for (String path : librarySearchPaths.split(":")) {
                if (isKnownAndroidPathFragment(path)) {
                    LOGGER.warn(
                            "[Create]: Found a library search path which seems to be hosted in an"
                                    + " Android filesystem: {}",
                            path);
                    return true;
                }
        ...
        if (workingDirectory != null) {
            if (isKnownAndroidPathFragment(workingDirectory)) {
                LOGGER.warn(
                        "[Create]: Working directory seems to be hosted in an Android filesystem:"
                                + " {}",
                        workingDirectory);
