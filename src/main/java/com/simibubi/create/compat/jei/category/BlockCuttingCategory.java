...
public class BlockCuttingCategory extends CreateRecipeCategory<CondensedBlockCuttingRecipe> {
    ...
    public void setRecipe(
            IRecipeLayoutBuilder builder, CondensedBlockCuttingRecipe recipe, IFocusGroup focuses) {
    ...
    @Override
    public void draw(
            CondensedBlockCuttingRecipe recipe,
    ...
    public static class CondensedBlockCuttingRecipe extends StonecutterRecipe {
        ...
        public List<List<ItemStack>> getCondensedOutputs() {
            ...
            for (ItemStack itemStack : outputs) {
                if (firstPass) result.add(new ArrayList<>());
    ...
    public static List<RecipeHolder<CondensedBlockCuttingRecipe>> condenseRecipes(
            List<RecipeHolder<?>> stoneCuttingRecipes) {
        List<RecipeHolder<CondensedBlockCuttingRecipe>> condensed = new ArrayList<>();
        Recipes:
        for (RecipeHolder<?> recipe : stoneCuttingRecipes) {
            Ingredient i1 = recipe.value().getIngredients().get(0);
            for (RecipeHolder<CondensedBlockCuttingRecipe> condensedRecipe : condensed) {
                if (ItemHelper.matchIngredients(
                        i1, condensedRecipe.value().getIngredients().get(0))) {
