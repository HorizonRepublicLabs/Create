...
public class ItemDrainCategory extends CreateRecipeCategory<EmptyingRecipe> {
    ...
    public static void consumeRecipes(
            Consumer<RecipeHolder<EmptyingRecipe>> consumer, IIngredientManager ingredientManager) {
        ObjectOpenCustomHashSet<ItemStack> emptiedItems =
        ...
        for (ItemStack stack : ingredientManager.getAllIngredients(VanillaTypes.ITEM_STACK)) {
            if (PotionFluidHandler.isPotionItem(stack)) {
                ...
                EmptyingRecipe recipe = new StandardProcessingRecipe.Builder<>(
                                EmptyingRecipe::new, id)
            ...
            // There can be a lot of duplicate empty tanks (e.g. from emptying tanks with different
            // fluids). Merge
            // them to reduce memory usage. If the item is exactly the same as the input, just use
            // the
            // input stack
            // instead of the copy.
            ...
            ResourceLocation fluidName =
                    RegisteredObjectsHelper.getKeyOrThrow(extracted.getFluid());

            ResourceLocation id =
                    Create.asResource("empty_" + itemName.getNamespace() + "_" + itemName.getPath()
                            + "_of_" + fluidName.getNamespace() + "_" + fluidName.getPath());
    ...
    public void setRecipe(
            IRecipeLayoutBuilder builder, EmptyingRecipe recipe, IFocusGroup focuses) {
