...
public record StockKeeperGuiContainerHandler(IIngredientManager ingredientManager)
    ...
    public Optional<IClickableIngredient<?>> getClickableIngredientUnderMouse(
            StockKeeperRequestScreen containerScreen, double mouseX, double mouseY) {
        return containerScreen
                .getHoveredIngredient((int) mouseX, (int) mouseY)
                .flatMap(pair -> ingredientManager.createClickableIngredient(
                        pair.getFirst(), pair.getSecond(), true));
    }
}
