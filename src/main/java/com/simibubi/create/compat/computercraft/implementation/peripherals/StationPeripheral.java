...
public class StationPeripheral extends SyncedPeripheral<StationBlockEntity> {
    ...
    public final void assemble() throws LuaException {
        if (!blockEntity.isAssembling()) throw new LuaException("station must be in assembly mode");
        ...
        if (!blockEntity.exitAssemblyMode()) throw new LuaException("failed to exit assembly mode");
    }
    ...
    public final String getStationName() throws LuaException {
        ...
        if (station == null) throw new LuaException("station is not connected to a track");
    ...
    public final void setStationName(String name) throws LuaException {
        if (!blockEntity.updateName(name)) throw new LuaException("could not set station name");
    }
    ...
    public final CreateLuaTable getSchedule() throws LuaException {
        ...
        if (schedule == null) throw new LuaException("train doesn't have a schedule");
    ...
    public final void setSchedule(IArguments arguments) throws LuaException {
        ...
        // We must check the completed schedule, because `toCompoundTag` normalizes all CompoundTag
        // keys
        // to CamelCase
        // and so `Entries`, `entries`, `EnTrIeS`, etc. will all be converted to `Entries` in the
        // schedule
     ...
     */
    private Pair<@org.jetbrains.annotations.Nullable DiscoveredPath, @NotNull Boolean> findPath(
            String destinationFilter) throws LuaException {
        Train train = getTrainOrThrow();
        ...
        if (best == null) return Pair.of(null, anyMatch);
        return Pair.of(best, true);
    }

    @LuaFunction
    public MethodResult canTrainReach(String destinationFilter) throws LuaException {
        Pair<@org.jetbrains.annotations.Nullable DiscoveredPath, @NotNull Boolean> path =
                findPath(destinationFilter);
        if (path.getFirst() != null) return MethodResult.of(true, null);
    ...
    public MethodResult distanceTo(String destinationFilter) throws LuaException {
        Pair<@org.jetbrains.annotations.Nullable DiscoveredPath, @NotNull Boolean> path =
                findPath(destinationFilter);
        if (path.getFirst() != null) return MethodResult.of(path.getFirst().distance, null);
    ...
    private @NotNull Train getTrainOrThrow() throws LuaException {
        ...
        if (train == null) throw new LuaException("there is no train present");
    ...
    private static @NotNull Object fromNBTTag(@Nullable String key, Tag tag) throws LuaException {
        ...
        if (type == Tag.TAG_BYTE && key != null && key.equals("Count"))
            ...
            return ((NumericTag) tag).getAsDouble();
                ...
                || type == Tag.TAG_BYTE_ARRAY
                || type == Tag.TAG_INT_ARRAY
                || type == Tag.TAG_LONG_ARRAY) {
    ...
    private static @NotNull Tag toNBTTag(@Nullable String key, Object value) throws LuaException {
        if (value instanceof Boolean v) return ByteTag.valueOf(v);
            ...
            if (v.intValue() == v.doubleValue()) return IntTag.valueOf(v.intValue());
            else return DoubleTag.valueOf(v.doubleValue());

        } else if (value instanceof String v) return StringTag.valueOf(v);
        else if (value instanceof Map<?, ?> v && v.containsKey(1.0)) { // List
            ListTag list = new ListTag();
            for (double i = 1; i <= v.size(); i++) {
                if (v.get(i) != null) list.add(toNBTTag(null, v.get(i)));
            }
            ...
            for (Object objectKey : v.keySet()) {
                ...
                compound.put(
                        ...
                        // Items store "count" in the same compound tag, so we can check for its
                        // presence to see
                        // if this is a serialized item
                        compoundKey.equals("id") && v.containsKey("count")
                                ? "id"
                                : StringHelper.snakeCaseToCamelCase(compoundKey),
