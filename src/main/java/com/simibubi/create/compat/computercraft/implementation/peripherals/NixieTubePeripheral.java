...
public class NixieTubePeripheral extends SyncedPeripheral<NixieTubeBlockEntity> {
    ...
    protected void onLastDetach() {
        // When detaching from the last computer, reset the entire nixie tube row back to redstone
        // display,
        // except if it's still being controlled from some other tube. onLastDetach runs after the
        // hasAttachedComputer flag is reset, so we can use walkNixies()'s computer control
        // rejection
        // for that.
        super.onLastDetach();
        Level world = blockEntity.getLevel();
        if (world == null) return;
        // Check if the nixie tube block is still there; if it isn't then the nixie was
        // removed/destroyed
    ...
    public void setText(IArguments arguments) throws LuaException {
        ...
        String tagElement = Component.Serializer.toJson(
                Component.literal(arguments.getString(0)), level.registryAccess());
    ...
    public void setTextColour(String colour) throws LuaException {
        ...
        DyeColor dye =
                LuaValues.checkEnum(1, DyeColor.class, colour.equals("grey") ? "gray" : colour);
    ...
    private void changeTextNixie(
            @Nullable String tagElement, @Nullable BlockState state, @Nullable DyeColor dye) {
        Level world = blockEntity.getLevel();
        if (world == null) return;
        NixieTubeBlock.walkNixies(
                world, blockEntity.getBlockPos(), true, (currentPos, rowPosition) -> {
                    if (tagElement != null)
                        ((NixieTubeBlock) blockEntity.getBlockState().getBlock())
                                .withBlockEntityDo(
                                        world,
                                        currentPos,
    ...
    public void setSignal(IArguments arguments) throws LuaException {
        if (arguments.optTable(0).isPresent()) setSignal(signal().first, arguments.getTable(0));
        if (arguments.optTable(1).isPresent()) setSignal(signal().second, arguments.getTable(1));
    }

    private void setSignal(
            NixieTubeBlockEntity.ComputerSignal.TubeDisplay display, @NotNull Map<?, ?> attrs)
            throws LuaException {
        ...
        if (attrs.containsKey("b")) display.b = constrainByte("r", 0, 255, attrs.get("b"));
