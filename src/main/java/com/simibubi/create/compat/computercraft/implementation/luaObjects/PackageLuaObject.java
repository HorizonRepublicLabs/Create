...
public class PackageLuaObject implements LuaComparable {
    ...
    public final boolean isEditable() {
        return (blockEntity != null
                && !blockEntity.heldBox.isEmpty()
                && blockEntity.heldBox == box);
    }

    @LuaFunction(mainThread = true)
    public final String getAddress() throws LuaException {
        if (isEditable()) this.address = PackageItem.getAddress(box);
        return this.address;
    }

    @LuaFunction(mainThread = true)
    public final void setAddress(String argument) throws LuaException {
        if (!isEditable()) throw new LuaException("Package is not editable");
    ...
    public Map<?, ?> getTableRepresentation() {
        try {
            ...
            if (hasOrderData()) map.put("orderData", getOrderData());
            return map;
