...
public class ComputerUtil {
    ...
    public static int bigItemStackToLuaTableFilter(BigItemStack entry, Map<?, ?> filter)
            throws LuaException {
        ...
        if (!deepEquals(new HashMap<>(filter), details)) return 0;
        return entry.count;
    }
    ...
    private static boolean deepEquals(Object fVal, Object iVal) throws LuaException {
        ...
        // Lua Objects can implement LuaComparable to provide a table representation for lazy
        // filtering
        ...
        if (fVal instanceof Map<?, ?> fMap
                && fMap.get("_op") instanceof String op
                && fMap.get("value") != null) {
            ...
            switch (op) {
                ...
                case "any", "all" -> { // Any / All operator
                    ...
                    if (!(fValue instanceof Map<?, ?> valueMap)) throw new LuaException(errorMsg);

                    List<?> values = toOrderedList(valueMap);
                    if (values == null) throw new LuaException(errorMsg);
        ...
        if (fColl == null || iColl == null) return false;
        ...
        switch (f.mode) {
            case EXACT -> {
                ...
                for (int k = 0; k < f.list.size(); k++)
                    if (!deepEquals(f.list.get(k), i.list.get(k))) return false;
                return true;
            }
            case CONTAINS -> {
                outer:
        ...
        switch (f.mode) {
            case EXACT -> {
                ...
                for (var e : f.map.entrySet()) {
                    if (!deepEquals(e.getValue(), i.map.get(e.getKey()))) return false;
                }
                return true;
            }
            case CONTAINS -> {
                for (var e : f.map.entrySet()) {
                    if (!i.map.containsKey(e.getKey())
                            || !deepEquals(e.getValue(), i.map.get(e.getKey()))) return false;
                }
                return true;
            }
            case CONTAINED -> {
                for (var e : i.map.entrySet()) {
                    if (!f.map.containsKey(e.getKey())
                            || !deepEquals(f.map.get(e.getKey()), e.getValue())) return false;
                }
                return true;
            }
        }
    ...
    // Contains: All elements in filter must be in the item, order doesn't matter, args removed from
    // filter as they are found
    // Contained: All elements in item must be in the filter, order doesn't matter, args removed
    // from
    // item as they are found
    private enum MatchMode {
        ...
        static MatchMode parse(Object t) throws LuaException {
            ...
            return switch (s.toLowerCase()) {
                ...
                default -> throw new LuaException("Invalid match mode: " + s
                        + ", expected 'exact', 'contained' or 'contains'");
            };
        }
    }
    ...
    private static boolean isArrayLike(Map<?, ?> map) {
        ...
        for (boolean ok : seen) if (!ok) return false;
    ...
    private static List<Object> toOrderedList(Map<?, ?> m) {
        ...
        for (var e : m.entrySet()) out.set(((Number) e.getKey()).intValue() - 1, e.getValue());
        return out;
    }
    ...
    public static Map<String, ?> getItemDetail(IItemHandler inventory, int slot)
            throws LuaException {

        int maxSlots = inventory.getSlots();
        if (slot < 1 || slot > maxSlots)
            throw new LuaException(String.format("Slot " + slot
                    + " out of range, available slots between " + 1 + " and " + maxSlots));
        var stack = inventory.getStackInSlot(slot - 1);
        return stack.isEmpty() ? null : VanillaDetailRegistries.ITEM_STACK.getDetails(stack);
    }

    public static Map<String, ?> getItemDetail(InventorySummary inventorySummary, int slot)
            throws LuaException {
