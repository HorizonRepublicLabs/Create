...
public class FramedBlocksInSchematics {
    ...
    public static CompoundTag prepareBlockEntityData(
        ...
        for (String key : data.getAllKeys())
            if (!KEYS_TO_RETAIN.contains(key)) keysToRemove.add(key);
        for (String key : keysToRemove) data.remove(key);

        if (data.getCompound("camo").contains("fluid")) data.remove("camo");

        if (data.getCompound("camo_two").contains("fluid")) data.remove("camo_two");

        return data;
    }

    public static ItemRequirement getRequiredItems(BlockState blockState, BlockEntity blockEntity) {
        if (blockEntity == null) return ItemRequirement.NONE;
        ...
        if (data.getBoolean("intangible"))
            list.add(new StackRequirement(
                    new ItemStack(Items.PHANTOM_MEMBRANE), ItemUseType.CONSUME));

        if (data.getBoolean("glowing"))
            list.add(
                    new StackRequirement(new ItemStack(Items.GLOWSTONE_DUST), ItemUseType.CONSUME));

        if (data.getBoolean("reinforced"))
            list.add(new StackRequirement(
                    new ItemStack(Mods.FRAMEDBLOCKS.getItem("framed_reinforcement")),
                    ItemUseType.CONSUME));

        if (data.contains("camo"))
            addCamoStack(
                    blockEntity.getLevel().holderLookup(Registries.BLOCK),
    ...
    private static void addCamoStack(
            HolderGetter<Block> level, CompoundTag tag, List<StackRequirement> list) {
        if (!tag.contains("state")) return;
        BlockState blockState = NbtUtils.readBlockState(level, tag.getCompound("state"));
        ItemStack itemStack = new ItemStack(blockState.getBlock());
        if (!itemStack.isEmpty()) list.add(new StackRequirement(itemStack, ItemUseType.CONSUME));
    }
}
