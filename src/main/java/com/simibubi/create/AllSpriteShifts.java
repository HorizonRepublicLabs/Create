...
public class AllSpriteShifts {
    ...
    public static final Map<WeatherState, CTSpriteShiftEntry> COPPER_SHINGLES =
            new EnumMap<>(WeatherState.class);
    public static final Map<WeatherState, CTSpriteShiftEntry> COPPER_TILES =
            new EnumMap<>(WeatherState.class);

    public static final Map<DyeColor, SpriteShiftEntry> DYED_BELTS = new EnumMap<>(DyeColor.class),
            DYED_OFFSET_BELTS = new EnumMap<>(DyeColor.class),
            DYED_DIAGONAL_BELTS = new EnumMap<>(DyeColor.class);
    public static final SpriteShiftEntry
            BURNER_FLAME = get("block/blaze_burner_flame", "block/blaze_burner_flame_scroll"),
            SUPER_BURNER_FLAME =
                    get("block/blaze_burner_flame", "block/blaze_burner_flame_superheated_scroll");
    public static final CTSpriteShiftEntry
            ANDESITE_SCAFFOLD = horizontal("scaffold/andesite_scaffold"),
            BRASS_SCAFFOLD = horizontal("scaffold/brass_scaffold"),
            COPPER_SCAFFOLD = horizontal("scaffold/copper_scaffold");
    public static final CTSpriteShiftEntry
            ANDESITE_SCAFFOLD_INSIDE = horizontal("scaffold/andesite_scaffold_inside"),
    ...
    public static final CTSpriteShiftEntry ANDESITE_CASING = omni("andesite_casing"),
            ...
            RAILWAY_CASING = omni("railway_casing"),
            RAILWAY_CASING_SIDE = omni("railway_casing_side"),
    ...
    public static final SpriteShiftEntry
            ELEVATOR_BELT = get("block/elevator_pulley_belt", "block/elevator_pulley_belt_scroll"),
    ...
    public static final SpriteShiftEntry BELT = get("block/belt", "block/belt_scroll"),
            ...
            ANDESIDE_BELT_CASING =
                    get("block/belt/brass_belt_casing", "block/belt/andesite_belt_casing"),
            CRAFTER_THINGIES = get("block/crafter_thingies", "block/crafter_thingies");

    public static final SpriteShiftEntry BOGEY_BELT =
            get("block/bogey/belt", "block/bogey/belt_scroll");
    ...
    private static void populateMaps() {
        ...
        Arrays.stream(supportedWoodTypes)
                .forEach(woodType -> WOODEN_WINDOWS.put(
                        woodType, vertical("palettes/" + woodType.name() + "_window")));
        ...
        for (DyeColor color : DyeColor.values()) {
            ...
            DYED_DIAGONAL_BELTS.put(
                    color, get("block/belt_diagonal", "block/belt/" + id + "_diagonal_scroll"));
        }

        for (WeatherState state : WeatherState.values()) {
            String pref = "copper/"
                    + (state == WeatherState.UNAFFECTED ? "" : Lang.asId(state.name()) + "_");
            COPPER_SHINGLES.put(
                    state,
                    getCT(AllCTTypes.ROOF, pref + "copper_roof_top", pref + "copper_shingles_top"));
            COPPER_TILES.put(
                    state,
                    getCT(AllCTTypes.ROOF, pref + "copper_roof_top", pref + "copper_tiles_top"));
        }
    }
    ...
    private static Couple<CTSpriteShiftEntry> vault(String name) {
        ...
        return Couple.createWithContext(medium -> CTSpriteShifter.getCT(
                AllCTTypes.RECTANGLE,
                Create.asResource(prefixed + "_small"),
    ...
    private static SpriteShiftEntry get(String originalLocation, String targetLocation) {
        return SpriteShifter.get(
                Create.asResource(originalLocation), Create.asResource(targetLocation));
    }

    private static CTSpriteShiftEntry getCT(
            CTType type, String blockTextureName, String connectedTextureName) {
