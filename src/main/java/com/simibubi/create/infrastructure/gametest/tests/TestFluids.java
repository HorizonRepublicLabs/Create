...
public class TestFluids {
    ...
    public static void steamEngine(CreateGameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            StressGaugeBlockEntity stress =
                    helper.getBlockEntity(AllBlockEntityTypes.STRESSOMETER.get(), stressometer);
            SpeedGaugeBlockEntity speed =
                    helper.getBlockEntity(AllBlockEntityTypes.SPEEDOMETER.get(), speedometer);
    ...
    public static void threePipeCombine(CreateGameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            if (moved != initialContents)
                helper.fail("Wrong amount of fluid amount. expected [%s], got [%s]"
    ...
    public static void threePipeSplit(CreateGameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            if (newTotalContents != totalContents) {
                helper.fail("Wrong total fluid amount. expected [%s], got [%s]"
    ...
    public static void waterwheelMaterials(CreateGameTestHelper helper) {
        ...
        helper.runAfterDelay(
                3,
                () -> chests.forEach(chest -> planks.forEach(plank -> ItemHandlerHelper.insertItem(
                        helper.itemStorageAt(chest), new ItemStack(plank), false))));
        ...
        helper.succeedWhen(() -> {
            ...
            WaterWheelBlockEntity smallWheelBe =
                    helper.getBlockEntity(AllBlockEntityTypes.WATER_WHEEL.get(), smallWheel);
            if (!smallWheelBe.material.is(block))
                helper.fail("Small waterwheel has not consumed "
                        + BuiltInRegistries.ITEM.getKey(plank));

            WaterWheelBlockEntity largeWheelBe =
                    helper.getBlockEntity(AllBlockEntityTypes.LARGE_WATER_WHEEL.get(), largeWheel);
            if (!largeWheelBe.material.is(block))
                helper.fail("Large waterwheel has not consumed "
                        + BuiltInRegistries.ITEM.getKey(plank));
