...
public class TestItems {
    ...
    public static void armMultiOutput(CreateGameTestHelper helper) {
        ...
        BlockPos[] blazeBurners = IntStream.rangeClosed(6, 8)
                .boxed()
                .flatMap(x -> IntStream.rangeClosed(1, 3).mapToObj(z -> new BlockPos(x, 2, z)))
    ...
    public static void attributeFilters(CreateGameTestHelper helper) {
        ...
        Holder<Enchantment> PROTECTION_ENCHANT = helper.getLevel()
                .registryAccess()
    ...
    public static void brassTunnelPreferNearest(CreateGameTestHelper helper) {
        ...
        helper.succeedWhen(() -> out.forEach(
                pos -> helper.assertContainerContains(pos, AllBlocks.BRASS_CASING.get())));
    }
    ...
    public static void brassTunnelSyncInput(CreateGameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            if (helper.secondsPassed() < 9) {
                ...
                outputs.forEach(
                        out -> helper.assertContainerContains(out, AllBlocks.BRASS_CASING.get()));
            }
        });
    }
    ...
    public static void smartObserverCounting(CreateGameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            if (chestNixieReading != totalChestItems)
                helper.fail("Chest nixie detected %s, expected %s"
            ...
            if (doubleChestNixieReading != totalDoubleChestItems)
                helper.fail("Double chest nixie detected %s, expected %s"
    ...
    public static void depotDisplay(CreateGameTestHelper helper) {
        ...
        FlapDisplayBlockEntity display = helper.getBlockEntity(
                        AllBlockEntityTypes.FLAP_DISPLAY.get(), displayPos)
                .getController();
        helper.succeedWhen(() -> {
            for (int i = 0; i < 2; i++) {
