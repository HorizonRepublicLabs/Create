...
public class TestContraptions {
    ...
    public static void controls(CreateGameTestHelper helper) {
        ...
        List<BlockPos> dirt =
                List.of(new BlockPos(4, 2, 6), new BlockPos(2, 2, 4), new BlockPos(4, 2, 2));
        List<BlockPos> wheat =
                List.of(new BlockPos(4, 3, 7), new BlockPos(1, 3, 4), new BlockPos(4, 3, 1));
        ...
        helper.succeedWhen(() -> {
            ...
            MechanicalBearingBlockEntity bearing =
                    helper.getBlockEntity(AllBlockEntityTypes.MECHANICAL_BEARING.get(), bearingPos);
    ...
    public static void elevator(CreateGameTestHelper helper) {
        ...
        helper.runAtTickTime(
                15, () -> helper.getBlockEntity(AllBlockEntityTypes.ELEVATOR_PULLEY.get(), pulley)
                        .clicked());
        helper.succeedWhen(() -> {
            ...
            if (!helper.getBlockState(lever)
                    .getValue(
                            LeverBlock
                                    .POWERED)) { // step 1: check entity, lamps, and secondary, then
                // move up
                helper.getFirstEntity(
                        AllEntityTypes.CONTROLLED_CONTRAPTION.get(),
                        elevatorStart); // make sure entity exists
                ...
                ElevatorPulleyBlockEntity secondary = helper.getBlockEntity(
                        AllBlockEntityTypes.ELEVATOR_PULLEY.get(), secondaryPulley);
                ...
                helper.getBlockEntity(AllBlockEntityTypes.ELEVATOR_PULLEY.get(), pulley)
                        .clicked();
            }
        });
    }
    ...
    public static void rollerFilling(CreateGameTestHelper helper) {
        ...
        List<BlockPos> existing = BlockPos.betweenClosedStream(
                        new BlockPos(1, 3, 2), new BlockPos(4, 2, 2))
                .toList();
        List<BlockPos> filled = BlockPos.betweenClosedStream(
                        new BlockPos(1, 2, 1), new BlockPos(4, 3, 3))
                .filter(pos -> !existing.contains(pos))
                .toList();
        List<BlockPos> tracks = BlockPos.betweenClosedStream(
                        new BlockPos(1, 4, 2), new BlockPos(4, 4, 2))
                .toList();
        helper.pullLever(lever);
        helper.succeedWhen(() -> {
    ...
    public static void rollerPavingAndClearing(CreateGameTestHelper helper) {
        ...
        List<BlockPos> paved = BlockPos.betweenClosedStream(
                        new BlockPos(1, 2, 1), new BlockPos(4, 2, 1))
                .toList();
