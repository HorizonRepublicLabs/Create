...
public class MechanicalDrillScenes {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (i == 1) {
                scene.overlay()
                        ...
                        .text("When given Rotational Force, a Mechanical Drill will break blocks"
                                + " directly in front of it");
            }
        }

        scene.world().hideSection(util.select().position(breakingPos), Direction.UP);
        ElementLink<EntityElement> plankEntity = scene.world()
                .createItemEntity(
    ...
    public static void contraption(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSectionAndMerge(util.select().position(3, 2, 3), Direction.EAST, contraption);
        scene.idle(5);
        scene.world()
                .showSectionAndMerge(util.select().position(3, 2, 2), Direction.EAST, contraption);
        ...
        scene.idle(10);
        scene.overlay()
                .showControls(util.vector().topOf(2, 3, 2), Pointing.DOWN, 60)
