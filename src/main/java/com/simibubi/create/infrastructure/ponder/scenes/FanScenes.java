...
public class FanScenes {
    ...
    public static void direction(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSection(
                        util.select().fromTo(3, 1, 0, 3, 1, 5).add(util.select().position(3, 2, 4)),
                        Direction.DOWN);
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSection(
                        util.select().layer(0).substract(util.select().position(0, 0, 4)),
                        Direction.UP);
        scene.idle(5);
        scene.world()
                .showSection(
                        util.select().fromTo(6, 1, 2, 5, 1, 2).add(util.select().position(1, 1, 2)),
                        Direction.DOWN);
        scene.idle(25);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 80)
        ...
        blockInFront = scene.world()
                .showIndependentSection(util.select().position(3, 1, 0), Direction.NORTH);
        scene.world().moveSection(blockInFront, util.vector().of(1, 0, 2), 0);
        scene.idle(50);

        scene.overlay()
                .showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60)
        ...
        scene.world()
                .modifyBlockEntityNBT(
                        util.select().position(depos),
                        DepotBlockEntity.class,
                        nbt -> nbt.put(
                                "HeldItem",
                                new TransportedItemStack(clay)
                                        .serializeNBT(scene.world().getHolderLookupProvider())));
        scene.effects()
                .emitParticles(
                        depotTop,
                        scene.effects().simpleParticleEmitter(ParticleTypes.SPIT, Vec3.ZERO),
        ...
        scene.world()
                .setKineticSpeed(
                        util.select().position(1, 2, 4).add(util.select().fromTo(3, 3, 1, 1, 3, 3)),
                        0);
    }
}
