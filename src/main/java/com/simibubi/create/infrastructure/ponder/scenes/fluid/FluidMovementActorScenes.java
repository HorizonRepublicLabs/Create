...
public class FluidMovementActorScenes {
    ...
    public static void transfer(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection pipes =
                util.select().fromTo(2, 1, 3, 0, 1, 3).add(util.select().position(0, 1, 4));
        ...
        scene.world()
                .modifyBlock(pumpPos, s -> s.setValue(PumpBlock.FACING, Direction.NORTH), false);
        ...
        scene.overlay()
                ...
                .placeNearTarget()
                .attachKeyFrame()
                .text("This component can interact with fluid tanks without the need to stop the"
                        + " contraption");
        scene.idle(90);
        ...
        scene.idle(17);
        scene.overlay()
                .showOutline(PonderPalette.GREEN, psi, util.select().fromTo(5, 3, 2, 6, 4, 2), 80);
        scene.idle(10);
        ...
        scene.overlay()
                ...
                .colored(PonderPalette.GREEN)
                .attachKeyFrame()
                .text("While engaged, the stationary interface will represent ALL Tanks on the"
                        + " contraption");
        scene.idle(80);

        ElementLink<WorldSectionElement> p =
        ...
        for (int i = 0; i < 16; i++) {
            if (i == 8)
                scene.overlay()
                        .showControls(
                                util.vector()
                                        .blockSurface(util.grid().at(5, 3, 2), Direction.WEST)
                                        .add(0, 0.5, 0),
                                Pointing.LEFT,
                                30)
                        .withItem(bucket);
            ...
            scene.world().modifyBlockEntity(st, type, be -> {
                IFluidHandler ifh = be.getLevel()
                        .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
                if (ifh != null) ifh.drain(1000, FluidAction.EXECUTE);
            });
        ...
        scene.world()
                .modifyBlock(pumpPos, s -> s.setValue(PumpBlock.FACING, Direction.SOUTH), true);
        ...
        scene.world().modifyBlockEntity(util.grid().at(2, 2, 3), type, be -> {
            IFluidHandler ifh = be.getLevel()
                    .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
            if (ifh != null) ifh.drain(8000, FluidAction.EXECUTE);
        });
        scene.idle(50);
        ...
        scene.overlay()
                ...
                .text("After no contents have been exchanged for a while, the contraption will"
                        + " continue on its way");
