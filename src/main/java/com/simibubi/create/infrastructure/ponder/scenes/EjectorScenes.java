...
public class EjectorScenes {
    ...
    public static void ejector(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(util.vector().topOf(targetPos), Pointing.DOWN, 50)
                .rightClick()
                .whileSneaking()
                .withItem(asStack);
        scene.idle(7);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.OUTPUT, slot, new AABB(targetPos), 160);
        ...
        scene.idle(80);
        scene.overlay()
                .showControls(util.vector().topOf(ejectorPos), Pointing.DOWN, 50)
                .rightClick()
                .withItem(asStack);
        scene.idle(7);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.GREEN, slot, bb.expandTowards(-15, 15, 0), 100);
        scene.idle(10);
        ...
        scene.idle(70);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.RED, new Object(), bb.move(-2, 0, -1), 60);
        scene.idle(10);
        scene.overlay()
                .showText(50)
                .colored(PonderPalette.RED)
                .text("They cannot however be off to a side")
                .pointAt(util.vector().blockSurface(targetPos.north().east(), Direction.WEST))
                .placeNearTarget();
        scene.idle(70);
        scene.overlay()
                .showOutlineWithText(util.select().position(ejectorPos.west()), 70)
                .colored(PonderPalette.OUTPUT)
                .text("If no valid Target was selected, it will simply target the block directly"
                        + " in front")
                .placeNearTarget();
        scene.idle(80);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(ejectorPos).add(0.5, 0, 0), Pointing.RIGHT, 30)
        ...
        scene.overlay()
                .showText(100)
                .text("It is now limited to this stack size, and only activates when its held"
                        + " stack reaches this amount")
                .pointAt(util.vector().topOf(ejectorPos))
                .placeNearTarget();
        for (int i = 0; i < 4; i++) {
            scene.idle(20);
        ...
        ElementLink<ParrotElement> birb = scene.special()
                .createBirb(
                        util.vector().topOf(ejectorPos).add(0, -3 / 16f, 0),
        ...
        scene.world()
                .modifyBlockEntity(
                        ...
                        ejector -> ejector.activateDeferred());
    ...
    public static void splitY(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showText(80)
                .attachKeyFrame()
                .text("Combined with Brass Tunnels, Ejectors can split item stacks by specific"
                        + " amounts")
                .pointAt(util.vector().topOf(ejectorPos))
                .placeNearTarget();
        scene.idle(90);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(tunnel), Pointing.DOWN, 80)
        ...
        scene.overlay()
                ...
                .text("First, configure the Brass Tunnel to 'Prefer Nearest', in order to"
                        + " prioritize its side output")
                .pointAt(util.vector().topOf(tunnel))
                .placeNearTarget();
        scene.idle(110);

        Vec3 input =
                util.vector().blockSurface(ejectorPos, Direction.NORTH).subtract(0, 2 / 16f, 0);
        Vec3 topOfSlot = input.add(0, 2 / 16f, 0);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(util.grid().at(4, 1, 3)), Pointing.DOWN, 20)
    ...
    public static void redstone(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            scene.world().toggleRedstonePower(observerRedstone);
            scene.idle(16);
            if (i == 3) scene.markAsFinished();
            if (i == 1) {
                scene.overlay()
                        .showText(60)
                        .attachKeyFrame()
                        .pointAt(util.vector()
                                .blockSurface(util.grid().at(4, 1, 1), Direction.NORTH))
                        .placeNearTarget()
