...
public class FunnelScenes {
    ...
    public static void intro(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection verticalFunnel =
                util.select().fromTo(2, -1, 4, 2, 4, 4).add(util.select().fromTo(1, 1, 4, 1, 4, 4));
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (i < 6) scene.world().createItemOnBelt(entryBeltPos, Direction.EAST, itemStack);
        }
        ...
        scene.world()
                .setBlocks(
                        util.select().fromTo(2, -1, 4, 2, 0, 4),
        ...
        for (int i = 0; i < 4; i++) {
            if (lastItemEntity != null) scene.world().modifyEntity(lastItemEntity, Entity::discard);
            if (i < 3)
                lastItemEntity = scene.world()
                        .createItemEntity(topItemSpawn, util.vector().of(0, -0.4, 0), itemStack);
            scene.idle(8);
        }
    ...
    public static void directionality(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        itemLink = scene.world()
                .createItemEntity(topCenter.add(0, 3, 0), util.vector().of(0, -0.2, 0), itemStack);
        scene.idle(10);
        ...
        // Wrench interaction
        scene.overlay()
                .showControls(topSide, Pointing.RIGHT, 40)
                .rightClick()
        ...
        itemLink = scene.world()
                .createItemEntity(
                        sideCenter.subtract(0, .45, 0), util.vector().of(0, 0, -0.1), itemStack);
        scene.idle(60);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            scene.world()
                    .modifyBlocks(
                            gearshiftAndLever, s -> s.cycle(BlockStateProperties.POWERED), false);
    ...
    public static void compat(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().showSection(util.select().layer(0), Direction.UP);
        Selection firstShow = util.select()
                .layer(1)
        ...
        scene.world()
                .createItemOnBeltLike(
                        sawFunnel.below(), Direction.SOUTH, new ItemStack(Blocks.OAK_LOG));
        scene.idle(40);
        ...
        scene.world()
                .createItemOnBeltLike(
                        depotFunnel.below(), Direction.SOUTH, new ItemStack(Items.GOLDEN_PICKAXE));
        scene.idle(40);
        ...
        scene.world()
                .createItemOnBeltLike(
                        drainFunnel.below(), Direction.SOUTH, new ItemStack(Items.WATER_BUCKET));
        scene.idle(40);
    ...
    public static void redstone(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        AABB redstoneBB =
                new AABB(funnel).inflate(-1 / 16f, -6 / 16f, -1 / 16f).move(0, -5 / 16f, 0);

        for (int i = 0; i < 4; i++) {
            if (lastItemEntity != null) scene.world().modifyEntity(lastItemEntity, Entity::discard);
            lastItemEntity = scene.world()
                    .createItemEntity(topItemSpawn, util.vector().of(0, -0.2, 0), itemStack);
            scene.idle(8);

            if (i == 3) {
                scene.world().modifyBlock(lever, s -> s.cycle(LeverBlock.POWERED), false);
                scene.world()
                        .modifyBlock(redstone, s -> s.setValue(RedStoneWireBlock.POWER, 15), false);
    ...
    public static void brass(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection beltAndStuff =
                util.select().fromTo(0, 1, 2, 4, 1, 2).add(util.select().fromTo(0, 1, 3, 0, 2, 5));
        Selection withoutBelt =
                util.select().layersFrom(1).substract(beltAndStuff).substract(depots);
        ...
        scene.idle(10);
        scene.world()
                .createItemOnBeltLike(andesiteFunnel.below().north(), Direction.SOUTH, itemStack);
        ...
        scene.world()
                .createItemOnBeltLike(
                        brassFunnel.below().north(), Direction.SOUTH, itemStack.copyWithCount(64));
        ...
        for (int i = 0; i < 14; i++) {
            ...
            scene.world().modifyEntities(ItemEntity.class, e -> {
                if (e.getY() < 1) e.discard();
            });

            if (i == 2) {
                scene.overlay().showFilterSlotInput(filter, Direction.NORTH, 40);
                scene.overlay()
                        .showControls(filter, Pointing.DOWN, 60)
                        .rightClick()
                        .withItem(emerald);
                scene.idle(10);
                scene.overlay()
                        .showText(80)
                        .text("Using items on the filter slot will restrict the funnel to only"
                                + " transfer matching stacks.")
                        .attachKeyFrame()
                        .pointAt(filter)
                        .placeNearTarget();
                scene.world()
                        .setFilterData(
                                util.select().position(brassFunnel),
    ...
    public static void transposer(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(funnelSelect, 40)
                .colored(PonderPalette.RED)
