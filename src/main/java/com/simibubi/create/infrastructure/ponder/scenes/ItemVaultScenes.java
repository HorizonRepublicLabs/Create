...
public class ItemVaultScenes {
    ...
    public static void storage(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink<WorldSectionElement> chestLink =
                scene.world().showIndependentSection(chests, Direction.NORTH);
        ...
        scene.overlay()
                .showOutline(
                        PonderPalette.GREEN,
                        "chestOutline",
                        util.select().fromTo(1, 1, 2, 0, 2, 3),
                        40);
        scene.idle(10);
        scene.overlay()
                .showLine(
                        PonderPalette.GREEN,
                        util.vector().of(2, 1, 2),
                        util.vector().of(3, 1, 1),
                        30);
        scene.overlay()
                .showLine(
                        PonderPalette.GREEN,
                        util.vector().of(2, 3, 2),
                        util.vector().of(3, 3, 1),
                        30);
        scene.overlay()
                .showLine(
                        PonderPalette.GREEN,
                        util.vector().of(2, 3, 4),
                        util.vector().of(3, 3, 4),
                        30);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(frontVault, Direction.NORTH), Pointing.RIGHT, 40)
        ...
        for (int i = 0; i < junk.length + 2; i++) {
            ...
            if (i < junk.length)
                scene.world()
                        .createItemOnBeltLike(util.grid().at(0, 1, 3), Direction.SOUTH, junk[i]);
            scene.idle(9);
        }
