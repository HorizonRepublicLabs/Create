...
public class DeployerScenes {
    ...
    public static void filter(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSection(
                        util.select().layer(0).add(util.select().position(1, 1, 2)), Direction.UP);
        scene.idle(5);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(deployerPos.west(2)), 60)
        ...
        scene.world()
                .modifyBlockEntityNBT(
                        ...
                        nbt -> nbt.put(
                                "HeldItem",
                                pot.saveOptional(scene.world().getHolderLookupProvider())));
        scene.idle(10);
        ...
        scene.world()
                .modifyBlockEntityNBT(
                        ...
                        nbt -> nbt.put(
                                "HeldItem",
                                tulip.saveOptional(scene.world().getHolderLookupProvider())));
        scene.idle(10);
        scene.overlay()
                .showText(40)
                .placeNearTarget()
        ...
        scene.world()
                .modifyBlockEntityNBT(
                        ...
                        nbt -> nbt.put(
                                "HeldItem",
                                shears.saveOptional(scene.world().getHolderLookupProvider())));
        scene.idle(10);
    ...
    public static void modes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(util.vector().topOf(deployerPos), Pointing.DOWN, 30)
                .withItem(tool);
        scene.idle(7);
        scene.world()
                .modifyBlockEntityNBT(
                        ...
                        nbt -> nbt.put(
                                "HeldItem",
                                tool.saveOptional(scene.world().getHolderLookupProvider())));
        scene.idle(45);
        ...
        scene.overlay()
                .showControls(frontVec, Pointing.LEFT, 40)
                .rightClick()
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                ...
                .placeNearTarget()
                .attachKeyFrame()
                .text("With a fitting held item, Deployers can process items provided beneath"
                        + " them");
        scene.idle(80);
        ...
        scene.world().moveDeployer(pressPos, -1, 30);
        scene.debug()
                .enqueueCallback(s -> SandPaperItem.spawnParticles(targetV, quartz, s.getWorld()));
        // particle
        ...
        ElementLink<BeltItemElement> ingot =
                scene.world().createItemOnBelt(beltPos, Direction.SOUTH, quartz);
        scene.idle(15);
    ...
    public static void contraption(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(flowers, 90)
                .attachKeyFrame()
                .colored(PonderPalette.GREEN)
                .text("They activate at each visited location, using items from inventories"
                        + " anywhere on the contraption");
        scene.idle(100);
        ...
        Vec3 filterSlot =
                util.vector().blockSurface(deployerPos.west(3), Direction.WEST).add(0, 0, 2 / 16f);
        ...
        for (int x = 0; x < 4; x++) {
            ...
            scene.world().moveDeployer(deployerPos, -1, 9);
            scene.world()
                    .setBlock(util.grid().at(1 + x, 1, 1), Blocks.POPPY.defaultBlockState(), false);
            scene.idle(18);
        }
    }
}
