...
public class DrainScenes {
    ...
    public static void emptying(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection pipes =
                util.select().fromTo(3, 1, 3, 3, 1, 4).add(util.select().fromTo(3, 2, 4, 2, 2, 4));
        ...
        ElementLink<WorldSectionElement> drainLink =
                scene.world().showIndependentSection(drain, Direction.DOWN);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(drainPos.west(), Direction.UP),
                        Pointing.DOWN,
                        40)
                .rightClick()
                .withItem(lavaBucket);
        ...
        scene.world().modifyBlockEntity(drainPos, ItemDrainBlockEntity.class, be -> {
            ...
            IFluidHandler fh = be.getLevel()
                    .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
            if (fh != null) fh.fill(new FluidStack(Fluids.LAVA, 1000), FluidAction.EXECUTE);
        });
        scene.idle(10);
        ...
        scene.world().modifyBlockEntity(drainPos, ItemDrainBlockEntity.class, be -> {
            IFluidHandler fh = be.getLevel()
                    .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
            if (fh != null) fh.drain(500, FluidAction.EXECUTE);
        });
