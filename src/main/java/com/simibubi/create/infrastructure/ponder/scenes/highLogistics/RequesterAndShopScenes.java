...
public class RequesterAndShopScenes {
    ...
    public static void requester(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showText(110)
                .text("Just like Stock tickers, Redstone requesters can order items from the"
                        + " logistics network")
                .attachKeyFrame()
                .placeNearTarget()
        ...
        scene.overlay()
                .showControls(util.vector().topOf(req), Pointing.DOWN, 80)
                .rightClick();
        scene.idle(10);
        ...
        scene.idle(15);
        ElementLink<WorldSectionElement> tickerL =
                scene.world().showIndependentSection(ticker, Direction.DOWN);
        scene.world().moveSection(tickerL, util.vector().of(0, 0, 1), 0);
        scene.idle(5);
        scene.special()
                .createBirb(
                        util.vector().centerOf(util.grid().at(4, 1, 1)),
                        FacePointOfInterestPose::new);
        scene.idle(20);

        Vec3 keeper = util.vector()
                .blockSurface(util.grid().at(4, 1, 1), Direction.WEST)
                .add(0, 0.5, 0);
        scene.overlay()
                .showControls(util.vector().topOf(util.grid().at(4, 1, 1)), Pointing.DOWN, 50)
                .rightClick()
                .withItem(linkItem);
        scene.idle(10);
