...
public class PulleyScenes {
    ...
    public static void movement(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .replaceBlocks(
                        util.select().fromTo(0, 2, 1, 2, 1, 1),
                        Blocks.OAK_PLANKS.defaultBlockState(),
                        false);

        scene.idle(5);
        scene.world()
                .showSectionAndMerge(util.select().position(2, 1, 0), Direction.SOUTH, chassis);
        scene.overlay()
                .showOutline(
                        PonderPalette.GREEN,
                        "glue",
    ...
    public static void movementModes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        scene.overlay()
                .showOutlineWithText(util.select().position(flowerPos), 70)
        ...
        scene.overlay()
                .showControls(util.vector().topOf(pulleyPos), Pointing.DOWN, 60)
                .rightClick();
        scene.overlay()
                .showText(70)
    ...
    public static void attachment(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(1, 1, 1, 1, 1, 2), 50)
                .colored(PonderPalette.GREEN)
                .placeNearTarget()
                .attachKeyFrame()
