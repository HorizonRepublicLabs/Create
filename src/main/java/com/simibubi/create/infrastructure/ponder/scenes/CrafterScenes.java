...
public class CrafterScenes {
    ...
    public static void setup(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .modifyBlocks(
                        crafters,
                        s -> s.setValue(MechanicalCrafterBlock.POINTING, Pointing.DOWN),
                        false);
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                scene.world()
                        .showSection(
                                util.select().position(y == 1 ? x + 1 : 3 - x, y + 1, 2),
                                Direction.DOWN);
                scene.idle(2);
            }
        }
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(2, 3, 2), Direction.NORTH),
                        Pointing.RIGHT,
                        40)
                .rightClick()
        ...
        for (BlockPos pos : positions) {
            scene.overlay()
                    .showControls(
                            util.vector().blockSurface(pos, Direction.NORTH), Pointing.RIGHT, 10)
                    .rightClick()
                    .withItem(AllItems.WRENCH.asStack());
            scene.idle(7);
        ...
        scene.overlay()
                ...
                .pointAt(util.vector()
                        .blockSurface(util.grid().at(1, 1, 2), Direction.WEST)
                        .add(0, 0, -.5f))
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(1, 3, 2), Direction.NORTH),
                        Pointing.RIGHT,
                        40)
                .rightClick()
                .withItem(planks);
        ...
        scene.overlay()
                ...
                .text("For recipes not fully occupying the crafter setup, the start can be"
                        + " forced using a Redstone Pulse")
    ...
    public static void connect(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 2; x++) {
                scene.world()
                        .showSection(
                                util.select().position(y == 1 ? x + 1 : 2 - x, y + 1, 2),
                                Direction.DOWN);
                scene.idle(2);
            }
        }
        ...
        BlockPos depotPos = util.grid().at(0, 1, 2);
        Selection funnel =
                util.select().fromTo(4, 1, 5, 4, 1, 2).add(util.select().fromTo(3, 2, 2, 3, 1, 2));
        Selection kinetics =
                util.select().position(3, 3, 2).add(util.select().fromTo(3, 3, 3, 3, 1, 3));
        scene.idle(5);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(2, 2, 2), 70)
                .attachKeyFrame()
                .placeNearTarget()
        ...
        scene.overlay()
                .showControls(v, Pointing.LEFT, 40)
                .rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.idle(7);
        ...
        scene.idle(40);
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(2, 2, 2, 1, 2, 2), 70)
                .attachKeyFrame()
                .placeNearTarget()
                .pointAt(v)
        ...
        scene.idle(80);
        scene.overlay()
                .showControls(v.add(0, 1, 0), Pointing.LEFT, 20)
                .rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.idle(7);
        ...
        scene.idle(20);
        scene.overlay()
                .showControls(v.add(.5, -.5, 0), Pointing.LEFT, 20)
                .rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.idle(7);
        ...
        scene.idle(40);
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(1, 1, 2, 2, 3, 2), 70)
                .attachKeyFrame()
                .placeNearTarget()
                .text("All connected Crafters can now be accessed by the same input location");
        scene.idle(60);
        scene.overlay()
                .showControls(util.vector().centerOf(util.grid().at(4, 2, 2)), Pointing.DOWN, 40)
                .withItem(planks);
        scene.idle(7);
        scene.world()
                .createItemOnBelt(
                        util.grid().at(4, 1, 2), Direction.EAST, planks.copyWithCount(16));
        scene.idle(22);
    ...
    public static void covers(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(emptyCrafter, 90)
                .attachKeyFrame()
                .colored(PonderPalette.RED)
        ...
        scene.overlay()
                ...
                .colored(PonderPalette.GREEN)
                .pointAt(util.vector().blockSurface(util.grid().at(2, 2, 2), Direction.NORTH))
                .text("Using Slot Covers, Crafters can be set to act as an Empty Slot in the"
                        + " arrangement")
                .placeNearTarget();
        scene.idle(100);
        scene.overlay()
                .showControls(
                        ...
                        Pointing.RIGHT,
                        50)
                .withItem(AllItems.CRAFTER_SLOT_COVER.asStack())
                .rightClick();
        scene.idle(7);
        scene.world()
                .modifyBlockEntityNBT(
                        emptyCrafter, type, compound -> compound.putBoolean("Cover", true));
        scene.idle(130);

        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(2, 3, 2), Direction.WEST),
                        Pointing.LEFT,
                        40)
        ...
        scene.overlay()
                .showOutlineWithText(
                        util.select().fromTo(3, 2, 2, 1, 2, 2).add(util.select().position(2, 1, 2)),
                        80)
                .attachKeyFrame()
                .pointAt(util.vector().blockSurface(util.grid().at(2, 2, 2), Direction.NORTH))
                .text("Shared Inputs created with the Wrench at the back can also reach across"
                        + " covered Crafters")
                .placeNearTarget();
        scene.idle(60);
        ...
        ElementLink<EntityElement> ingot = scene.world()
                .createItemEntity(
                        util.vector().centerOf(4, 4, 2), util.vector().of(0, 0.2, 0), iron);
        scene.idle(17);
