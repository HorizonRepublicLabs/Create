...
public class TunnelScenes {
    ...
    public static void andesite(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int i = 0; i < 3; i++) {
            tunnels.add(scene.world()
                    .showIndependentSection(util.select().position(1 + i, 2, 4), Direction.DOWN));
        ...
        for (int i = 0; i < 3; i++) {
            ...
            scene.world()
                    .modifyBlockEntityNBT(
                            ...
                            nbt -> NBTHelper.writeEnum(
                                    nbt, "Casing", BeltBlockEntity.CasingType.ANDESITE),
                            true);
            scene.idle(4);
        }
        ...
        scene.overlay()
                .showControls(util.vector().topOf(util.grid().at(4, 1, 2)), Pointing.DOWN, 20)
    ...
    public static void brass(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int i = 0; i < 3; i++) {
            tunnels.add(scene.world()
                    .showIndependentSection(util.select().position(2 + i, 2, 4), Direction.DOWN));
            scene.world().moveSection(tunnels.get(i), util.vector().of(0, 0, -2), 0);
            scene.idle(4);
        }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            scene.world()
                    .modifyBlockEntityNBT(
                            ...
                            BeltBlockEntity.class,
                            nbt -> NBTHelper.writeEnum(
                                    nbt, "Casing", BeltBlockEntity.CasingType.BRASS),
                            true);
            scene.idle(4);
        }
        ...
        scene.world().modifyBlockEntity(tunnelPos, tunnelClass, be -> be.getBehaviour(
                        SidedFilteringBehaviour.TYPE)
        ...
        for (int i = 0; i < 6; i++) {
            scene.world()
                    .createItemOnBelt(
                            util.grid().at(5, 1, 2), Direction.EAST, i % 2 == 0 ? zinc : copper);
            scene.idle(12);
        }
        ...
        scene.overlay()
                ...
                .pointAt(tunnelTop)
                .placeNearTarget()
                .text("Whenever a passing item has multiple valid exits, the distribution mode"
        ...
        for (int i = 0; i < 3; i++) {
            ...
            scene.world()
                    .createItemOnBelt(
                            util.grid().at(5, 1, 2),
                            Direction.EAST,
                            AllItems.BRASS_INGOT.asStack(63));
        }
        scene.idle(30);
        ...
        ElementLink<WorldSectionElement> newBelt = scene.world()
                .showIndependentSection(
                        util.select()
                                .fromTo(3, 3, 2, 0, 3, 4)
                                .add(util.select().fromTo(5, 3, 3, 4, 3, 3)),
                        Direction.DOWN);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            scene.world()
                    .showSectionAndMerge(
                            util.select().position(3, 4, 2 + i), Direction.DOWN, newBelt);
        }

        scene.overlay()
                .showOutlineWithText(util.select().fromTo(3, 1, 1, 3, 2, 3), 80)
                .attachKeyFrame()
                .placeNearTarget()
        ...
        scene.overlay()
                .showControls(tunnelFilterVec.add(0, 0, -1), Pointing.RIGHT, 20)
                .withItem(item1);
        scene.world().modifyBlockEntity(newTunnelPos.north(), tunnelClass, be -> be.getBehaviour(
                        SidedFilteringBehaviour.TYPE)
                .setFilter(Direction.WEST, item1));
        scene.idle(4);
        scene.overlay().showControls(tunnelFilterVec, Pointing.DOWN, 20).withItem(item2);
        scene.world().modifyBlockEntity(newTunnelPos, tunnelClass, be -> be.getBehaviour(
                        SidedFilteringBehaviour.TYPE)
                .setFilter(Direction.WEST, item2));
        scene.idle(4);
        scene.overlay()
                .showControls(tunnelFilterVec.add(0, 0, 1), Pointing.LEFT, 20)
                .withItem(item3);
        scene.world().modifyBlockEntity(newTunnelPos.south(), tunnelClass, be -> be.getBehaviour(
                        SidedFilteringBehaviour.TYPE)
        ...
        scene.overlay()
                .showText(80)
                .pointAt(util.vector().blockSurface(tunnelPos.above().north(), Direction.WEST))
                .placeNearTarget()
    ...
    public static void brassModes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSection(
                        util.select()
                                .fromTo(1, 1, 1, 5, 1, 5)
                                .add(util.select().fromTo(3, 2, 5, 1, 2, 5)),
                        Direction.DOWN);
        scene.idle(10);
        for (int i = 0; i < 3; i++) {
        ...
        for (int i = 0; i < 32; i++) {
            if (i < 30)
                scene.world()
                        .createItemOnBelt(
                                util.grid().at(1, 1, 5),
                                Direction.EAST,
                                new ItemStack(Items.SNOWBALL, 12));
            ...
            if (i == 0) {
                scene.overlay()
                        ...
                        .pointAt(tunnelTop)
                        .placeNearTarget()
                        .text("'Split' will attempt to distribute the stack evenly between"
                                + " available outputs");
            }
            ...
            if (i == 4) {
                scene.overlay()
                        .showControls(modeVec, Pointing.RIGHT, 140)
                        .showing(AllIcons.I_TUNNEL_FORCED_SPLIT);
                scene.world()
                        .modifyBlockEntity(
                                util.grid().at(1, 2, 3), tunnelClass, be -> be.getBehaviour(
                                                ScrollOptionBehaviour.TYPE)
                                        .setValue(
                                                BrassTunnelBlockEntity.SelectionMode.FORCED_SPLIT
                                                        .ordinal()));
            }
            ...
            if (i == 5) {
                scene.overlay()
                        .showText(80)
                        .attachKeyFrame()
                        .text("'Forced Split' will never skip outputs, and instead wait until"
                                + " they are free")
                        .pointAt(util.vector().blockSurface(util.grid().at(1, 1, 2), Direction.UP))
                        .placeNearTarget()
                ...
                scene.world()
                        .setBlock(util.grid().at(1, 1, 0), Blocks.AIR.defaultBlockState(), false);
                scene.world().multiplyKineticSpeed(util.select().everywhere(), 1.5f);
            }
                ...
                scene.world()
                        .modifyBlockEntity(
                                util.grid().at(1, 2, 3), tunnelClass, be -> be.getBehaviour(
                                                ScrollOptionBehaviour.TYPE)
                                        .setValue(
                                                BrassTunnelBlockEntity.SelectionMode.ROUND_ROBIN
                                                        .ordinal()));
                scene.overlay()
                        .showControls(modeVec, Pointing.RIGHT, 140)
                        .showing(AllIcons.I_TUNNEL_ROUND_ROBIN);
                scene.overlay()
                        .showText(80)
                        .attachKeyFrame()
                        .pointAt(tunnelTop)
                        .placeNearTarget()
                        .text("'Round Robin' keeps stacks whole, and cycles through outputs"
                                + " iteratively");
            }
            ...
            if (i == 13) {
                scene.overlay()
                        .showText(60)
                        .text("Once Again, if an output is unable to take more items, it will be"
                                + " skipped")
                        .placeNearTarget()
                        .pointAt(util.vector().blockSurface(util.grid().at(1, 1, 2), Direction.UP))
                        .colored(PonderPalette.GREEN);
                ...
                scene.world()
                        .modifyBlockEntity(
                                util.grid().at(1, 2, 3), tunnelClass, be -> be.getBehaviour(
                                                ScrollOptionBehaviour.TYPE)
                                        .setValue(
                                                BrassTunnelBlockEntity.SelectionMode
                                                        .FORCED_ROUND_ROBIN
                                                        .ordinal()));
            }
                ...
                scene.world()
                        .modifyBlockEntity(
                                util.grid().at(1, 2, 3), tunnelClass, be -> be.getBehaviour(
                                                ScrollOptionBehaviour.TYPE)
                                        .setValue(
                                                BrassTunnelBlockEntity.SelectionMode.PREFER_NEAREST
                                                        .ordinal()));
                scene.world().moveSection(blockage, util.vector().of(1, 0, 0), 10);
                scene.world().setBlock(util.grid().at(1, 1, 0), barrier, false);
                scene.overlay()
                        .showText(70)
                        .attachKeyFrame()
                        .text("'Prefer Nearest' prioritizes the outputs closest to the items'"
                                + " input location")
                        .pointAt(util.vector().blockSurface(util.grid().at(1, 1, 2), Direction.UP))
                        .placeNearTarget()
                        .colored(PonderPalette.GREEN);
            ...
            if (i == 25) {
                ...
                scene.world()
                        .setBlock(util.grid().at(2, 1, 0), Blocks.AIR.defaultBlockState(), false);
            }

            if (i == 26) {
                scene.overlay()
                        .showControls(modeVec, Pointing.RIGHT, 140)
                        .showing(AllIcons.I_TUNNEL_RANDOMIZE);
                scene.world()
                        .modifyBlockEntity(
                                util.grid().at(1, 2, 3), tunnelClass, be -> be.getBehaviour(
                                                ScrollOptionBehaviour.TYPE)
                                        .setValue(
                                                BrassTunnelBlockEntity.SelectionMode.RANDOMIZE
                                                        .ordinal()));
            }
        ...
        scene.overlay()
                .showControls(modeVec, Pointing.RIGHT, 140)
                .showing(AllIcons.I_TUNNEL_SYNCHRONIZE);
        scene.world().modifyBlockEntity(util.grid().at(1, 2, 3), tunnelClass, be -> be.getBehaviour(
                        ScrollOptionBehaviour.TYPE)
