...
public class BeltScenes {
    ...
    public static void beltConnector(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(util.vector().topOf(backEnd), Pointing.DOWN, 57)
                .rightClick()
                .withItem(beltItem);
        scene.idle(7);

        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.GREEN, backEnd, shaftBB.move(backEnd), 42);
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.BLACK, backEndCenter, connectBB, 50);
        scene.idle(20);

        scene.overlay()
                .showControls(util.vector().topOf(frontEnd), Pointing.DOWN, 37)
                .rightClick()
                .withItem(beltItem);
        scene.idle(7);

        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.GREEN, frontEnd, shaftBB.move(frontEnd), 17);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.BLACK, backEndCenter, connectBB.expandTowards(-4, 0, 0), 20);
        scene.idle(20);
        ...
        scene.overlay()
                .showControls(falseSelection, Pointing.DOWN, 37)
                .rightClick()
                .withItem(beltItem);
        scene.idle(7);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.RED, backEnd, shaftBB.move(backEnd.south(1)), 50);
        ...
        scene.overlay()
                .showControls(falseSelection, Pointing.DOWN, 20)
                .rightClick()
                .withItem(beltItem)
                .whileSneaking();
        scene.idle(60);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(shaftLocation), Pointing.DOWN, 50)
                .rightClick()
                .withItem(AllBlocks.SHAFT.asStack());
        scene.idle(7);
        scene.world()
                .modifyBlock(shaftLocation, s -> s.setValue(BeltBlock.PART, BeltPart.PULLEY), true);
        scene.idle(10);
        ...
        scene.idle(7);
        scene.world()
                .modifyBlock(shaftLocation, s -> s.setValue(BeltBlock.PART, BeltPart.MIDDLE), true);
        scene.idle(10);
        scene.overlay()
                .showText(50)
        ...
        scene.overlay()
                .showControls(util.vector().topOf(shaftLocation.east()), Pointing.DOWN, 50)
                .rightClick()
    ...
    public static void directions(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink<WorldSectionElement> belt =
                scene.world().showIndependentSection(firstBelt, Direction.SOUTH);
        ...
        scene.overlay()
                .showText(160)
                .text("These are all possible directions. Belts can span any Length between 2"
                        + " and 20 blocks");
    ...
    public static void transport(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink<EntityElement> item = scene.world()
                .createItemEntity(
                        util.vector().centerOf(0, 4, 2), util.vector().of(0, 0, 0), stack);
        scene.idle(13);
        ...
        ElementLink<ParrotElement> parrot = scene.special()
                .createBirb(
                        util.vector().topOf(0, 1, 2).add(0, -3 / 16f, 0),
    ...
    public static void beltsCanBeEncased(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(util.vector().topOf(beltPos), Pointing.DOWN, 20)
                .rightClick()
        ...
        scene.overlay()
                .showControls(util.vector().topOf(beltPos2), Pointing.DOWN, 20)
                .rightClick()
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(beltPos3, Direction.EAST), Pointing.RIGHT, 20)
                .rightClick()
                .withItem(brassCasingItem);
        scene.idle(7);
        ...
        for (int z = 1; z <= 3; z++) brassBelts.add(beltPos.south(z));
        for (int x = 1; x <= 3; x++) brassBelts.add(beltPos3.east(x).below(x));
        for (int x = 1; x <= 3; x++) andesiteBelts.add(beltPos2.east(x));
        ...
        scene.overlay()
                .showControls(util.vector().topOf(beltPos.south()), Pointing.DOWN, 40)
                .rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.idle(7);
    ...
    public static void depot(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(topOf, Pointing.DOWN, 20)
                .rightClick()
                .withItem(new ItemStack(Items.COPPER_BLOCK));
        scene.idle(7);
        scene.world()
                .createItemOnBeltLike(depotPos, Direction.NORTH, new ItemStack(Items.COPPER_BLOCK));
        scene.idle(10);
        scene.overlay()
                .showText(70)
                .attachKeyFrame()
        ...
        scene.world().removeItemsFromBelt(depotPos);
        scene.world()
                .createItemOnBeltLike(depotPos, Direction.UP, new ItemStack(Items.WATER_BUCKET));
        scene.world()
                .modifyBlockEntityNBT(
                        util.select().position(depotPos.above(2)),
                        SpoutBlockEntity.class,
        ...
        ElementLink<WorldSectionElement> spout = scene.world()
                .showIndependentSection(
                        util.select().position(depotPos.above(2).west()), Direction.SOUTH);
        ...
        Selection fanSelect = util.select()
                .fromTo(4, 1, 3, 5, 2, 2)
        ...
        scene.world()
                .showSection(
                        util.select().fromTo(2, 1, 4, 2, 1, 5).add(util.select().position(2, 0, 5)),
                        Direction.DOWN);
