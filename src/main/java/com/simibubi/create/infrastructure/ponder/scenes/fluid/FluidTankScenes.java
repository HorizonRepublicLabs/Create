...
public class FluidTankScenes {
    ...
    public static void storage(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection kinetics1 =
                util.select().fromTo(2, 1, 5, 2, 1, 3).add(util.select().position(2, 2, 4));
        ...
        Selection pipe = util.select()
                .fromTo(1, 1, 2, 1, 1, 5)
                .add(util.select().position(1, 0, 5))
                .add(util.select().position(2, 1, 2));

        ElementLink<WorldSectionElement> tankLink =
                scene.world().showIndependentSection(tank, Direction.NORTH);
        scene.world().moveSection(tankLink, util.vector().of(0, 0, -1), 0);
        scene.idle(5);
        ElementLink<WorldSectionElement> chocLink =
                scene.world().showIndependentSection(chocolate, Direction.NORTH);
        scene.world().moveSection(chocLink, util.vector().of(2, -4, 3), 0);
        scene.idle(10);

        scene.overlay()
                .showOutline(
                        PonderPalette.GREEN, chocLink, util.select().fromTo(3, 1, 3, 2, 4, 4), 40);
        scene.idle(10);
        scene.overlay()
                .showLine(
                        PonderPalette.GREEN,
                        util.vector().of(3, 1, 2),
                        util.vector().of(2, 1, 3),
                        30);
        scene.overlay()
                .showLine(
                        PonderPalette.GREEN,
                        util.vector().of(3, 3, 2),
                        util.vector().of(2, 5, 3),
                        30);
        scene.overlay()
                .showLine(
                        PonderPalette.GREEN,
                        util.vector().of(4, 3, 2),
                        util.vector().of(4, 5, 3),
                        30);
        scene.overlay()
                .showOutline(
                        PonderPalette.GREEN, tankLink, util.select().fromTo(3, 1, 1, 3, 2, 1), 40);
        scene.idle(10);
        ...
        scene.world()
                .modifyBlockEntity(tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory()
        ...
        scene.world()
                .modifyBlock(pumpPos, s -> s.setValue(PumpBlock.FACING, Direction.SOUTH), false);
        scene.idle(10);
        scene.world().showSection(largeCog1, Direction.UP);
        scene.world().showSection(kinetics1, Direction.WEST);
        scene.idle(10);
        scene.world()
                .setBlock(util.grid().at(1, -1, 5), AllBlocks.FLUID_TANK.getDefaultState(), false);
        ...
        for (int i = 0; i < 4; i++) {
            scene.world()
                    .modifyBlockEntity(
                            tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory()
        ...
        scene.world()
                .modifyBlock(pumpPos, s -> s.setValue(PumpBlock.FACING, Direction.NORTH), true);
        scene.world().propagatePipeChange(pumpPos);
        for (int i = 0; i < 4; i++) {
            scene.world()
                    .modifyBlockEntity(
                            tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory()
                                    .fill(
                                            FluidHelper.copyStackWithAmount(content, 2000),
                                            FluidAction.EXECUTE));
            scene.idle(5);
        }
        scene.idle(40);
        ...
        scene.overlay()
                ...
                .placeNearTarget()
                .pointAt(util.vector()
                        .blockSurface(util.grid().at(2, 1, 1), Direction.DOWN)
                        .add(0, 1 / 8f, 0));
        scene.idle(50);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(2, 2, 2), Direction.NORTH),
                        Pointing.RIGHT,
                        40)
        ...
        scene.idle(7);
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(2, 1, 2, 2, 2, 2), 70)
        ...
        scene.world().modifyBlockEntity(util.grid().at(4, 3, 0), SpoutBlockEntity.class, be -> {
            IFluidHandler handler = be.getLevel()
                    .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
            if (handler != null) handler.fill(content, FluidAction.EXECUTE);
        });
        ...
        scene.idle(7);
        ElementLink<WorldSectionElement> spoutLink =
                scene.world().showIndependentSection(spoutstuff, Direction.SOUTH);
        ElementLink<WorldSectionElement> largeCogLink =
                scene.world().showIndependentSection(largeCog2, Direction.UP);
        scene.world().moveSection(spoutLink, util.vector().of(-1, 0, 1), 0);
        scene.world().moveSection(largeCogLink, util.vector().of(-1, 0, 1), 0);
        scene.idle(20);
        scene.overlay()
                .showOutline(
                        PonderPalette.GREEN, new Object(), util.select().position(2, 1, 1), 50);
        scene.idle(5);
        scene.overlay()
                .showOutline(
                        PonderPalette.GREEN, new Object(), util.select().position(3, 3, 1), 50);
        scene.idle(5);

        scene.overlay()
                .showText(80)
                .text("You can use Basins, Item Drains and Spouts to drain or fill fluid"
                        + " containing items")
                .attachKeyFrame()
        ...
        ItemStack chocBucket = AllFluids.CHOCOLATE
                .get()
                .getFluidType()
                .getBucket(new FluidStack(
                        FluidHelper.convertToStill(AllFluids.CHOCOLATE.get()), 1000));
    ...
    public static void sizes(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        ElementLink<WorldSectionElement> s1 =
                scene.world().showIndependentSection(single, Direction.DOWN);
        ...
        ElementLink<WorldSectionElement> s2 =
                scene.world().showIndependentSection(single, Direction.DOWN);
        scene.world().moveSection(s2, util.vector().of(2, -2, 3), 0);
        scene.idle(5);
        ElementLink<WorldSectionElement> s3 =
                scene.world().showIndependentSection(single, Direction.DOWN);
        scene.world().moveSection(s3, util.vector().of(3, -2, 3), 0);
        scene.idle(5);
        ElementLink<WorldSectionElement> s4 =
                scene.world().showIndependentSection(single, Direction.DOWN);
        ...
        ElementLink<WorldSectionElement> d =
                scene.world().showIndependentSectionImmediately(single2);
        ...
        scene.idle(3);
        ElementLink<WorldSectionElement> s5 =
                scene.world().showIndependentSection(single, Direction.DOWN);
        ...
        ElementLink<WorldSectionElement> t =
                scene.world().showIndependentSectionImmediately(single3);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(3, 3, 1), Direction.NORTH),
                        Pointing.RIGHT,
                        60)
                .rightClick()
        ...
        scene.world()
                .modifyBlocks(
                        full2,
                        s -> s.setValue(FluidTankBlock.SHAPE, FluidTankBlock.Shape.PLAIN),
                        false);
        scene.idle(30);
    ...
    public static void creative(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink<WorldSectionElement> cTankLink =
                scene.world().showIndependentSection(cTank, Direction.DOWN);
