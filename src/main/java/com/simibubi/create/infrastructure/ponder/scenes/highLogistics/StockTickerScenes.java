...
public class StockTickerScenes {
    ...
    public static void stockTicker(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink<WorldSectionElement> linkL =
                scene.world().showIndependentSection(link1S, Direction.DOWN);
        scene.world().moveSection(linkL, util.vector().of(0, -2, 0), 0);
        scene.idle(15);

        ItemStack linkItem = AllBlocks.STOCK_TICKER.asStack();
        scene.overlay()
                .showControls(util.vector().topOf(link1.below(2)), Pointing.DOWN, 50)
                .rightClick()
                .withItem(linkItem);
        scene.idle(5);
        ...
        scene.overlay()
                .showOutline(PonderPalette.BLUE, link1, util.select().position(3, 2, 6), 120);
        scene.overlay()
                .showOutline(
                        PonderPalette.BLUE, link2, util.select().fromTo(4, 2, 6, 5, 3, 6), 120);
        scene.idle(30);

        scene.overlay()
                .showText(100)
                .text("Stock-linked packagers make their attached inventory available to the"
                        + " network")
                .attachKeyFrame()
                .placeNearTarget()
                .pointAt(util.vector().blockSurface(util.grid().at(3, 2, 5), Direction.WEST));
        scene.idle(110);
        ...
        scene.idle(10);
        scene.special()
                .createBirb(util.vector().centerOf(ticker.east()), FacePointOfInterestPose::new);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(ticker.east()), Pointing.DOWN, 50)
                .rightClick();
        scene.idle(10);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(ticker), Pointing.DOWN, 100)
                .rightClick()
                .withItem(filterItem);
        scene.idle(10);
        ...
        scene.overlay()
                .showText(100)
                .text("Using attribute or list filters, categories can be added to the item"
                        + " listings")
                .attachKeyFrame()
                .colored(PonderPalette.BLUE)
                .placeNearTarget()
    ...
    public static void stockTickerAddress(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection belt1 =
                util.select().fromTo(8, 1, 4, 2, 1, 4).add(util.select().position(4, 2, 4));
        ...
        ElementLink<WorldSectionElement> extrasL =
                scene.world().showIndependentSection(postBox, Direction.DOWN);
