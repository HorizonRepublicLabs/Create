...
public class ArmScenes {
    ...
    public static void setup(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(armSel, 70)
                .attachKeyFrame()
                .colored(PonderPalette.RED)
                .text("Mechanical Arms have to be assigned their in- and outputs before they are"
                        + " placed")
                .pointAt(armSurface)
                .placeNearTarget();
        scene.idle(80);
        ...
        scene.idle(20);
        scene.overlay()
                .showControls(depotSurface, Pointing.RIGHT, 50)
                .rightClick()
                .withItem(armItem);
        scene.idle(7);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, new Object(), depotBounds.move(4, 2, 1), 400);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(outputDepot, Direction.NORTH),
                        Pointing.RIGHT,
                        20)
                .rightClick()
                .withItem(armItem);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, second, depotBounds.move(0, 2, 1), 100);
        scene.idle(25);
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(outputDepot, Direction.NORTH),
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.OUTPUT, second, depotBounds.move(0, 2, 1), 280);
        scene.overlay()
                .showText(70)
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, new Object(), depotBounds.move(1, 1, 0), 43);
        ...
        scene.idle(35);
        scene.overlay()
                .showControls(util.vector().topOf(util.grid().at(1, 1, 0)), Pointing.DOWN, 30)
                .leftClick()
                .withItem(armItem);
        scene.idle(50);
        ...
        scene.world()
                .showSection(
                        util.select().fromTo(2, 1, 5, 2, 1, 3).add(util.select().position(2, 0, 5)),
                        Direction.DOWN);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.OUTPUT, new Object(), depotBounds.move(0, 2, 1), 60);
        scene.idle(5);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, new Object(), depotBounds.move(4, 2, 1), 60);
        scene.idle(5);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.OUTPUT, new Object(), depotBounds.move(1, 1, 0), 60);
        scene.idle(5);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, new Object(), depotBounds.move(1, 3, 4), 60);
        scene.idle(5);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, new Object(), depotBounds.move(4, 1, 2), 60);
        scene.idle(5);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.RED, in, chestBounds.move(4, 2, 3), 120);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.RED, out, chestBounds.move(0, 2, 3), 120);
        scene.overlay()
                .showText(80)
                .attachKeyFrame()
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.INPUT, in, depotBounds.move(4, 1, 2), 80);
        scene.idle(5);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.OUTPUT, out, funnelBounds.move(0, 2, 2), 80);
        scene.idle(5);
        ...
        scene.overlay()
                .showControls(util.vector().topOf(inputDepot), Pointing.RIGHT, 30)
                .withItem(sword);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(0, 2, 3), Direction.WEST),
                        Pointing.LEFT,
                        30)
                .withItem(sword);
    }
    ...
    public static void filtering(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                scene.world()
                        .showSection(
                                util.select().position(y == 1 ? x + 3 : 5 - x, y + 1, 1),
                                Direction.DOWN);
                scene.idle(2);
            }
        }
        ...
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(2, 1, 4, 1, 1, 4), 60)
                .text("Inputs")
                .placeNearTarget()
                .colored(PonderPalette.INPUT);
        scene.idle(50);
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(5, 3, 1, 3, 1, 1), 40)
                .text("Outputs")
                .placeNearTarget()
                .colored(PonderPalette.OUTPUT);
        scene.idle(50);
        ...
        scene.overlay()
                .showText(80)
                .attachKeyFrame()
                .text("Sometimes it is desirable to restrict targets of the Arm by matching a"
                        + " filter")
                .placeNearTarget()
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(4, 1, 4), 80)
                .colored(PonderPalette.RED)
        ...
        for (int y = 0; y < 3; y++) {
            scene.world()
                    .showSection(util.select().fromTo(5, y + 1, 2, 3, y + 1, 2), Direction.NORTH);
            scene.idle(2);
        }
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                ...
                scene.overlay()
                        .showControls(filterSlot.add(2 - x, -y, 0), Pointing.LEFT, 5)
                        .rightClick()
                        .withItem(item);
                scene.idle(7);
                scene.world()
                        .setFilterData(
                                util.select().position(5 - x, 3 - y, 2),
        ...
        for (int i = 0; i < 4; i++) {
            ...
            scene.world()
                    .instructArm(armPos, Phase.SEARCH_INPUTS, i == 3 ? ItemStack.EMPTY : sand, -1);
            scene.world()
                    .modifyBlockEntity(
                            funnelPos.north(),
                            MechanicalCrafterBlockEntity.class,
                            mct -> mct.getInventory().insertItem(0, sand.copy(), false));
            scene.idle(10);
        }
        ...
        for (int i = 0; i < 5; i++) {
            ...
            scene.world()
                    .instructArm(
                            armPos, Phase.SEARCH_INPUTS, i == 4 ? ItemStack.EMPTY : sulphur, -1);
            scene.world()
                    .modifyBlockEntity(
                            funnelPos.north(),
                            MechanicalCrafterBlockEntity.class,
                            mct -> mct.getInventory().insertItem(0, sulphur.copy(), false));
            scene.idle(10);
        }
    ...
    public static void modes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.INPUT, depotBox, depotBox.move(1, 1, 4), 60);
        scene.overlay()
                .showText(30)
                .text("Input")
        ...
        scene.idle(40);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.OUTPUT, depotBox, beltBox.move(2, 1, 2), 40);
        scene.overlay()
                .showText(40)
                .text("Outputs")
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (i == 2) {
                scene.overlay()
                        .showText(60)
                        .attachKeyFrame()
                        .text("Round Robin mode simply cycles through all outputs that are"
                                + " available")
                        .pointAt(util.vector().blockSurface(util.grid().at(2, 1, 2), Direction.UP))
                        .placeNearTarget()
                        .colored(PonderPalette.OUTPUT);
            ...
            if (i == 12) {
                ...
                scene.world()
                        .setBlock(
                                util.grid().at(3, 1, 0), Blocks.BARRIER.defaultBlockState(), false);
            }
            ...
            if (i == 13) {
                scene.world()
                        .setBlock(
                                util.grid().at(2, 1, 0), Blocks.BARRIER.defaultBlockState(), false);
                ...
                scene.overlay()
                        .showText(60)
                        .attachKeyFrame()
                        .text("Prefer First prioritizes the outputs selected earliest when"
                                + " configuring this Arm")
                        .pointAt(util.vector().blockSurface(util.grid().at(3, 1, 2), Direction.UP))
                        .placeNearTarget()
                        .colored(PonderPalette.GREEN);
                index = 0;
            ...
            if (i == 9) {
                scene.overlay()
                        .showText(80)
                        .attachKeyFrame()
                        .text("Forced Round Robin mode will never skip outputs, and instead wait"
                                + " until they are free")
                        .pointAt(util.vector().blockSurface(util.grid().at(3, 1, 2), Direction.UP))
                        .placeNearTarget()
                        .colored(PonderPalette.RED);
                ...
                scene.world().moveSection(blockage, util.vector().of(1, 0, 0), 10);
                scene.world()
                        .setBlock(util.grid().at(3, 1, 0), Blocks.AIR.defaultBlockState(), false);
                scene.idle(50);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (i == 2) {
                scene.overlay()
                        ...
                        .pointAt(util.vector().topOf(armPos))
                        .placeNearTarget()
                        .text("Thus, a negative pulse can be used to trigger exactly one"
                                + " activation cycle");
            }
