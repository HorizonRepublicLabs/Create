...
public class PackagerScenes {
    ...
    public static void packager(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection cogNBelt =
                util.select().fromTo(6, 1, 2, 0, 1, 2).add(util.select().position(6, 1, 3));
        ...
        ElementLink<WorldSectionElement> chestL =
                scene.world().showIndependentSection(chest1, Direction.DOWN);
        scene.world().moveSection(chestL, util.vector().of(-2, -1, 0), 0);
        scene.idle(10);
        ElementLink<WorldSectionElement> packagerL =
                scene.world().showIndependentSection(packager1S, Direction.SOUTH);
        scene.world().moveSection(packagerL, util.vector().of(-2, -1, 0), 0);
        scene.idle(20);

        ItemStack dirt = new ItemStack(Items.DIRT);
        scene.overlay()
                .showControls(util.vector().of(2.5, 3, 2.5), Pointing.DOWN, 40)
                .withItem(dirt);
        scene.idle(20);
        ...
        scene.idle(20);
        scene.overlay()
                .showControls(util.vector().topOf(util.grid().at(1, 2, 4)), Pointing.DOWN, 40)
                .withItem(dirt);
        scene.idle(20);

        scene.overlay()
                .showText(90)
                .text("Packages inserted will be destroyed, unpacking the contents into the"
                        + " inventory")
                .attachKeyFrame()
                .placeNearTarget()
        ...
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(1, 2, 3, 1, 2, 4), 90)
    ...
    public static void packagerAddress(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(40);
        PonderHilo.packageHopsOffBelt(
                scene, util.grid().at(4, 1, 0), Direction.NORTH, warehouseBox);
        scene.idle(40);
        scene.world().multiplyKineticSpeed(util.select().everywhere(), 1 / 32f);
        scene.overlay()
                .showText(100)
