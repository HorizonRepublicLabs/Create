...
public class PumpScenes {
    ...
    public static void flow(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .modifyBlock(
                        pumpPos,
                        s -> s.setValue(
                                PumpBlock.FACING, s.getValue(PumpBlock.FACING).getOpposite()),
                        false);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, in, bb1.move(util.vector().centerOf(3, 1, 3)), 3);
        scene.idle(2);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, in, bb2.move(util.vector().centerOf(3, 1, 2)), 50);
        scene.idle(10);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.OUTPUT, out, bb2.move(util.vector().centerOf(1, 1, 2)), 50);
        scene.idle(10);
        ...
        scene.idle(25);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.RED, in, new AABB(leverPos.below()), 30);
        scene.idle(45);

        scene.overlay()
                .showControls(util.vector().topOf(pumpPos), Pointing.DOWN, 40)
                .rightClick()
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.INPUT, in, bb2.move(util.vector().centerOf(3, 1, 2)), 30);
        scene.idle(15);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.OUTPUT,
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.OUTPUT, out, bb2.move(util.vector().centerOf(1, 1, 2)), 30);
        scene.idle(25);
    }
    ...
    public static void speed(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection cogs =
                util.select().fromTo(5, 1, 4, 2, 1, 4).add(util.select().position(2, 1, 3));
        ...
        scene.world()
                .modifyBlock(pumpPos, s -> s.setValue(PumpBlock.FACING, Direction.WEST), false);
        scene.world()
                .modifyBlockEntity(
                        util.grid().at(0, 1, 2),
        ...
        scene.idle(10);
        ElementLink<WorldSectionElement> mp1 =
                scene.world().showIndependentSection(megapipe1, Direction.EAST);
        scene.world().moveSection(mp1, util.vector().of(0, -3, 0), 0);
        scene.idle(5);
        ElementLink<WorldSectionElement> mp2 =
                scene.world().showIndependentSection(megapipe2, Direction.WEST);
        ...
        scene.overlay()
                ...
                .pointAt(util.vector().topOf(pumpPos))
                .placeNearTarget()
                .text("Regardless of speed, Mechanical Pumps affect pipes connected up to 16"
                        + " blocks away");
        scene.idle(75);
        ...
        scene.world()
                .modifyBlock(
                        pumpPos.south(), s -> s.setValue(PumpBlock.FACING, Direction.WEST), true);
        scene.world().setKineticSpeed(util.select().everywhere(), 0);
        scene.world().showSection(northPump, Direction.DOWN);
        scene.world()
                .modifyBlock(
                        pumpPos.north(), s -> s.setValue(PumpBlock.FACING, Direction.EAST), false);
        scene.idle(4);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(2, 1, 1, 2, 1, 3), 60)
                .attachKeyFrame()
        ...
        scene.idle(30);
        scene.overlay()
                .showControls(util.vector().topOf(pumpPos.north()), Pointing.DOWN, 30)
                .rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.idle(7);
        scene.world()
                .modifyBlock(
                        pumpPos.north(), s -> s.setValue(PumpBlock.FACING, Direction.WEST), true);
        scene.idle(30);

        scene.overlay()
                .showText(70)
                .attachKeyFrame()
                .pointAt(util.vector().topOf(pumpPos.north()).subtract(0.5f, 0.125f, 0))
                .placeNearTarget()
