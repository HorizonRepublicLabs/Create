...
public class ChainDriveScenes {
    ...
    public static void chainDriveAsRelay(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.addKeyframe();
        scene.overlay()
                .showControls(util.vector().topOf(rotatedECD), Pointing.DOWN, 30)
                .rightClick()
    ...
    public static void adjustableChainGearshift(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink<WorldSectionElement> lever = scene.world()
                .showIndependentSection(
                        util.select().fromTo(leverPos, leverPos.south()), Direction.UP);
        ...
        scene.world()
                .showSection(
                        util.select()
                                .fromTo(eastDrive, eastDrive.west(2))
                                .add(util.select().position(eastDrive.above())),
                        Direction.DOWN);
        scene.idle(10);
        ...
        scene.world()
                .showSection(util.select().fromTo(eastGauge, eastGauge.below()), Direction.DOWN);
        scene.idle(5);
        scene.world()
                .showSection(
                        util.select().fromTo(middleGauge, middleGauge.below()), Direction.DOWN);
        scene.idle(5);
        ...
        scene.world()
                .modifyKineticSpeed(
                        util.select().fromTo(westGauge.below(), middleGauge), f -> 2 * f);
        ...
        scene.overlay()
                .showText(80)
                .text("When Powered, the speed transmitted to other Chain Drives in the row is"
                        + " doubled")
                .attachKeyFrame()
                .placeNearTarget()
        ...
        scene.world()
                .hideSection(util.select().fromTo(eastDrive, eastDrive.west(2)), Direction.SOUTH);
        scene.idle(15);
        scene.world().toggleRedstonePower(util.select().fromTo(leverPos, leverPos.south(2)));
        Selection newDriveSelect =
                util.select().fromTo(eastDrive.south(2), eastDrive.south(2).west(2));
        ElementLink<WorldSectionElement> drives =
                scene.world().showIndependentSection(newDriveSelect, Direction.NORTH);
        scene.world()
                .modifyKineticSpeed(
                        util.select().fromTo(westGauge.below(), middleGauge), f -> .5f * f);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.MEDIUM, eastDrive, bb.expandTowards(-15 / 16f, 0, 0), 160);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.SLOW, eastDrive.west(), bb.move(-2, 0, 0), 160);
        scene.idle(20);

        scene.overlay()
                .showText(80)
                .text("Whenever the Powered Gearshift is not at the source, its speed will be"
                        + " halved instead")
                .attachKeyFrame()
                .placeNearTarget()
        ...
        for (BlockPos gauge : new BlockPos[] {eastGauge, middleGauge, westGauge}) {
            ...
            scene.overlay()
                    ...
                    .sharedText(
                            gauge == westGauge
                                    ? "rpm8"
                                    : gauge == eastGauge ? "rpm16_source" : "rpm16")
        ...
        scene.overlay()
                .showText(100)
                .text("In both cases, Chain Drives in the row always run at 2x the speed of the"
                        + " Powered Gearshift")
                .attachKeyFrame()
                .placeNearTarget()
                .pointAt(util.vector().blockSurface(eastDrive.west(2), Direction.WEST));
        ...
        scene.world()
                .showSection(util.select().fromTo(analogPos, analogPos.south()), Direction.DOWN);
        ...
        scene.world()
                .modifyBlock(analogPos.south(), s -> s.setValue(RedStoneWireBlock.POWER, 8), false);
        scene.world().toggleRedstonePower(util.select().position(1, 1, 4));
        ...
        scene.overlay()
                .showText(100)
                .text("Using analog signals, the ratio can be adjusted more precisely between 1"
                        + " and 2")
                .attachKeyFrame()
                .placeNearTarget()
                .pointAt(util.vector().blockSurface(eastDrive.west(2), Direction.WEST));
        ...
        for (BlockPos gauge : new BlockPos[] {eastGauge, middleGauge, westGauge}) {
            scene.idle(5);
            TextElementBuilder textBuilder = scene.overlay()
                    .showText(180)
                    .colored(gauge == westGauge ? PonderPalette.SLOW : PonderPalette.MEDIUM)
                    .placeNearTarget()
                    .pointAt(util.vector().blockSurface(gauge, Direction.NORTH));
            if (gauge == westGauge) textBuilder.text("12 RPM");
            else textBuilder.sharedText(gauge == eastGauge ? "rpm16_source" : "rpm16");
        }
    }
}
