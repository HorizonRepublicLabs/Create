...
public class DetectorScenes {
    ...
    public static void smartObserver(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        ElementLink<WorldSectionElement> chestLink =
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(observerPos.north(), Direction.NORTH),
                        Pointing.RIGHT,
                        40)
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(observerPos, Direction.UP), Pointing.DOWN, 60)
                .rightClick()
                .withItem(amethystItem);
        scene.idle(7);
        scene.world()
                .setFilterData(
                        util.select().position(observerPos),
        ...
        scene.idle(10);
        ElementLink<WorldSectionElement> amethystLink =
                scene.world().showIndependentSection(amethyst, Direction.EAST);
        ...
        scene.overlay()
                ...
                .attachKeyFrame()
                .pointAt(util.vector().blockSurface(observerPos.above().east(), Direction.WEST))
                .placeNearTarget();
        scene.idle(60);
    ...
    public static void thresholdSwitch(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection cogs =
                util.select().fromTo(4, 0, 6, 5, 0, 6).add(util.select().position(5, 0, 5));
        ...
        Selection basePlate =
                util.select().fromTo(0, 0, 1, 2, 0, 5).add(util.select().fromTo(4, 0, 5, 4, 0, 1));

        scene.world().showSection(basePlate, Direction.UP);
        ElementLink<WorldSectionElement> stripLink =
                scene.world().showIndependentSection(baseStrip, Direction.UP);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (i % 2 == 1) scene.world().cycleBlockProperty(switchPos, ThresholdSwitchBlock.LEVEL);
        }
            ...
            if (i % 3 == 1)
                scene.world()
                        .modifyBlock(
                                switchPos,
                                s -> s.setValue(
                                        ThresholdSwitchBlock.LEVEL,
                                        s.getValue(ThresholdSwitchBlock.LEVEL) - 1),
                                false);
        }
        scene.world().multiplyKineticSpeed(util.select().everywhere(), 1 / 8f);
        ...
        scene.overlay()
                .showText(90)
                .text("The redstone output can now be used to control item supply, keeping the"
                        + " buffer filled")
                .pointAt(util.vector().blockSurface(switchPos.north(), Direction.DOWN))
                .attachKeyFrame()
                .placeNearTarget();
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(switchPos, Direction.UP), Pointing.DOWN, 60)
                .rightClick();
        scene.idle(7);
        scene.overlay()
                .showText(70)
        ...
        ElementLink<WorldSectionElement> tankLink =
                scene.world().showIndependentSection(fluidTank, Direction.DOWN);
        ...
        scene.idle(15);
        ElementLink<WorldSectionElement> hole =
                scene.world().makeSectionIndependent(util.select().position(2, 0, 3));
