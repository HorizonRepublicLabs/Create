...
public class GantryScenes {
    ...
    private static void intro(SceneBuilder builder, SceneBuildingUtil util, boolean pinion) {
        ...
        String text = pinion
                ? "Gantry Carriages can mount to and slide along a Gantry Shaft."
                : "Gantry Shafts form the basis of a gantry setup. Attached Carriages will move"
                        + " along them.";
        ...
        scene.world()
                .showSectionAndMerge(
                        util.select().layersFrom(3).substract(planks), Direction.DOWN, gantry);
        scene.world()
                .replaceBlocks(
                        util.select().fromTo(5, 3, 2, 3, 4, 2),
        ...
        scene.overlay()
                .showOutline(
                        ...
                        40);
        scene.overlay()
                .showControls(util.vector().centerOf(util.grid().at(3, 3, 2)), Pointing.UP, 40)
    ...
    public static void direction(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .modifyBlocks(
                        ...
                        s -> s.hasProperty(RedStoneWireBlock.POWER)
                                ? s.setValue(RedStoneWireBlock.POWER, 15)
                                : s,
                        false);
    ...
    public static void subgantry(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSection(
                        util.select()
                                ...
                                .add(util.select().fromTo(2, 1, 3, 4, 1, 3)),
                        Direction.UP);
        scene.idle(10);
        ...
        scene.world()
                .showSectionAndMerge(
                        util.select().fromTo(0, 1, 2, 4, 1, 2), Direction.EAST, gantry);
        scene.idle(15);
