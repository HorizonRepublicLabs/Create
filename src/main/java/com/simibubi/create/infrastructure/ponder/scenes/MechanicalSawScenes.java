...
public class MechanicalSawScenes {
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .modifyBlockEntityNBT(
                        sawSelect, SawBlockEntity.class, nbt -> nbt.putInt("RecipeIndex", 0));
        ...
        ElementLink<EntityElement> logItem =
                scene.world().createItemEntity(itemSpawn, util.vector().of(0, 0, 0), log);
        scene.idle(12);
        ...
        scene.world()
                .modifyEntity(logItem, e -> e.setDeltaMovement(util.vector().of(0.05, 0.2, 0)));
        scene.idle(12);
        ...
        scene.idle(12);
        scene.overlay()
                .showControls(itemSpawn.add(2, 0, 0), Pointing.DOWN, 20)
                .withItem(strippedLog);
        scene.idle(30);
        ...
        scene.world()
                .setBlock(
                        shaftPos,
                        AllBlocks.COGWHEEL.getDefaultState().setValue(ShaftBlock.AXIS, Axis.Z),
                        true);
        scene.idle(3);
        scene.addKeyframe();
        ...
        ElementLink<WorldSectionElement> beltSection =
        ...
        scene.overlay()
                .showText(80)
                .attachKeyFrame()
                .text("When an ingredient has multiple possible outcomes, the filter slot can"
                        + " specify it")
                .pointAt(filter)
                .placeNearTarget();
        scene.idle(90);
    ...
    public static void treeCutting(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .setBlock(util.grid().at(2, 0, 2), Blocks.GRASS_BLOCK.defaultBlockState(), false);
        scene.world()
                .showSection(
                        util.select()
                                .layer(0)
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (i == 1) {
                scene.overlay()
                        ...
                        .text("When given Rotational Force, a Mechanical Saw will cut trees"
        ...
        scene.world()
                .replaceBlocks(
                        util.select().fromTo(2, 2, 2, 2, 6, 2),
        ...
        scene.world()
                .replaceBlocks(util.select().layersFrom(4), Blocks.AIR.defaultBlockState(), false);
        ...
        scene.world()
                .hideSection(
                        util.select().layersFrom(2).add(util.select().fromTo(2, 1, 2, 1, 1, 3)),
                        Direction.UP);
        scene.world().modifyEntities(ItemEntity.class, Entity::discard);
        scene.idle(15);
        scene.world()
                .setBlocks(
                        util.select().fromTo(2, 1, 2, 1, 20, 3),
        ...
        scene.world()
                .hideSection(
                        util.select()
                                .fromTo(2, 1, 2, 1, 1, 3)
                                .substract(util.select().position(breakingPos)),
                        Direction.WEST);
        scene.idle(10);
        scene.overlay()
                .showOutlineWithText(util.select().position(breakingPos), 90)
                .attachKeyFrame()
                .colored(PonderPalette.GREEN)
                .placeNearTarget()
                .text("In order to cut the tree fully, the Saw has to break the last block"
        ...
        for (int i = 0; i < 30; i++) {
            scene.world()
                    .replaceBlocks(
                            util.select().fromTo(2, i + 1, 2, 1, i + 1, 3),
                            Blocks.AIR.defaultBlockState(),
                            true);
            for (int x = 1; x <= 2; x++) {
                for (int z = 2; z <= 3; z++) {
                    ...
                    float distance =
                            (float) dropPos.distanceTo(util.vector().centerOf(breakingPos));
                    scene.world()
                            .createItemEntity(
                                    dropPos,
                                    util.vector().of(-distance / 20, 0, 0),
    ...
    public static void contraption(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.scaleSceneView(.9f);
        scene.world()
                .setBlock(util.grid().at(2, 0, 3), Blocks.GRASS_BLOCK.defaultBlockState(), false);
        scene.world()
                .showSection(
                        util.select()
                                .layer(0)
        ...
        scene.idle(5);
        scene.world()
                .showSectionAndMerge(util.select().position(4, 1, 3), Direction.EAST, contraption);
        scene.idle(5);
        scene.world()
                .showSectionAndMerge(util.select().position(4, 1, 2), Direction.EAST, contraption);
        scene.idle(5);
        ...
        Selection tree =
                util.select().fromTo(2, 1, 3, 2, 7, 3).add(util.select().layersFrom(3));
        ...
        scene.world()
                .replaceBlocks(
                        util.select().fromTo(2, 2, 3, 2, 6, 3),
                        Blocks.AIR.defaultBlockState(),
                        true);
        ...
        scene.idle(10);
        scene.overlay()
                .showControls(util.vector().topOf(3, 2, 2), Pointing.DOWN, 60)
