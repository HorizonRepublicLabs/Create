...
public class BearingScenes {
    ...
    public static void windmillsAsSource(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .setBlock(
                        ...
                        AllBlocks.SAIL
                                .getDefaultState()
                                .setValue(SailBlock.FACING, Direction.NORTH),
                        false);
        scene.idle(5);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(anchorPos), 60)
        ...
        for (Direction d : Iterate.directions)
            if (d.getAxis() != Axis.Z)
                scene.world()
                        .showSectionAndMerge(
                                util.select()
                                        .fromTo(anchorPos.relative(d, 1), anchorPos.relative(d, 2)),
                                d.getOpposite(),
                                structure);
        scene.idle(10);
        ...
        scene.world()
                .showSectionAndMerge(
                        util.select()
                                .fromTo(
                                        anchorPos.above().east(),
                                        anchorPos.above(3).east()),
                        Direction.WEST,
                        structure);
        scene.world()
                .showSectionAndMerge(
                        util.select()
                                .fromTo(
                                        anchorPos.below().west(),
                                        anchorPos.below(3).west()),
                        Direction.EAST,
                        structure);
        scene.world()
                .showSectionAndMerge(
                        util.select()
                                .fromTo(
                                        anchorPos.east().below(),
                                        anchorPos.east(3).below()),
                        Direction.UP,
                        structure);
        scene.world()
                .showSectionAndMerge(
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.GREEN, bb1, bb1.expandTowards(-4, 0, 0), 75);
        scene.idle(5);
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.GREEN, bb2, bb2, 1);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.GREEN, bb2, bb2.expandTowards(0, -4, 0), 80);
        scene.idle(10);
        scene.overlay()
                .showControls(util.vector().centerOf(util.grid().at(5, 2, 0)), Pointing.RIGHT, 40)
        ...
        scene.overlay()
                .showControls(util.vector().topOf(windmill).subtract(.5, 0, 0), Pointing.DOWN, 60)
                .rightClick();
        scene.idle(7);
        ...
        scene.overlay()
                .showText(60)
                .pointAt(util.vector().topOf(windmill).subtract(.5, 0, 0))
                .placeNearTarget()
                .attachKeyFrame()
                .text("Activated with Right-Click, the Windmill Bearing will start providing"
                        + " Rotational Force");
        scene.idle(70);
    ...
    public static void windmillsAnyStructure(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .showSectionAndMerge(
                        util.select().fromTo(3, 2, 2, 3, 3, 1), Direction.SOUTH, contraption);
        scene.world()
                .showSectionAndMerge(
                        util.select().fromTo(3, 2, 4, 3, 3, 5), Direction.NORTH, contraption);
        scene.idle(5);
        scene.world()
                .showSectionAndMerge(util.select().position(3, 1, 5), Direction.NORTH, contraption);
        scene.world()
                .showSectionAndMerge(util.select().position(3, 4, 2), Direction.DOWN, contraption);
        scene.idle(5);
        scene.world()
                .showSectionAndMerge(util.select().position(2, 1, 5), Direction.EAST, contraption);
        scene.world()
                .showSectionAndMerge(util.select().position(3, 3, 3), Direction.DOWN, contraption);
        scene.idle(5);
        ...
        scene.overlay()
                .showOutline(
                        PonderPalette.BLUE, bearingPos, util.select().fromTo(3, 2, 1, 3, 3, 2), 80);
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(3, 2, 4, 3, 3, 5), 80)
                .colored(PonderPalette.BLUE)
                .attachKeyFrame()
                .text("Any Structure can count as a valid Windmill, as long as it contains at"
                        + " least 8 sail-like Blocks.");

        scene.idle(90);
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(bearingPos, Direction.WEST), Pointing.LEFT, 40)
                .rightClick();
        scene.idle(7);
    ...
    public static void mechanicalBearing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(bearingPos.above()), 60)
                .colored(PonderPalette.GREEN)
        ...
        ElementLink<WorldSectionElement> plank = scene.world()
                .showIndependentSection(
                        util.select().position(bearingPos.above().east().north()), Direction.DOWN);
        ...
        scene.overlay()
                ...
                .placeNearTarget()
                .attachKeyFrame()
                .text("Upon receiving Rotational Force, it will assemble it into a Rotating"
                        + " Contraption");
        scene.idle(37 * 2);
        ...
        ElementLink<WorldSectionElement> contraption = scene.world()
                .showIndependentSection(
                        util.select().layersFrom(3).substract(plank2), Direction.DOWN);
        scene.world()
                .replaceBlocks(
                        util.select().fromTo(2, 4, 3, 4, 3, 3),
        ...
        scene.overlay()
                .showOutline(
                        ...
                        40);
        scene.overlay()
                .showControls(util.vector().centerOf(util.grid().at(4, 3, 3)), Pointing.RIGHT, 40)
                .withItem(AllItems.SUPER_GLUE.asStack());
    ...
    public static void bearingModes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                ...
                .attachKeyFrame()
                .text("When Stopped, the Bearing will place the structure at the nearest"
                        + " grid-aligned Angle");
        scene.idle(110);

        Vec3 blockSurface =
                util.vector().blockSurface(bearingPos, Direction.NORTH).add(0, 2 / 16f, 0);
        ...
        scene.overlay()
                ...
                .text("It can be configured never to revert to solid blocks, or only near the"
                        + " angle it started at");
        scene.idle(90);
    }
    ...
    public static void stabilizedBearings(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .configureCenterOfRotation(contraption, util.vector().centerOf(parentBearingPos));
        scene.idle(20);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(bearingPos), 60)
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(bearingPos.north()), 60)
                .colored(PonderPalette.GREEN)
        ...
        scene.world()
                .configureCenterOfRotation(
                        subContraption, util.vector().centerOf(parentBearingPos));
    ...
    public static void clockwork(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                ...
                .placeNearTarget()
                .attachKeyFrame()
                .text("Upon receiving Rotational Force, the structure will be rotated according"
        ...
        scene.overlay()
                .showControls(util.vector().topOf(bearingPos), Pointing.DOWN, 60)
                .rightClick();
        scene.idle(7);
        ...
        scene.world()
                .replaceBlocks(
                        util.select().fromTo(3, 3, 1, 3, 4, 2),
                        Blocks.OAK_PLANKS.defaultBlockState(),
                        false);
        ...
        scene.idle(15);
        scene.overlay()
                .showControls(util.vector().centerOf(util.grid().at(3, 4, 1)), Pointing.RIGHT, 40)
                .withItem(AllItems.SUPER_GLUE.asStack());
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(3, 3, 1, 3, 4, 2), 80)
                .placeNearTarget()
                .attachKeyFrame()
                .colored(PonderPalette.GREEN)
                .sharedText("movement_anchors");
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(bearingPos, Direction.WEST), Pointing.LEFT, 20)
                .rightClick();
        scene.idle(7);
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(bearingPos.north(3)), 80)
                .placeNearTarget()
                .colored(PonderPalette.BLUE)
                .attachKeyFrame()
        ...
        scene.world()
                .setBlock(
                        ...
                        Blocks.STONE_BRICK_WALL
                                .defaultBlockState()
                                .setValue(WallBlock.SOUTH_WALL, WallSide.TALL),
                        false);
        ...
        scene.overlay()
                .showOutline(
                        PonderPalette.BLUE, minuteHand, util.select().fromTo(3, 3, 0, 3, 6, 0), 85);
        scene.overlay()
                .showOutlineWithText(util.select().fromTo(3, 3, 1, 3, 4, 2), 80)
                .placeNearTarget()
                .colored(PonderPalette.GREEN)
        ...
        for (int i = 0; i < 40; i++) {
            scene.idle(23);
            scene.world().rotateSection(minuteHand, 0, 0, 6, 3);
            if (i == 29) scene.world().rotateSection(hourHand, 0, 0, 30, 20);
        }
    }
    ...
    private static void sails(SceneBuilder builder, SceneBuildingUtil util, boolean frame) {
            ...
            for (Direction d : Iterate.horizontalDirections) {
                ...
                if (frame)
                    scene.world()
                            .modifyBlock(
                                    location,
                                    s -> AllBlocks.SAIL_FRAME
                                            .getDefaultState()
                                            .setValue(
                                                    SailBlock.FACING, s.getValue(SailBlock.FACING)),
                                    false);
                scene.world()
                        .showSectionAndMerge(
                                util.select().position(location), d.getOpposite(), plank);
                scene.idle(2);
            }
        }
        ...
        if (!frame) {
            ...
            scene.overlay()
                    .showControls(
                            util.vector().blockSurface(util.grid().at(2, 3, 1), Direction.NORTH),
                            Pointing.RIGHT,
                            30)
