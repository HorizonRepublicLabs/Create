...
public class PostboxScenes {
    ...
    public static void postbox(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        for (int i = 7; i >= 0; i--) {
            scene.world()
                    .showSectionAndMerge(util.select().position(i, 1, 5), Direction.DOWN, tracksL);
            scene.idle(1);
        }
        scene.idle(5);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.WHITE, box, bb1.inflate(0.025, 0.025, 0.025), 50);
        scene.idle(26);
        ...
        ElementLink<WorldSectionElement> postboxE =
                scene.world().showIndependentSection(boxS, Direction.DOWN);
        scene.world().moveSection(postboxE, util.vector().of(0, -1, 0), 0);

        scene.idle(15);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.GREEN, box, bb1.inflate(0.025, 0.025, 0.025), 50);
        ...
        scene.idle(10);
        scene.overlay()
                .showLine(PonderPalette.GREEN, util.vector().topOf(box.below()), fromTarget, 40);
        scene.idle(45);

        scene.overlay()
                .showControls(util.vector().topOf(box.below()), Pointing.DOWN, 40)
                .rightClick();
        scene.idle(7);
        ...
        scene.overlay()
                ...
                .pointAt(util.vector().topOf(box.below()))
                .placeNearTarget();
        scene.idle(80);
        ...
        scene.overlay()
                ...
                .colored(PonderPalette.BLUE)
                .text("Warehouse")
                .pointAt(util.vector().blockSurface(box, Direction.NORTH).add(-.5, 0, 0))
                .placeNearTarget();
        scene.idle(5);
        scene.overlay()
                .showText(40)
        ...
        ElementLink<WorldSectionElement> train1L =
        ...
        scene.idle(14);
        ElementLink<WorldSectionElement> train2L =
                scene.world().showIndependentSection(train2, null);
        ...
        scene.overlay()
                ...
                .attachKeyFrame()
                .pointAt(util.vector().topOf(6, 2, 4))
                .placeNearTarget();
        scene.idle(50);
        ...
        scene.overlay()
                ...
                .colored(PonderPalette.OUTPUT)
                .text("\u2192 Warehouse")
                .pointAt(util.vector().centerOf(3, 2, 2).add(0, -.25, 0))
                .placeNearTarget();

        scene.idle(50);

        ElementLink<WorldSectionElement> tracksL2 =
                scene.world().showIndependentSection(tracks, Direction.EAST);
        ...
        ElementLink<WorldSectionElement> outpostL =
                scene.world().showIndependentSection(glass, Direction.UP);
        scene.world().moveSection(outpostL, util.vector().of(-4, -1, -1), 0);
        scene.idle(1);
        scene.world().moveSection(outpostL, util.vector().of(4, 0, 0), 40);
        scene.idle(9);
        ElementLink<WorldSectionElement> stationL =
                scene.world().showIndependentSection(stationS.add(box2S), Direction.DOWN);
