...
public class ChuteScenes {
    ...
    public static void downward(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .createItemEntity(
                        util.vector().centerOf(util.grid().at(3, 3, 2)),
                        util.vector().of(0, -0.1, 0),
                        stack);
        scene.idle(20);
        ElementLink<EntityElement> remove = scene.world()
                .createItemEntity(
                        util.vector().centerOf(util.grid().at(1, 5, 2)),
                        util.vector().of(0, 0.1, 0),
                        stack);
        scene.idle(15);
        ...
        scene.world()
                .modifyBlock(
                        util.grid().at(3, 3, 3),
                        s -> s.setValue(ChuteBlock.SHAPE, ChuteBlock.Shape.WINDOW),
                        false);
        scene.overlay()
                .showText(50)
                .attachKeyFrame()
        ...
        scene.world()
                .modifyBlock(
                        util.grid().at(3, 2, 3),
                        s -> s.setValue(ChuteBlock.SHAPE, ChuteBlock.Shape.ENCASED),
                        false);
        scene.overlay()
                .showText(50)
        ...
        scene.world()
                .modifyBlock(util.grid().at(2, 2, 1), s -> s.setValue(SHAPE, Shape.NORMAL), false);
        scene.world()
                .modifyBlock(
                        util.grid().at(2, 3, 2), s -> s.setValue(SHAPE, Shape.INTERSECTION), false);
        scene.world().showSection(util.select().fromTo(2, 1, 1, 2, 2, 1), Direction.DOWN);
        scene.idle(30);
        ItemStack chuteItem = AllBlocks.CHUTE.asStack();
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(2, 2, 1), Direction.SOUTH),
                        Pointing.LEFT,
                        30)
                .rightClick()
                .withItem(chuteItem);
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(2, 3, 2), Direction.SOUTH),
                        Pointing.LEFT,
                        30)
                .rightClick()
                .withItem(chuteItem);
    ...
    public static void upward(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(
                        util.vector().blockSurface(util.grid().at(1, 2, 2), Direction.NORTH),
                        Pointing.RIGHT,
                        50)
                .withItem(AllItems.GOGGLES.asStack());
        scene.overlay()
                .showText(70)
                .text("Inspecting chutes with Engineers' Goggles reveals information about the"
                        + " movement direction")
                .attachKeyFrame()
                .pointAt(surface)
                .placeNearTarget();
        ...
        scene.world()
                .showSection(
                        util.select().fromTo(2, 2, 2, 4, 1, 5).add(util.select().position(3, 0, 5)),
                        Direction.DOWN);
        ItemStack stack = new ItemStack(Items.COPPER_BLOCK);
        ...
        scene.overlay()
                ...
                .attachKeyFrame()
                .pointAt(util.vector().centerOf(util.grid().at(3, 1, 2)).add(0, 3 / 16f, 0))
                .placeNearTarget();
        scene.idle(32);
    ...
    public static void smart(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(filter.add(0, 0.125, 0), Pointing.DOWN, 40)
                .rightClick()
                .withItem(copper);
        scene.idle(7);
        scene.world()
                .setFilterData(util.select().position(smarty), SmartChuteBlockEntity.class, copper);
        ...
        for (int i = 0; i < 18; i++) {
            ...
            if (i == 13) scene.world().showSection(lever, Direction.NORTH);
        }
