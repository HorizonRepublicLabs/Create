...
public class HosePulleyScenes {
    ...
    public static void intro(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection pipes =
                util.select().fromTo(3, 1, 1, 3, 5, 1).add(util.select().position(2, 5, 1));
        BlockPos hosePos = util.grid().at(1, 5, 1);
        Selection hose = util.select().position(1, 5, 1);
        Selection crank = util.select().position(0, 5, 1);

        ElementLink<WorldSectionElement> hoselink =
                scene.world().showIndependentSection(hose, Direction.UP);
        ...
        scene.world()
                .modifyBlockEntity(util.grid().at(1, 5, 1), HosePulleyBlockEntity.class, be -> {
        ...
        scene.world()
                .modifyBlock(
                        util.grid().at(3, 2, 1),
                        s -> s.setValue(PumpBlock.FACING, Direction.DOWN),
                        true);
        ...
        scene.overlay()
                .showText(120)
                .text("Fill and Drain speed of the pulley depends entirely on the fluid"
                        + " networks' throughput")
                .placeNearTarget()
    ...
    public static void level(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showOutlineWithText(util.select().position(hosePos.below()), 50)
        ...
        scene.world().showSectionAndMerge(pipes, Direction.WEST, hoselink);
        scene.world()
                .modifyBlockEntity(util.grid().at(1, 6, 1), HosePulleyBlockEntity.class, be -> {
                    IFluidHandler ifh = be.getLevel()
                            .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
                    if (ifh != null)
                        ifh.fill(new FluidStack(Fluids.WATER, 100), FluidAction.EXECUTE);
        ...
        scene.idle(3);
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.MEDIUM, bb, bb.expandTowards(0, -2, 0), 70);
        scene.idle(20);
        ...
        scene.world()
                .modifyBlock(
                        util.grid().at(3, 2, 1),
                        s -> s.setValue(PumpBlock.FACING, Direction.UP),
                        true);
        scene.world().propagatePipeChange(util.grid().at(3, 2, 1));
        scene.idle(16);
        scene.world().setKineticSpeed(kinetics, 0);
        ...
        scene.overlay()
                .chaseBoundingBoxOutline(PonderPalette.MEDIUM, bb, bb.move(0, -3 + 2 / 8f, 0), 3);
        scene.idle(3);
        scene.overlay()
                .chaseBoundingBoxOutline(
                        PonderPalette.MEDIUM, bb, bb.expandTowards(0, -3 + 2 / 8f, 0), 120);
        scene.idle(20);
    ...
    public static void infinite(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection kinetics =
                util.select().fromTo(5, 1, 2, 4, 2, 2).add(util.select().position(5, 0, 2));
        ...
        scene.idle(40);
        scene.world()
                .modifyBlockEntity(util.grid().at(1, 3, 2), HosePulleyBlockEntity.class, be -> {
                    IFluidHandler ifh = be.getLevel()
                            .getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
                    if (ifh != null)
        ...
        scene.overlay()
                .showOutlineWithText(pulleyPos, 60)
