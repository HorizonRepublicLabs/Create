...
public class SpoutScenes {
    ...
    public static void filling(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world()
                .modifyBlock(
                        util.grid().at(2, 3, 3),
                        s -> s.setValue(PumpBlock.FACING, Direction.NORTH),
                        false);
        ...
        ItemStack bucket = AllFluids.HONEY.get().getFluidType().getBucket(honey);
        scene.overlay()
                .showControls(
        ...
        scene.idle(7);
        scene.overlay()
                .showOutlineWithText(util.select().position(2, 3, 2), 50)
        ...
        scene.world()
                .modifyBlockEntityNBT(
                        spoutS, SpoutBlockEntity.class, nbt -> nbt.putInt("ProcessingTicks", 20));
        scene.idle(20);
        scene.world().removeItemsFromBelt(depotPos);
        ItemStack potion = new ItemStack(Items.HONEY_BOTTLE);
        scene.world().createItemOnBeltLike(depotPos, Direction.UP, potion);
        ParticleOptions fluidParticle =
                FluidFX.getFluidParticle(new FluidStack(AllFluids.HONEY.get(), 1000));
        for (int i = 0; i < 10; i++) {
            scene.effects()
                    .emitParticles(
        ...
        ElementLink<BeltItemElement> ingot =
                scene.world().createItemOnBelt(beltPos, Direction.SOUTH, bottle);
        scene.idle(15);
