...
public class RemapHelper {
    ...
    private static void remapPaletteBlock(String type, String newType, boolean vanilla) {
        reMap.put(
                "%s_cobblestone_stairs".formatted(type),
                asResource("cut_%s_stairs".formatted(newType)));
        reMap.put(
                "%s_cobblestone_slab".formatted(type),
                asResource("cut_%s_slab".formatted(newType)));
        reMap.put(
                "%s_cobblestone_wall".formatted(type),
                asResource("cut_%s_wall".formatted(newType)));

        if (!vanilla) {
            if (type != "gabbro")
                reMap.put("%s_cobblestone".formatted(type), asResource("%s".formatted(newType)));
            reMap.put(
                    "polished_%s".formatted(type),
                    asResource("polished_cut_%s".formatted(newType)));
            reMap.put(
                    "polished_%s_stairs".formatted(type),
                    asResource("polished_cut_%s_stairs".formatted(newType)));
            reMap.put(
                    "polished_%s_slab".formatted(type),
                    asResource("polished_cut_%s_slab".formatted(newType)));
            reMap.put(
                    "polished_%s_wall".formatted(type),
                    asResource("polished_cut_%s_wall".formatted(newType)));
        }

        reMap.put("%s_bricks".formatted(type), asResource("cut_%s_bricks".formatted(newType)));
        reMap.put(
                "%s_bricks_stairs".formatted(type),
                asResource("cut_%s_brick_stairs".formatted(newType)));
        reMap.put(
                "%s_bricks_slab".formatted(type),
                asResource("cut_%s_brick_slab".formatted(newType)));
        reMap.put(
                "%s_bricks_wall".formatted(type),
                asResource("cut_%s_brick_wall".formatted(newType)));
        reMap.put(
                "fancy_%s_bricks".formatted(type),
                asResource("small_%s_bricks".formatted(newType)));
        reMap.put(
                "fancy_%s_bricks_stairs".formatted(type),
                asResource("small_%s_brick_stairs".formatted(newType)));
        reMap.put(
                "fancy_%s_bricks_slab".formatted(type),
                asResource("small_%s_brick_slab".formatted(newType)));
        reMap.put(
                "fancy_%s_bricks_wall".formatted(type),
                asResource("small_%s_brick_wall".formatted(newType)));
        reMap.put("paved_%s".formatted(type), asResource("small_%s_bricks".formatted(newType)));
        reMap.put(
                "paved_%s_stairs".formatted(type),
                asResource("small_%s_brick_stairs".formatted(newType)));
        reMap.put(
                "paved_%s_slab".formatted(type),
                asResource("small_%s_brick_slab".formatted(newType)));
        reMap.put(
                "paved_%s_wall".formatted(type),
                asResource("small_%s_brick_wall".formatted(newType)));

        if (!vanilla)
            reMap.put(
                    "chiseled_%s".formatted(type),
                    asResource("polished_cut_%s".formatted(newType)));
    ...
    public static void remap(RegisterEvent event) {
        ...
        if (registry == Registries.BLOCK || registry == Registries.ITEM) {
            reMap.forEach((string, resourceLocation) ->
                    registry.addAlias(asResource(string), resourceLocation));
        }
        ...
        if (registry == Registries.BLOCK_ENTITY_TYPE) {
            ...
            registry.addAlias(
                    asResource("adjustable_pulley"),
                    AllBlockEntityTypes.ADJUSTABLE_CHAIN_GEARSHIFT.getId());
        }
    }
}
