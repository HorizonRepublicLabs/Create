...
public class DumpRailwaysCommand {
    ...
    static ArgumentBuilder<CommandSourceStack, ?> register() {
        return Commands.literal("trains").requires(cs -> cs.hasPermission(2)).executes(ctx -> {
    ...
    static void fillReport(
        ...
        List<TrackGraph> nearest = railways.trackNetworks.values().stream()
                .sorted((tg1, tg2) -> Float.compare(
                        tg1.distanceToLocationSqr(level, location),
        ...
        List<Train> nearestTrains = railways.trains.values().stream()
                .sorted((t1, t2) -> Float.compare(
                        t1.distanceToLocationSqr(level, location),
        ...
        if (trainCount > 0 && !nearestTrains.isEmpty()) {
            ...
            for (Train train : nearestTrains) {
                chat.accept(
                        ...
                        bright);
                if (train.derailed) chat.accept("├─Derailed", orange);
                ...
                if (owner != null) chat.accept("├─Owned by " + owner.getName().getString(), blue);
                GlobalStation currentStation = train.getCurrentStation();
                if (currentStation != null) {
                    chat.accept("├─Waiting at: " + currentStation.name, blue);
                } else if (train.navigation.destination != null)
                    chat.accept(
                            "├─Travelling to " + train.navigation.destination.name + " ("
                                    + Mth.floor(train.navigation.distanceToDestination) + "m away)",
                            darkBlue);
                ScheduleRuntime runtime = train.runtime;
                if (runtime.getSchedule() != null) {
                    chat.accept(
                            "├─Schedule, Entry " + runtime.currentEntry + ", "
                                    + (runtime.paused
                                            ? "Paused"
                                            : runtime.state.name().replaceAll("_", " ")),
                            runtime.paused ? darkBlue : blue);
                } else chat.accept("├─Idle, No Schedule", darkBlue);
                ...
                presentDimensions.forEach(key -> chat.accept(
                        "├─In %1$s near [%2$s]"
                                .formatted(
                                        key.location(),
    ...
    private static Component createDeleteButton(Train train) {
        return Component.literal("└─")
                .withStyle(style -> style.withColor(blue))
                .append(ComponentUtils.wrapInSquareBrackets(Component.literal("Remove")
                                .withStyle(style -> style.withColor(orange)))
                        .withStyle(style -> {
                            return style.withColor(blue)
                                    .withClickEvent(new ClickEvent(
                                            ClickEvent.Action.RUN_COMMAND,
                                            "/c train remove " + train.id.toString()))
                                    .withHoverEvent(new HoverEvent(
                                            HoverEvent.Action.SHOW_TEXT,
                                            Component.literal("Click to remove ")
                                                    .append(train.name)));
                        }));
    }
    ...
    private static Component createTeleportButton(Train train) {
        return Component.literal("├─")
                .withStyle(style -> style.withColor(darkBlue))
                .append(ComponentUtils.wrapInSquareBrackets(Component.literal("Teleport")
                                .withStyle(style -> style.withColor(orange)))
                        .withStyle(style -> {
                            return style.withColor(darkBlue)
                                    .withClickEvent(new ClickEvent(
                                            ClickEvent.Action.RUN_COMMAND,
                                            "/c train tp " + train.id.toString()))
                                    .withHoverEvent(new HoverEvent(
                                            HoverEvent.Action.SHOW_TEXT,
                                            Component.literal("Click to teleport to ")
                                                    .append(train.name)));
                        }));
    }
}
