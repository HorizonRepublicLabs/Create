...
public class CouplingCommand {
    ...
    static {
        SAME_DIMENSION = new SimpleCommandExceptionType(
    ...
    public static final DynamicCommandExceptionType TWO_CARTS =
            new DynamicCommandExceptionType(a -> {
                return Component.literal("Your selector targeted " + a
                        + " entities. You can only couple 2 Minecarts at a time.");
            });
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> register() {
        ...
        return Commands.literal("coupling")
                        ...
                        .then(Commands.argument("carts", EntityArgument.entities())
                                .executes(ctx -> {
                                    Collection<? extends Entity> entities =
                                            EntityArgument.getEntities(ctx, "carts");
                                    if (entities.size() != 2)
                                        throw TWO_CARTS.create(entities.size());

                                    ArrayList<? extends Entity> eList =
                        ...
                        .then(Commands.argument("cart1", EntityArgument.entity())
                                .then(Commands.argument("cart2", EntityArgument.entity())
                                        .executes(ctx -> {
                                            ...
                                            MinecartController cart1Capability = cart1.getData(
                                                    AllAttachmentTypes.MINECART_CONTROLLER);
                                            ...
                                            int cart1Couplings =
                                                    (cart1Capability.isConnectedToCoupling()
                                                                    ? 1
                                                                    : 0)
                                                            + (cart1Capability.isLeadingCoupling()
                                                                    ? 1
                                                                    : 0);
                                            if (cart1Couplings == 0) {
                                                ctx.getSource()
                                            ...
                                            for (boolean bool : Iterate.trueAndFalse) {
                                                UUID coupledCart =
                                                        cart1Capability.getCoupledCart(bool);
                                                if (coupledCart == null) continue;

                                                if (coupledCart != cart2.getUUID()) continue;

                                                MinecartController cart2Controller =
                                                        CapabilityMinecartController.getIfPresent(
                                                                cart1.getCommandSenderWorld(),
                                                                coupledCart);
                ...
                .then(Commands.literal("removeAll")
                        .then(Commands.argument("cart", EntityArgument.entity()).executes(ctx -> {
                            Entity cart = EntityArgument.getEntity(ctx, "cart");
                            if (!(cart instanceof AbstractMinecart))
                                throw ONLY_MINECARTS_ALLOWED.create();

                            MinecartController capability =
                                    cart.getData(AllAttachmentTypes.MINECART_CONTROLLER);
                            if (capability == MinecartController.EMPTY) {
                                ctx.getSource()
                                        .sendSuccess(
                                                () -> {
                                                    return Component.literal(
                                                            "Minecart has no Couplings Attached");
                                                },
                                                true);
                                return 0;
                            }

                            int couplings = (capability.isConnectedToCoupling() ? 1 : 0)
                                    + (capability.isLeadingCoupling() ? 1 : 0);
                            if (couplings == 0) {
                                ctx.getSource()
                                        .sendSuccess(
                                                () -> {
                            ...
                            ctx.getSource()
                                    .sendSuccess(
                                            () -> {
                                                return Component.literal("Removed " + couplings
                                                        + " couplings from the Minecart");
                                            },
                                            true);
