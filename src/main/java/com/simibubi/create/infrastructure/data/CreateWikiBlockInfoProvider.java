...
public class CreateWikiBlockInfoProvider implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cachedOutput) {
        return CompletableFuture.allOf(BuiltInRegistries.BLOCK.stream()
                .filter(b ->
                        RegisteredObjectsHelper.getKeyOrThrow(b).getNamespace().equals(Create.ID))
                .map(block -> {
                    ...
                    if (item != null)
                        element.addProperty(
                                "stackable", item.asItem().getDefaultInstance().getMaxStackSize());
                    ...
                    element.addProperty(
                            "flammable", ((FireBlock) Blocks.FIRE).getBurnOdds(state) > 0);
