...
public class Label extends AbstractSimiWidget {
    ...
    public void setTextAndTrim(Component newText, boolean trimFront, int maxWidthPx) {
        ...
        for (int i = startIndex; i != endIndex; i += step) {
            String sub =
                    builder.substring(trimFront ? i : startIndex, trimFront ? endIndex + 1 : i + 1);
            if (fontRenderer.width(Component.literal(sub).setStyle(newText.getStyle())) + trimWidth
                    <= maxWidthPx) {
                text = Component.literal(trimFront ? trim + sub : sub + trim)
                        .setStyle(newText.getStyle());
                return;
            }
        }
    }
    ...
    protected void doRender(
        ...
        if (suffix != null && !suffix.isEmpty()) copy.append(suffix);
