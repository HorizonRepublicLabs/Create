...
public class ScrollInput extends AbstractSimiWidget {
    ...
    public ScrollInput setState(int state) {
        ...
        clampState();
        updateTooltip();
        if (displayLabel != null) writeToLabel();
        return this;
    }
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pScrollY) {
        ...
        state += step;
        if (shifted) state -= state % shiftStep;
    ...
    protected void clampState() {
        if (state >= max) state = max - 1;
        if (state < min) state = min;
    }

    public void onChanged() {
        if (displayLabel != null) writeToLabel();
        if (onScroll != null) onScroll.accept(state);
        updateTooltip();
    }
    ...
    protected void updateTooltip() {
        ...
        toolTip.add(scrollToModify
                .plainCopy()
                .withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY));
        toolTip.add(shiftScrollsFaster
                .plainCopy()
                .withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY));
    }
}
