...
public abstract class DirectionalAxisBlockStateGen extends SpecialBlockStateGen {
    ...
    protected int getXRotation(BlockState state) {
        ...
        if ((direction.getAxis() == Axis.X) == alongFirst) return 90;
    ...
    protected int getYRotation(BlockState state) {
        ...
        if (direction.getAxis().isVertical()) return alongFirst ? 90 : 0;

        return horizontalAngle(direction) + 90;
    }

    public abstract <T extends Block> String getModelPrefix(
            DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state);

    @Override
    public <T extends Block> ModelFile getModel(
            DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state) {
        boolean vertical = state.getValue(GaugeBlock.FACING).getAxis().isVertical();
