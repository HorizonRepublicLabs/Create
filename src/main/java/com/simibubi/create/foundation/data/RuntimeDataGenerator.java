...
public class RuntimeDataGenerator {
    // (1. variant_prefix, optional, can be null)stripped_(2. wood name)(3. type)(4. empty
    // group)endofline
    ...
    // (1. wood name)(2. type)(3. variant_suffix, optional)_stripped(4. 2nd variant_suffix,
    // optional)
    ...
    // startofline(not preceded by stripped_)(1. wood_name)(2. type)(3. (4. variant suffix),
    // optional,
    // that doesn't end in _stripped, can be null)endofline
    ...
    private static final Object2ObjectOpenHashMap<ResourceLocation, JsonElement> JSON_FILES =
            new Object2ObjectOpenHashMap<>();
    private static final Map<ResourceLocation, ResourceLocation> MISMATCHED_WOOD_NAMES =
            ImmutableMap.<ResourceLocation, ResourceLocation>builder()
                    .put(
                            Mods.ARS_N.asResource("blue_archwood"),
                            Mods.ARS_N.asResource(
                                    "archwood")) // Generate recipes for planks -> everything else
                    // .put(Mods.UUE.asResource("chorus_cane"), Mods.UUE.asResource("chorus_nest"))
                    // // Has a
                    // weird setup with both normal and stripped planks, that it already provides
                    // cutting
                    // recipes for
    ...
    // planks -> stairs, slabs, fences, fence gates, doors, trapdoors, pressure plates, buttons and
    // signs
    private static void cuttingRecipes(ResourceLocation itemId) {
        ...
        if (hasFoundMatch) {
            ...
            String suffix =
                    !strippedInPrefix && !noStrippedVariant ? match.group(3) + match.group(4) : "";
            ...
            ResourceLocation nonStrippedId =
                    matched_name.withSuffix(type).withPrefix(prefix).withSuffix(suffix);
            ResourceLocation planksId = base.withSuffix("_planks");
            ResourceLocation stairsId = base.withSuffix(
                    base.getNamespace().equals(Mods.BTN.getId()) ? "_planks_stairs" : "_stairs");
            ResourceLocation slabId = base.withSuffix(
            ...
            if (!noStrippedVariant) {
                ...
                ResourceLocation tag = Create.asResource(
            ...
            if (!path.contains("_wood")
                    && !path.contains("_hyphae")
                    && BuiltInRegistries.ITEM.containsKey(planksId)) {
    ...
    private static class StandardBuilder<T extends StandardProcessingRecipe<?>>
        ...
        public StandardBuilder(
            ...
            super(
                    factory,
                    Create.asResource(
                            "runtime_generated/compat/" + modid + "/" + from + "_to_" + to));
        }
        ...
        public T build() {
            ...
            Optional<JsonElement> serialized = CatnipCodecUtils.encode(
                    Recipe.CONDITIONAL_CODEC,
                    JsonOps.INSTANCE,
                    Optional.of(new WithConditions<>(recipe)));
