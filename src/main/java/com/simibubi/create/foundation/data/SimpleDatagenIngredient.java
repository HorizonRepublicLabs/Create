...
public class SimpleDatagenIngredient implements ICustomIngredient {
    ...
    */
    private static final MapCodec<SimpleDatagenIngredient> INTERNAL_CODEC =
            RecordCodecBuilder.mapCodec(instance -> instance.group(ResourceLocation.CODEC
                            .fieldOf("item")
                            .forGetter(i -> i.mod.asResource(i.id)))
                    .apply(instance, location -> {
                        ...
                        throw new AssertionError("ID " + location.getNamespace()
                                + " doesn't correspond to any compat mod."
                                + " SimpleDatagenIngredient is not meant for"
                                + " deserialization anyway");
                    }));

    private static final MapCodec<SimpleDatagenIngredient> CODEC =
            RecordCodecBuilder.mapCodec(instance -> instance.group(INTERNAL_CODEC
                            .codec()
                            .listOf()
                            .fieldOf("ingredients")
                            .forGetter(List::of))
                    .apply(instance, list -> {
                        assert list.size() == 1
                                : "SimpleDatagenIngredient should only be serialized as a"
                                        + " single-element list, and shouldn't be deserialized"
                                        + " anyway";
    ...
    public @NotNull IngredientType<?> getType() {
        if (!didRegistryInjection) {
            // Need to do some registry injection to get the Registry#byNameCodec to encode the
            // right type
            // for this
            ...
            if (NeoForgeRegistries.INGREDIENT_TYPES instanceof MappedRegistryAccessor<?> mra) {
                ...
                //noinspection DataFlowIssue - it is ok to pass null as the owner, because this is
                // only
                // being used for serialization
                mra$.getByValue()
                        .put(INGREDIENT_TYPE, Holder.Reference.createStandAlone(null, wrappedKey));
                ...
                throw new AssertionError("SimpleDatagenIngredient will not be able to serialize"
                    + " without injecting into a registry. Expected"
                    + " NeoForgeRegistries.INGREDIENT_TYPES to be of class MappedRegistry, is of"
                    + " class "
