...
public class DataFixerHelper {
    ...
    public static final Set<BlockPosFixer> BLOCK_POS_FIXERS_VIEW =
            Collections.unmodifiableSet(BLOCK_POS_FIXERS);
    ...
    static {
        ...
        addBlockPosFixer(
                References.BLOCK_ENTITY,
                Set.of("item_vault", "fluid_tank"),
                Set.of("LastKnownPos", "Controller"));
        ...
        addBlockPosFixer(
                References.BLOCK_ENTITY,
                Set.of("rope_pulley", "elevator_pulley"),
                Set.of("MirrorChildren"));
        addBlockPosFixer(
                References.BLOCK_ENTITY,
                Set.of("rope_pulley", "elevator_pulley"),
    ...
    private static void addBlockPosFixer(
            DSL.TypeReference reference, Set<String> ids, Set<String> fields) {
        for (String id : ids) addBlockPosFixer(reference, id, fields);
    }

    private static void addBlockPosFixer(
            DSL.TypeReference reference, String id, Set<String> fields) {
    ...
    private static void addBlockPosFixer(
            DSL.TypeReference reference,
            Set<String> ids,
            Function<Dynamic<?>, Dynamic<?>> customFixer) {
        for (String id : ids) addBlockPosFixer(reference, id, customFixer);
    }

    private static void addBlockPosFixer(
            DSL.TypeReference reference, String id, Function<Dynamic<?>, Dynamic<?>> customFixer) {
        BLOCK_POS_FIXERS.add(new BlockPosFixer(
                reference, id.contains(":") ? id : "create:" + id, null, customFixer));
    }

    private static void addBlockPosFixer(
            DSL.TypeReference reference, String id, Map<String, String> renames) {
        BLOCK_POS_FIXERS.add(new BlockPosFixer(
                reference, id.contains(":") ? id : "create:" + id, renames, null));
    }

    private static Dynamic<?> convertListOfBlockPositions(String tagKey, Dynamic<?> data) {
        return data.update(
                tagKey,
                dynamic -> dynamic.createList(dynamic.asStream().map(listData -> {
                    ...
                    if (x.isPresent() && y.isPresent() && z.isPresent()) {
                        ...
                        return listData.set(
                                "Pos",
                                listData.createIntList(IntStream.of(
                                        x.get().intValue(),
                                        y.get().intValue(),
                                        z.get().intValue())));
                    } else {
