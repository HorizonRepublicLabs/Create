...
public class CameraAngleAnimationService {
    ...
    public static void tick() {
        ...
        if (Minecraft.getInstance().player != null) {
            if (!yRotation.settled()) Minecraft.getInstance().player.setYRot(yRotation.getValue(1));

            if (!xRotation.settled()) Minecraft.getInstance().player.setXRot(xRotation.getValue(1));
        }
    }
    ...
    public static void setYawTarget(float yaw) {
        ...
        setupChaser(
                yRotation,
                currentYaw + AngleHelper.getShortestAngleDiff(currentYaw, Mth.wrapDegrees(yaw)));
    }
    ...
    public static void setPitchTarget(float pitch) {
        ...
        setupChaser(
                xRotation,
                currentPitch
                        + AngleHelper.getShortestAngleDiff(currentPitch, Mth.wrapDegrees(pitch)));
    }
    ...
    private static void setupChaser(LerpedFloat rotation, float target) {
        if (animationMode == Mode.LINEAR) {
            rotation.chase(
                    target, animationSpeed > 0 ? animationSpeed : 2, LerpedFloat.Chaser.LINEAR);
        } else if (animationMode == Mode.EXPONENTIAL) {
            rotation.chase(
                    target, animationSpeed > 0 ? animationSpeed : 0.25, LerpedFloat.Chaser.EXP);
        }
    }
