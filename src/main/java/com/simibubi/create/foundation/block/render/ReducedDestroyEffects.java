...
public class ReducedDestroyEffects implements IClientBlockExtensions {
    ...
    public boolean addDestroyEffects(
            BlockState state, Level worldIn, BlockPos pos, ParticleEngine manager) {
        if (!(worldIn instanceof ClientLevel world)) return true;
                ...
                for (int yIndex = 0; yIndex < yParts; ++yIndex) {
                    for (int zIndex = 0; zIndex < zParts; ++zIndex) {
                        ...
                        manager.add(new TerrainParticle(
                                        world, x, y, z, d4 - 0.5D, d5 - 0.5D, d6 - 0.5D, state, pos)
