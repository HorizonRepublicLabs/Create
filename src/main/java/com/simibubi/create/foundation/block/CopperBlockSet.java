...
public class CopperBlockSet {
    ...
    protected static final Map<WeatherState, Supplier<Block>> BASE_BLOCKS =
            new EnumMap<>(WeatherState.class);
    ...
    protected final NonNullBiConsumer<DataGenContext<Block, ?>, RegistrateRecipeProvider>
            mainBlockRecipe;
    ...
    public CopperBlockSet(
        ...
        for (boolean waxed : Iterate.falseAndTrue) {
            for (Variant<?> variant : this.variants) {
                BlockEntry<?>[] entries = waxed
                        ? this.entries.get(variant)
                        : new BlockEntry<?>[WEATHER_STATE_COUNT * 2];
                for (WeatherState state : WEATHER_STATES) {
                    ...
                    if (waxed) {
                        ...
                        CopperRegistries.addWeathering(
                                entries[getIndex(WEATHER_STATES[state.ordinal() - 1], false)],
                                entry);
                    }
                }
                if (!waxed) this.entries.put(variant, entries);
            }
        }
    }

    protected <T extends Block> BlockEntry<?> createEntry(
            AbstractRegistrate<?> registrate,
            Variant<T> variant,
            WeatherState state,
            boolean waxed) {
        ...
        if (!suffix.equals("")) name = Lang.nonPluralId(name);

        name += suffix;

        Supplier<Block> baseBlock = BASE_BLOCKS.get(state);
        BlockBuilder<T, ?> builder = registrate
                .block(name, variant.getFactory(this, state, waxed))
                .initialProperties(() -> baseBlock.get())
                .loot((lt, block) -> variant.generateLootTable(lt, block, this, state, waxed))
                .blockstate(
                        (ctx, prov) -> variant.generateBlockState(ctx, prov, this, state, waxed))
                .recipe((c, p) -> variant.generateRecipes(
                        entries.get(BlockVariant.INSTANCE)[state.ordinal()], c, p))
        ...
        if (variant == StairVariant.INSTANCE) builder.tag(BlockTags.STAIRS);

        if (variant == SlabVariant.INSTANCE) builder.tag(BlockTags.SLABS);
                ...
                ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, ctx.get())
                        ...
                        .save(
                                ...
                                ResourceLocation.fromNamespaceAndPath(
                                        ctx.getId().getNamespace(),
                                        "crafting/" + generalDirectory + ctx.getName()
                                                + "_from_honeycomb"));
            });
        }
    ...
    public interface Variant<T extends Block> {
        ...
        NonNullFunction<Properties, T> getFactory(
                CopperBlockSet blocks, WeatherState state, boolean waxed);

        default void generateLootTable(
                RegistrateBlockLootTables lootTable,
                T block,
                CopperBlockSet blocks,
                WeatherState state,
                boolean waxed) {
    ...
    public static class BlockVariant implements Variant<Block> {
        ...
        @Override
        public NonNullFunction<Properties, Block> getFactory(
                CopperBlockSet blocks, WeatherState state, boolean waxed) {
            if (waxed) {
                return Block::new;
            } else {
        ...
        public void generateBlockState(
                DataGenContext<Block, Block> ctx,
                RegistrateBlockstateProvider prov,
                CopperBlockSet blocks,
                WeatherState state,
                boolean waxed) {
            ...
            String baseLoc = ModelProvider.BLOCK_FOLDER + "/" + blocks.generalDirectory
                    + getWeatherStatePrefix(state);
    ...
    public static class SlabVariant implements Variant<SlabBlock> {
        ...
        @Override
        public NonNullFunction<Properties, SlabBlock> getFactory(
                CopperBlockSet blocks, WeatherState state, boolean waxed) {
            if (waxed) {
        ...
        @Override
        public void generateBlockState(
                DataGenContext<Block, SlabBlock> ctx,
                RegistrateBlockstateProvider prov,
                CopperBlockSet blocks,
                WeatherState state,
                boolean waxed) {
            ResourceLocation fullModel = prov.modLoc(ModelProvider.BLOCK_FOLDER + "/"
                    + getWeatherStatePrefix(state) + blocks.getName());
        ...
        public void generateRecipes(
                ...
                RegistrateRecipeProvider prov) {
            prov.slab(
                    DataIngredient.items(blockVariant.get()),
    ...
    public static class StairVariant implements Variant<StairBlock> {
        ...
        @Override
        public NonNullFunction<Properties, StairBlock> getFactory(
                CopperBlockSet blocks, WeatherState state, boolean waxed) {
            if (!blocks.hasVariant(parent)) {
                throw new IllegalStateException("Cannot add StairVariant '" + this
                        + "' without parent Variant '" + parent.toString() + "'!");
            }
            if (waxed) {
        ...
        @Override
        public void generateBlockState(
                DataGenContext<Block, StairBlock> ctx,
                RegistrateBlockstateProvider prov,
                CopperBlockSet blocks,
                WeatherState state,
                boolean waxed) {
