...
public enum AllCTTypes implements CTType {
    ...
    OMNIDIRECTIONAL(8, ContextRequirement.builder().all().build()) {
        ...
        public int getTextureIndex(CTContext context) {
            ...
            int tileX = 0, tileY = 0;
            int borders =
                    (!c.up ? 1 : 0) + (!c.down ? 1 : 0) + (!c.left ? 1 : 0) + (!c.right ? 1 : 0);
            ...
            if (borders == 2) {
                if ((c.up && c.left && c.topLeft)
                        || (c.down && c.left && c.bottomLeft)
                        || (c.up && c.right && c.topRight)
                        || (c.down && c.right && c.bottomRight)) tileX += 3;
            }
    ...
    ROOF(4, ContextRequirement.builder().all().build()) {
        ...
        public int getTextureIndex(CTContext context) {
            ...
            if (upDrops) {
                if (leftDrops) return context.bottomRight ? 0 : 5;
                if (rightDrops) return context.bottomLeft ? 2 : 5;
                return 1;
            }

            if (downDrops) {
                if (leftDrops) return context.topRight ? 8 : 5;
                if (rightDrops) return context.topLeft ? 10 : 5;
                return 9;
            }
            ...
            if (!context.up || !context.down || !context.left || !context.right) return 5;

            if (context.bottomLeft && context.topRight) {
                if (context.topLeft && !context.bottomRight) return 12;
                if (context.bottomRight && !context.topLeft) return 15;
                if (!context.bottomRight && !context.topLeft) return 7;
            }

            if (context.bottomRight && context.topLeft) {
                if (context.topRight && !context.bottomLeft) return 13;
                if (context.bottomLeft && !context.topRight) return 14;
                if (!context.bottomLeft && !context.topRight) return 11;
            }
    ...
    ROOF_STAIR(4, ContextRequirement.builder().axisAligned().build()) {
        private static final int[][] MAPPING = {
            {1, 6, 9, 4}, {14, 12, 13, 15}, {2, 10, 8, 0}, {5, 5, 5, 5}
        };
