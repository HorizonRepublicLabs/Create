...
public class RotatedPillarCTBehaviour extends HorizontalCTBehaviour {
    ...
    public boolean connectsTo(
        ...
        if (primaryOffset != null
                && primaryOffset.getAxis() != stateAxis
                && !ConnectedPillarBlock.getConnection(state, primaryOffset)) return false;
    ...
    protected boolean reverseUVsVertically(BlockState state, Direction face) {
        Axis axis = state.getValue(LayeredBlock.AXIS);
        if (axis == Axis.X && face == Direction.NORTH) return false;
        if (axis == Axis.Z && face == Direction.WEST) return false;
    ...
    protected Direction getUpDirection(
        ...
        Axis axis = state.getValue(LayeredBlock.AXIS);
        if (axis == Axis.Y) return super.getUpDirection(reader, pos, state, face);
        ...
        return Direction.fromAxisAndDirection(
                axis, alongX ? AxisDirection.POSITIVE : AxisDirection.NEGATIVE);
    }
    ...
    protected Direction getRightDirection(
            BlockAndTintGetter reader, BlockPos pos, BlockState state, Direction face) {
        Axis axis = state.getValue(LayeredBlock.AXIS);
        if (axis == Axis.Y) return super.getRightDirection(reader, pos, state, face);
    ...
    public CTSpriteShiftEntry getShift(
            BlockState state, Direction direction, TextureAtlasSprite sprite) {
        return super.getShift(
                state,
                direction.getAxis() == state.getValue(LayeredBlock.AXIS)
                        ? Direction.UP
                        : Direction.SOUTH,
                sprite);
    }
