...
public class BigOutlines {
    ...
    public static void pick() {
        ...
        double maxRange = mc.hitResult == null
                ? Double.MAX_VALUE
                : mc.hitResult.getLocation().distanceToSqr(origin) + 0.5;
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        BlockHitResult hit = blockState
                                .getInteractionShape(mc.level, p)
                        ...
                        result =
                                new BlockHitResult(vec, hit.getDirection(), hitPos, hit.isInside());
                    }
                }
            }
