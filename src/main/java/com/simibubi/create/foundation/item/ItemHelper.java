...
public class ItemHelper {
    ...
    public static void dropContents(Level world, BlockPos pos, IItemHandler inv) {
        for (int slot = 0; slot < inv.getSlots(); slot++)
            Containers.dropItemStack(
                    world, pos.getX(), pos.getY(), pos.getZ(), inv.getStackInSlot(slot));
    }
    ...
    public static List<Pair<Ingredient, MutableInt>> condenseIngredients(
        ...
        for (Ingredient igd : recipeIngredients) {
            for (Pair<Ingredient, MutableInt> pair : actualIngredients) {
                ...
                if (stacks1.length != stacks2.length) continue;
    ...
    public static boolean matchIngredients(Ingredient i1, Ingredient i2) {
        ...
        if (stacks1 == stacks2) return true;
        if (stacks1.length == stacks2.length) {
            for (int i = 0; i < stacks1.length; i++)
                if (!ItemStack.isSameItem(stacks1[i], stacks2[i])) return false;
            return true;
        }
        return false;
    }
    ...
    public static boolean matchAllIngredients(NonNullList<Ingredient> ingredients) {
        ...
        for (int i = 1; i < ingredients.size(); i++)
            if (!matchIngredients(firstIngredient, ingredients.get(i))) return false;
        return true;
    }
    ...
    public static ItemStack extract(
            IItemHandler inv, Predicate<ItemStack> test, int exactAmount, boolean simulate) {
    ...
    public static ItemStack extract(
        ...
        do {
            ...
            for (int slot = 0; slot < inv.getSlots(); slot++) {
                ...
                if (extracting.isEmpty()) extracting = stack.copy();
                else extracting.grow(stack.getCount());

                if (!simulate && hasEnoughItems) inv.extractItem(slot, stack.getCount(), false);
            ...
            if (checkHasEnoughItems) checkHasEnoughItems = false;
            else break;

        } while (true);

        if (amountRequired && extracting.getCount() < amount) return ItemStack.EMPTY;
    ...
    public static ItemStack extract(
            IItemHandler inv,
            Predicate<ItemStack> test,
            Function<ItemStack, Integer> amountFunction,
            boolean simulate) {
        ItemStack extracting = ItemStack.EMPTY;
        ...
        for (int slot = 0; slot < inv.getSlots(); slot++) {
            if (extracting.isEmpty()) {
                ...
                if (stackInSlot.isEmpty() || !test.test(stackInSlot)) continue;
            ...
            ItemStack stack =
                    inv.extractItem(slot, maxExtractionCount - extracting.getCount(), true);

            if (!test.test(stack)) continue;
            if (!extracting.isEmpty() && !canItemStackAmountsStack(stack, extracting)) continue;

            if (extracting.isEmpty()) extracting = stack.copy();
            else extracting.grow(stack.getCount());

            if (!simulate) inv.extractItem(slot, stack.getCount(), false);
    ...
    public static ItemStack findFirstMatch(IItemHandler inv, Predicate<ItemStack> test) {
        ...
        else return inv.getStackInSlot(slot);
    }
    ...
    public static int findFirstMatchingSlotIndex(IItemHandler inv, Predicate<ItemStack> test) {
        for (int slot = 0; slot < inv.getSlots(); slot++) {
            ...
            if (test.test(toTest)) return slot;
        }
        return -1;
    }

    public static ItemStack fromItemEntity(Entity entityIn) {
        if (!entityIn.isAlive()) return ItemStack.EMPTY;
