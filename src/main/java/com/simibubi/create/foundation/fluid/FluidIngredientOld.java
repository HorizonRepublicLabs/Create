...
public class FluidIngredientOld {
    private static final Codec<SizedFluidIngredient> FLUID_STACK =
            RecordCodecBuilder.create(i -> i.group(
                    ...
                    .apply(
                            i,
                            (type, fluid, components, amount) -> new SizedFluidIngredient(
                                    DataComponentFluidIngredient.of(
                                            false, components.split().added(), fluid),
                                    amount)));

    private static final Codec<SizedFluidIngredient> FLUID_TAG =
            RecordCodecBuilder.create(i -> i.group(
                    ...
                    .apply(
                            i,
                            (type, tag, amount) ->
                                    new SizedFluidIngredient(TagFluidIngredient.tag(tag), amount)));

    public static final Codec<SizedFluidIngredient> CODEC =
            Codec.withAlternative(FLUID_STACK, FLUID_TAG);

    private static <T> RecordCodecBuilder<T, String> validatedType(String requiredType) {
        return Codec.STRING
                .validate(s -> s.equals(requiredType)
                        ? DataResult.success(s)
                        : DataResult.error(() -> "Invalid Type: " + s))
                .fieldOf("type")
