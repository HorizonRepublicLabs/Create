...
public class FluidHelper {
    ...
    public static boolean tryFillItemFromBE(
        ...
        for (int i = 0; i < capability.getTanks(); i++) {
            ...
            ItemStack out = GenericItemFilling.fillItem(
    ...
    private static FluidExchange exchange(
        ...
        for (int tankSlot = 0; tankSlot < fluidTank.getTanks(); tankSlot++) {
            for (int slot = 0; slot < fluidItem.getTanks(); slot++) {
                ...
                boolean canMoveToTank = (undecided || lockedExchange == FluidExchange.ITEM_TO_TANK)
                        && tankCapacity > 0;
                boolean canMoveToItem = (undecided || lockedExchange == FluidExchange.TANK_TO_ITEM)
                        && itemCapacity > 0;
