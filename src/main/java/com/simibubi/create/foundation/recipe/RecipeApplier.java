...
public class RecipeApplier {
    public static void applyRecipeOn(
            ItemEntity entity, Recipe<?> recipe, boolean returnProcessingRemainder) {
        List<ItemStack> stacks =
                applyRecipeOn(entity.level(), entity.getItem(), recipe, returnProcessingRemainder);
        if (stacks.isEmpty()) {
            entity.discard();
            return;
        }
        ...
        for (ItemStack additional : stacks) {
            ItemEntity entityIn = new ItemEntity(
                    entity.level(), entity.getX(), entity.getY(), entity.getZ(), additional);
            entityIn.setDeltaMovement(entity.getDeltaMovement());
            entity.level().addFreshEntity(entityIn);
        }
    }

    public static List<ItemStack> applyRecipeOn(
        ...
        if (recipe instanceof ProcessingRecipe<?, ?> pr) {
            ...
            for (int i = 0; i < stackIn.getCount(); i++) {
                List<ProcessingOutput> outputs = pr instanceof ManualApplicationRecipe mar
                        ? mar.getRollableResults()
                        : pr.getRollableResults();
                for (ItemStack stack : pr.rollResults(outputs, level.random)) {
                    for (ItemStack previouslyRolled : stacks) {
                        if (stack.isEmpty()) continue;
                        if (!ItemStack.isSameItemSameComponents(stack, previouslyRolled)) continue;
                        int amount = Math.min(
                                previouslyRolled.getMaxStackSize() - previouslyRolled.getCount(),
                                stack.getCount());
