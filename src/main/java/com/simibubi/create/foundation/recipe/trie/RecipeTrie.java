...
public class RecipeTrie<R extends Recipe<?>> {
    private static final int MAX_CACHE_SIZE =
            Integer.getInteger("create.recipe_trie.max_cache_size", 512);
    ...
    private final Cache<Set<AbstractVariant>, IntSet> ingredientCache =
    ...
    public static @NotNull Set<AbstractVariant> getVariants(
            @Nullable IItemHandler itemStorage, @Nullable IFluidHandler fluidStorage) {
    ...
    public static class Builder<R extends Recipe<?>> {
        ...
        private final Object2IntOpenHashMap<AbstractVariant> variantToId =
                new Object2IntOpenHashMap<>();
        private int nextVariantId = 0;

        private final Object2IntMap<AbstractIngredient> ingredientToId =
                new Object2IntOpenHashMap<>();
        private int nextIngredientId = 0;
        private final int universalIngredientId;

        private final Int2ObjectOpenHashMap<IntSet> variantToIngredients =
        ...
        private int getOrAssignId(AbstractIngredient ingredient) {
            return ingredientToId.computeIfAbsent(ingredient, $ -> {
                ...
                for (AbstractVariant variant : ingredient.variants) {
                    variantToIngredients
                            .computeIfAbsent(getOrAssignId(variant), $1 -> new IntOpenHashSet())
                            .add(id);
                }
                return id;
            });
        ...
        private AbstractVariant getOrAssignVariant(Item item) {
            AbstractVariant variant =
                    variantCache.computeIfAbsent(item, $ -> new AbstractVariant.AbstractItem(item));
        ...
        private AbstractVariant getOrAssignVariant(Fluid fluid) {
            AbstractVariant variant = variantCache.computeIfAbsent(
                    fluid, $ -> new AbstractVariant.AbstractFluid(fluid));
            getOrAssignId(variant);
            return variant;
        }
        ...
        public RecipeTrie<R> build() {
            ...
            Create.LOGGER.info(
                    "RecipeTrie of depth {} with {} nodes built with {} variants, {} ingredients,"
                            + " and {} recipes",
                    trie.getMaxDepth(),
                    trie.getNodeCount(),
                    variantToId.size(),
