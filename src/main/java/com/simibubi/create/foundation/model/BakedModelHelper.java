...
public class BakedModelHelper {
    ...
    public static int[] cropAndMove(
            int[] vertexData, TextureAtlasSprite sprite, AABB crop, Vec3 move) {
        ...
        float uScale =
                (float) Math.round((getUnInterpolatedU(sprite, u3) - getUnInterpolatedU(sprite, u0))
                        / xyz3.distanceTo(xyz0));
        float vScale =
                (float) Math.round((getUnInterpolatedV(sprite, v1) - getUnInterpolatedV(sprite, v0))
        ...
        if (uScale == 0) {
            ...
            uScale = (float)
                    Math.round((getUnInterpolatedU(sprite, u1) - getUnInterpolatedU(sprite, u0))
                            / xyz1.distanceTo(xyz0));
            vScale = (float)
                    Math.round((getUnInterpolatedV(sprite, v3) - getUnInterpolatedV(sprite, v0))
                            / xyz3.distanceTo(xyz0));
        }
        ...
        for (int vertex = 0; vertex < 4; vertex++) {
            ...
            if (diff.lengthSqr() > 0) {
                ...
                BakedQuadHelper.setU(
                        vertexData, vertex, sprite.getU(getUnInterpolatedU(sprite, u) + uDiff));
                BakedQuadHelper.setV(
                        vertexData, vertex, sprite.getV(getUnInterpolatedV(sprite, v) + vDiff));
            }
    ...
    public static BakedModel generateModel(
            BakedModel template, UnaryOperator<TextureAtlasSprite> spriteSwapper) {
        ...
        for (Direction cullFace : Iterate.directions) {
            ...
            List<BakedQuad> quads =
                    template.getQuads(null, cullFace, random, ModelData.EMPTY, RenderType.solid());
            culledFaces.put(cullFace, swapSprites(quads, spriteSwapper));
        }

        random.setSeed(42L);
        List<BakedQuad> quads =
                template.getQuads(null, null, random, ModelData.EMPTY, RenderType.solid());
    ...
    public static List<BakedQuad> swapSprites(
            List<BakedQuad> quads, UnaryOperator<TextureAtlasSprite> spriteSwapper) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int vertex = 0; vertex < 4; vertex++) {
                ...
                float v = BakedQuadHelper.getV(vertexData, vertex);
                BakedQuadHelper.setU(
                        vertexData, vertex, newSprite.getU(getUnInterpolatedU(sprite, u)));
                BakedQuadHelper.setV(
                        vertexData, vertex, newSprite.getV(getUnInterpolatedV(sprite, v)));
            }
