...
public class ValueBoxRenderer {
    ...
    public static void renderItemIntoValueBox(
            ItemStack filter, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
    ...
    private static float customZOffset(Item item) {
        ...
        if (item instanceof BlockItem) {
            ...
            if (block instanceof FenceBlock) return nudge;
            if (block.builtInRegistryHolder().is(BlockTags.BUTTONS)) return nudge;
            if (block == Blocks.END_ROD) return nudge;
        }
        return 0;
    }
}
