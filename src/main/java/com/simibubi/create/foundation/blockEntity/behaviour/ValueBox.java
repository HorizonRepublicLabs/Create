...
public class ValueBox extends ChasingAABBOutline {
    ...
    public void render(PoseStack ms, SuperRenderTypeBuffer buffer, Vec3 camera, float pt) {
        ...
        if (hasTransform && !transform.shouldRender(levelAccessor, pos, blockState)) return;

        ms.pushPose();
        ms.translate(pos.getX() - camera.x, pos.getY() - camera.y, pos.getZ() - camera.z);
        if (hasTransform) transform.transform(levelAccessor, pos, blockState, ms);
    ...
    public static class ItemValueBox extends ValueBox {
        ...
        public ItemValueBox(
                Component label, AABB bb, BlockPos pos, ItemStack stack, MutableComponent count) {
            super(label, bb, pos);
            this.stack = stack;
            this.count = count;
        }
        ...
        @Override
        public AllIcons getOutline() {
            if (!stack.isEmpty()) return AllIcons.VALUE_BOX_HOVER_6PX;
    ...
    public static class TextValueBox extends ValueBox {
        ...
        public TextValueBox(
                Component label, AABB bb, BlockPos pos, BlockState state, Component text) {
        ...
        public void renderContents(PoseStack ms, MultiBufferSource buffer) {
            ...
            ms.translate(
                    singleDigit ? stringWidth / 2 : 0,
                    singleDigit ? -verticalMargin : verticalMargin,
                    0);

            int overrideColor = transform.getOverrideColor();
            if (overrideColor == -1) drawString8x(ms, buffer, text, 0, 0, 0xEDEDED);
            else drawString(ms, buffer, text, 0, 0, overrideColor);
        }
    }
    ...
    private static void drawString(
            PoseStack ms, MultiBufferSource buffer, Component text, float x, float y, int color) {
