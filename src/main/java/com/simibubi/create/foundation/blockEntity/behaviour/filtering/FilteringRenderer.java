...
public class FilteringRenderer {
    public static void tick() {
        ...
        for (BlockEntityBehaviour b : sbe.getAllBehaviours()) {
            ...
            if (!behaviour.slotPositioning.shouldRender(world, pos, state)) continue;
            if (!behaviour.mayInteract(mc.player)) continue;
            ...
            boolean hit = behaviour.slotPositioning.testHit(
                    world, pos, state, target.getLocation().subtract(Vec3.atLowerCornerOf(pos)));

            AABB emptyBB = new AABB(Vec3.ZERO, Vec3.ZERO);
            AABB bb = isFilterSlotted ? emptyBB.inflate(.45f, .31f, .2f) : emptyBB.inflate(.25f);

            ValueBox box =
                    new ItemValueBox(label, bb, pos, filter, behaviour.getCountLabelForValueBox());
            box.passive(!hit || behaviour.bypassesInput(mainhandItem));

            Outliner.getInstance()
                    .showOutline(
                            Pair.of("filter" + behaviour.netId(), pos),
                            box.transform(behaviour.slotPositioning))
                    .lineWidth(1 / 64f)
            ...
            if (showCount) tip.add(behaviour.getAmountTip());
        ...
        for (BlockEntityBehaviour b : be.getAllBehaviours()) {
            ...
            if (!be.isVirtual()) {
                ...
                if (cameraEntity != null && level == cameraEntity.level()) {
                    ...
                    if (cameraEntity.position().distanceToSqr(VecHelper.getCenterOf(blockPos))
                            > (max * max)) {
                        continue;
                    }
                }
            ...
            if (slotPositioning instanceof Sided sided) {
                ...
                for (Direction d : Iterate.directions) {
                    ...
                    if (AllBlocks.CONTRAPTION_CONTROLS.has(blockState))
                        ValueBoxRenderer.renderFlatItemIntoValueBox(
                                filter, ms, buffer, light, overlay);
                    else
                ...
                ValueBoxRenderer.renderItemIntoValueBox(
                        behaviour.getFilter(), ms, buffer, light, overlay);
                ms.popPose();
            }
        }
    }
