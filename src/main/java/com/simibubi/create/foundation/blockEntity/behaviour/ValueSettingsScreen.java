...
public class ValueSettingsScreen extends AbstractSimiScreen {
    ...
    protected void init() {
        ...
        Vec2 coordinateOfValue =
                getCoordinateOfValue(initialSettings.row(), initialSettings.value());
    ...
    public ValueSettings getClosestCoordinate(int mouseX, int mouseY) {
        ...
        for (; column <= board.maxValue(); column++) {
            Vec2 coord = getCoordinateOfValue(
                    row, milestonesOnly ? column * board.milestoneInterval() : column);
        ...
        return new ValueSettings(
                row,
                milestonesOnly
                        ? Math.min(column * board.milestoneInterval(), board.maxValue())
                        : column);
    }
    ...
    public Vec2 getCoordinateOfValue(int row, int column) {
        ...
        float xOut = guiLeft
                + ((Math.max(1, column) - 1) / board.milestoneInterval()) * milestoneSize
                + column * scale
                + 1.5f;
        xOut += maxLabelWidth + 14 + 4;

        if (column % board.milestoneInterval() == 0) xOut += milestoneSize / 2;
        if (column > 0) xOut += milestoneSize;
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        Component tip = CreateLang.translateDirect(
                "gui.value_settings.release_to_confirm", Component.keybind("key.use"));
        ...
        if (iconMode)
            for (int i = 0; i <= board.maxValue(); i++)
                fattestLabel = Math.max(
                        fattestLabel,
                        font.width(board.formatter().format(new ValueSettings(0, i))));
        ...
        if (fadeInWidth > fattestLabel) {
            ...
            graphics.drawString(
                    font, title, textX - font.width(title) / 2, y - 14, 0xdddddd, false);
            graphics.drawString(
                    font,
                    tip,
        ...
        for (Component component : board.rows()) {
            ...
            if (!iconMode) {
                ...
                for (int w = 0;
                        w < valueBarWidth;
                        w += AllGuiTextures.VALUE_SETTINGS_BAR.getWidth() - 1)
                    UIRenderHelper.drawCropped(
                            graphics,
                            valueBarX + w,
                            y + 1,
                            Math.min(
                                    AllGuiTextures.VALUE_SETTINGS_BAR.getWidth() - 1,
                                    valueBarWidth - w),
                            8,
                            zLevel,
            ...
            for (int milestone = 0; milestone < milestoneCount; milestone++) {
                if (iconMode)
                    AllGuiTextures.VALUE_SETTINGS_WIDE_MILESTONE.render(
                            graphics, milestoneX, y + 1);
                else AllGuiTextures.VALUE_SETTINGS_MILESTONE.render(graphics, milestoneX, y + 1);
            ...
            if (soundCoolDown == 0) {
                ...
                minecraft
                        ...
                        .play(SimpleSoundInstance.forUI(
                                AllSoundEvents.SCROLL_VALUE.getMainEvent(), pitch, 0.25F));
    ...
    protected void renderBrassFrame(GuiGraphics graphics, int x, int y, int w, int h) {
        ...
        if (h > 8) {
            UIRenderHelper.drawStretched(
                    graphics, x, y + 4, 3, h - 8, zLevel, AllGuiTextures.BRASS_FRAME_LEFT);
            UIRenderHelper.drawStretched(
                    graphics, x + w - 3, y + 4, 3, h - 8, zLevel, AllGuiTextures.BRASS_FRAME_RIGHT);
        }

        if (w > 8) {
            UIRenderHelper.drawCropped(
                    graphics, x + 4, y, w - 8, 3, zLevel, AllGuiTextures.BRASS_FRAME_TOP);
            UIRenderHelper.drawCropped(
                    graphics,
                    x + 4,
                    y + h - 3,
    ...
    public void renderBackground(
            @NotNull GuiGraphics graphics, int pMouseX, int pMouseY, float pPartialTick) {
        int a = ((int) (0x50
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pScrollY) {
        ...
        int column = closest.value()
                + ((int) Math.signum(pScrollY)) * (hasShiftDown() ? board.milestoneInterval() : 1);
        column = Mth.clamp(column, 0, board.maxValue());
        if (column == closest.value()) return false;
