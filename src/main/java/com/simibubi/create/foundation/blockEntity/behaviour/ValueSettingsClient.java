...
public class ValueSettingsClient implements LayeredDraw.Layer {
    ...
    public void cancelIfWarmupAlreadyStarted(PlayerInteractEvent.RightClickBlock event) {
        if (interactHeldTicks != -1 && event.getPos().equals(interactHeldPos)) {
    ...
    public void startInteractionWith(
            BlockPos pos, BehaviourType<?> behaviourType, InteractionHand hand, Direction side) {
        interactHeldTicks = 0;
        interactHeldPos = pos;
    ...
    public void tick() {
        ...
        if (interactHeldTicks == -1) return;
        Player player = mc.player;

        if (!ValueSettingsInputHandler.canInteract(player)
                || AllBlocks.CLIPBOARD.isIn(player.getMainHandItem())) {
            cancelInteraction();
            return;
        }
        HitResult hitResult = mc.hitResult;
        if (!(hitResult instanceof BlockHitResult blockHitResult)
                || !blockHitResult.getBlockPos().equals(interactHeldPos)) {
            cancelInteraction();
            return;
        }
        BlockEntityBehaviour behaviour =
                BlockEntityBehaviour.get(mc.level, interactHeldPos, interactHeldBehaviour);
        ...
        if (!mc.options.keyUse.isDown()) {
            ...
            valueSettingBehaviour.onShortInteract(
                    player, interactHeldHand, interactHeldFace, blockHitResult);
            cancelInteraction();
            return;
        }

        if (interactHeldTicks > 3) player.swinging = false;
        if (interactHeldTicks++ < 5) return;
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (mc.options.hideGui || !ValueSettingsInputHandler.canInteract(mc.player)) return;
        if (hoverTicks == 0 || lastHoverTip == null) return;
