...
public class ContinuousOBBCollider extends OBBCollider {
    ...
    public static ContinuousSeparationManifold separateBBs(
            Vec3 cA, Vec3 cB, Vec3 eA, Vec3 eB, Matrix3d m, Vec3 motion) {
        ...
        !(separate(mf, uA0, diff.x, eA.x, a00 * eB.x + a01 * eB.y + a02 * eB.z, motion.x, true)
                || separate(
                        mf, uA1, diff.y, eA.y, a10 * eB.x + a11 * eB.y + a12 * eB.z, motion.y, true)
                || separate(
                        mf, uA2, diff.z, eA.z, a20 * eB.x + a21 * eB.y + a22 * eB.z, motion.z, true)
    ...
    static boolean separate(
        ...
        if (!discreteCollision && signum(projectedMotion) == signum(TL)) return true;
        ...
        if (!discreteCollision) {
            ...
            if (abs(seperation) > abs(projectedMotion)) return true;
        ...
        // boolean isBestSeperation = discreteCollision && checkCount == 5; // Debug specific
        // separations
        ...
        if (dot != 0 && discreteCollision) {
            ...
            if (!cross.equals(Vec3.ZERO)) {
                ...
                stepSeparationVec = sepVec.subtract(
                        axisPlane.scale(sepVec.dot(stepPlane) / axisPlane.dot(stepPlane)));
                ...
                if (abs(mf.stepSeparation) > stepSeparation) mf.stepSeparation = stepSeparation;
            }
        }
        ...
        if (isBestSeperation) {
            ...
            mf.collisionPosition =
                    normalizedAxis.scale(signum(TL) * (-rB) - signum(seperation) * .125f);
        }
    ...
    public static class ContinuousSeparationManifold extends SeparationManifold {
        ...
        public double getTimeOfImpact() {
            if (latestCollisionEntryTime == UNDEFINED) return UNDEFINED;
            if (latestCollisionEntryTime > earliestCollisionExitTime) return UNDEFINED;
