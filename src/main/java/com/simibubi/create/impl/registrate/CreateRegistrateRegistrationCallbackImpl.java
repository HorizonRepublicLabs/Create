...
public class CreateRegistrateRegistrationCallbackImpl {
    ...
    private static final Map<String, Either<List<CallbackImpl<?, ?>>, CreateRegistrate>> CALLBACKS =
            new HashMap<>();
            ...
            if (either != null) {
                ...
                if (optionalCallbacks.isEmpty()) { // in other words, either.right().isPresent()
                    throw new IllegalArgumentException(
                            "Tried to register a duplicate CreateRegistrate instance for mod ID: "
                                    + modid);
                }
    ...
    public static <R, T extends R> void register(
        ...
        synchronized (CALLBACKS) {
            either = CALLBACKS.computeIfAbsent(
                    id.getNamespace(), k -> Either.left(new ArrayList<>()));
            // must be synchronized here, because if `registerRegistrate` were called between these
            // two
            // calls,
        ...
        // This is safe to call outside the synchronized block, because a registrate will only ever
        // be
        // added once.
