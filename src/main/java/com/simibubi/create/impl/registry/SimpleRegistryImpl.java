...
public abstract sealed class SimpleRegistryImpl<K, V> implements SimpleRegistry<K, V>
        permits SingleImpl, MultiImpl {
    ...
    static final class SingleImpl<K, V> extends SimpleRegistryImpl<K, V> {
        ...
        public synchronized V get(K object) {
            ...
            // new providers are added to the start, so normal iteration is reverse-registration
            // order
    ...
    static final class MultiImpl<K, V> extends SimpleRegistryImpl<K, List<V>>
        ...
        private record ProviderWrapper<K, V>(Provider<K, V> wrapped)
                implements Provider<K, List<V>> {
            @Override
